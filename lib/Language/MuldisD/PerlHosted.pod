...
This document is Language::MuldisD::Dialect::HDMD_Perl_Tiny version 0.8.0.
...
This document outlines the specification of I<Hosted Data Muldis D> as
hosted in either Perl 5 or Perl 6, and as composed of just|mainly core Perl
types; for brevity, the term I<PHMD> will be used to refer to this spec.
...
Hosted Data Muldis D is the natural form that one would code in if they
were primarily writing their application in a separate host language, such
as Perl, and any Muldis D code was being specified in terms of host
language code, such as Perl arrays, hashes, and scalars.  Hosted Data
Muldis D code consists of quasi-hierarchical but actually relational
collection values, typically catalog tuples.  Hosted Data Muldis D is the
only representation format used by the API the Muldis DB (specifically
...
emulators), the Hosted Data form is the easiest to use and the least error
prone since no values have to be escaped or stitched together as strings,
which prevents many injection security holes. Hosted Data Muldis D is also
...
name, which for the C<Muldis_D> spec is the range of authority string
...
from C<Muldis_D> as well if that seems appropriate), to prevent ambiguity.
...
C<Muldis_D> plus C<cpan:DUNCAND> spec is the normal 3-integer-parts
...
Examples of a root node:
    ...
    [ 'Muldis_D', 'cpan:DUNCAND', '1.2.3', [ 'Bool', 'md_enum', 'false' ] ]
...
Note that a Hosted Data Muldis D implementation may eschew the above

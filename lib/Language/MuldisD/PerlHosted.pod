...
This document is Language::MuldisD::PerlHosted version 0.4.0.
...
This node type represents a non-empty character string value; it is
interpreted as a Muldis D C<sys.Core.Text.NEText>.  Its format is the same
as for C<sys.Core.Text.Text> but that the node type is 'Text', and the
payload may not be the empty string.
...
Examples:
    ...
    [ 'Cat.NameChain', ['glo','the_db','gene','sorted_person_name'] ]

    [ 'Cat.NameChain', 'glo.the_db.stats.samples_by_order' ]

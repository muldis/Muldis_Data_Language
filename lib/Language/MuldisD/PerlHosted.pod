...
=item The payload; a Perl C<Hash|Mapping> value.
...
=item The payload; a Perl C<Array|Seq|Set|KeySet> of C<Hash|Mapping> value.
...
=item Node type: the Perl C<Str> value C<Set>.

=item Type name; a Perl C<Str> value (or char-mode Perl scalar).

=item The payload; a Perl C<Array|Seq|Set|KeySet> value.
...
=item Node type: the Perl C<Str> value C<Seq>.

=item Type name; a Perl C<Str> value (or char-mode Perl scalar).

=item The payload; a Perl C<Array|Seq> value.
...
=item Node type: the Perl C<Str> value C<Bag>.

=item Type name; a Perl C<Str> value (or char-mode Perl scalar).

=item Format; one of: C<aoa_counted>, C<array_repeated>, C<perl_bag> (p6).

=item The payload; a Perl C<Bag|KeyBag> value or C<Array|Seq> or
...
=item If the format is C<aoa_counted>, then the payload must be a Perl
...
=item If the format is C<array_repeated>, then the payload must be a Perl
...
=item If the format is C<perl_bag>, then the payload must be a Perl 6

...
This node is interpreted as a Muldis D C<sys.type.Rat> value as follows:
...
interpreted as a Muldis D C<sys.type.URat>.  Its format is the same as for
C<sys.type.Rat> but that the node type is 'URat', its formats are
...
Muldis D C<sys.type.PRat>.  Its format is the same as for C<sys.type.URat>

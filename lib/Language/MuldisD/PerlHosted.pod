...
The AST language/schema version, encoded as a Perl C<Str> value.  The
version string is interpreted within the context of both the
...
official spec version to use is shown in the VERSION and DESCRIPTION of the
C<MuldisD.pm> file, when corresponding to the spec containing that file.

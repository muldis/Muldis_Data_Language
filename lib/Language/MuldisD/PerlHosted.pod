...
This document is Language::MuldisD::PerlHosted version 0.0.0.
...
=item The AST language/schema name, specifically the Perl C<Str> value
C<MuldisD> for this spec.

=item The AST language/schema version plus authority, encoded either as a
single Perl C<Str> value, or as a Perl C<Array> value containing separate
values for the language version and authority.

=item The payload is any other PHMD node.
...
=item Node type: the Perl C<Str> value C<Bool>.

=item Format; one of: C<md_enum>, C<perl_bool>, C<any_perl>.
...
=item If the format is C<md_enum>, then the payload must be a Perl C<Str>
...
=item If the format is C<perl_bool>, then:  Under Perl 6, the payload must
...
=item If the format is C<any_perl>, then the payload may be any Perl value,
...
=item Node type: the Perl C<Str> value C<Order>.

=item Format; one of: C<md_enum>, C<perl_order>.
...
=item If the format is C<perl_order>, then:  Under Perl 6, the payload must
...
=item Node type: the Perl C<Str> value C<Int>.

=item Format; one of: C<md_int>, C<perl_int>, C<any_perl>.

=item Only when format is C<md_int>; the max-col-val.
...
=item If the format is C<md_int>, then the max-col-val must be a Perl
...
=item If the format is C<perl_int>, then:  Under Perl 6, the payload must
...
=item Node type: the Perl C<Str> value C<Blob>.

=item Format; one of: C<md_blob>, C<perl_blob>.

=item Only when format is C<md_blob>; the max-col-val.
...
=item If the format is C<md_blob>, then the max-col-val must be a Perl
...
=item If the format is C<perl_blob>, then:  Under Perl 6, the payload must
...
=item Node type: the Perl C<Str> value C<Text>.
...
making a PHMD node.  Under Perl 6, the payload must be a Perl C<Text>,
which is mapped directly.  Under Perl 5, the payload must be just a
canonical Perl character string, which is a scalar whose utf-8 flag is
true, or that doesn't contain any octets with a C<1>-valued highest bit.
...
This node type represents a tuple value.  It has 4 elements:

=over

=item Node type: the Perl C<Str> value C<Tuple>.

=item Type name; a Perl C<Str> value (or char-mode Perl scalar).

=item Format; one of: C<perl_hash>, C<perl_aoa>.

=item The payload.

=back

This node is interpreted as a Muldis D C<sys.type.Tuple> value as follows:

=over

=item If the format is C<perl_hash>, then the payload must be a Perl
C<Hash>, each key+value pair of which defines a named attribute of the new
...
=item If the format is C<perl_aoa>, then the payload must be a Perl
C<Array>, each element of which is a 2-element Perl C<Array> that defines a
named attribute of the new tuple; the 2 sub-array elements are, in order, a
Perl C<Str> that specifies the attribute name, and a PHMD node that
specifies the attribute value.

=back

The list of named values in the payload must correspond to the attribute
names and types declared by the tuple type named by the tuple PHMD node
type name; each attribute type may be any.

Examples:

    [ 'Tuple', 'sys.type.Tuple.D0', 'perl_hash', {} ]

    [ 'Tuple', 'nat.type.account.user.t', 'perl_hash', {
        'login_name' => [ 'Text', 'hartmark' ],
        'login_pass' => [ 'Text', 'letmein' ],
        'is_special' => [ 'Bool', 'md_enum', 'true' ],
    } ]

    [ 'Tuple', 'nat.type.gene.person.t', 'perl_aoa', [
        [ 'name', [ 'Text', 'Michelle' ] ],
        [ 'age' , [ 'Int', 'perl_int', 17 ] ],
    ] ]
...
Examples:
    ...
    [ 'Database', 'sys.type.Database.D0', 'perl_hash', {} ]

    [ 'Database', 'nat.type.account', 'perl_hash', {
        'user' => [ 'Relation', 'nat.type.account.user.r', ... ],
    } ]

    [ 'Database', 'nat.type.gene', 'perl_aoa', [
        'person', [ 'Relation', 'nat.type.gene.person.r', ... ],
    ] ]

=head2 sys.type.Relation

This node type represents a relation value.  It has 4 elements:

=over

=item Node type: the Perl C<Str> value C<Relation>.

=item Type name; a Perl C<Str> value (or char-mode Perl scalar).

=item Format; one of: C<perl_aoh>, C<perl_aoaoa>.

=item The payload.

=back

This node is interpreted as a Muldis D C<sys.type.Relation> value as
follows:

=over

=item If the format is C<perl_aoh>, then the payload must be a Perl
C<Array>, of which each element defines a tuple of the new relation; each
element is a Perl C<Hash>, as per the payload of a tuple-defining PHMD node
whose format is C<perl_hash>.

=item If the format is C<perl_aoaoa>, then the payload must be a Perl
C<Array>, of which each element defines a tuple of the new relation; each
element is a Perl C<Array>, as per the payload of a tuple-defining PHMD
node whose format is C<perl_aoa>.

=back

All of the payload elements must be of the same degree and must declare the
same attribute names and compatible types, each payload element as per the
payload of a tuple-defining PHMD.

Examples:

    [ 'Relation', 'sys.type.Relation.D0C0', 'perl_hash', [] ]

    [ 'Relation', 'sys.type.Relation.D0C1', 'perl_hash', [ {} ] ]

    [ 'Relation', 'nat.type.account.user.r', 'perl_hash', [
        {
            'login_name' => [ 'Text', 'hartmark' ],
            'login_pass' => [ 'Text', 'letmein' ],
            'is_special' => [ 'Bool', 'md_enum', 'true' ],
    ...
    [ 'Relation', 'nat.type.gene.person.r', 'perl_aoa', [
        [
            [ 'name', [ 'Text', 'Michelle' ] ],
            [ 'age' , [ 'Int', 'perl_int', 17 ] ],
        ],

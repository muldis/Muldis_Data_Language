...
C<sys.Core.Database.Database>.  Its format is the same as for
...
This node is interpreted as a Muldis D C<sys.Core.Set.Set> value whose
...
C<sys.Core.Maybe.Maybe>.  Its format is the same as for C<sys.Core.Set.Set>
but that the node type is 'Maybe', and the payload must have at most 1
element.
...
This node is interpreted as a Muldis D C<sys.Core.Seq.Seq> value whose
...
element index is used as the C<index> attribute o the tuple.
...
This node is interpreted as a Muldis D C<sys.Core.Bag.Bag> value whose

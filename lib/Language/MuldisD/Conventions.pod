...
This document is Language::MuldisD::Conventions version 0.0.0.
...
many situations not to do this in (eg, C<Bool>, C<Order>).  For examples,
C<Int> or C<RatRoundRule> or C<E_ENK>.
...
C<is_equal> or C<is_a_member>.  While not a noun per se, such names

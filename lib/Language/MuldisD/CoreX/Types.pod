...
This document is Language::MuldisD::Core::Types version 0.19.1.
...
=item C<sys.Core.Spec.(Set|Maybe|Seq|Bag)OfScalar>

A C<(Set|Maybe|Seq|Bag)OfScalar> is a completely defined proper
subtype of C<(Set|Maybe|Seq|Bag)> whose C<value> attribute has a declared
type of a C<Scalar> subtype.

=item C<sys.Core.Spec.(Set|Maybe|Seq|Bag)Of(Bool|Int|UInt|Blob|Text)>

A C<(Set|Maybe|Seq|Bag)Of(Bool|Int|UInt|Blob|Text)> is a completely
defined proper subtype of C<(Set|Maybe|Seq|Bag)> whose C<value> attribute
has a declared type of a C<(Bool|Int|UInt|Blob|Text)> subtype.

...
Language::MuldisD::SeeAlso -
External resources that you really ought to look at

=head1 VERSION

This document is Language::MuldisD::SeeAlso version 0.3.0.

=head1 DESCRIPTION

This document is a central location within the L<Language::MuldisD>
...
=item B<Muldis DB for Perl 5> - L<Muldis::DB>
...
GNU GPL version 3 or later.  It is a sub-language implementation, in a
manner of speaking, such that users would typically specify Concrete Muldis
D code using native Perl 5 data structures and compile/execute them against
a Perl 5 module embedded in their otherwise Perl 5 application.

=item B<Muldis DB for Perl 6>

This is a parallel first full implementation whose only main difference is
that it is written in Perl 6, for embedding in Perl 6 programs.  Also, it
is currently distributed only with the Perl 6 implementation called Pugs
(L<http://pugscode.org/>) in its C<ext/> directory.  Over the long term,
the Perl 5 version is deprecated in favor of the Perl 6 version.
...
It is desired to make a Parrot (L<http://parrotcode.org/>) hosted
...
libraries are written in Concrete Muldis D source files, compiled, and

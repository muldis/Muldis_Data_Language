...
This document is Language::MuldisD::Basics version 0.15.0.
...
current multi-document: L<Language::MuldisD::Dialect::PTMD_Tiny>,
L<Language::MuldisD::Dialect::HDMD_Perl_Tiny>,
L<Language::MuldisD::Hierarchical>.  Other, unofficial Muldis D dialects
...
system-defined operators are root types.  The 7 most important
system-defined root types are: C<Bool>, C<Int>, C<Rat>, C<Blob>, C<Text>,
C<Tuple>, C<Relation>.  All user-defined root types are scalar types that
are defined not in terms of other types except for that any components of
their I<possreps> (possible representations) have declared types.
I<Perhaps it should be said that all root types are defined by this last
sentence?>  A I<leaf type> is a data type that has no proper subtypes save
for C<Empty>.
...
C<Empty> type (zero values), while all of the other 6 most important
system-defined root types (C<Int>, C<Rat>, C<Blob>, C<Text>, C<Tuple>,
C<Relation>) are infinite, as are the C<Universal>, C<Scalar>,
C<QuasiScalar>, C<QuasiTuple>, C<QuasiRelation>, C<Remnant> types.
...
Conceptually speaking, the Muldis D language core has just a single numeric
...
with some non-core language type, either some system-defined extension or
some user-defined type.  For example, if you want division by zero to
produce a special infinity value rather than fail / throw an exception,
you'll need a non-core type.  Or likewise if you want your math to process
approximate/measured numbers with proper significant figure handling.
...
you can have that as a simple proper subtype of the core numeric type.

Now, the Muldis D language core actually has 2 numeric data types which are
disjoint: C<Int> and C<Rat>.  The latter one is equal to the conceptual
single numeric type as to what numeric values it can represent; the first
one by contrast is conceptually a proper subtype which just contains all
the integers, that is the values where the conceptual I<exponent> is zero
(or I<denominator> is 1) and so the I<mantissa> (or I<numerator>) by itself
is the value.  The reason that Muldis D has these 2 types disjoint is to
assist ease of use and implementation; moreover, C<Int> is conceptually a
lot simpler, and C<Rat> is conceptually defined in terms of C<Int>.

Note that only the C<Int> type is required for bootstrapping the Muldis D
language core and must always be provided; but the C<Rat> type may be left
out of a minimal Muldis D implementation if it isn't needed for user data.
...
C<function> or C<updater> are C<procedure> instead solely because they are
...
non-hosted Plain Text Muldis D application.  A C<main> is the same as a
...
Plain Text Muldis D application, and the only job of that code is to mount
...
Muldis D "main program" (it runs at DBMS startup, and the DBMS shuts down
when it ends), or host language routines (the DBMS exists for the life of
some host language object that represents it), as applicable.  This root
...
specifically C<fed|dep|sdp|pkg>.  All non-global variables are just of the
user data variety, and use the C<pkg|lex> primary namespaces (yes, C<pkg>
...
addressable relvar, type, and routine unqualified names.  So the
...
except procedures, or the C<main> if it exists, or the host language if it
exists.

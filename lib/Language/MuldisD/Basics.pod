...
This is the hierarchy of invocation namespaces of DBMS entities:
    ...
    fed  # the transac-synced federation of curr mounted depots w mount nms
        fed.<depot>(.<namespace>){0,}
            fed.<depot>(.<namespace>){0,}.<relvar>
            fed.<depot>(.<namespace>){0,}.<type>
            fed.<depot>(.<namespace>){0,}.<routine>
            fed.<depot>(.<namespace>){0,}.<package>
                fed.<depot>(.<namespace>){0,}.<package>.<relvar>
                fed.<depot>(.<namespace>){0,}.<package>.<type>
                fed.<depot>(.<namespace>){0,}.<package>.<routine>
    dep  # entities in a self-sufficient depot ref their own depot w this
        dep(.<namespace>){0,}
            dep(.<namespace>){0,}.<relvar>
            dep(.<namespace>){0,}.<type>
            dep(.<namespace>){0,}.<routine>
            dep(.<namespace>){0,}.<package>
                dep(.<namespace>){0,}.<package>.<relvar>
                dep(.<namespace>){0,}.<package>.<type>
                dep(.<namespace>){0,}.<package>.<routine>
    nsp  # entities in a generic public namespace ref their own nsp w this
        nsp.<relvar>
        nsp.<type>
        nsp.<routine>
        nsp.<package>
...
specifically C<app|fed|dep|nsp>.  All non-global variables are just of the
user data variety, and use the C<pkg|lex> primary namespaces.
...
routines, variables) that I<do not> live in any depots.  Variables, types
and routines can exist either standalone or as parts of packages, and
otherwise go straight under C<app>, so their names are either 2 or 3 parts
total.  Variables under C<app> may be of any data types at all, including
of database types.  Variables in C<app> are not considered "part of the
database", but rather part of the temporary working memory of the
application.

The C<fed> primary namespace is for all user-defined entities that I<do>
live in depots.  Entities under C<fed> are organized similarly to C<app>
...
of just relations).  Beneath C<fed>, each secondary namespace is the name
...
at a time.  Under each depot name is an optional tree of generic
namespaces, adding 0..N name parts, each of which we refer to simply as a
I<generic public namespace>.  After that we have the aforementioned
arrangement like C<app> has.
...
Similarly, the primary namespace C<nsp> is for self-references between
entities in a common generic namespace; if there are 2 or more layers of
generic namespace, then this is specifically to the most immediate one of
the referencer; if there are zero layers of generic namespace, then this is
an alias for C<dep>.
...
The entire virtual machine as a whole is synchronized with respect to
transactions; all depots mounted at the same time are a I<federation> whose
updates must be collectively atomic, and commit or rollback as one, such as
if they are all managed by the same actual DBMS or DBMS cluster.  (Also,
generally speaking, no depot may be mounted or unmounted while an explicit
...
Each individual depot is the maximum scope of self-sufficiency.  All
entities in a depot must be fully definable using only system-defined
entities and/or user-defined entities in the same depot (so, for example,
any foreign key constraints can only be between relvars in the same depot).
This allows each depot to maintain an independent existence as far as its
interpretability and integrity goes.  If the storage mechanism is based on
files (eg, SQLite), and each file can exist separately, but several can
optionally be used at the same time, then each file should be represented
by a separate depot.  If the storage mechanism is represented by a SQL
...
If the self-sufficient-entities criteria of a depot is to be strictly
adhered to, then nothing in a depot is allowed to reference other depots,
and therefore any routines that would do cross-depot queries must exist
somewhere else; so the C<app> namespace is provided to be their home.  But
since the C<app> namespace requires runtime definition of its entities,
such as by the C<main> routine or a host-language routine, that solution
makes it more difficult to manage large amounts of cross-database-query
code, so a better solution may need to be developed for this task.

The division of a depot into multiple generic public namespaces is
optional, and this construct is provided to allow a perception of the
storage system that is as reasonably unabstracted as possible; the native
namespace hierarchy of the storage system can be exploited with little
difficulty.  Assuming the previously described meaning of a depot is
adhered to, there will typically be either zero (SQLite) or one
(PostgreSQL, Oracle) layers of generic namespaces; where there is one, it
typically corresponds to the storage system's concept of a I<schema>; but N
layers are provided by Muldis D "just in case".  This said, all entities
that are directly under a generic namespace should all be considered public
...
typically be applied either at the generic namespace level or to other
individual entities under depots.

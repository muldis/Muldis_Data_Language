...
This document is Language::MuldisD::Basics version 0.4.0.
...
L<Language::MuldisD::Core>, L<Language::MuldisD::Catalog>,
L<Language::MuldisD::Grammar>, L<Language::MuldisD::PerlHosted>,
L<Language::MuldisD::Ext::Num>, L<Language::MuldisD::Ext::Temporal>,
...
Language::MuldisD::Basics, is the human readable authoritative
design document for that language, and for the Muldis D virtual machine
...
also be used by an interactive shell interface over Muldis::DB::Interface,
when users submit commands at runtime, or in any other situation where it
makes sense to take input in that form.
...
code, such as Perl objects of Muldis::DB::Literal classes.  Abstract Muldis
D code consists of quasi-hierarchical but actually relational collection
values, typically catalog tuples.  Abstract Muldis D is the only
representation format used by the API of L<Muldis::DB::Interface> itself,
and is what any Perl code typically should be using.  When generating
Muldis D code from arbitrary Perl data structures (which includes the work
of, eg, SQL DBMS emulators), the Abstract form is the easiest to use and
the least error prone since no values have to be escaped or stitched
together as strings, which prevents many injection security holes.
Abstract Muldis D is also what is used when Muldis D code is defined to
generate/prepare and execute other Muldis D code at runtime (by reading or
updating the meta-model / system catalog), which is "data definition".

See L<Language::MuldisD::Catalog> first for details of the Muldis D
...
Muldis D virtual machine; it must always be defined (but sometimes
implicitly), and its execution begins simultaneously with the startup of
the virtual machine, and its end of execution coincides with the shutdown
of the virtual machine.  A C<main> is the same as a C<procedure>, but that
it can not be invoked by any other Muldis D routine, it can not live in any
depot, and it can not have any parameters.  In a non-hosted Concrete Muldis
D application, this is fairly normal and straightforward; also, said
routine is actually the only content of the program file, and its body
defines any other routines or causes them to be loaded from depots, for
subsequent invocation.  In a mixed-language application, where Muldis D
code is invoked by another host language, the Muldis D C<main> is akin to a
coroutine (with some limitations) whose execution is interleaved with the
host language's routines; more specifically, there is an implicit default
C<main> per virtual machine, having no statements in its body, and the host
language explicitly defines, injects, and has executed its component
statements on an ad-hoc basis (several can be done as a unit); the orders
to execute by the host language of these statements constitutes the actual
running of the C<main> routine, and the same statements can be re-run
without repeated compilation.  A host language can not invoke a named
Muldis D routine directly, but must go by way of a C<main> statement.
Similarly to statements, the host language can declare lexical variables of
the C<main>, and these can optionally be bound to host language variables;
in fact, using such bound variables is the only canonical way to pass data
directly between the virtual machine and the host language.  A C<main>
lexical variable can't be un-declared without shutting down the virtual
machine, though its memory footprint can be lessened by setting it to a
small value, such as the default value for its type.

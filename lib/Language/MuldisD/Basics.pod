...
Language::MuldisD::Language -
Design document of the Muldis D language

=head1 VERSION

This document is Language::MuldisD::Language version 0.3.1.
...
L<Language::MuldisD::Grammar>, L<Language::MuldisD::Ext::Num>,
...
Language::MuldisD::Language, is the human readable authoritative
...
updating the Muldis D DBMS source code itself.  Invoking a
...
other routine, and that can not live in any depot.  In a pure Muldis D
application, this is the type of the single "main" routine; assuming it is
Concrete Muldis D, said routine is actually the only content of the program
file, and its body defines any other routines or causes them to be loaded
from depots, for subsequent invocation.  In a mixed-language application,
where Muldis D code is invoked by another host language, a C<host_gate> is
defined and injected into the DBMS by the host language, and only the host
language can invoke it; moreover, a C<host_gate> is the I<only> kind of
routine that a host language may directly invoke, hence its name.  Aside
from not being invokable by any other Muldis D routine, a C<host_gate> is
the same as a C<procedure>.
...
By contrast, every C<procedure> and C<host_gate> is neither implicitly a

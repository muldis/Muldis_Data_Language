...
C<Language::MuldisD::Grammar> or C<Language::MuldisD::PerlHosted>, along
with the system-built-in data types and routines defined in
C<Language::MuldisD::Core>, and using just those instructions to write your
...
the entity names should all be resolvable at compile time, and moreover any
routine et al should be compilable in isolation from any others.
...
for example, '=' is provided as an alias for 'is_equal', and '<' an alias
for 'is_ascending', and '*' as an alias for 'Int.product', and so-forth.
...
apply to user-defined routines.  A routine must be compilable in isolation
to named-argument syntax.

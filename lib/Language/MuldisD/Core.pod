...
C<Order:same>, C<Order:decrease>.  It is the result type of any
C<comparison> routine that is used on 2 values of a
C<sys.Core.Ordered.Ordered> subtype.  Its default value is C<Order:same>;
its minimum and maximum values are, respectively, C<Order:increase> and
C<Order:decrease>.  The cardinality of this type is 3.
...
C<Rat>, C<Blob>, C<Text>, C<Cat.Name>, C<Cat.NameChain>, C<Cat.E_RM>,
...
=item C<sys.Core.Ordered.comparison of Order (Ordered $v1, Ordered $v2)>
...
type that declares itself an C<Ordered> subtype.  Note that C<comparison>
is considered the only fundamental ordered-specific operator, and all
others are defined over it.

=item C<sys.Core.Ordered.reverse_comparison of Order (Ordered $v1, Ordered
$v2)>
...
would result in C<Order:increase> when given the same arguments, and
C<Bool:false> otherwise.

=item C<sys.Core.Ordered.is_decrease of Bool (Ordered $v1, Ordered $v2)>

This function results in C<Bool:true> iff C<sys.Core.Ordered.comparison>
would result in C<Order:decrease> when given the same arguments, and
C<Bool:false> otherwise.

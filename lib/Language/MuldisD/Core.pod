...
Its default value's only tuple's only element has the value C<Bool:false>.
The C<Single> type is complementary to the C<Nothing> type under C<Maybe>;
C<Nothing> consists of exactly 1 of C<Maybe>'s values, and C<Single>
consists of all of C<Maybe>'s other values.
...
C<Blob>, C<Text>, C<Cat.Name>, C<Cat.NameChain>, C<Cat.E_RM>, C<Cat.E_TK>,
C<Cat.E_TDM>, C<Cat.E_ENK>, C<Cat.E_PSAK>.  Its default value is
C<Bool:false>.

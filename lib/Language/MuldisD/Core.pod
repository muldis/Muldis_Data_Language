...
L<Language::MuldisD::Catalog> document; all the other core entities are
described in the current C<Core> document.  TODO: EDIT THIS PARAGRAPH.

This current C<Core> document features the boolean, order, exact numeric
(integer, rational, floating-point), bit string, and character string types
and operators, plus the tuple and relation type constructors (and quasi-
variants) and operators, plus the type system minimal and maximal types,
and the polymorphic operators that all types, or some types including core
types, have defined over them.
...
values are non-negative / greater than or equal to zero.  Its minimum value
is zero.

=item C<sys.type.PInt>

A C<PInt> (positive integer) is a proper subtype of C<UInt> where all
member values are positive / greater than or equal to one.  Its default and
minimum value is 1.

=item C<sys.type.Rat>

A C<Rat> is a single exact rational number of any magnitude.  It is
conceptually a I<numerator> (C<Int>) divided by a I<denominator> (<PInt>).
...
specify maximum denominators that are powers of either 2 or 10.

=item C<sys.type.RatI>

A C<RatI> (rational: integer) is a proper subtype of C<Rat> where the
denominator is 1; every value of this type maps to exactly 1 C<Int> value
and vice-versa, so it is conceptually like an integer, without being
disjoint to C<Rat>.

=item C<sys.type.RatB>

A C<RatB> (rational: binary) is a proper subtype of C<Rat> where the
denominator is a power of 2; it is the best option to exactly represent
rational numbers that are conceptually binary or octal or hexadecimal.

=item C<sys.type.RatD>

A C<RatD> (rational: decimal) is a proper subtype of C<Rat> where the
denominator is a power of 10; it is the best option to exactly represent
rational numbers that are conceptually the decimal numbers that lay-people
work with.

=item C<sys.type.Float>

A C<Float> is a single exact floating-point number of any magnitude.  It is
conceptually the result of a mantissa (C<Int>) multiplied by the result of
a radix (C<PInt>) raised to the power of an exponent (C<Int>), where the
mantissa's absolute value is a multiple of the radix.  Its default value is
zero; its minimum and maximum values are conceptually infinities and
practically impossible.  The cardinality of this type is infinity; to
define a most-generalized finite C<Float> subtype, you must specify the
greatest magnitude value radix, the greatest magnitude value mantissa, plus
the the 2 integer end-points of the inclusive range of the value exponent.
Common subtypes specify radixes that are either 2 or 10.

=item C<sys.type.FloatI>

A C<FloatI> (float: integer) is a proper subtype of C<Float> where the
radix is 1, or the exponent is zero; every value of this type maps to
exactly 1 C<Int> value and vice-versa, so it is conceptually like an
integer, without being disjoint to C<Float>.

=item C<sys.type.FloatB>

A C<FloatB> (float: binary) is a proper subtype of C<Float> where the radix
is 2; it is the best option to exactly represent floating-point numbers
that are conceptually binary or octal or hexadecimal.  Note that the
domains of the IEEE 754 standard binary floating-point numerics are roughly
the same as C<FloatB> subtypes, aside from C<Float> not having separate +/-
zero, nor infinities, nor NaNs; for the 32-bit version, specify a greatest
mantissa of 2^23 (24 bits) and exponent range of -128..127 (8 bits); for
the 64-bit version, specify a greatest mantissa of 2^52 (53 bits) and
exponent range of -1024..1023 (11 bits).  I<Note: some or all details of
what maps to an IEEE 754 float should be reviewed and corrected.>

=item C<sys.type.FloatD>

A C<FloatD> (float: decimal) is a proper subtype of C<Float> where the
radix is 10; it is the best option to exactly represent floating-point
numbers that are conceptually the decimal numbers that lay-people work
with.
...
the Muldis D system catalog / meta-model (see L<Language::MuldisD::Catalog>
for more details), and some others are system-defined for convenience since
they are the types of many core operators (see further below in the current
...
=item C<sys.type.(Set|Maybe|Seq|Bag)Of(Bool|Int|Blob|Text)>

A C<(Set|Maybe|Seq|Bag)Of(Bool|Int|Blob|Text)> is a completely defined
proper subtype of C<(Set|Maybe|Seq|Bag)> whose C<value> attribute has a
declared type of a C<(Bool|Int|Blob|Text)> subtype.
...
=item C<sys.rtn.Universal.is_value_of_type of Bool (RO: Cat.LongName $type,
Universal $v)>
...
=item C<sys.rtn.Universal.treat of Some.Universal (RO: Cat.LongName $as,
Some.Universal $v)>
...
=item C<sys.rtn.Universal.default of Some.Universal (RO: Cat.LongName $of)>
...
=item C<sys.rtn.Ordinal.min of Some.Ordinal (RO: Set{Some.Ordinal} $topic)>
...
=item C<sys.rtn.Ordinal.max of Some.Ordinal (RO: Set{Some.Ordinal} $topic)>

This function is exactly the same as C<sys.rtn.Ordinal.min> except that it
...
=item C<sys.rtn.Ordinal.maybe_min of Maybe{Some.Ordinal} (RO:
Set{Some.Ordinal} $topic)>
...
=item C<sys.rtn.Ordinal.maybe_max of Maybe{Some.Ordinal} (RO:
Set{Some.Ordinal} $topic)>

This function is to C<sys.rtn.Ordinal.max> as C<sys.rtn.Ordinal.maybe_min>
is to C<sys.rtn.Ordinal.min>.
...
=item C<sys.rtn.Tuple.rename of Tuple (RO: Tuple $topic, Cat.AttrRenameMap
$map)>
...
=item C<sys.rtn.Tuple.project of Tuple (RO: Tuple $topic, Cat.ShortNameSet
$attrs)>
...
=item C<sys.rtn.Tuple.remove of Tuple (RO: Tuple $topic, Cat.ShortNameSet
$attrs)>
...
=item C<sys.rtn.Tuple.wrap of Tuple (RO: Tuple $topic, Cat.ShortNameSet
...
=item C<sys.rtn.Tuple.unwrap of Tuple (RO: Tuple $topic, Cat.ShortName
$outer)>

...
=item C<constraint> - C<Cat.TypeConstraint> ... default one always ret True
...
composed entirely of a single expression tree.  Only functions may contain
C<Cat.ExprNode>, so for any procedures that would conceptually include
them, those portions of the procedures need to be separated out and
encapsulated by named functions or inner-functions.
...
A C<Cat.InnerFunc> has these 4 attributes:
...
A C<Cat.InnerFunc> has these 3 attributes:
...
A C<Cat.SetOfName> is a C<(Set)> whose C<value> attribute has a
declared type of C<Cat.Name>.

=head2 sys.Core.Cat.SetOfSetOfName

A C<Cat.SetOfSetOfName> is a C<(Set)> whose C<value> attribute has a
declared type of C<Cat.SetOfName>.

=head2 sys.Core.Cat.SetOfNameMap

A C<Cat.SetOfNameMap> is a C<(Set)> whose C<value> attribute has a
declared type of C<Cat.NameMap>.

=head2 sys.Core.Cat.SetOfNameChain

A C<Cat.SetOfNameChain> is a C<(Set)> whose C<value> attribute has a
declared type of C<Cat.NameChain>.
...
=item C<sys.Core.Set.reduce of Some.ScaTupRel (Set $topic, Cat.NameChain
...
parameter is curried with the same-named argument of C<reduce>, and the
first 2 parameters are the 2 input scalar or nonscalar values for an
invocation.  If C<$topic> has zero values, then C<reduce> results in the
...
=item C<sys.Core.Set.maybe_reduce of Some.ScaTupRel (Set $topic,
Cat.NameChain $func, Tuple $assuming)>

This function is exactly the same as C<sys.Core.Set.reduce> except that it
does not take a C<$assuming> argument, and it results in a C<Maybe> of what
is otherwise the result type, and that result has zero elements if the
...
attribute of C<$topic> named by C<$name>.  It is a short-hand for a
unary projection of just the named attribute plus its renaming to C<value>.
...
=item C<sys.Core.Seq.reduce of Some.ScaTupRel (Seq $topic, Cat.NameChain
$func, Tuple $assuming, Some.ScaTupRel $identity)>

This function is the same as C<sys.Core.Set.reduce>, including that input
...
function named in C<$func> is only associative, and I<not> commutative.

=item C<sys.Core.Seq.maybe_reduce of Some.ScaTupRel (Seq $topic,
Cat.NameChain $func, Tuple $assuming)>

This function is to C<sys.Core.Set.maybe_reduce> as C<sys.Core.Seq.reduce>
is to C<sys.Core.Set.reduce>.
...
=item C<sys.Core.Bag.reduce of Some.ScaTupRel (Bag $topic, Cat.NameChain
$func, Tuple $assuming, Some.ScaTupRel $identity)>

This function is the same as C<sys.Core.Set.reduce>, including that input
values for the reduction come from the C<value> attribute of C<$topic>,
except that it works with a C<Bag> rather than a C<Set>.

=item C<sys.Core.Bag.maybe_reduce of Some.ScaTupRel (Bag $topic,
Cat.NameChain $func, Tuple $assuming)>

This function is to C<sys.Core.Set.maybe_reduce> as C<sys.Core.Bag.reduce>
is to C<sys.Core.Set.reduce>.

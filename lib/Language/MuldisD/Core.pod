...
This document is Language::MuldisD::Core version 0.12.0.
...
=head2 sys.Core.Some.Universal

Contexts defined by the C<Some.Universal> pseudo-type accept values of
potentially any type of C<Universal>.

=head2 sys.Core.Some.ScaTupRel

Contexts defined by the C<Some.ScaTupRel> pseudo-type accept values of
potentially any type of C<Scalar> or C<Tuple> or C<Relation>.

=head2 sys.Core.Some.Ordered
...
C<Cat.E_TK>, C<Cat.E_TDM>, C<Cat.E_EK>, ...
...
values C<Cat.E_EK:half_up>, C<Cat.E_EK:to_even>, C<Cat.E_EK:to_floor>,
C<Cat.E_EK:to_ceiling>, C<Cat.E_EK:to_zero>.  When a value of some ordered
...
=head2 sys.Core.Cat.E_EK

A C<Cat.E_EK> (expression kind) is an enumeration consisting of the 10
values C<Cat.E_EK:default> (default value of expression's declared type),
...
C<Cat.E_EK:upd_param> (on-entry value of expression-containing update
operator subject-to-update parameter), C<Cat.E_EK:ro_param> (value of
...
=item C<coll_lit> - C<Cat.SetOfNameMap>
...
C<Cat.E_EK:upd_param> or C<Cat.E_EK:ro_param>, then the expression
...
=item C<func_args> - C<Cat.NameMap>
...
=item C<ro_args> - C<Cat.NameMap>

These are the 0..N read-only arguments to the update operator invocation.
Each element defines one argument value, with the element C<key> matching
the parameter name, and the element C<value> naming a local C<ExprNode>
which defines the value.

=back

The default value of C<Cat.UpdateStmt> will unconditionally update 1
subject-to-update operator of name C<$topic> with the value C<Bool:false>.
...
operator named C<$topic> and declared type C<Bool> and will unconditionally
update it to the value C<Bool:false>; it has zero read-only parameters.
...
A C<Cat.UpdateOper> has these 3 attributes:
...
This defines the entire body of the update operator, which is composed of
one or more inner update operators, plus its public interface / parameter
list, which is defined by one of those.
...
relation types (for which it is named I<heading>), and it is used also for
...
=head2 sys.Core.Cat.NameMap

A C<Cat.NameMap> specifies a map of short entity names to other short
...
C<Cat.Name>; the C<key> attribute is a unary key.  Its default value
has zero tuples.

=head2 sys.Core.Cat.BiDiNameMap

A C<Cat.BiDiNameMap> is a proper subtype of C<Cat.NameMap> where
...
=head2 sys.Core.Cat.SetOfNameMap

A C<Cat.SetOfNameMap> is a C<Set> whose C<value> attribute has a declared
type of C<Cat.NameMap>.
...
=item C<sys.Core.Universal.treat of Some.Universal (Cat.NameChain $as,
Some.Universal $v)>
...
=item C<sys.Core.Universal.default of Some.Universal (Cat.NameChain $of)>
...
=item C<sys.Core.Ordered.comparison of Order (Some.Ordered $v1,
Some.Ordered $v2)>
...
=item C<sys.Core.Ordered.reverse_comparison of Order (Some.Ordered $v1,
Some.Ordered $v2)>
...
=item C<sys.Core.Blob.catenation of Blob (Seq{Blob} $topic)>
...
=item C<sys.Core.Text.catenation of Text (Seq{Text} $topic)>
...
=item C<sys.Core.Tuple.attr_from_Tuple of Some.ScaTupRel (Tuple $topic)>
...
=item C<sys.Core.Tuple.Tuple_from_attr of Tuple (Cat.Name $name,
Some.ScaTupRel $value)>
...
=item C<sys.Core.Tuple.attr of Some.ScaTupRel (Tuple $topic, Cat.Name
$name)>
...
=item C<sys.Core.Tuple.rename of Tuple (Tuple $topic, Cat.BiDiNameMap
$map)>
...
=item C<sys.Core.Relation.power_set of Set{Relation} (Relation $topic)>
...
=item C<sys.Core.Relation.maybe_reduction of Maybe{Tuple} (Relation $topic,
...
=item C<sys.Core.Set.is_member of Bool (Set $set, Some.ScaTupRel $value)>
...
=item C<sys.Core.Set.is_not_member of Bool (Set $set, Some.ScaTupRel
$value)>

This function is exactly the same as C<sys.Core.Set.is_member> except that
it results in the opposite boolean value when given the same arguments.

=item C<sys.Core.Set.insertion of Set (Set $set, Some.ScaTupRel $value)>
...
=item C<sys.Core.Set.deletion of Set (Set $set, Some.ScaTupRel $value)>
...
=item C<sys.Core.Set.reduction of Some.ScaTupRel (Set $topic, Cat.NameChain
$func, Tuple $assuming, Some.ScaTupRel $identity)>
...
=item C<sys.Core.Set.Set_from_wrap of Set{Tuple} (Relation $topic)>
...
=item C<sys.Core.Maybe.attr_or_default of Some.ScaTupRel (Tuple $topic)>
...
=item C<sys.Core.Maybe.attr_or_value of Some.ScaTupRel (Tuple $topic,
Some.ScaTupRel $value)>
...
=item C<sys.Core.Seq.value of Some.ScaTupRel (Seq $topic, UInt $index)>
...
=item C<sys.Core.Seq.insertion of Seq (Seq $topic, UInt $index,
Some.ScaTupRel $value)>
...
=item C<sys.Core.Seq.is_element of Bool (Seq $topic, Some.ScaTupRel
$value)>
...
=item C<sys.Core.Seq.is_not_element of Bool (Seq $topic, Some.ScaTupRel
$value)>

This function is exactly the same as C<sys.Core.Seq.is_element> except that
it results in the opposite boolean value when given the same arguments.

=item C<sys.Core.Seq.reduction of Some.ScaTupRel (Seq $topic, Cat.NameChain
$func, Tuple $assuming, Some.ScaTupRel $identity)>
...
=item C<sys.Core.Seq.catenation of Seq (Seq{Seq} $topic)>
...
=item C<sys.Core.Bag.is_member of Bool (Bag $bag, Some.ScaTupRel $value)>
...
=item C<sys.Core.Bag.is_not_member of Bool (Bag $bag, Some.ScaTupRel
$value)>

This function is exactly the same as C<sys.Core.Bag.is_member> except that
it results in the opposite boolean value when given the same arguments.

=item C<sys.Core.Bag.count of UInt (Bag $bag, Some.ScaTupRel $value)>
...
=item C<sys.Core.Bag.insertion of Bag (Bag $bag, Some.ScaTupRel $value)>
...
=item C<sys.Core.Bag.deletion of Bag (Bag $bag, Some.ScaTupRel $value)>
...
=item C<sys.Core.Bag.reduction of Some.ScaTupRel (Bag $topic, Cat.NameChain
$func, Tuple $assuming, Some.ScaTupRel $identity)>
...
=item C<sys.Core.Bag.Bag_from_wrap of Bag{Tuple} (Relation $topic)>
...
=item C<sys.Core.Bag.union of Bag (Set{Bag} $topic)>
...
=item C<sys.Core.Bag.intersection of Bag (Set{Bag} $topic)>

    ...
    sys.Core.Universal.Universal
        ...
        sys.Core.Relation.Relation
            sys.Core.Relation.Set
                ...
                sys.Core.Spec.SetOfTuple
                sys.Core.Spec.SetOfRelation
                    sys.Core.Spec.SetOfBag
...
of attributes of scalar possreps or tuples or relations.
...
C<Cat.E_TK>, C<Cat.E_TDM>, C<Cat.E_ENK>, C<Cat.E_PSAK>, ...
...
same as C<NEText> in all ways but that it is specifically intended for use
...
C<Cat.E_TDM:negation> (defined as explicit negation of another type).
...
(a C<PInt2_N>), C<max_denom> (a C<PInt>), and C<round_meth> (a
...
A C<Cat.InnerFunc> has these 5 attributes:
...
=head2 sys.Core.Cat.UpdateStmt

A C<Cat.UpdateStmt> is a C<Tuple>.  It specifies a single update statement
(of 1..N variables), which is the most fundamental component of procedural
...
C<Cat.UpdateStmt>, either directly, or indirectly by way of update operator
invocations, as each update operator body is itself composed entirely of
1 or more update statements (grouped as 1 multi-update).  Only update
operators may contain C<Cat.UpdateStmt>, so for any procedures that would
...
A C<Cat.UpdateStmt> has these 3 attributes:

=over

=item C<update_oper> - C<Cat.NameChain>
...
The default value of C<Cat.UpdateStmt> will unconditionally update 1
subject-to-update parameter of name C<$topic> with the value C<Bool:false>.

=head2 sys.Core.Cat.InnerUpdateOper

A C<Cat.InnerUpdateOper> is a C<Tuple>.  It specifies a named
C<update_operator> routine, which is the main component type of a complete
implicitly atomic procedural routine; a complete update operator consists
of at least one inner update operator, which is its main body, likely plus
more when it would conceptually be code refactored.

A C<Cat.InnerUpdateOper> has these 6 attributes:
...
=item C<stmt> - C<Cat.MultiUpdateStmt>
...
The default value of C<Cat.InnerUpdateOper> has 1 subject-to-update
parameter named C<$topic> and declared type C<Bool> and will
unconditionally update it to the value C<Bool:false>; it has zero read-only
parameters.

=head2 sys.Core.Cat.UpdateOper

A C<Cat.UpdateOper> is a C<Tuple>.  It specifies a named C<update operator>
...
A C<Cat.UpdateOper> has these 4 attributes:
...
=item C<root_inner_update_oper> - C<Cat.Name>
...
=item C<inner_update_opers> - C<Cat.InnerUpdateOperTree>
...
The default value of C<Cat.UpdateOper> is as per the default of
C<Cat.InnerUpdateOper>.
...
This (save for C<inner_update_opers> and C<inner_funcs>) defines the entire
...
=head2 sys.Core.Cat.MultiUpdateStmt

A C<Cat.MultiUpdateStmt> is a C<Relation> with the same heading as
C<Cat.UpdateStmt>; it is a multi-update statement, which is the narrowest
...
value of C<Cat.MultiUpdateStmt> has one tuple that is the default value of
C<Cat.UpdateStmt>.

=head2 sys.Core.Cat.InnerUpdateOperSet

A C<Cat.InnerUpdateOperSet> is a C<Relation> with the same heading as
C<Cat.InnerUpdateOper>; it specifies a set of inner update operators (that
...
=head2 sys.Core.Cat.InnerUpdateOperTree

A C<Cat.InnerUpdateOperTree> is a proper subtype of
C<Cat.InnerUpdateOperSet> which is likely to only be applicable if the
complete routine is a update operator.  It must have at least 1 member
tuple.  All member tuples must define a single invocation tree but that
cycles are allowed, including that invoke what is conceptually the tree
root (main update operator if applicable).  The default value of
C<Cat.InnerUpdateOperTree> has one tuple that is the default value of
C<Cat.InnerUpdateOper>.
...
A C<Cat.InnerProcTree> is to C<Cat.InnerProc> what
C<Cat.InnerUpdateOperTree> is to C<Cat.InnerUpdateOper>.
...
A C<PInt2_36> is a proper subtype of C<PInt2_N> where all member values are
between 2 and 36.  (The significance of the number 36 is 10 digits plus 26
letters.)  Its maximum value is 36.  The cardinality of this type is 35.
...
=item C<sys.Core.Universal.is_equal of Bool (Universal $v1, Universal $v2)>
...
=item C<sys.Core.Universal.is_not_equal of Bool (Universal $v1,
Universal $v2)>
...
=item C<sys.Core.Universal.is_value_of_type of Bool (Cat.NameChain $type,
Universal $v)>
...
=item C<sys.Core.Universal.treat of Universal (Cat.NameChain $as,
Universal $v)>
...
=item C<sys.Core.Universal.default of Universal (Cat.NameChain $of)>
...
=item C<sys.Core.Ordered.comparison of Cat.Order (Ordered $v1, Ordered
$v2)>
...
the type) over the value of the C<$v1> argument, and otherwise it results
in C<Cat.Order:decrease> as the reverse of the last condition would be
true.  This function's arguments must be of compatible declared types; in
this case, 2 declared types are compatible iff they are both subtypes of a
...
=item C<sys.Core.Ordered.reverse_comparison of Cat.Order (Ordered $v1,
Ordered $v2)>
...
=item C<sys.Core.Ordered.is_increase of Bool (Ordered $v1, Ordered $v2)>
...
=item C<sys.Core.Ordered.is_decrease of Bool (Ordered $v1, Ordered $v2)>

This function results in C<Bool:true> iff C<sys.Core.Ordered.comparison>
would result in C<Cat.Order:decrease> when given the same arguments, and
C<Bool:false> otherwise.

=item C<sys.Core.Ordered.is_inside_range of Bool (Ordered $topic, Ordered
$min, Ordered $max, Bool $min_is_inside, Bool $max_is_inside)>

This function results in C<Bool:true> iff its C<$topic> argument is within
the range whose bounds are defined by its C<$min> and C<$max> arguments.
If C<$min_is_inside> or C<$max_is_inside> are C<Bool:true>, then C<$topic>
is considered to be within the range if it is equal to C<$min> or C<$max>,
...
=item C<sys.Core.Ordered.is_outside_range of Bool (Ordered $topic, Ordered
$min, Ordered $max, Bool $min_is_inside, Bool $max_is_inside)>
...
=item C<sys.Core.Ordered.min of Ordered (SetOfOrdered $topic)>
...
=item C<sys.Core.Ordered.max of Ordered (SetOfOrdered $topic)>
...
=item C<sys.Core.Ordered.maybe_min of MaybeOfOrdered (SetOfOrdered $topic)>
...
=item C<sys.Core.Ordered.maybe_max of MaybeOfOrdered (SetOfOrdered $topic)>
...
=item C<sys.Core.Bool.and of Bool (SetOfBool $topic)>
...
=item C<sys.Core.Bool.or of Bool (SetOfBool $topic)>
...
=item C<sys.Core.Bool.xor of Bool (BagOfBool $topic)>
...
=item C<sys.Core.Int.increase of Int (Int $topic)>

This function results in its argument increased by 1.

=item C<sys.Core.Int.decrease of Int (Int $topic)>
...
=item C<sys.Core.Int.sum of Int (BagOfInt $addends)>
...
=item C<sys.Core.Int.product of Int (BagOfInt $factors)>
...
=item C<sys.Core.Int.maybe_quotient of MaybeOfInt (Int $dividend, Int
$divisor)>
...
=item C<sys.Core.Int.maybe_remainder of MaybeOfUInt (Int $dividend, Int
$divisor)>
...
=item C<sys.Core.Int.range of Int (SetOfInt $topic)>
...
=item C<sys.Core.Int.median of SetOfInt (BagOfInt $topic)>
...
=item C<sys.Core.Int.mode of SetOfInt (BagOfInt $topic)>
...
=item C<sys.Core.Int.factorial of PInt (UInt $topic)>
...
=item C<sys.Core.Rat.sum of Rat (BagOfRat $addends)>
...
=item C<sys.Core.Rat.product of Rat (BagOfRat $factors)>
...
=item C<sys.Core.Rat.maybe_quotient of MaybeOfRat (Rat $dividend, Rat
$divisor)>
...
=item C<sys.Core.Rat.range of Rat (SetOfRat $topic)>

This function results in the difference between the lowest and highest
element values of its argument.  If C<$topic> has zero values, then
C<range> results in the rational zero.

=item C<sys.Core.Rat.mean of Rat (BagOfRat $topic)>
...
=item C<sys.Core.Rat.maybe_mean of MaybeOfRat (BagOfRat $topic)>
...
=item C<sys.Core.Rat.median of SetOfRat (BagOfRat $topic)>
...
=item C<sys.Core.Rat.mean_of_median of Rat (BagOfRat $topic)>
...
=item C<sys.Core.Rat.mode of SetOfRat (BagOfRat $topic)>
...
=item C<sys.Core.Rat.round of Rat (Rat $topic, Cat.RatRoundRule
$round_rule)>
...
=item C<sys.Core.Rat.log of Rat (PRat $topic, PRat $radix, Cat.RatRoundRule
$round_rule)>
...
would be an irrational number in the general case, the C<$round_rule>
argument specifies how to coerce the conceptual result into a rational
number that is the actual result.

=item C<sys.Core.Rat.natural_power of PRat (Rat $exponent, Cat.RatRoundRule
$round_rule)>
...
=item C<sys.Core.Rat.natural_log of Rat (PRat $topic, Cat.RatRoundRule
$round_rule)>
...
=item C<sys.Core.Rat.Int_from_Rat of Int (Rat $rat, Cat.E_RM $round_meth)>
...
=item C<sys.Core.Blob.catenation of Blob (SeqOfBlob $topic)>
...
=item C<sys.Core.Blob.is_substr of Bool (Blob $look_in, Blob $look_for,
...
=item C<sys.Core.Blob.is_not_substr of Bool (Blob $look_in, Blob $look_for,
Bool $fixed_start, Bool $fixed_end)>
...
=item C<sys.Core.Blob.and of Blob (SetOfBlob $topic)>
...
=item C<sys.Core.Blob.or of Blob (SetOfBlob $topic)>
...
=item C<sys.Core.Blob.xor of Blob (BagOfBlob $topic)>
...
=item C<sys.Core.Text.catenation of Text (SeqOfText $topic)>
...
=item C<sys.Core.Text.is_substr of Bool (Text $look_in, Text $look_for,
Bool $fixed_start, Bool $fixed_end)>
...
=item C<sys.Core.Text.is_not_substr of Bool (Text $look_in, Text $look_for,
Bool $fixed_start, Bool $fixed_end)>

This function is exactly the same as C<sys.Core.Text.is_substr> except that
it results in the opposite boolean value when given the same arguments.

=item C<sys.Core.Text.case_folded_to_upper (Text $topic)>
...
=item C<sys.Core.Text.case_folded_to_lower (Text $topic)>

This function results in the transformation of its argument where any
letters considered to be (capital) uppercase are folded to (small)
lowercase.

=item C<sys.Core.Text.whitespace_trimmed (Text $topic)>
...
=item C<sys.Core.Tuple.attr_from_Tuple of ScaTupRel (Tuple $topic)>
...
=item C<sys.Core.Tuple.Tuple_from_attr of Tuple (Cat.Name $name, ScaTupRel
$value)>
...
=item C<sys.Core.Tuple.attr of ScaTupRel (Tuple $topic, Cat.Name $name)>
...
=item C<sys.Core.Tuple.update_attr of Tuple (Tuple $topic, Cat.Name $name,
ScaTupRel $value)>

This function results in its C<$topic> argument but that its attribute
whose name is C<$name> has been updated with a new scalar or nonscalar
value given by C<$value>.  This function will fail if C<$name> specifies an
attribute name that C<$topic> doesn't have, or if the declared type of
C<$value> isn't a subtype of the declared type of the attribute.

=item C<sys.Core.Tuple.rename of Tuple (Tuple $topic, Cat.AttrRenameMap
$map)>
...
with the C<before> and C<after> attributes of a C<$map> tuple representing
the old and new names of a C<$topic> attribute, respectively.  As a trivial
...
=item C<sys.Core.Tuple.projection of Tuple (Tuple $topic, Cat.SetOfName
$attrs)>
...
=item C<sys.Core.Tuple.cmpl_projection of Tuple (Tuple $topic,
Cat.SetOfName $attrs)>
...
=item C<sys.Core.Tuple.wrap of Tuple (Tuple $topic, Cat.SetOfName $inner,
Cat.Name $outer)>
...
of C<$topic>, then the result has a single attribute whose value is the
same as C<$topic>.  This function supports the new attribute having the
same name as an old one being wrapped into it.  This function will fail if
C<$inner> specifies any attribute names that C<$topic> doesn't have, or if
C<$outer> is the same as C<$topic> attributes that aren't being wrapped.

=item C<sys.Core.Tuple.cmpl_wrap of Tuple (Tuple $topic, Cat.SetOfName
$cmpl_inner, Cat.Name $outer)>
...
=item C<sys.Core.Tuple.unwrap of Tuple (Tuple $topic, Cat.Name $outer)>
...
have, or if an attribute of C<$topic{$outer}> is the same as another
C<$topic> attribute.
...
=item C<sys.Core.Tuple.product of Tuple (QuasiSetOfTuple $topic)>
...
=item C<sys.Core.Relation.is_member of Bool (Relation $r, Tuple $t)>
...
=item C<sys.Core.Relation.is_not_member of Bool (Relation $r, Tuple $t)>
...
=item C<sys.Core.Relation.insertion of Relation (Relation $r, Tuple $t)>
...
=item C<sys.Core.Relation.deletion of Relation (Relation $r, Tuple $t)>
...
the result is C<$t> deleted from C<$r>.  As a trivial case, if C<$t>
already doesn't exist in C<$r>, then the result is just C<$r>.

=item C<sys.Core.Relation.empty of Relation (Relation $topic)>

This function results in the empty relation of the same heading of its
argument; it has zero tuples.

=item C<sys.Core.Relation.universal of Relation (Relation $topic)>
...
=item C<sys.Core.Relation.power_set of SetOfRelation (Relation $topic)>
...
=item C<sys.Core.Relation.negation of Relation (Relation $topic)>
...
=item C<sys.Core.Relation.projection of Relation (Relation $topic,
Cat.SetOfName $attrs)>
...
=item C<sys.Core.Relation.cmpl_projection of Relation (Relation $topic,
Cat.SetOfName $attrs)>
...
=item C<sys.Core.Relation.wrap of Relation (Relation $topic, Cat.SetOfName
$inner, Cat.Name $outer)>
...
=item C<sys.Core.Relation.cmpl_wrap of Relation (Relation $topic,
Cat.SetOfName $cmpl_inner, Cat.Name $outer)>
...
=item C<sys.Core.Relation.unwrap of Relation (Relation $topic, Cat.Name
$outer)>

This function is the inverse of C<sys.Core.Relation.wrap> as
C<sys.Core.Tuple.unwrap> is to C<sys.Core.Tuple.wrap>.

=item C<sys.Core.Relation.group of Relation (Relation $topic, Cat.SetOfName
$inner, Cat.Name $outer)>

This function is similar to C<sys.Core.Relation.wrap> but that the
C<$topic> attribute-wrapping transformations result in new
C<Relation>-typed attributes rather than new C<Tuple>-typed attributes, and
moreover multiple C<$topic> tuples may be combined into fewer tuples whose
new C<Relation>-typed attributes have multiple tuples.  This function takes
a relation of N tuples and divides the tuples into M groups where all the
...
trivial case, if C<$inner> is empty, then the result has all the same
tuples and attributes as before plus a new relation-typed attribute of
degree zero whose value per tuple is of cardinality one; or, if C<$inner>
lists all attributes of C<$topic>, then the result has a single tuple of a
single attribute whose value is the same as C<$topic>.  This function
supports the new attribute having the same name as an old one being grouped
into it.  This function will fail if C<$inner> specifies any attribute
names that C<$topic> doesn't have, or if C<$outer> is the same as C<$topic>
attributes that aren't being grouped.

=item C<sys.Core.Relation.cmpl_group of Relation (Relation $topic,
Cat.SetOfName $group_per, Cat.Name $outer)>
...
specifies any attribute name that C<$topic> doesn't have, or if an
attribute of C<$topic{$outer}> is the same as another C<$topic> attribute.

=item C<sys.Core.Relation.transitive_closure of Relation (Relation $topic)>
...
=item C<sys.Core.Relation.reduction of Tuple (Relation $topic,
Cat.NameChain $func, Tuple $assuming, Tuple $identity)>
...
C<$identity> may be changed to take a function name rather than a value,
for consistency with C<$func>.>  This function will fail if the declared
headings of C<$identity> and C<$topic> aren't compatible.

=item C<sys.Core.Relation.maybe_reduction of MaybeOfTuple (Relation $topic,
Cat.NameChain $func, Tuple $assuming)>

This function is exactly the same as C<sys.Core.Relation.reduction> except
that it does not take an C<$assuming> argument, and it results in a
C<Maybe> of what is otherwise the result type, and that result has zero
elements if the argument has zero elements.

=item C<sys.Core.Relation.restriction of Relation (Relation $topic,
Cat.NameChain $func, Tuple $assuming)>
...
C<sys.Core.Relation.restriction> to implement some common kinds of
relational restrictions (those composed simply of anded or ored tests for
attribute value equality), due to the former's greater simplicity.

=item C<sys.Core.Relation.cmpl_restriction of Relation (Relation
$topic, Cat.NameChain $func, Tuple $assuming)>
...
=item C<sys.Core.Relation.extension of Relation (Relation $topic,
Cat.NameChain $func, Tuple $assuming)>
...
then this function results simply in C<$topic>.

=item C<sys.Core.Relation.substitution of Relation (Relation $topic,
Cat.NameChain $func, Tuple $assuming)>
...
heading of C<$topic>; corresponding values resulting from the function
named in C<$func> will replace the values of the tuples of C<$topic>.  The
result relation has a cardinality that is the same as that of C<$topic>,
unless the result of any substitutions was redundant tuples, in which case
the result has appropriately fewer tuples.  As a trivial case, if C<$func>
is defined to unconditionally result in either the degree-zero tuple or in
the same tuple as its own C<$topic> argument, then this function results
...
=item C<sys.Core.Relation.substitution_in_restriction of Relation (Relation
$topic, Cat.NameChain $restr_func, Tuple $restr_assuming, Cat.NameChain
$subst_func, Tuple $subst_assuming)>
...
C<$subst_assuming>), then the result of the substitution is unioned with
the un-transformed group.  Note that the C<substitution_in_semijoin>
function is recommended to be used instead for many common substitutions.

=item C<sys.Core.Relation.map of Relation (Relation $topic, Cat.NameChain
$func, Tuple $assuming)>
...
argument, then this function results simply in C<$topic>; or, if C<$func>
is defined to have a static result, then this function's result will have
just 0..1 tuples.

=item C<sys.Core.Relation.summary of Relation (Relation $topic,
Cat.SetOfName $group_per, Cat.NameChain $summ_func, Tuple $summ_assuming)>
...
C<$assuming>); the curried function has, rather than the typical 1
C<$topic> varying parameter, 2 varying parameters named C<$summarize> and
C<$per>, which are valued with the relation-valued attribute and
tuple-valued attribute, respectively.  As per a function that C<map>
applies, the function named by C<$summ_func> effectively takes a whole
post-grouping input tuple and results in a whole tuple; the applied
...
relation at once (except by chance of it resolving to 1 group); you should
instead invoke your summarize-all C<$func> directly, or inline it, rather
than by way of C<summary>, especially if you want a single-tuple result on
an empty C<$topic> (which C<summary>) won't do.
...
=item C<sys.Core.Relation.is_subset of Bool (Relation $look_in, Relation
$look_for)>
...
=item C<sys.Core.Relation.is_not_subset of Bool (Relation $look_in,
Relation $look_for)>
...
=item C<sys.Core.Relation.is_proper_subset of Bool (Relation $look_in,
Relation $look_for)>

This function is exactly the same as C<sys.Core.Relation.is_subset> except
that it results in C<Bool:false> if its 2 arguments are equal.

=item C<sys.Core.Relation.is_not_proper_subset of Bool (Relation $look_in,
Relation $look_for)>
...
=item C<sys.Core.Relation.union of Relation (SetOfRelation $topic)>
...
=item C<sys.Core.Relation.exclusion of Relation (BagOfRelation $topic)>
...
=item C<sys.Core.Relation.intersection of Relation (SetOfRelation $topic)>
...
=item C<sys.Core.Relation.join of Relation (QuasiSetOfRelation $topic)>
...
=item C<sys.Core.Relation.product of Relation (QuasiSetOfRelation $topic)>
...
=item C<sys.Core.Relation.composition of Relation (Relation $r1, Relation
$r2)>
...
=item C<sys.Core.Relation.substitution_in_semijoin of Relation (Relation
$topic, Relation $restr, Cat.NameChain $subst_func, Tuple $subst_assuming)>
...
=item C<sys.Core.Set.is_member of Bool (Set $set, ScaTupRel $value)>

This function results in C<Bool:true> iff its C<$value> argument matches
the sole attribute of a tuple of its C<$set> argument (that is, iff
...
=item C<sys.Core.Set.is_not_member of Bool (Set $set, ScaTupRel $value)>

This function is exactly the same as C<sys.Core.Set.is_member> except that
it results in the opposite boolean value when given the same arguments.

=item C<sys.Core.Set.insertion of Set (Set $set, ScaTupRel $value)>
...
is, conceptually the result is C<$value> inserted into C<$set>.  As a
trivial case, if C<$value> already exists in C<$set>, then the result is
just C<$set>.

=item C<sys.Core.Set.deletion of Set (Set $set, ScaTupRel $value)>
...
C<$value>; that is, conceptually the result is C<$value> deleted from
C<$set>.  As a trivial case, if C<$value> already doesn't exist in C<$set>,
then the result is just C<$set>.

=item C<sys.Core.Set.reduction of ScaTupRel (Set $topic, Cat.NameChain
$func, Tuple $assuming, ScaTupRel $identity)>
...
value given in C<$identity>.  I<Note that C<$identity> may be changed to
take a function name rather than a value, for consistency with C<$func>.>
This function will fail if the declared type of C<$identity> isn't a
subtype of the declared type of the sole attribute of C<$topic>.

=item C<sys.Core.Set.maybe_reduction of Maybe (Set $topic, Cat.NameChain
$func, Tuple $assuming)>
...
=item C<sys.Core.Set.Set_from_wrap of SetOfTuple (Relation $topic)>
...
=item C<sys.Core.Set.Set_from_attr of Set (Relation $topic, Cat.Name
$name)>
...
=item C<sys.Core.Maybe.attr_or_default of ScaTupRel (Tuple $topic)>
...
=item C<sys.Core.Maybe.attr_or_value of ScaTupRel (Tuple $topic, ScaTupRel
$value)>
...
=item C<sys.Core.Seq.value of ScaTupRel (Seq $topic, UInt $index)>

This function results in the scalar or nonscalar C<value> attribute of the
tuple of C<$topic> whose C<index> attribute is C<$index>.  This function
will fail if no tuple exists in C<$topic> with the specified index.

=item C<sys.Core.Seq.update_value of Seq (Seq $topic, UInt $index,
ScaTupRel $value)>
...
has been updated with a new scalar or nonscalar value given by C<$value>.
This function will fail if no tuple exists in C<$topic> with the specified
index, or if the declared type of C<$value> isn't a subtype of the declared
type of the C<value> attribute.

=item C<sys.Core.Seq.insertion of Seq (Seq $topic, UInt $index, ScaTupRel
$value)>
...
had theirs increased by 1.  As a trivial case, if C<$index> is equal to
...
=item C<sys.Core.Seq.deletion of Seq (Seq $topic, UInt $index)>
...
function will fail if no tuple exists in C<$topic> with the specified
index.

=item C<sys.Core.Seq.is_element of Bool (Seq $topic, ScaTupRel $value)>

This function results in C<Bool:true> iff its C<$value> argument matches
the C<value> attribute of at least one tuple of its C<$topic> argument
(that is, iff conceptually C<$value> is an element of C<$topic>), and
C<Bool:false> otherwise.  This function will fail if the declared type of
C<$value> isn't a subtype of the declared type of that attribute.

=item C<sys.Core.Seq.is_not_element of Bool (Seq $topic, ScaTupRel $value)>

This function is exactly the same as C<sys.Core.Seq.is_element> except that
it results in the opposite boolean value when given the same arguments.

=item C<sys.Core.Seq.reduction of ScaTupRel (Seq $topic, Cat.NameChain
$func, Tuple $assuming, ScaTupRel $identity)>
...
=item C<sys.Core.Seq.maybe_reduction of Maybe (Seq $topic, Cat.NameChain
$func, Tuple $assuming)>

This function is to C<sys.Core.Set.maybe_reduction> as
C<sys.Core.Seq.reduction> is to C<sys.Core.Set.reduction>.

=item C<sys.Core.Seq.catenation of Seq (SeqOfSeq $topic)>
...
=item C<sys.Core.Seq.repeat of Seq (Seq $topic, UInt $count)>

This function results in the catenation of C<$count> instances of
C<$topic>.

=item C<sys.Core.Seq.reverse of Seq (Seq $topic)>
...
=item C<sys.Core.Seq.is_subseq of Bool (Seq $look_in, Seq $look_for)>
...
=item C<sys.Core.Seq.is_not_subseq of Bool (Seq $look_in, Seq $look_for)>
...
=item C<sys.Core.Bag.cardinality of UInt (Bag $topic)>
...
=item C<sys.Core.Bag.is_member of Bool (Bag $bag, ScaTupRel $value)>

This function is the same as C<sys.Core.Set.is_member>, including that
matching of C<$value> is done against the C<value> attribute, except that
it works with a C<Bag> rather than a C<Set>.

=item C<sys.Core.Bag.is_not_member of Bool (Bag $bag, ScaTupRel $value)>

This function is exactly the same as C<sys.Core.Bag.is_member> except that
it results in the opposite boolean value when given the same arguments.

=item C<sys.Core.Bag.count of UInt (Bag $bag, ScaTupRel $value)>
...
=item C<sys.Core.Bag.insertion of Bag (Bag $bag, ScaTupRel $value)>

This function is the same as C<sys.Core.Set.insertion> as per C<is_member>
but that its result differs depending on whether C<$value> already exists
in C<$bag>; if it does, then no new tuple is added, but the C<count>
attribute for the matching tuple is increased by 1; if it does not, then a
new tuple is added where its C<value> is C<$value> and its C<count> is 1.

=item C<sys.Core.Bag.deletion of Bag (Bag $bag, ScaTupRel $value)>
...
=item C<sys.Core.Bag.reduction of ScaTupRel (Bag $topic, Cat.NameChain
$func, Tuple $assuming, ScaTupRel $identity)>
...
=item C<sys.Core.Bag.maybe_reduction of Maybe (Bag $topic, Cat.NameChain
$func, Tuple $assuming)>

This function is to C<sys.Core.Set.maybe_reduction> as
C<sys.Core.Bag.reduction> is to C<sys.Core.Set.reduction>.

=item C<sys.Core.Bag.Set_from_Bag of Set (Bag $topic)>

This function results in the C<Set> that is the projection of the C<value>
attribute of its C<Bag> argument.

=item C<sys.Core.Bag.Bag_from_Set of Bag (Set $topic)>
...
=item C<sys.Core.Bag.Bag_from_wrap of BagOfTuple (Relation $topic)>

This function results in a C<Bag> whose C<value> attribute is tuple-typed
and that attribute's values are all the tuples of C<$topic>; is a
...
=item C<sys.Core.Bag.Bag_from_attr of Bag (Relation $topic, Cat.Name
$name)>

This function results in a C<Bag> consisting of all the values of the
attribute of C<$topic> named by C<$name>.  It is a short-hand for first
doing a relational group on all attributes of C<$topic> besides C<$name> to
produce a new relation-typed attribute, and then extending the result of
the group with a new positive integer attribute whose values are the
...
=item C<sys.Core.Bag.is_subset of Bool (Bag $look_in, Bag $look_for)>
...
=item C<sys.Core.Bag.is_not_subset of Bool (Bag $look_in, Bag $look_for)>

This function is like C<sys.Core.Relation.is_not_subset> as per
C<is_subset>.

=item C<sys.Core.Bag.is_proper_subset of Bool (Bag $look_in, Bag
$look_for)>

This function is like C<sys.Core.Relation.is_proper_subset> as per
C<is_subset>.  I<TODO: What is its definition?>

=item C<sys.Core.Bag.is_not_proper_subset of Bool (Bag $look_in, Bag
$look_for)>

This function is like C<sys.Core.Relation.is_not_proper_subset> as per
C<is_subset>.  I<TODO: What is its definition?>

=item C<sys.Core.Bag.union of Bag (SetOfBag $topic)>
...
=item C<sys.Core.Bag.intersection of Bag (SetOfBag $topic)>
...
=item C<sys.Core.Bag.difference of Bag (Bag $source, Bag $filter)>
...
=item C<sys.Core.Cat.Order_(increase|same|decrease) of Cat.Order ()>
...
=item C<sys.Core.Cat.Order_reverse of Cat.Order (Cat.Order $topic)>

This function results in the reverse value of its argument; an
C<Cat.Order:increase> or C<Cat.Order:decrease> argument results in the other one of
the two; an C<Cat.Order:same> argument results in itself.
...
=item C<sys.Core.Universal.assign (UPD: Universal $target ; RO: Universal
$v)>
...
=item C<sys.Core.Control.fail (RO: Cat.Exception $topic)>
...
=item C<sys.Core.Control.try_catch (UPD: Tuple $try_updating, Tuple
$catch_updating ; RO: Cat.NameChain $try, Cat.NameChain $catch, Tuple
$try_assuming, Tuple $catch_assuming)>

This procedure invokes the procedure named in its C<$try> argument, giving
it the arguments C<$try_updating> and C<$try_assuming> as its C<$updating>
and C<$assuming> arguments, respectively.  If the C<$try> procedure throws
...
C<$topic> whose value is a C<Cat.Exception>; if the C<$catch> procedure

...
This document is Language::MuldisD::Core version 0.7.0.
...
Note that C<sys.Core.Empty.Empty> is a proper subtype of all of the other
types in this graph, but every other type has only one immediate supertype
shown, and hence the graph of them is a simple hierarchy.
...
=item C<sys.Core.Universal.is_value_of_type of Bool (RO: Cat.LongName
$type, Universal $v)>
...
=item C<sys.Core.Universal.default of Some.Universal (RO: Cat.LongName
$of)>
...
=item C<sys.Core.Ordered.min of Some.Ordered (RO: Set{Some.Ordered}
$topic)>
...
=item C<sys.Core.Ordered.max of Some.Ordered (RO: Set{Some.Ordered}
$topic)>
...
and its body contains every tuple that is in any of the input relations. If
C<$topic> has zero values, then C<union> results in the empty relation with

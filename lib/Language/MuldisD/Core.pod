...
This document is Language::MuldisD::Core version 0.16.0.
        ...
        sys.Core.QuasiRelation.QuasiRelation
            ...
            sys.Core.QuasiRelation.QuasiSet
                sys.Core.QuasiRelation.QuasiMaybe
                    ...
                    sys.Core.QuasiRelation.QuasiJust
...
A C<Just> is a proper subtype of C<Maybe> that has exactly 1 element.  Its
default value's only tuple's only element has the value C<Bool:false>.  The
C<Just> type is complementary to the C<Nothing> type under C<Maybe>;
C<Nothing> consists of exactly 1 of C<Maybe>'s values, and C<Just> consists
of all of C<Maybe>'s other values.
...
=head2 sys.Core.QuasiRelation.QuasiJust

A C<QuasiJust> is a proper subtype of C<QuasiMaybe> in the corresponding
manner to C<Just> being a proper subtype of C<Maybe>.  Its cardinality is
infinite.
...
A C<Cat.Func> has these 2 attributes:
...
A C<Cat.Updater> has these 3 attributes:
...
A C<Cat.Proc> has these 4 attributes:
...
=item C<function sys.Core.Ordered.is_increase_or_same result Bool params {
v1(Ordered), v2(Ordered) }>

This function is exactly the same as C<sys.Core.Ordered.is_decrease> except
that it results in the opposite boolean value when given the same
arguments.

=item C<function sys.Core.Ordered.is_decrease_or_same result Bool params {
v1(Ordered), v2(Ordered) }>

This function is exactly the same as C<sys.Core.Ordered.is_increase> except
that it results in the opposite boolean value when given the same
arguments.
...
=item C<function sys.Core.Maybe.just result Just params { value(ScaTupRel)
}>
...
=item C<function sys.Core.Maybe.attr result ScaTupRel params { topic(Just)
}>

This function results in the scalar or nonscalar value of the sole
attribute of the sole tuple of its argument, which always exists when the
argument is a C<Just>.

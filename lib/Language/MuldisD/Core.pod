...
specify both that all numerators are multiples of a particular I<radix>
(<PInt>), and that all denominators are positive powers of that same radix,
where the radix is usually either 2 or 10; types such as these will easily
map exactly to common human or physical numeric representations, so they

...
=item C<sys.rtn.Ordinal.min of Some.Ordinal (RO: Set{Some.Ordinal} $v)>
...
function's result.  If C<$v> has zero values, then C<min> results in the
result type's concept of positive infinity, which is the identity value for
I<min>.  This function will fail on a C<$v> of zero values if the result
...
=item C<sys.rtn.Ordinal.max of Some.Ordinal (RO: Set{Some.Ordinal} $v)>
...
=item C<sys.rtn.Blob.and of Blob (RO: Set{Blob} $v)>
...
of the function's result.  If C<$v> has zero values, then C<and> will
result in an appropriate-length string of identity/1 valued bits.

=item C<sys.rtn.Blob.or of Blob (RO: Set{Blob} $v)>
...
=item C<sys.rtn.Blob.xor of Blob (RO: Bag{Blob} $v)>
...
=item C<sys.rtn.Relation.is_empty of Bool (RO: Relation $v)>
...
=item C<sys.rtn.Relation.is_not_empty of Bool (RO: Relation $v)>
...
=item C<sys.rtn.Relation.not of Relation (RO: Relation $v)>
...
=item C<sys.rtn.Relation.restrict of Relation (RO: Relation $source,
Cat.LongName $filter, Tuple $assuming)>

This function results in the relational restriction of its C<$source>
argument as determined by applying the C<Bool>-returning function named in
its C<$filter> argument when the latter function is curried by its
C<$assuming> argument.  The result relation has the same heading as
C<$source>, and its body contains the subset of C<$source> tuples where,
for each tuple, the function named by C<$filter> results in C<Bool:true>
when passed the tuple as its C<$topic> argument and C<$assuming> as its
C<$assuming> argument.  As a trivial case, if C<$filter> is defined to
unconditionally result in C<Bool:true>, then this function results simply
in C<$source>; or, for an unconditional C<Bool:false>, this function
...
relations.  If C<$v> has zero values, then C<intersection> results in the
universal relation with the same heading (that is, the relation having all
the tuples that could ever exist in a relation with that heading), which is
the per-distinct-heading identity value for relational intersection.  Note
that this I<intersection> operator is conceptually a special case of
C<join>, applicable when the headings of the inputs are the same, and the
other will produce the same result as this when given the same inputs, but
with the exception that I<intersection> has a different identity value when
given zero inputs.  This function will fail on a C<$v> of zero values if
the result type's universal relation is impossible or impractically large
to represent, such as when any attributes are of infinite types.

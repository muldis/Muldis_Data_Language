...
explicit count of occurrences for C<value>.  Its default value has zero
tuples.
...
=item C<sys.Core.Tuple.attr_from_Tuple of Some.Universal (Tuple $topic)>
...
attribute's declared type is the same as that of C<$value>.  This function
will fail if its argument is not of a scalar or nonscalar type.

=item C<sys.Core.Tuple.extract_attr of Some.Universal (Tuple $topic,
Cat.Name $name)>
...
transformations that otherwise would require the chaining of up to a

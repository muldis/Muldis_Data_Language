...
This document is Language::MuldisD::Core version 0.6.0.
...
as the declared parameter types of various core operators; but they are not
specific to defining the system catalog:
    ...
    sys.type.Universal
        ...
        sys.type.Relation
            sys.type.Set
                ...
                sys.type.SetOfRelation

                sys.type.Maybe

                    # The following are all regular maybe types.

                    sys.type.MaybeOfBool
                    sys.type.MaybeOfInt
                    sys.type.MaybeOfRat
                    sys.type.MaybeOfBlob
                    sys.type.MaybeOfText
        ...
        sys.type.QuasiRelation
            sys.type.QuasiSet
                ...
                sys.type.QuasiSetOfRelation

Note that, in later operator documentation, if you see something like
C<Foo{Bar}> as a declared type, it corresponds to C<sys.type.FooOfBar>.
...
These system-defined subtypes are specific to defining the system catalog:
    ...
    sys.type.Universal
        ...
        sys.type.Relation
            ...
            sys.type.ShortNameMap
            sys.type.BiDiShortNameMap

            sys.type.Set

                # The following are all regular set types.

                sys.type.SetOfShortName
                sys.type.SetOfShortNameMap
...
routine that is used on 2 values of a C<sys.type.Ordered> subtype.  Its
default value is C<Order:same>; its minimum and maximum values are,
...
various core operators; but they are not specific to defining the system
catalog.

=over

=item C<sys.type.PInt1_4>
...
=item C<sys.type.(Set|Maybe|Seq|Bag)Of(Bool|Int|Rat|Blob|Text)>
...
=item C<sys.type.(Set|Bag)OfRelation>
...
=item C<sys.type.QuasiSetOfRelation>
...
=head2 C<sys.type.ScalarLiteral>

A C<ScalarLiteral> is a union type over all the system-defined scalar
...
C<Rat>, C<Blob>, C<Text>, C<ShortName>, C<LongName>, C<E.EK>,
...

=head2 C<sys.type.ShortName>

A C<ShortName> is a canonical name for some DBMS entities (others use
C<LongName>), such as the declared names of attributes of scalar
...
C<LongName> consists of a sequence (1..N) of C<ShortName>, the
elements being ordered from greatest to least significance.  Its default
value is a single element that is the default value of C<ShortName>,
the empty string.

=head2 C<sys.type.E.EK>

A C<E.EK> (expression kind) is an enumeration consisting of the 8
values C<E.EK:default> (default value of expression's type),
C<E.EK:(scalar|tuple|relation|quasi_tuple|quasi_relation)> (hard-coded
literal scalar|tuple|relation|quasi-tuple|quasi-relation value),
C<E.EK:param> (value of expression-containing function parameter),
C<E.EK:func> (result of function invocation, or inlining of function
body).  Its default value is C<E.EK:scalar>.
...
=head2 C<sys.type.Expr>

A C<Expr> is a C<Tuple>.  It specifies a named expression node, which
...
composed of just C<Expr>, either directly, or indirectly by way of
function invocations, as each function body is itself composed entirely of
a single expression tree.  Only functions may contain C<Expr>, so for
...
A C<Expr> has these 8 attributes:

=over

=item C<name> - C<ShortName>
...
=item C<scal_lit> - C<ScalarLiteral>

Iff C<kind> is C<E.EK.scalar>, then the expression represents a
...
=item C<coll_lit> - C<SetOfShortNameMap>

Iff C<kind> is C<E.EK.(|quasi_)(tuple|relation)>, then the expression
...
another local C<Expr> which defines the value.  The value of
...
=item C<param> - C<ShortName>

Iff C<kind> is C<E.EK.param>, then the expression represents the value
of the containing function's parameter which this names.

=item C<func> - C<LongName>

Iff C<kind> is C<E.EK:func>, then the expression represents the result
of invoking a named function with specific arguments, and this the name of
that function.

=item C<func_args> - C<ShortNameMap>
...
Note that whenever a C<Expr> attribute isn't significant, given the
expression type, and should be ignored, its value is the default for its
type.

The default value of C<Expr> represents the literal scalar value
C<Bool:false>.
...
=head2 C<sys.type.ShortNameMap>

A C<ShortNameMap> specifies a map of short entity names to other short
...
C<ShortName>; the C<key> attribute is a unary key.  Its default value
has zero tuples.

=head2 C<sys.type.BiDiShortNameMap>

A C<BiDiShortNameMap> is a proper subtype of C<ShortNameMap> where
its C<value> attribute is also a unary key.  It is used as a specification
for how to rename attributes of a relation.

=head2 C<sys.type.SetOfShortName>

A C<SetOfShortName> is a C<(Set)> whose C<value> attribute has a
declared type of C<ShortName>.

=head2 C<sys.type.SetOfShortNameMap>

A C<SetOfShortNameMap> is a C<(Set)> whose C<value> attribute has a
declared type of C<ShortNameMap>.
...
=item C<sys.rtn.Universal.is_equal of Bool (RO: Some.Universal $v1,
Some.Universal $v2)>
...
=item C<sys.rtn.Universal.is_not_equal of Bool (RO: Some.Universal $v1,
Some.Universal $v2)>

This function is exactly the same as C<sys.rtn.Universal.is_equal> except
that it results in the opposite boolean value when given the same
arguments.

=item C<sys.rtn.Universal.is_value_of_type of Bool (RO: LongName $type,
Universal $v)>
...
=item C<sys.rtn.Universal.treat of Some.Universal (RO: LongName $as,
Some.Universal $v)>
...
=item C<sys.rtn.Universal.default of Some.Universal (RO: LongName $of)>
...
=item C<sys.rtn.Ordered.compare of Order (RO: Some.Ordered $v1,
Some.Ordered $v2)>
...
=item C<sys.rtn.Ordered.reverse_compare of Order (RO: Some.Ordered $v1,
Some.Ordered $v2)>

This function is exactly the same as C<sys.rtn.Ordered.compare> except that
...
C<sys.rtn.Ordered.compare> with the same arguments.

=item C<sys.rtn.Ordered.is_increase of Bool (RO: Some.Ordered $v1,
Some.Ordered $v2)>

This function results in C<Bool:true> iff C<sys.rtn.Ordered.compare> would
result in C<Order:increase> when given the same arguments, and
C<Bool:false> otherwise.

=item C<sys.rtn.Ordered.is_decrease of Bool (RO: Some.Ordered $v1,
Some.Ordered $v2)>
...
=item C<sys.rtn.Ordered.min of Some.Ordered (RO: Set{Some.Ordered} $topic)>
...
=item C<sys.rtn.Ordered.max of Some.Ordered (RO: Set{Some.Ordered} $topic)>

This function is exactly the same as C<sys.rtn.Ordered.min> except that it
...
=item C<sys.rtn.Ordered.maybe_min of Maybe{Some.Ordered} (RO:
...
=item C<sys.rtn.Ordered.maybe_max of Maybe{Some.Ordered} (RO:
Set{Some.Ordered} $topic)>

This function is to C<sys.rtn.Ordered.max> as C<sys.rtn.Ordered.maybe_min>
is to C<sys.rtn.Ordered.min>.
...
=item C<sys.rtn.Bool.not of Bool (RO: Bool $topic)>

This function results in the logical I<not> of its argument.

=item C<sys.rtn.Bool.and of Bool (RO: Set{Bool} $topic)>
...
=item C<sys.rtn.Bool.or of Bool (RO: Set{Bool} $topic)>
...
=item C<sys.rtn.Bool.xor of Bool (RO: Bag{Bool} $topic)>
...
=item C<sys.rtn.Order.reverse of Order (RO: Order $topic)>
...
=item C<sys.rtn.Int.abs of UInt (RO: Int $topic)>
...
=item C<sys.rtn.Int.maybe_quotient of Maybe{Int} (RO: Int $dividend, Int
$divisor)>

This function is exactly the same as C<sys.rtn.Int.quotient> except that it
results in a C<Maybe> of what is otherwise the result, and that result has
zero elements if C<$divisor> is zero.

=item C<sys.rtn.Int.maybe_remainder of Maybe{UInt} (RO: Int $dividend, Int
$divisor)>

This function is exactly the same as C<sys.rtn.Int.remainder> except that
it results in a C<Maybe> of what is otherwise the result, and that result
has zero elements if C<$divisor> is zero.

=item C<sys.rtn.Int.range of Int (RO: Set{Int} $topic)>
...
=item C<sys.rtn.Int.median of Set{Int} (RO: Bag{Int} $topic)>
...
middle values, if the count of input values is even.  If C<$topic> has zero
values, then the result set is empty.

=item C<sys.rtn.Int.mode of Set{Int} (RO: Bag{Int} $topic)>
...
=item C<sys.rtn.Int.power of Int (RO: Int $radix, UInt $exponent)>
...
=item C<sys.rtn.Int.Int_from_NEText of Int (RO: NEText $text, PInt2_36
$radix)>
...
=item C<sys.rtn.Int.NEText_from_Int of NEText (RO: Int $int, PInt2_36
$radix)>
...
=head2 Functions for sys.type.Rat
...
=item C<sys.rtn.Rat.Rat_from_Int_pair of Rat (RO: Int $numerator, PInt
$denominator)>
...
=item C<sys.rtn.Rat.numerator of Int (RO: Rat $topic)>
...
=item C<sys.rtn.Rat.denominator of PInt (RO: Rat $topic)>
...
=item C<sys.rtn.Rat.abs of URat (RO: Rat $topic)>

This function results in the absolute value of its argument.

=item C<sys.rtn.Rat.sum of Rat (RO: Bag{Rat} $addends)>
...
=item C<sys.rtn.Rat.difference of Rat (RO: Rat $minuend, Rat $subtrahend)>

This function results in the difference when its C<$subtrahend> argument is
subtracted from its C<$minuend> argument.

=item C<sys.rtn.Rat.product of Rat (RO: Bag{Rat} $factors)>
...
=item C<sys.rtn.Rat.quotient of Rat (RO: Rat $dividend, Rat $divisor)>

This function results in the quotient when its C<$dividend> argument is
divided by its C<$divisor> argument using rational division.  This function
will fail if C<$divisor> is zero.

=item C<sys.rtn.Rat.maybe_quotient of Maybe{Rat} (RO: Rat $dividend, Rat
$divisor)>

This function is exactly the same as C<sys.rtn.Rat.quotient> except that it
results in a C<Maybe> of what is otherwise the result, and that result has
zero elements if C<$divisor> is zero.

=item C<sys.rtn.Rat.range of Rat (RO: Set{Rat} $topic)>

This function results in the difference between the lowest and highest
element values of its argument.  If C<$topic> has zero values, then
C<range> results in the rational zero.

=item C<sys.rtn.Rat.mean of Rat (RO: Bag{Rat} $topic)>
...
=item C<sys.rtn.Rat.maybe_mean of Maybe{Rat} (RO: Bag{Rat} $topic)>

This function is exactly the same as C<sys.rtn.Rat.mean> except that it
results in a C<Maybe> of what is otherwise the result, and that result has
zero elements if C<$topic> has zero values.

=item C<sys.rtn.Rat.median of Set{Rat} (RO: Bag{Rat} $topic)>
...
=item C<sys.rtn.Rat.mean_of_median of Rat (RO: Bag{Rat} $topic)>

This function is a wrapper over C<sys.rtn.Rat.median> that will return the
mean of its result elements; it will fail if there are zero elements.

=item C<sys.rtn.Rat.mode of Set{Rat} (RO: Bag{Rat} $topic)>
...
=item C<sys.rtn.Rat.power of PRat (RO: PRat $radix, Rat $exponent)>
...
=item C<sys.rtn.Rat.log of Rat (RO: PRat $topic, PRat $radix, PInt
$rebase_radix, PInt $rebase_max_denom, RoundMeth $rebase_round)>
...
parameters of the C<sys.rtn.Rat.rebase> function.

=item C<sys.rtn.Rat.natural_power of PRat (RO: Rat $exponent, PInt
$rebase_radix, PInt $rebase_max_denom, RoundMeth $rebase_round)>
...
=item C<sys.rtn.Rat.natural_log of Rat (RO: PRat $topic, PInt
$rebase_radix, PInt $rebase_max_denom, RoundMeth $rebase_round)>
...
=item C<sys.rtn.Rat.Rat_from_NEText of Rat (RO: NEText $text, PInt2_36
$radix)>
...
=item C<sys.rtn.Rat.NEText_from_Rat of NEText (RO: Rat $rat, PInt2_36
$radix)>
...
=item C<sys.rtn.Rat.Rat_from_Int of Rat (RO: Int $int)>

This selector function results in the C<Rat> value that is conceptually
equal to its C<Int> argument.

=item C<sys.rtn.Rat.round_half_up of Int (RO: Rat $topic)>
...
=item C<sys.rtn.Rat.round_to_even of Int (RO: Rat $topic)>
...
=item C<sys.rtn.Rat.round_to_floor of Int (RO: Rat $topic)>
...
=item C<sys.rtn.Rat.round_to_ceiling of Int (RO: Rat $topic)>

This rounding function results in the integer that is either equal to its
argument or that is the nearest integer that is below the argument.

=item C<sys.rtn.Rat.round_to_zero of Int (RO: Rat $topic)>
...
=item C<sys.rtn.Rat.rebase of Rat (RO: Rat $topic, PInt $radix, PInt
$max_denom, RoundMeth $round)>
...
=item C<sys.rtn.Blob.catenate of Blob (RO: Seq{Blob} $topic)>
...
=item C<sys.rtn.Blob.repeat of Blob (RO: Blob $topic, UInt $count)>

This function results in the catenation of C<$count> instances of
C<$topic>.

=item C<sys.rtn.Blob.length_in_bits of UInt (RO: Blob $topic)>
...
=item C<sys.rtn.Blob.not of Blob (RO: Blob $topic)>

This function results in the bitwise I<not> of its argument.

=item C<sys.rtn.Blob.and of Blob (RO: Set{Blob} $topic)>
...
=item C<sys.rtn.Blob.or of Blob (RO: Set{Blob} $topic)>

This function is the same as C<sys.rtn.Blob.and> but that it recursively
does a bitwise inclusive-or rather than a bitwise I<and>, and its identity
value is composed of zero valued bits.

=item C<sys.rtn.Blob.xor of Blob (RO: Bag{Blob} $topic)>

This function is the same as C<sys.rtn.Blob.or> but that it recursively
...
=item C<sys.rtn.Blob.Blob_from_Text of Blob (RO: Text $text, PInt1_4
$size)>
...
=item C<sys.rtn.Blob.Text_from_Blob of Text (RO: Blob $blob, PInt1_4
$size)>
...
=item C<sys.rtn.Text.catenate of Text (RO: Seq{Text} $topic)>
...
=item C<sys.rtn.Text.repeat of Text (RO: Text $topic, UInt $count)>

This function results in the catenation of C<$count> instances of
C<$topic>.

=item C<sys.rtn.Text.length_in_nfd_graphs of UInt (RO: Text $topic)>

This function results in the length of its argument in Unicode
Normalization Form D graphemes.

=item C<sys.rtn.Text.length_in_nfc_graphs of UInt (RO: Text $topic)>

This function results in the length of its argument in Unicode
Normalization Form C graphemes.

=item C<sys.rtn.Text.length_in_nfd_codes of UInt (RO: Text $topic)>

This function results in the length of its argument in Unicode
Normalization Form D code points.

=item C<sys.rtn.Text.length_in_nfc_codes of UInt (RO: Text $topic)>
...
=item C<sys.rtn.Text.fold_case_to_upper (RO: Text $topic)>

This function results in the transformation of its argument where any Latin
letters are folded to (capital) uppercase.

=item C<sys.rtn.Text.fold_case_to_lower (RO: Text $topic)>

This function results in the transformation of its argument where any Latin
letters are folded to (small) lowercase.

=item C<sys.rtn.Text.trim_whitespace (RO: Text $topic)>
...
=head2 Functions for sys.type.Tuple Having Single Input Tuples

=over

=item C<sys.rtn.Tuple.degree of UInt (RO: Tuple $topic)>

This function results in the degree of its argument (that is, the count of
attributes it has).

=item C<sys.rtn.Tuple.rename of Tuple (RO: Tuple $topic,
BiDiShortNameMap $map)>
...
with the C<key> and C<value> attributes of a C<$map> tuple representing
...
=item C<sys.rtn.Tuple.project of Tuple (RO: Tuple $topic,
SetOfShortName $attrs)>
...
=item C<sys.rtn.Tuple.remove of Tuple (RO: Tuple $topic, SetOfShortName
$attrs)>
...
=item C<sys.rtn.Tuple.wrap of Tuple (RO: Tuple $topic, SetOfShortName
...
=item C<sys.rtn.Tuple.unwrap of Tuple (RO: Tuple $topic, ShortName
$outer)>
...
=head2 Functions for sys.type.Tuple Having Multiple Input Tuples

=over

=item C<sys.rtn.Tuple.product of Tuple (RO: QuasiSet{Tuple} $topic)>

This function is similar to C<sys.rtn.Relation.product> but that it works
...
=head2 Functions for sys.type.Relation Having Single Input Relations

=over

=item C<sys.rtn.Relation.degree of UInt (RO: Relation $topic)>

This function results in the degree of its argument (that is, the count of
attributes it has).

=item C<sys.rtn.Relation.cardinality of UInt (RO: Relation $topic)>

This function results in the cardinality of its argument (that is, the
count of tuples its body has).

=item C<sys.rtn.Relation.is_empty of Bool (RO: Relation $topic)>
...
=item C<sys.rtn.Relation.is_not_empty of Bool (RO: Relation $topic)>
...
=item C<sys.rtn.Relation.Tuple_from_Relation of Tuple (RO: Relation
$topic)>
...
=item C<sys.rtn.Relation.Relation_from_Tuple of Relation (RO: Tuple
$topic)>
...
=item C<sys.rtn.Relation.evacuate of Relation (RO: Relation $topic)>

This function results in a C<Relation> that has the same heading as its
argument, but with an empty body.

=item C<sys.rtn.Relation.not of Relation (RO: Relation $topic)>
...
=item C<sys.rtn.Relation.rename of Relation (RO: Relation $topic,
BiDiShortNameMap $map)>
...
=item C<sys.rtn.Relation.project of Relation (RO: Relation $topic,
SetOfShortName $attrs)>
...
=item C<sys.rtn.Relation.remove of Relation (RO: Relation $topic,
SetOfShortName $attrs)>

This function is the same as C<sys.rtn.Tuple.remove> but that it operates
on and results in a C<Relation> rather than a C<Tuple>.

=item C<sys.rtn.Relation.wrap of Relation (RO: Relation $topic,
SetOfShortName $inner, ShortName $outer)>
...
=item C<sys.rtn.Relation.unwrap of Relation (RO: Relation $topic,
ShortName $outer)>

This function is the inverse of C<sys.rtn.Relation.wrap> as
C<sys.rtn.Tuple.unwrap> is to C<sys.rtn.Tuple.wrap>.

=item C<sys.rtn.Relation.group of Relation (RO: Relation $topic,
SetOfShortName $inner, ShortName $outer)>

I<TODO.>

=item C<sys.rtn.Relation.ungroup of Relation (RO: Relation $topic,
ShortName $outer)>

I<TODO.>

=item C<sys.rtn.Relation.tclose of Relation (RO: Relation $topic)>
...
=item C<sys.rtn.Relation.restrict of Relation (RO: Relation $topic,
LongName $func, Tuple $assuming)>
...
C<$topic>, and its body contains the subset of C<$topic> tuples where,
for each tuple, the function named by C<$func> results in C<Bool:true>
...
in C<$topic>; or, for an unconditional C<Bool:false>, this function
results in the empty relation with the same heading.  Note that this
operation is also legitimately known as I<where>.  Note that
C<sys.rtn.Relation.semijoin> is recommended for use instead of
C<sys.rtn.Relation.restrict> to implement some common kinds of relational
...
=item C<sys.rtn.Relation.extend of Relation (RO: Relation $topic,
LongName $func, Tuple $assuming)>
...
=head2 Functions for sys.type.Relation Having Multiple Input Relations
...
=item C<sys.rtn.Relation.union of Relation (RO: Set{Relation} $topic)>
...
=item C<sys.rtn.Relation.exclusion of Relation (RO: Bag{Relation} $topic)>
...
=item C<sys.rtn.Relation.intersection of Relation (RO: Set{Relation}
$topic)>
...
=item C<sys.rtn.Relation.join of Relation (RO: QuasiSet{Relation} $topic)>
...
=item C<sys.rtn.Relation.product of Relation (RO: QuasiSet{Relation}
$topic)>
...
=item C<sys.rtn.Relation.compose of Relation (RO: Relation $r1, Relation
$r2)>
...
=item C<sys.rtn.Relation.quotient of Relation (RO: Relation $dividend,
Relation $divisor)>

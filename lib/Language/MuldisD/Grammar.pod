...
This document is Language::MuldisD::Dialect::PTMD_Tiny version 0.11.0.
...
This document outlines the grammar of I<Plain Text Muldis D>.  This grammar
...
Whether or not these literals are valid is up to the I<Hosted Data Muldis D>
compiler, whose input is the AST resulting from parsing I<Plain Text Muldis
D>.  This grammar primarily tests for well-formedness.

See also, for example, L<Language::MuldisD::Dialect::HDMD_Perl_Tiny> for
a description of the core Perl data structures that you would probably
use instead of the grammar below when using a Muldis D implementation in
a Perl application; moreover, that other document is necessary to
understand the meaning of some parts of this PTMD_Tiny document.

I<This documentation is pending.>

Plain Text Muldis D is the natural form that one would code in if they were
...
desired to be implemented in the near future).  Plain Text Muldis D would
...
correctly.  Any remaining errors should be corrected as soon as possible.
    ...
    # grammar Muldis_D:auth<cpan:DUNCAND>:ver<0.11.0>;
    grammar Muldis_D-0.11.0-cpan:DUNCAND;
    ...
    token relation {
          <generic_relation_empty_body>
        | <generic_relation_with_tuples>
        | <set>
        | <nothing>
        | <single>
        | <seq>
    ...
    token generic_relation_empty_body {
        (Relation)
        <spec_sep>
        <list_open>
            (((<cat_name>) <list_sep>)*)
    ...
    token generic_relation_with_tuples {
        (Relation)
        <spec_sep>
        <list_open>
            (((<tuple_body>) <list_sep>)*)
...
The following are fragments of actual Plain Text Muldis D code.
    ...
    Muldis_D:'cpan:DUNCAND':'1.2.3':{ Bool:false }

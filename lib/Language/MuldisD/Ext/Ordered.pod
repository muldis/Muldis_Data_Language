...
This document is Language::MuldisD::Ext::Ordered version 0.19.1.
...
=item C<function sys.Ordered.Ordered.is_before result Bool params {
...
=item C<function sys.Ordered.Ordered.is_not_before result Bool params {
topic(Universal), other(Universal) }>

This function is exactly the same as C<sys.Ordered.Ordered.is_before>
except that it results in the opposite boolean value when given the same
arguments.  (It could alternately be called "is after or is identical".)

=item C<function sys.Ordered.Ordered.is_inside_range result Bool params {
...
types as per C<sys.Ordered.Ordered.is_before>.  This function will fail if
C<max> is before C<min>.

=item C<function sys.Ordered.Ordered.is_outside_range result Bool params {
topic(Universal), min(Universal), max(Universal), min_is_inside(Bool),
max_is_inside(Bool) }>

This function is exactly the same as C<sys.Ordered.Ordered.is_inside_range>
except that it results in the opposite boolean value when given the same
arguments.

=item C<function sys.Ordered.Ordered.min result Universal params {
topic(Set) }>
...
=item C<function sys.Ordered.Ordered.max result Universal params {
topic(Set) }>

This function is exactly the same as C<sys.Ordered.Ordered.min> except that
it results in the maximum input element value rather than the minimum one,
and its identity value is the result type's concept of negative infinity.

=item C<function sys.Ordered.Ordered.maybe_min result Maybe params {
topic(Set) }>

This function is exactly the same as C<sys.Ordered.Ordered.min> except that
it results in a C<Maybe> of what is otherwise the result type, and that
result has zero elements if the argument has zero elements.

=item C<function sys.Ordered.Ordered.maybe_max result Maybe params {
topic(Set) }>

This function is to C<sys.Ordered.Ordered.max> as
C<sys.Ordered.Ordered.maybe_min> is to C<sys.Ordered.Ordered.min>.

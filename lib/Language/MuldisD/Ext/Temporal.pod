...
This document is Language::MuldisD::Ext::Temporal version 0.3.0.
...
type is infinity; to define a most-generalized finite C<Temporal.DateTime>
...
C<Temporal.Time> subtype, you must specify its least magnitude fraction of
a second.
...
difference between two C<Temporal.DateTime> values, and is likewise assumed
...
most-generalized finite C<Temporal.Duration> subtype, you must specify the
...
cardinality of this type is infinity; to define a most-generalized finite
C<Temporal.DurationOfDays> subtype, you must specify the minimum and
...
=item C<sys.type.Temporal.DurationNoTZ>

A C<Temporal.DurationNoTZ> is the same as C<Temporal.Duration> in all
...
C<Temporal.DateTimeNoTZ> values.  Matters of its cardinality are the same
as for C<Temporal.Duration>.

=item C<sys.type.Temporal.DurationOfDaysNoTZ>

A C<Temporal.DurationOfDaysNoTZ> is to C<Temporal.DurationOfDays> what
C<Temporal.DurationNoTZ> is to C<Temporal.Duration>.

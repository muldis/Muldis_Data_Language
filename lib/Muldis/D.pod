...
control of other parties.  This being said, B<Muldis D> actually includes 2
syntax/grammar specifications, meaning 4 in total.

For the official Muldis D by authority C<http://muldis.com>, the default 4
...
C<Muldis_D_Foundation 'http://muldis.com' '0.201.0.-9'>.

=item *

B<Muldis D Entry Notation> (B<MDEN>) -
C<Muldis_D_Entry_Notation 'http://muldis.com' '0.201.0.-9'>.

=item *

B<Muldis D Plain Text> (B<MDPT>) -
C<Muldis_D_Plain_Text 'http://muldis.com' '0.201.0.-9'>.

=item *

B<Muldis D Standard Library> (B<MDSL>) -
C<Muldis_D_Standard_Library 'http://muldis.com' '0.201.0.-9'>.

=back

In this context, the B<Muldis D> language aggregate specification is mostly
just a top-level catalogue pointing to those 4 primary components, as well
...
The B<Muldis D Entry Notation> specification defines the grammar of the
...
However, B<Muldis D Entry Notation> is the only syntax for which support by
...
being much more likely to be stable over time.  B<Muldis D Entry Notation>
is intended to be the primary standard format of interchange of both code
and data between all Muldis D implementations.  B<Muldis D Entry Notation>
...
See L<Muldis::D::Entry_Notation> for the B<Muldis D Entry Notation> specification.

The B<Muldis D Plain Text> specification defines the grammar of the
larger of the two official concrete Muldis D language syntaxes that every
non-trivial Muldis D implementation should reasonably support as options.
However, B<Muldis D Plain Text> is the official syntax for which support by
every Muldis D implementation is merely considered highly recommended
rather than mandatory; this is justified partly by that any implementation
that supports the mandatory B<Muldis D Entry Notation> can bootstrap
B<Muldis D Plain Text> support over it.  B<Muldis D Plain Text> is intended
to be the secondary standard format of interchange of both code and data
between all Muldis D implementations.
...
name of a concrete grammar it conforms to (either a B<MDEN> or B<MDPT>
version or alternate), except that this concept doesn't apply to the B<MDF>
...
fundamental dependency of its own kind.  In the cases of each of the
B<MDEN> and B<MDPT>, its name is also referred to by each source code
written in that syntax, so it is explicitly clear on how to parse that code.
...
The expected fully-qualified name of a B<Muldis D Entry Notation> or
B<Muldis D Plain Text> version as
...
character strings C<Muldis_D> and C<Entry_Notation> respectively, for
B<MDEN>; the I<family name> and I<syntax name> are simply the 2
...
Note that a leading MDEN/MDPT declaration in source code should be effective
as a "magic number" for identifying it as being MDEN/MDPT source code.
...
C<Muldis_D>, C<Muldis_D_Foundation>, C<Muldis_D_Entry_Notation>,
C<Muldis_D_Plain_Text>, and C<Muldis_D_Standard_Library>.
...
base HTTP url belonging to the authority (example C<http://muldis.com>) or
a qualified user identifier at some well-known compilation unit repository
(example C<http://github.com/muldis> or C<cpan:DUNCAND>).

For all official/original works by Muldis Data Systems, Inc., the
I<authority> has always been C<http://muldis.com> and is expected to remain
...
A I<version number> for authority C<http://muldis.com> is an ordered
sequence of integers, the order of these being from most significant to
least, with 4 positions [MAJOR,MINOR,PATCH,MATURITY] and further ones
...
while MATURITY may be any integer.  MAJOR is always (except when
...
MATURITY must be a negative integer for a pre-production version
(experimental, alpha, beta, RC, etc) and be a non-negative integer for a
production version.  Typically the magnitude of MATURITY indicates
perceived stability; zero is the default for production while a larger
positive number can suggest greater confidence in quality; for
pre-production it is suggested that a release candidate is negative-one,
and other states such as beta are successively more negative.  A toolchain
for installing or upgrading packages should default to ignoring any whose
I<version number> has a negative integer in any position for users' safety.
...
should have received copies of the AL2 as part of the Muldis D

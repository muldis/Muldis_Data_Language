...
An enumeration contract associated with a I<singleton type> is
...
reference to or a definition of an enumeration contract is allowed, a
...
general form enumeration contract with an omitted I<S1>, a I<P1> that tests
its argument for equality with I<V1>, and a default value of I<V1>.
...
expression, and similarly, the function call's argument must be defined by
a value expression.  The ultimate roots of value expression call chains are
typically procedure statements.
...
means to define an enumeration contract associated with a singleton type;

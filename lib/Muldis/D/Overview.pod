...
Muldis::D::Overview -
High-Level View of Muldis D Plain Text

=head1 VERSION

This document is Muldis::D::Overview version 0.200.1.
...
This document provides a high-level view of the B<Muldis D Plain Text>
language, or I<Muldis D> less formally.  It provides the basics of how the
language is designed and works, as a foundation upon which to understand
the other parts of the language spec.
...
(not valid for the I<empty type>).  Note that the Muldis D Plain Text
keyword C<type> actually declares a I<contract>, despite its contrary name;
for all practical purposes its users are talking about its associated type.

Every I<contract> is of exactly 1 of 2 kinds based on how it is declared;
it is either an I<enumeration contract> or an I<interface contract>; these
two declaration methods are polar opposites of each other.

Each given I<enumeration contract>, I<E1>, explicitly declares for itself
what its associated type / value set is.  The form this takes is a set of
(typically exactly one) other contracts' names, I<S1>, plus a predicate
expression, I<P1>.  The type of I<E1> is defined as taking the union of the
types of I<S1> as source values and then applying I<P1> as a filter; the
type of I<E1> consists of just the values for which I<P1> results in
I<true>.  Having a I<S1> is optional; the value source of an I<E1> without
a I<S1> is implicitly the universal type.  Having a I<P1> is also optional;
for an I<E1> without a I<P1>, its implicit predicate is unconditionally
I<true>.  The canonical way to associate an I<E1> with the universal type
...
enumeration contract, I<E1>, may explicitly declare this value for itself;
if I<E1> does not do this, then the default value of its immediate sources
is implicitly inherited as its own; if the set of default values of the
direct sources of I<E1>, after being filtered by the predicate of I<E1>, is
...
latter may be chained, but ultimately each end composer must be an
I<enumeration contract> as only they can originate default values.  For
example, one may have 2 interface contracts and an enumeration contract to
...
argument I<A1> of the type associated with I<I1>.  For each given composing
type I<S1> of I<I1>, for each virtual routine I<R1>, there must exist a
(possibly virtual) routine I<R2> which explicitly declares that it
I<implements> I<R1>, where the argument I<A2> corresponding to I<A1> is of
the type associated with I<S1>.  That is, any contract composing an
interface contract must implement the latter's required interface; failing
to do so is an error.

And so, regardless of whether types are manifested in a program by way of a
an enumeration contract or an interface contract, users can be confident
...
The primary determinant for whether you would declare a contract as an
enumeration contract or an interface contract is whether you want the
contract's definition to be closed, or open, respectively.  If you use an
enumeration contract, then assuming you have control over all the types it
...
call/invocation takes exactly 1 input I<argument> value and gives exactly 1
output I<result> value.  The result value of a function call is determined
entirely by its argument value, and a function is guaranteed to give
exactly the same single result value for every call with the same argument
value.  Multiple distinct arguments may yield the same result, but multiple
calls with the same argument never give different results.
...
the latter.  A function has no lexical variables at all, although they can
be faked for code factoring purposes by giving explicit names to its
component sub-expressions, the current value for each of which is
defined/set once within a function call but referenceable multiple times.
A function call, like any value expression in general, is logically an
atomic operation.  Value expressions are also conceptually lazy and can be
gated to only be evaluated conditionally.
...
expressions or called functions must explicitly return failure-indicating
...
A I<constant> is similar to a I<function> but that it takes no argument at
all and it always gives the same result.  A constant may be invoked
anywhere that a function may be, and a constant may invoke functions.  The
single value a constant denotes may be arbitrarily complex, and repeated
portions may be factored under their own names, like in a function.
...
package, or site external definition or content dependencies of such, with
...
I<TODO: Other related notes.  A folded option is idiomatic for a database
while not using that option is idiomatic for normal code.>

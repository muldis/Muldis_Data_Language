...
These are keywords within a package declaration that are external to any
...
Grammar:
    ...
    <package> ::=
        ...
        '{' <sp>
            <pkg_identity> <sp> ';' <sp>
            <pkg_foundation> <sp> ';' <sp>
            [<pkg_uses> <sp> ';' <sp>]?
            [<pkg_entry_point> <sp> ';' <sp>]?
    ...
    <pkg_entry_point> ::=
        entry <sp> <entry_point_kind> <sp> by <sp> <entry_point_rtn_name>
...
I<Note that "entry" is the "entry point" for a program.  It is NOT automatically
...
Grammar:
    ...
    <delimiting_expr> ::=
        '(' <sp> [
            <naming_expr> | <aliasing_expr> | <result_expr> | ''
        ] % [<sp> ';' <sp>] <sp> ')'
...
An C<< <Attr_Names> >> node represents a value of the Muldis D
C<Attr_Names> type, which is an arbitrarily-large unordered collection of
attribute names.  An C<< <Attr_Name> >> node represents a value of the Muldis
D C<Attr_Name> type, which is a subtype by constraint of the C<Attr_Names>
type; C<< <Attr_Name> >> provides a terser alternative syntax for the
common special case of C<< <Attr_Names> >> having exactly 1 attribute.
...
The C<Attr_Names> type is not a foundational type of the Muldis D type
system, but rather is a subtype by constraint of the C<Tuple> type, and all
C<Attr_Names> values can be selected in terms of C<< <Tuple> >> grammar
nodes.  However, C<< <Attr_Names> >> is the canonical grammar for all
C<Attr_Names> values, except for all C<Attr_Name> values, for which
...
=head2 Identifier and Reference Literals
...
I<TODO.  Note that Function_Name and Contract_Name are aliases for Reference.>
...
Grammar:
    ...
    <attr_commalist> ::=
        [<anon_attr> | <named_attr> | <same_named_attr> | ''] % [<sp> ',' <sp>]
...
Grammar:
    ...
    <Capsule> ::=
        '\\:' <sp> '(' <sp> <wrapper_expr> <sp> ',' <sp> <c_asset_expr> <sp> ')'
...
Grammar:
    ...
    <postcircumfixed_func_invo_expr> ::=
        <generic_func_name> <sp> '::'? <sp> '(' <sp> <attr_commalist> <sp> ')'

I<TODO.>

A C<< <generic_func_call> >> denotes a C<Function_Call> value, which pairs
a C<Function_Name> (C<Reference>) value naming a function (or constant or
...
I<< Also cover \<0 shorthand here or something much like "=" is. >>

I<TODO: FORGET NOT THE ps5_nonquoted_symbolic_grouping.>

=head2 Foundation Constant and Function Invocation Expressions

Grammar:

    <foundation_func_invo_expr> ::=
          [<generic_func_args> <sp> '-->^' <sp> <foundation_func_name>]
        | [<foundation_func_name> <sp> '^<--' <sp> <generic_func_args>]
...
A C<::=> is I<what-binding> while a C<::?=> is I<why-binding>.
    ...
    + - numbers or bags (and octets and tuple-bags)
    ...
    * - wildcard or "any" or "all" (example is \<*args expr node)
    ! - booleans or excuses
    ^ - raising something up - bootstrap on foundation, or attribute projection (or number to a power, but not in Core)
    . - collection item accessor or existence test
    = - association of 2 things, either for comparison or binding or assignment
    < - "from" or "readonly" or "before" or "LHS" or "first"
    > - "to" or "writeable" or "after" or "RHS" or "second"
...
Combining from/to eg as C<< \<>foo >> refers to variable or subject to update parameter.
...
I<TODO.  These are listed from tightest at the top to loosest at the bottom.>
    ...
    symbolic in/pre/postfix (bareword)
    alpha in/pre/postfix (bareword)

...
section in this file named L<Comment Literals>.
...
annotation is an internal code comment expressed as a C<Comment> literal,
...
A C<< <Comment> >> node represents a value of the Muldis D C<Comment> type,
...
The C<Comment> type is not a foundational type of the Muldis D type system, but
rather is a subtype by constraint of the C<Capsule> type, and all C<Comment>
values can be selected in terms of C<< <Capsule> >> grammar nodes.
However, C<< <Comment> >> is the canonical grammar for all C<Comment> values.
...
C<Comment> literals or quoted identifiers, and the valid formats for both

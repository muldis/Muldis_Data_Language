...
Note that C<whitespace> is meant to be more comprehensive and include
any character that Unicode considers to be whitespace.

I<TODO: Figure out how to say in grammar when (insignificant) dividing space is
only conditionally needed such as to mark the transition points between
otherwise-adjacent significant tokens where that can't be told otherwise.>
I<Clues would seem to be at http://doc.perl6.org/language/regexes#Look-around_assertions
and http://doc.perl6.org/language/regexes#Matching_Whitespace .>
...
consisting of multiple C<ps3_bracketing_char> into multiple tokens such that
...
consisting of multiple C<ps3_symbolic_char> into multiple tokens where those
...
string or nonquoted C<ps3_whitespace_char>, beneath a parse node
...
C<Blob> type, which are a particular run of C<ps3_alphanumeric_char>
...
C<ps3_alphanumeric_char> optionally with some C<ps3_symbolic_char>,
and further optionally split (when long) using dividing space.

Grammar:

    <ps10_numeric> ::=
        <[0..9]> <ps10_numeric_seg>? ['.' <ps10_numeric_seg>]?

    <ps10_numeric_seg> ::=
        <ps3_alphanumeric_char>+ % [<sp> ',,' <sp>]

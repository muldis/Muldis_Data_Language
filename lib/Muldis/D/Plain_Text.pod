...
Grammar:
    ...
    <generic_invo_expr> ::=
        <generic_const_invo_expr> | <generic_func_invo_expr>

    <generic_const_invo_expr> ::=
          ['(' <sp> '-->' <sp> <generic_const_ref> <sp> ')']
        | ['(' <sp> <generic_const_ref> <sp> '<--' <sp> ')']
...
A C<< <generic_[const|func]_ref> >> denotes a C<Reference> value naming a function;
...
Grammar:
    ...
    <pcf_invo_expr> ::=
        <pcf_const_invo_expr> | <pcf_func_invo_expr>

    <pcf_const_invo_expr> ::=
        <generic_const_name> <sp> '::'? <sp> '(' <sp> ')'
...
A C<< <pcf_invo_expr> >> is subject to the additional rule that, iff its
C<< <generic_*_name> >> element has no C<::> within itself, then that
element must have a trailing C<::> element.

A C<< <pcf_func_invo_expr> >> is subject to the additional rule that its
C<< <attr_commalist> >> must have at least 1 C<< <*_attr> >> element.
...
Grammar:
    ...
    <foundation_const_invo_expr> ::=
        ...
        | ['(' <sp> <foundation_const_name> <sp> '^<--' <sp> ')']

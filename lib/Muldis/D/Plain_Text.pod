...
Grammar:
    ...
    <generic_func_invo_expr> ::=
        ...
        | [<generic_func_ref> <sp> '<--' <sp> <generic_func_args>]
...
=head2 Postcircumfixed Constant and Function Invocation Expressions
...
A C<< <pcf_func_invo_expr> >> is subject to the additional rule that, iff
its C<< <generic_func_name> >> element has no C<::> within itself, then
that element must have a trailing C<::> element.

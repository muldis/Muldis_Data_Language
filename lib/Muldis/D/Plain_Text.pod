...
this is the canonical grammar for them.  Conceptually, but not actually, a
C<< <Boolean> >> node is prefixed by C<\?>.
...
grammar for them.  Conceptually, but not actually, an C<< <Integer> >> node
is always prefixed by C<\+>, whether quoted or not.
...
nominal.  Conceptually, but not actually, a C<< <Fraction> >> node is
always prefixed by C<\+>, whether quoted or not.
...
I<TODO.  A :: is optional for a constant call, mandatory for function call.>
...
I<FORGET NOT THE MANDATORY :: IN FOO::(...).>
...
A C<::=> is I<what-binding> while a C<::?=> is I<why-binding>.
    ...
    ~ - arrays (and text and bits and blob)

...
in the form of Blob/Bits vs Text values or files vs not files.>
...
This is effectively a "magic number" for identifying source code as MDPT.
...
When C<< <foundational_indicator> >> is present, the effective grammar is a
reasonably minimal subset capable of representing all Muldis D code in a
highly normalized manner, meaning that when the full Muldis D Plain Text
grammer gives multiple syntax options that all parse into the same
C<Package> values ignoring the optional I<decoration> attributes, just the
most generic of each set of alternate syntax options will be supported;
when C<< <foundational_indicator> >> is absent, the full Muldis D Plain
Text grammar is supported, albeit still limited by the character repertoire
specified by C<< <script_name> >>.

Iff C<< <foundational_indicator> >> is present, then the following grammar
definitions are in effect:

    <script_name> ::=
        <script_name__minimal>

    <regular_symbolic_char> ::=
        <regular_symbolic_char__minimal>

    <escaped_char> ::=
        <escaped_char__minimal>
...
following grammar definitions are in effect:
    ...
    <script_name> ::=
        <script_name__maximal>

    <regular_symbolic_char> ::=
        <regular_symbolic_char__maximal>

    <escaped_char> ::=
        <escaped_char__maximal>
...
have multiple parts, such as C<Unicode(9.0,UTF-8,canon)>, indicating
...
Examples:
    ...
    script Unicode:9.0:UTF-8:canon;
...
Otherwise, if C<< <foo> >> is C<:=> then C<< <sp> >> is allowed to be empty.
...
These are keywords within the outer areas of source code that are external
to the root package declaration:
...
These are keywords within the heading of a material declaration:
...
including the headings of nested material declarations:
...
Grammar:
    ...
    <XXX_foundation_func_name> ::=
        <alphanumeric_name>

    <XXX_foundation_proc_name> ::=
        <alphanumeric_name>
...
package entities (types, constants, functions, procedures, aliases, etc)
which live within a multi-level namespace.

=head1 PACKAGE SPECIFICATION XXX REMOVE THIS WHOLE SECTION AS HAS NO SYNTAX OF ITS OWN
...
I<TODO.  Valid attr names in the "Package" Tuple commalist are these:
{identity, foundation, uses, entry, floating, materials};
the 6 pkg_* above denote the attribute asset for each;
{identity,foundation} are mandatory attrs, the rest are not.>

I<TODO.  The package attr asset for "materials" is a Tuple node each of
whose attributes is a material definition or a folder definition; the latter
is the case iff it is also a Tuple node or if the atter name is the format X::Y.>

I<TODO.  Note that "entry" is the "entry point" for a program.  It is NOT automatically
invoked when loading a package, but is informative for an explicit post-load call.>

I<TODO.  Each entry point kind represents a distinct kind of routine or
...
=head1 MATERIAL SPECIFICATION XXX REMOVE THIS WHOLE SECTION AS HAS NO SYNTAX OF ITS OWN

Grammar:

    <material> ::=
        <alias> | <constant> | <selection> | <interface> | <enumeration> | <function> | <procedure>

    <alias> ::=
        alias <sp> of <sp> <pkg_entity_name>
...
=head2 Enumeration Specification
...
be a completely foldable constant.  It may not contain a C<< <source_expr> >>.
...
Examples:
    ...
    myfoo ::?= 'this is a comment on the elsewhere-declared node named myfoo'
...
is an arbitrarily-long sequence of abstract I<characters>.
...
I<TODO.  Note that Function_Name and Contract_Name are aliases for Identity_Identifier.>
...
Grammar:
    ...
    <attr_commalist> ::=
        [<anon_attr> | <named_attr> | <nested_named_attr> | <same_named_attr> | ''] % [<sp> ',' <sp>]
...
every possible C<< <delimiting_expr> >>.
...
Grammar:
    ...
    <Capsule> ::=
        <wrapper_expr> <sp> '>:<' <sp> <c_asset_expr>
...
=head2 Generic Constant and Function Call Specification Selectors
...
Grammar:
    ...
    <invocation_expr> ::=
        ...
        | <XXX_foundation_func_invo_expr>

I<TODO.>

I<Also TODO is adding things like -->? and -->! etc to test if something
is invokable or indicate a result if one isn't invokable.>

=head2 Generic Constant and Function Invocation Expressions

Grammar:

    <generic_func_invo_expr> ::=
        <long_arrowed_func_invo_expr> | <postcircumfixed_func_invo_expr>

    <long_arrowed_func_invo_expr> ::=
          [<generic_func_args> <sp> '-->' <sp> <generic_func_call>]
        | [<generic_func_call> <sp> '<--' <sp> <generic_func_args>]
...
A C<< <generic_func_call> >> denotes a C<Function_Call> value, which pairs
a C<Function_Name> (C<Identity_Identifier>) value naming a function (or constant or
type) with a C<Tuple> value giving arguments to pass to it; typically this
is defined either with a C<< <Function_Call> >> selector expression or with
an inline-defined C<function>; the C<< <generic_func_args> >> gives more args.

Note that explicitly passing a nullary tuple will also match a constant,
and explicitly passing a unary tuple with a positional argument will also
match a type.
...
A C<< <generic_func_name> >> may alternately be a constant name or a type
...
has exactly zero C<< <*_attr> >> elements, either a constant or a niladic
...
=head2 Fixed Constant and Function Invocation Expressions
...
Say that it is just syntactic sugar for a specific XXX_foundation_func_invo_expr
but we special-case it because we don't want "=" to change based on what
packages are in scope, same as ":?:!" or ":=" etc don't change.
Note, this only affects '=' when called like an infix op syntactically;
calling it as -->"=" or "="::() gets the reg func if exists.>
...
=head2 XXX Foundation Constant and Function Invocation Expressions

Grammar:

    <XXX_foundation_func_invo_expr> ::=
          [<generic_func_args> <sp> '-->^' <sp> <XXX_foundation_func_name>]
        | [<XXX_foundation_func_name> <sp> '^<--' <sp> <generic_func_args>]
...
as changes to them WOULD change the Plain_Text version.  ?OBSOLETE?>
...
I<TODO.  Make 'if' keyword optional/noiseword, for parity with non-short.>
...
I<TODO.  Make 'given' keyword optional/noiseword, for parity with non-short.>

=head1 GENERIC PERFORMANCE STATEMENTS
...
A C<::=> is I<what-binding> while a C<::?=> is I<why-binding>.
    ...
    ? - booleans or sets (and bits and relations) or some predicate eg for existence
    + - numbers or bags (and octets and tuple-bags) or element insertion
    - - numbers or element removal
    ...
    & - OBSOLETE: subject-to-update parameter/etc or procedure
    * - OBSOLETE?: wildcard or "any" or "all"
    ! - booleans or excuses
    ^ - XXX raising something up - bootstrap on foundation, or interval endpoint exclusion (or number to a power, but not in Core)
...
I<TODO.  These are listed from tightest at the top to loosest at the bottom.>
    ...
    terms - base literals or delimited anything or special selector syntax,
        including inline func/type/etc decls, or foo::bar::etc or func foo::(...) or \foo::(...)
    generic universal routine call infix: args --> rtn or rtn <-- args or args \--> rtn etc or args -->^ rtn etc
    symbolic in/pre/postfix (bareword) (postfix indicated with 'pipe' keyword)
        and also infix >:<, prefix <:, postfix :> (no 'pipe' keyword needed)
    alpha in/pre/postfix (bareword) (postfix indicated with 'pipe' keyword)
    conditional ternaries or n-aries: :?:! / if-then-else / :??:!! / given-when-default
    why-binding infix ::?=
    what-expr-binding infix ::=
    "returns" expression prefix
...
MDPT is Copyright © 2002-2016, Muldis Data Systems, Inc.

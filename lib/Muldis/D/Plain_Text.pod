...
common cases of function (or singleton type definer) invocations where an unqualified
...
=head2 Generic Singleton Type Definer and Function Call Specification Selectors
...
=head2 Generic Singleton Type Definer and Function Invocation Expressions
...
a C<Function_Name> (C<Identity_Identifier>) value naming a function (or singleton type definer or
...
Note that explicitly passing a nullary tuple will also match a singleton type definer,
...
OR (INTERNALLY RECURSIVE) SINGLETON TYPE DEFINER IS DECLARED INLINE WITH ITS CALL! >>
...
=head2 Fixed Singleton Type Definer and Function Invocation Expressions

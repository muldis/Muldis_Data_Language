...
INDICATOR>, L</SCRIPT NAME>, L</GENERIC VALUE EXPRESSIONS>, and
L</Capsule Selectors> for more details.

A C<< <Package> >> node is expected to evaluate to a value of the Muldis D
C<Package> type; its canonical form is a C<< <Capsule> >> node whose
C<< <wrapper_expr> >> is the value C<\Package>.
...
These are keywords only within an C<< <expr> >> or C<< <stmt> >>, that is,
the root package declaration itself and any constituent parts thereof:
...
Examples:
    ...
    `The Muldis D Foundation function FDN__Integer_plus, from any perspective.`
    \@foundation::FDN__Integer_plus

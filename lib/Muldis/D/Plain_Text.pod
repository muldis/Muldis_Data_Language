...
The C<Bits> type is not a foundational type of the Muldis D type system, but
rather is a subtype by constraint of the C<Capsule> type, and all C<Bits>
...
is a general purpose octet string.
...
When using Ob, the bits must come in groups of 8; when using 0x, the hexits
must come in groups of 2.

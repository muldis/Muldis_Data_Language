...
Grammar:
    ...
    <Pair> ::=
        '\\:' '(' <sp> <p_key_expr> <sp> ',' <sp> <p_asset_expr> <sp> ')'
...
Grammar:
    ...
    <generic_func_invo_expr> ::=
        <generic_func_args> <sp> '-->' <sp> <generic_func_name>
...
Say that it is just syntactic sugar for a specific bootstrap_func_invo_expr
...
Grammar:
    ...
    <bootstrap_func_invo_expr> ::=
        <generic_func_args> <sp> '-->^' <sp> <bootstrap_func_name>
...
=head2 Logical-Not Meta Operator Expressions
...
empathetically does NOT parse into a routine call, not even a bootstrap.>
...
A C<::=> is I<what-binding> while a C<::?=> is I<why-binding>.
    ...
    : - pairs or dicts (and renamings) - also key/asset separator in most pairwise collections
    ? - booleans or sets (and relations)
    ...
    () - heterogeneous aordered collection of field - structures/tuples/pairs - nominal+universal

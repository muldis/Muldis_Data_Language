...
abstract codepoint number or name.  If the C<...> looks like an C<<
<Integer> >>, sans that dividing space isn't allowed here, then the C<...>
is interpreted as a Unicode abstract codepoint number.  If the C<...>
consists of just uppercased (not lowercased) letters and the space
character, then the C<...> is interpreted as a Unicode character name.  One
reason for this feature is to empower more elegant passing of Unicode-savvy
Plain_Text source code through a communications channel that is more
limited, such as to 7-bit ASCII.
...
Grammar:
    ...
    <expr> ::=
        <named_expr> | <commented_expr> | <anon_expr>
...
Iff an C<< <expr> >> is a C<< <commented_expr> >>, then the C<< <expr> >>
element of the C<< <commented_expr> >> is being given an explicit
introspectable code comment whose text is the C<< <Text> >> element.  Keep
in mind that C<::?=> binds tighter than nearly anything, so you may have to
use parenthesis when you want the comment to associate with a non-leaf node.
...
C<\~external~'...'> or C<"..."> represent character data, either C<Text> or
C<External> literals or quoted identifiers, and the valid formats for both

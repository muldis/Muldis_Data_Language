...
granularity how large the effective Muldis D grammar is that a programmer
may employ with their Muldis D code.
...
highly normalized manner, meaning that when the full Muldis D grammer gives
multiple syntax options that all parse into the same C<Package> values
ignoring the optional I<decoration> attributes, just the most generic of
each set of alternate syntax options will be supported; when C<<
<foundational_indicator> >> is absent, the full Muldis D grammar is
supported, albeit still limited by the character repertoire specified by
C<< <script_name> >>.
...
rather is a subtype by constraint of the C<Capsule> type, and all C<Ratio>
...
rather is a subtype by constraint of the C<Capsule> type, and all C<Float>
values can be selected in terms of C<< <Capsule> >> grammar nodes.

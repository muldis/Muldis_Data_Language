...
type, which is a general purpose integer numeric value.  The C<Integer>
type is a fundamental type of the Muldis D type system, and this is the
canonical grammar for them.
...
formatting, such as for C<10_000_000>.  An C<< <Integer> >> may optionally
be split into 1..N segments where each pair of consecutive segments is
separated by dividing space or the optional token C<,,> which is mainly
valuable to prevent mis-parsing of hexadecimal literals when the first
character of a non-first segment isn't a digit.
...
=head2 Decimal/Binary Ratio Literals

Grammar:

    <dec_bin_Ratio> ::=
        <dec_bin_2> | <dec_bin_8> | <dec_bin_10> | <dec_bin_16>
...
A C<< <dec_bin_Ratio> >> node represents a value of the Muldis D C<Ratio>
type, which is a general purpose rational numeric value.  To be specific,
C<< <dec_bin_Ratio> >> only provides for a subset of C<Ratio> values,
namely those whose denominators are integral powers of 2 or 10.
...
nodes.  However, C<< <dec_bin_Ratio> >> is the canonical grammar for all
C<Ratio> values whose denominators are integral powers of 2 or 10, and the
canonical syntax for all other C<Ratio> is a value expression that divides
2 coprime C<Integer> values where the divisor is positive.

...
I<TODO: Figure out how to say in grammar when (insignificant) whitespace is
...
whitespace, and becomes non-semantic metadata for its wider context.

=head3 Pipeline Stage 7

Stage 7 in the parsing pipeline is to collect any runs of consecutive
(apart from any invervening whitespace) quoted context tokens or parse
...
and further optionally split (when long) using whitespace.

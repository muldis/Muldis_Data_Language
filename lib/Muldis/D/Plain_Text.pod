...
section in this file named L<Internal-Comment Literals>.
...
annotation is an internal code comment expressed as a C<Internal_Comment> literal,
...
The C<Blob> type is not a foundational type of the Muldis D type system, but
rather is a subtype by constraint of the C<Capsule> type, and all C<Blob>
values can be selected in terms of C<< <Capsule> >> grammar nodes.
...
The C<Text> type is not a foundational type of the Muldis D type system, but
rather is a subtype by constraint of the C<Capsule> type, and all C<Text>
values can be selected in terms of C<< <Capsule> >> grammar nodes.
...
=head2 Internal-Comment Literals
...
A C<< <Internal_Comment> >> node represents a value of the Muldis D C<Internal_Comment> type,
which is a generic character string comment for annotating source code when
the comment is meant to be introspectable as such.

The C<Internal_Comment> type is not a foundational type of the Muldis D type system, but
rather is a subtype by constraint of the C<Capsule> type, and all C<Internal_Comment>
values can be selected in terms of C<< <Capsule> >> grammar nodes.
However, C<< <Internal_Comment> >> is the canonical grammar for all C<Internal_Comment> values.

For less important comments or visual dividers that are intended to be
treated like insignificant whitespace and not be introspectable as
comments, see the section in this file named L<DIVIDING SPACE>.

Examples:

    \~?'This is an introspectable comment.'
...
Grammar:
    ...
    <Capsule> ::=
        '\\:' '(' <sp> <wrapper_expr> <sp> ',' <sp> <c_asset_expr> <sp> ')'
...
C<Internal_Comment> literals or quoted identifiers, and the valid formats for both
...
MDPT is Copyright © 2002-2015, Muldis Data Systems, Inc.

...
of symbolic characters in the original source code.  I<Details pending.>
...
section in this file named C<INTROSPECTABLE COMMENTS>.
...
general case, an expression denoting any value of the C<Universal> type may
be used where you see C<< <expr> >>, any further restrictions would be
provided by the context.

Iff an C<< <expr> >> is an C<< <expr_name> >>, then this typically means
...
<expr_name> >> is the invocation name of that child.  Alternately, the C<<
<expr_name> >> may be the invocation name of one of the
expression-containing routine's parameters, in which case the C<< <expr> >>
in question represents the current argument to that parameter.
...
=head2 Non-Negative Integer Literals

Grammar:

    <nn_Integer> ::=
        <nnint_2> | <nnint_8> | <nnint_10> | <nnint_16>
    ...
    <nnint_16> ::=
        '0x' <[ 0..9 A..F _ a..f ]>+ % <num_sp>

    <num_sp> ::=
        <sp> [',,' <sp>]?

A C<< <nn_Integer> >> node represents a value of the Muldis D C<Integer>
type, which is a general purpose integer numeric value.  To be specific,
C<< <nn_Integer> >> only provides for a subset of C<Integer> values,
namely those which are non-negative.

The C<Integer> type is a fundamental type of the Muldis D type system and
includes both negative and non-negative integers.  In the Muldis D Plain
Text language however, the C<Integer>-specific C<< <nn_Integer> >> only
provides the canonical syntax for non-negative C<Integer> values, and the
canonical syntax for negative C<Integer> values is a value expression that
takes the opposite or additive inverse of a positive C<Integer> value.

Compared with most common programming languages, this Muldis D Plain Text
restriction of negative values from its most fundamental numeric literal
formats is probably strange.  However, the primary rationale is that this
keeps the Muldis D Plain Text grammar simpler by avoiding the only special
case for a symbolic character immediately preceding an alphanumeric token.
...
same way with respect to the symbolic even when it contains a C<->.  Since
the customary unary C<-> operator is system-defined however, users may not
even notice what is missing, since C<-3> will still yield what they expect.
Note that the non-plain-text Muldis D native source code syntax in terms of
data structures still supports negative values in its nodes for C<Integer>
literals, but Muldis D Plain Text just doesn't use it, without folding.
...
formatting, such as for C<10_000_000>.  A C<< <nn_Integer> >> may
optionally be split into 1..N segments where each pair of consecutive
segments is separated by dividing space or the optional token C<,,> which
is mainly valuable to prevent mis-parsing of hexadecimal literals when the
first character of a non-first segment isn't a digit.
...
=head2 Non-Negative Decimal/Binary Ratio Literals

Grammar:

    <nn_dec_bin_Ratio> ::=
        <nnrat_2> | <nnrat_8> | <nnrat_10> | <nnrat_16>

    <nnrat_2> ::=
        '0b' <[ 0..1 _ ]>+ % <num_sp> '.' <[ 0..1 _ ]>+ % <num_sp>

    <nnrat_8> ::=
        '0o' <[ 0..7 _ ]>+ % <num_sp> '.' <[ 0..7 _ ]>+ % <num_sp>

    <nnrat_10> ::=
        '0d'? <[ 0..9 _ ]>+ % <num_sp> '.' <[ 0..9 _ ]>+ % <num_sp>

    <nnrat_16> ::=
        '0x' <[ 0..9 A..F _ a..f ]>+ % <num_sp>
            '.' <[ 0..9 A..F _ a..f ]>+ % <num_sp>

A C<< <nn_dec_bin_Ratio> >> node represents a value of the Muldis D
C<Ratio> type, which is a general purpose rational numeric value.  To be
specific, C<< <nn_dec_bin_Ratio> >> only provides for a subset of C<Ratio>
values, namely those which are non-negative and whose denominators are
integral powers of 2 or 10.

The C<Ratio> type is not a fundamental type of the Muldis D type system,
but rather is a subtype by constraint of the C<Structure> type, and all
C<Ratio> values can be selected in terms of C<< <Structure> >> grammar
nodes.  However, C<< <nn_dec_bin_Ratio> >> is the canonical grammar for all
non-negative C<Ratio> values whose denominators are integral powers of 2 or
10, and the canonical syntax for all other C<Ratio> is a value expression
that divides 2 coprime C<Integer> values where the divisor is positive.
...
The meanings of the simple escape sequences are:
    ...
    ----+-----------+-----------------+-----+------------------------------
    \a  | 0x27      | APOSTROPHE      | '   | delimit Text literals
...
like an C<< <Integer> >>, sans that underscores and dividing space aren't
...
passing of Unicode-savvy Plain_Text source code through a communications
...
An C<< <External> >> node represents a value of the Muldis D C<External>
type, which represents a native value of some other programming language,
typically expressed in Muldis D Plain Text as source code of the other
programming language.  In practice however, C<External> values tend to be
strictly transient and their content implementation-dependent on whatever
the host environment of the Muldis D implementation is, and they are not
typically serialized to persistent source code, Muldis D Plain Text or
otherwise.  The C<External> type is a fundamental type of the Muldis D type
system, and this is the canonical grammar for them within Plain Text code.

Examples:

    `With a Perl 5 host, the Perl undef.`
    \~external~'undef'
...
I<FORGET NOT THE ps5_nonquoted_symbolic_grouping.>

=head2 Bootstrap Function Invocation Expressions
...
Because the above sequences are special to C<Plain_Text>, dividing space is
optional between them and symbolic operator calls/syntax because there is
considered no ambiguity.

As this special group is minimized as possible, just to the 10 characters
C<< !,-:;<=>?\ >>, that leaves 12 more ASCII symbolic characters completely
free in any combination for user-definable nonquoted operator names, though
the former 10 are also still available for nonquoted operator names that
aren't superstrings of any C<< <ps5_nonquoted_symbolic_grouping> >>.
...
Stage 8 in the parsing pipeline is to isolate any literals specific to the
C<Blob> type, which are a particular run of C<< <alphanumeric_char> >>
followed immediately by a logically single-quoted string, where the latter
may actually be a run of said that was collected by stage 7.

Grammar:

    <ps6_blob> ::=
        0 <[box]> <ps2_single_quoted>

Note that the single C<< <ps2_single_quoted> >> above actually represents a run.

Note that the above actually allows invalid C<Blob> literals, however
anything matching the above pattern will be a syntax error if it doesn't
otherwise match the stricter C<Blob> definition.

=head3 Pipeline Stage 9

Stage 9 in the parsing pipeline is to take any tokens from stage 2
...
actual characters they represent.  This applies only to single-quoted and
double-quoted strings, and for the former, only to those not already
collected into a literal specific to the C<Blob> type.  These all represent
character data, either C<Text> literals or quoted identifiers, and the
valid formats for both within the quoted contexts are identical.  Note that
for a run, each individual quoted context is treated in isolation, as some
may be formatted for escape sequences and some may not.

Grammar:

    <ps9_quoted_sans_delimiters> ::=
        <ps9_nonescaped> | <ps9_escaped>
...
=head3 Pipeline Stage 10

Stage 10 in the parsing pipeline is to isolate any literals specific to
...
Grammar:
    ...
    <ps10_numeric> ::=
        <digit_char> <ps10_numeric_seg>? ['.' <ps10_numeric_seg>]?

    <ps10_numeric_seg> ::=
        <alphanumeric_char>+ % <num_sp>

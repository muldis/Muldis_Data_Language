...
This document is Muldis::D::Basics version 0.22.0.
...
and generic database or table state constraints.  Only database transition
constraints are not part of the database's type, and are applied directly
to the C<dep.data> variable (and C<fed.data> also) by the depot's catalog
in another way.
...
C<(.(|quasi_)(set|maybe|single|seq|bag)_of){0,}>; this C<of> element takes
the same position as the C<from> element in the syntax, just after the
C<type>; or they can both be used together in which case the C<of>
...
sequential array of integers (that is, a C<Seq> whose C<value> attribute
has the type C<Int>), you can just use it implicitly by saying
C<type.seq_of.type.sys.std.Core.Type.Int>, the same as you would use the

...
material itself.  So for example, if a material C<sdp.lib.foo.""> exists,
then C<sdp.lib.foo> will implicitly refer to the same material in any
...
the empty string; so for example, a material C<sdp.lib.foo."".""> can also
be referenced by both C<sdp.lib.foo> and C<sdp.lib.foo."">.  Note that this
...
former's C<dep.cat.data> or C<sdp.cat.data> is a C<Single>), then the
...
is the empty string.  (This also means that C<dep.lib> alone will reference
the type of the depot's C<dep.data>, when the convention is followed.)
...
accomplished by setting that depot's C<dep.cat.data> to a C<Single> value
that names the declared database type of that depot's C<dep.data>;
...
a self-local dbvar, that is iff the subdepot's C<sdp.cat.data> is a
C<Single>, then the value of C<dep.data> must have a corresponding
database-typed attribute that matches by name,
recursively.  For example, if a depot has a
...
type to be the same type as the child subdepot's C<sdp.cat.data> names;

...
and only procedures and recipes may be what is thusly invoked.  Both
...
=head2 Functions, Recipes, and Procedures

Every Muldis D routine is exactly one of these 3 main routine kinds, where
the 3 kinds are mutually exclusive: C<function>, C<recipe>,
C<procedure>.  Each of these 3 kinds is very distinct with regards to what
it conceptually represents or where it may be used, and Muldis D has
disjoint catalog data types for defining routines of each of the 3 kinds:
C<Function>, C<Recipe>, C<Procedure>.  Just functions and
recipes are collectively also known as I<atomic routines>;
just recipes and procedures are collectively also known as
...
deterministic.  A function's capabilities are a proper subset of a
recipe's; the most that can be accomplished just by invoking a function is
any set of operations
that can be accomplished just with values supplied
as its explicit arguments, and also a function
is more limited in how it can be invoked.
...
function, and to do so requires a recipe intermediary; a recipe
may directly invoke only recipes or functions, and a function may only
directly invoke functions.

Functions differ from imperative routines (recipes and procedures)
...
Recipes differ from other kinds of routines (functions and
...
function or procedure can I<not> directly see any global
variables.  A procedure can only see or update any global variables by
invoking a recipe to be its proxy for that purpose.

Procedures differ from atomic routines (functions and recipes)
...
statement.  Unlike either functions, recipes, or procedures, which may
have zero
...
Muldis D routine of any kind (function, recipe, or procedure), that has at
...
(besides using a C<recipe> rather than a C<procedure>).
...
using C<fed> except a procedure (or recipe?), or the host language if it

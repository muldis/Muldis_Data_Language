...
This document is Muldis::D::Basics version 0.105.0.
...
=item interval, interval-union, non-union interval

Within this documentation, any uses of the unqualified term I<interval> in
general are meant to be taken in a more liberal sense than the externally
normal meaning of the term; quite often within this documentation, any
unqualified references to I<interval> may also be interpreted as referring
to what would normally be called an I<interval-union>, and the shorter term
is used to mean either of them mainly for purposes of brevity.  The
system-defined types C<Interval> (a tuple type) and C<IntervalUnion> (a
relation type) reflect the externally normal meanings of the terms, with
I<interval> being a pair of ordered endpoints and I<interval-union> being a
set of I<interval>.  But the grammar node kinds like C<Interval> actually
represent C<IntervalUnion>, and the shorthands for declaring types like the
C<interval_of.Foo> prefix actually declare interval-unions of C<Foo>, and
generally any attribute or parameter names like C<interval> actually take
interval-union values.  And so, within this documentation, in places other
than where the above 2 types are declared, the term I<non-union interval>
will be used to unambiguously refer to an C<Interval> specifically in
contrast to an C<IntervalUnion>.
...
C<of> would appear first.  Note that C<[|dh_]interval_of> actually declares
C<[|DH]IntervalUnion> types, not C<[|DH]Interval> types; the shorter syntax

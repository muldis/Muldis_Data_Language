...
I<external type>.  A data type that is a primitive or structure or
external type is also called a I<root type>; a data type that is an
...
system-defined, not user-defined.  There are just 3 of these: C<Int>,
C<Tuple>, C<Relation>.  I<TODO: Further correct this desc.>
...
types include: C<Bool.True>, C<Rat>, C<Blob>, C<Text>, C<Name>, C<Comment>,
C<Order.Increase>, C<TAIInstant>, C<TAIDuration>.  All user-defined
structure types are scalar types.  I<TODO: Further correct this desc.>
...
C<DHSet>, C<DHArray>, C<Maybe>, C<Bag>, C<String>, C<UTCDateTime>.  The
system-defined types defined in terms of a union (sometimes not an
explicitly enumerated one like with user-defined types) include:
C<Universal>, C<Empty>, C<Scalar>, C<DHScalar>, C<Bool>,
C<Order>.  In the case of a scalar subset enumeration
...
All non-external values in the Muldis D type system are broadly
...
enumeration type named C<External> which is a union over all external
types.  The 4 types are all mutually disjoint, and C<Universal> is a union
type over all of them.

Most non-external data types each consist exclusively of values from
...
which all scalar root types except C<Int> do), so such do not need to be
...
that either has no possreps (it is an C<Int>) or all of its possreps are
...
system-defined C<Int> type which doesn't have any possreps, so you don't

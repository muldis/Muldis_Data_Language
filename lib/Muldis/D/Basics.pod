...
This document is Muldis::D::Basics version 0.82.0.
...
order to use all Muldis D features:
    ...
    !   | F;21 | EXCLAMATION MARK       | factorial op, other ops
    "   | F;22 | QUOTATION MARK         | double-quot; delim quot Name lits
    #   | F;23 | NUMBER SIGN            | delim Comment lit, cardinality op
    $   | F;24 | DOLLAR SIGN            | sigil for data/value entity names
    %   | F;25 | PERCENT SIGN           | modulus/remainder op
    ...
    *   | F;2A | ASTERISK               | numeric multiply op, Rat literals
    ...
    @   | F;40 | COMMERCIAL AT          | sigil used in wrap,group,etc ops
...
for attributes of tuple and scalar (but not relation
...
With scalars, referencing a possrep attribute requires 2 name elements,
...
With relations, this kind of terse syntax may also be used in some,
though not all, situations as the syntax may be with tuples and scalar
possreps, since in the general case, addressing a relation attribute is
conceptually referring to a set of 0..N items rather than exactly 1.  So
for the present, relation attributes may only be referred to using this
terse syntax in situations where said attributes of *all* of the tuples
in the relation at once are being referenced.  An example of this is some
...
rather than the whole relation attribute.  (For the present, other parts
of the Muldis D documentation ignore for simplicity that an RVA of a
relation can be drilled into, but you in fact can do this where it makes
sense.)

Note that in general, any value expression can denote a pseudo-variable,
but only components of tuples or scalar possreps, and sometimes
components of relations, get the special short-hand where an extended

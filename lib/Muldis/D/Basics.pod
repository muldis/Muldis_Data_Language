...
C<Universal>, C<Empty>, C<QScalar>, C<Scalar>, C<Reference>,
C<String>.  In the case of a q/scalar subset enumeration
...
C<Relation>, C<Set>, C<Maybe>, C<Single>, C<Array>, C<Bag>] are actually
not nonscalar types like their namesakes at all, but they are all
...
the C<Universal>, C<QScalar>, C<Reference> types.
...
With q/scalars, referencing a possrep attribute requires 2 name elements,
...
might expect, you can also just reference a q/scalar possrep as a whole, as
if it were a q/tuple-typed pseudo-variable, by using just 1 name element;

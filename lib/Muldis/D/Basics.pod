...
This document is Muldis::D::Basics version 0.73.0.
...
order to use all Muldis D features:
    ...
    *   | F;2A | ASTERISK               | numeric multiply op, Rat literals
    ...
    /   | F;2F | SOLIDUS                | numeric division op
...
just the 7-bit ASCII repertoire:
    ...
    ±   | F;B1   | PLUS-MINUS SIGN            | <not in use>
    ×   | F;D7   | MULTIPLICATION SIGN        | relational cross-product op
    ÷   | F;F7   | DIVISION SIGN              | relational divide
    ∃   | F;2203 | THERE EXISTS               | <not in use>
    ∄   | F;2204 | THERE DOES NOT EXIST       | <not in use>
    ∅   | F;2205 | EMPTY SET                  | <not in use>
    ...
    ∖   | F;2216 | SET MINUS                  | relational difference op
    ∧   | F;2227 | LOGICAL AND                | logical and/all op
    ∨   | F;2228 | LOGICAL OR                 | logical or/any op
    ...
    ⊉   | F;2289 | NEITHER A SUPERSET OF NOR  | is-not-superset op
        |        |   EQUAL TO                 |
    ⊿   | F;22BF | RIGHT TRIANGLE             | antijoin/semidifference op
    ⋉   | F;22C9 | LEFT NORMAL FACTOR         | semijoin op
        |        |   SEMIDIRECT PRODUCT       |
    ⋊   | F;22CA | RIGHT NORMAL FACTOR        | <not in use>
        |        |   SEMIDIRECT PRODUCT       |
    ⟕   | F;27D5 | LEFT OUTER JOIN            | half-outer-join op
    ⟖   | F;27D6 | RIGHT OUTER JOIN           | <not in use>
    ⟗   | F;27D7 | FULL OUTER JOIN            | <not in use>
    ⨝   | F;2A1D | JOIN                       | (natural inner) join op 

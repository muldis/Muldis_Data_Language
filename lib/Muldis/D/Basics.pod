...
types:
     ...
     8 | Identifier | $lit etc - also special syntax :*expr will produce Global_Identifier from Identifier expr
     9 | Capsule    | expr->expr - that is Global_Identifier->Tuple eg :*$Date=>:%{year:500,month:4,day:24}
...
These are the I<payload> structures for those:
    ...
    Identifier : List[3] :
        ...
        ==> proper subtype Global_Identifier
            - kind must = 1
...
At program runtime, some contexts require the C<Global_Identifier> subtype,
...
still relative to its place of declaration.

Typically in Muldis D code a special expression node must be wrapped around
any C<Identifier> selector so that it is resolved to a C<Global_Identifier>

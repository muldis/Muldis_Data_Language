...
default order function for type, built into type def,
only definable for scalar root types
invoke using sys.std.Core.Scalar.\w+ named func

also, every system-defined scalar type without possreps is ordered; for
those with possreps, only some are

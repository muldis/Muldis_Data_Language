...
This document is Muldis::D::Basics version 0.52.0.
...
system-defined, not user-defined.  There are just 6 of these: C<Int>,
C<String>, C<Tuple>, C<Relation>, C<QuasiTuple>, C<QuasiRelation>.  I<TODO:
Further correct this description.>
...
of a subset include: C<NNInt>, C<BString>, C<Database>, C<Set>, C<Array>,
C<QuasiMaybe>, C<QuasiBag>, C<UTCDateTime>.  The system-defined types
defined in terms of a union include: C<Universal>, C<Empty>, C<Scalar>,
C<QuasiScalar>, C<Reference>, C<Remnant>, C<ScaTupRel>.  In the case of a
scalar subset enumeration type, typically it will define extra scalar
possreps or selectors.  All user-defined nonscalar or quasi-nonscalar types
...
categorized into 6 complementary sets called I<scalar values>, I<tuple
values>, I<relation values>, I<quasi-scalar values>, I<quasi-tuple values>,
and I<quasi-relation values>; tuple and relation values are collectively
known as I<nonscalar values>; quasi-tuple and quasi-relation values are
...
types.  The 7 types are all mutually disjoint, and C<Universal> is a union
type over all of them.

Most non-reference data types each consist exclusively of values from
exactly one of the above first 6 categories, and each such type does not
include values from several of them.  Therefore, every such data type is
said to be either a I<scalar type>, a I<tuple type>, a I<relation type>, a
I<quasi-scalar> type, a I<quasi-tuple type>, or a I<quasi-relation type>,
depending which category all of its values come from.  In similar fashion,
a I<nonscalar type> is generally any type that is not a scalar type, if we
ignored quasi- types, meaning it is either a tuple type or a relation type.
...
7 categories, according to their type definition rules.  The remnant
...
by a type that fits in one of the 8 categories, save C<Empty> itself.  The
...
which all user-defined scalar root types do), so such do not need to be
explicitly defined per such type.  They are all in the
C<sys.std.Core.Scalar.\w+> namespace.  These generic operators take
advantage of the fact that each scalar possrep looks like a tuple, and they
look like basic tuple operators but for taking an extra argument to say
which possrep we're dealing with, and possibly a second extra attribute to
say what type, in the case of the generic scalar value selector.
...
The Muldis D implementation can choose for itself as to how the scalar type
...
a relation has attributes even if it has no tuples), but that the
...
automatically work with all tuple and relation types, so they do not need
...
The explicit heading of a tuple or relation value only defines the names of
its attributes, not their types; the types of tuple or relation attributes
are simply derived from the values of those attributes, specifically their
MSTs, recursively in the case of TVAs and RVAs.  Declared tuple or relation
attribute types are only applicable to explicit tuple or relation type
...
of the function is a C<Cat.Order>.  This function results in
C<Cat.Order:same> iff its 2 arguments are exactly the same value, and
otherwise it results in C<Cat.Order:increase> if the value of the C<other>
argument is considered to be an increase (as defined by the function's
algorithm) over the value of the C<topic> argument, and otherwise it
results in C<Cat.Order:decrease> as the reverse of the last condition would
be true.  The function also has an optional C<Tuple>-typed third parameter,
named C<assuming>, which carries optional customization details for the
algorithm; this permits the function to implement a choice between multiple
(typically similar) ordering algorithms rather than just one, which reduces
the number of functions needed for supporting that choice; if the algorithm
is not customizable, then a tuple argument would be of degree zero.
...
system-defined scalar types that don't have any possreps, so you don't need
to define any yourself for these most-common cases.  Note that a nonscalar
type can't have a default ordering function, and a subtype of a scalar type
...
function is attached to a scalar root type.
...
routine or its sibling routines.  Conceptually speaking, a
...
C<topic>.  If the data type being selected is a scalar type or subtype with
...
attributes of the tuple match those of the possrep; such a tuple argument
provides the candidate components of the scalar value being selected.  Or,
...
A C<type_default> is the same as a C<inner_function>, but that it is part
...
A C<possrep_map> is the same as a C<inner_function>, but that it is part of
the definition of a (|quasi-)scalar data type, and it is used to convert a
value from one of that type's possreps to another.  A C<possrep_map> has a
...
tuple whose attributes match those of the possrep being converted to.  Note
...
on a per-tuple basis, one subset of that type's attributes is defined to be
...
parameter whose declared type is a tuple whose attributes match those of
...
A C<order_determination> is the same as a C<inner_function>, but that it is
structured to fill the role of a I<system-compatible fundamental
order-determination function>; see the L</Ordered Types> pod section in
this file for more details.  A C<order_determination> has 3 arguments named
[C<topic>, C<other>, C<assuming>] and results in a C<Cat.Order>.
...
for attributes of tuple and possrep-having-scalar (but not relation in the
...
not all, situations as the syntax may be with tuples and scalar possreps,
...
apply a referential constraint to elements of a TVA or RVA of a relation,
rather than the whole relation attribute.  (For the present, other parts of
the Muldis D documentation ignore for simplicity that an RVA of a relation
...
but only components of tuples or scalar possreps, and sometimes components
of relations, get the special short-hand where an extended entity name can
...
packages, while namespaces under C<data> are defined in terms of tuple
(database) attributes that are themselves tuples (databases) rather than
relations.  However, in order for any given depot|subdepot|package to
...
to the fact that one tends to declare tuple+relation type pairs by
...
taking the declared type where a tuple+relation type was declared together,
you use extra syntax to say whether it is a tuple type or relation type

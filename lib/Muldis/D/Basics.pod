...
structure types, plus examples of literal syntax:
     ...
     1 | String        | ideas are \+[expr,...] and \~[expr,...] and Blob-inspired versions like 0x[lit,...]/etc
                       |    Maybe \~lit is a String literal where 'lit' is
                       |    interpreted the same way as in SC_Heading etc,
                       |    so basic piecewise building block for building
                       |    say Tuple, eg: "unary_tuple(\~atnm,atvl)" means "\%{atnm:atvl}".
     ...
     7 | SC_Identifier | Several kinds, all mutually disjoint:
...
These are the I<payload> structures for the 11 structure types:
    ...
    Identifier : Universal[4] :
        0 : package name base : Universal[0..N] of String-payload
        1 : package name extensions : Universal[0..E] of String-payload
            0 : authority
            1 : version number
            2+ : available for something else?
        2 : relative starts K levels up : Integer-payload in 0..J
        ...
        ==> proper subtype Reference (aka "Global Identifier")
            - where Identifier kind is "identity"
    Capsule : Universal[2] :

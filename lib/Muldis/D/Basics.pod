...
This document is Muldis::D::Basics version 0.113.0.
...
of data having those types.  As a consequence of this, it is also feasible
to have structure values that have both a metadata component and a payload
component where the former defines a type constraint that applies to the
latter; selecting such a structure value will only succeed if the payload
component satisfies the constraint defined by the metadata component.
I<TODO:  Add a type factory to Muldis D for creating said structures, and
make a depot such a structure, and let data-manipulation routines be
updater-routines that work with such structure values.>
...
B<ACTUALLY, THE CONCEPT OF RECIPE IS NO LONGER NEEDED EXACTLY,
NOW THAT ALL MULDIS-D VALUES ARE DECLARED BY SYSTEM TYPES,
SO ORDINARY UPDATER-ROUTINES SHOULD BE ABLE TO DO WHAT RECIPES WERE FOR;
SO THIS AND RELATED SECTIONS ARE OUTDATED AND NEED TO BE UPDATED/REPLACED;
ALSO, TRANSACTIONING IS SUPPOSED TO BE MVCC, AND FULLY ISOLATED.>
...
Muldis D routine of any kind (function, updater, or procedure), that has at
...
(besides using an C<updater> rather than a C<procedure>).

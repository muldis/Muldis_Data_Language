...
This document is Muldis::D::Basics version 0.89.0.
...
order to use all Muldis D features:
    ...
    #   | F;23 | NUMBER SIGN            | delim Comment lit, cardinality op
    ...
    *   | F;2A | ASTERISK               | numeric multiply op, Rat literals
    ...
    :   | F;3A | COLON                  | value literal elem sep
...
rejected in favor of some other alternatives for now:
    ...
    ----+--------+-------------------------+-------------------------------
    `   | F;60   | GRAVE ACCENT            | use still to determine
...
C<ImpRef>, C<TypeRef>.  I<Note: It is possible that a future Muldis D
...
And so, the 8 system-defined enumeration types [C<DHTuple>, C<Database>,
C<DHRelation>, C<DHSet>, C<DHMaybe>, C<DHSingle>, C<DHArray>, C<DHBag>] are
...
the routine kind C<order_determination>) must have at least 3 parameters,
...
the N similar argument values come by way of a single nonscalar typed
parameter.

Or conceptually speaking, all Muldis D routines actually have exactly 1 or
...
correspond to the official named parameters.  With all functional routines,
...
system-defined routines such as C<sys.std.Core.Control.func_invo>, where
...
Now there are actually a lot more distinct routine kinds, but they are
special cases of the above-mentioned ones:
...
All of the leaf nodes in the second hierarchy above are distinguished by
their API (parameters and result) and intended use.  All of the ones under
I<functional> are also C<function> routines, so any context where a generic
C<function> can be used, so can they.
...
routines.  A C<function> invocation is trivially
...
Sometimes a C<function> is composed as part of the definition of another
C<function> (or C<updater>).  Conceptually speaking, former
C<function> is part of the definition of the body of the parent
...
A C<type_constraint> is the same as an C<function>, but that it is
...
C<topic>.  If the data type being selected is a scalar type or subtype
...
A C<named_value> is the same as an C<function>, but that it has only
zero parameters and unconditionally results in the same single value.  A
C<named_value> is often used when you want to declare a program constant
value that is easy to reference on a non-lexical scale.  A C<named_value>
...
A C<possrep_map> is the same as an C<function>, but that it is part
of the definition of a scalar data type, and it is used to convert a value
from one of that type's possreps to another.  A C<possrep_map> has exactly
1 parameter whose name is C<topic> and whose declared type is a tuple whose
...
=head2 order_determination

An C<order_determination> is the same as an C<function>, but that it
...
this file for more details.  An C<order_determination> has at least 3
parameters named [C<topic>, C<other>, C<is_reverse_order>] and
always results in an C<Order>.

=head2 value_filter

A C<value_filter> is the same as an C<function>, but that it has
at least 1 parameter, where that 1 is named C<topic>, and the function
always results in a C<Bool>; C<topic> is usually a C<Tuple> but doesn't
have to be.  Any additional parameters besides the above-named 1 are hereby
collectively referred to as I<misc params>.  A C<value_filter> is similar
to a C<type_constraint> in that it evaluates C<topic> for membership in a
particular value domain, resulting in C<Bool:true> if C<topic> is a member
and C<Bool:false> otherwise; but a C<value_filter> differs from a
C<type_constraint> in that it would be explicitly invoked possibly in any
context, and its criteria for evaluating C<topic> can be customized at
runtime by I<misc params>.  A C<value_filter> is what would be used in
operations like the general case of relational restriction.

=head2 value_map

A C<value_map> is the same as an C<function>, but that it has at
least 1 parameter, where that 1 is named C<topic>; C<topic> is usually a
C<Tuple> but doesn't have to be.  A C<value_map> is what would be used in
operations like the general case of relational extension or substitution.

=head2 value_reduction

A C<value_reduction> is the same as an C<function>, but that it has
at least 2 parameters, where those 2 are named [C<v1>, C<v2>]; C<v1> and
C<v2> must have the same declared type as each
other, and the C<value_reduction> function's declared result type must be
the same as that of C<v1> and C<v2>.  A C<value_reduction> is what would
typically be used in N-ary operations that can be defined in terms of a
repetition of binary operations, such that a C<value_reduction> would
...
invoke C<function> and C<updater>
...
relational-assignment short-hands such as "assign_insertion",
"assign_substitution", "assign_deletion".

=head2 system_service
...
(both catalog and data), and is the only kind of routine that
...
transactions over multiple consecutive statements, and is the
only kind of routine that can; specifically, those statements are parcelled
together into a separate C<procedure>, and then that is invoked
...
C<procedure>, then invoke that by way of the
...
Muldis D C<updater> and C<system_service>, and by
...
This is the hierarchy of invocation namespaces of DBMS entities:
    ...
    tpc  # time-varying opt user-def aliases for fed.* invoc namespaces
        tpc.cat  # upd sys cat db desc ent under tpc.[dep|sdp]
        tpc.[dep|sdp]
            tpc.[dep|sdp].[cat|lib|data]
                # and so on like under dep|sdp top-levels
    ...
    sdp  # entities in a subdepot ref their own subdepot with this
...
catalog variables all exist as the C<[sys|mnt|fed|dep|sdp].cat>
secondary namespaces (C<sys.cat> is a constant).  The global user data
variables all live as the C<[fed|dep|sdp].data> secondary namespaces.
...
except a procedure, or the host language if it exists.
...
Similarly, the primary namespace C<lex> refers to entities within
the same private scope as the referencer.  Variables
...
built-in type definition and constraints).  So the primary namespace C<sdp>
is I<also> used for individual post-split materials to refer to other
post-split materials within the same conceptual larger material.  I<TODO:
Maybe split the C<sdp> primary somehow so there are distinct top-levels for
post-split siblings and the parent subdepot of the whole post-split group.>
...
collection of material (type and routine) definitions where some parts of
the collection are private and others are public.  All entities that are
under a non-C<lex> namespace should all be considered possibly public or
globally referenceable (database user privileges notwithstanding).  By
...
namespaces of C<fed|dep|sdp> are fully independent in definition, such
...
The 3 system-defined user-data variables named C<[fed|dep|sdp].data>
are all of (almost) "just" the C<Database> type (which is a C<Tuple> proper
...
corresponding depot's default C<dep.data> value or it is the zero-attribute
...
accomplished by adding a material with the empty string entity declaration
name that is a database type definition directly within the depot's root
namespace, then the declared type of that depot's C<dep.data> is that same
database type.
...
and are applied directly to the C<dep.data> variable (and C<fed.data> also)
by the depot's catalog in another way.

Conceptually speaking, for the ultimate freedom from constraints, the
declared type of C<dep.data> can be a simple alias for the C<Database>
type, meaning that users can update C<dep.data> (or more specifically, C<<
fed.data.<depot-mount-name> >>) with any C<Database> value at all, and
...
below as, strictly speaking, some database values can't be in C<dep.data>.

If a depot has any subdepots, then the value of C<dep.data>
must have corresponding attributes that match by name, recursively, iff the
subdepot has a self-local dbvar.  For example, if a depot has a
root-child subdepot named C<foo> and a child subdepot of that named C<bar>,
...
C<dep.data.foo.bar> is the pseudo-dbvar of C<sdp.data> as seen by entities
in the C<bar> subdepot.  The type of C<dep.data> may have additional
attributes besides those matching subsepots, but it may not
lack any corresponding ones with subdepots that have self-local dbvars.
...
its C<dep.data>, goes also for the catalog of a subdepot and
its C<sdp.data>, respectively.  Note that while a depot or
...
Strictly speaking, the type of a depot/subdepot's self-local dbvar
can I<never> be "just a database" (an alias of C<Database>).  To start
with, a depot/etc has at least an empty string named child type iff it has
a self-local dbvar, and so said dbvar's type has to at least exclude every
database value with an attribute whose name is the empty string.  For a
depot/etc with no child subdepots or materials besides that
single type, that would be the only limitation.  Regarding any other child
...
type to be C<sdp.lib.foo."">; if it doesn't, then the parent's type must
...
I<A further feature under consideration is that, for any routine or type
whose declared name is the empty string, its parent subdepot is a proxy for
it for referencing purposes; so for example, C<sdp.lib.foo> is a proxy for
C<sdp.lib.foo."">, which if that was a relvar attribute's type, might be
the type for C<sdp.data.foo>.  But for now, that feature does not exist.>
...
syntax.  In particular this affects any code that needs certain kinds of
portability such as when dumping the contents of one depot as code and
wanting to run that code to load a different-mount-named depot.

The C<tpc> primary namespace exists to bring the benefits of the
C<dep|sdp> namespaces to referencing code living outside the same
depot|subdepot as the referenced entity.  For example, while the
...
Similarly, the catalog variable C<tpc.cat.topic_subdepot>
exists.  While C<tpc.cat.topic_depot> etc is not
valued (that is, it is C<nothing>), referencing C<tpc.dep> etc will fail.

This "topic depot|subdepot" feature is analagous to the "current
...
Dealing with a nonscalar type is slightly more complicated yet, due
...
taking the declared type where a tuple+relation type was declared
together, you use extra syntax to say whether it is a tuple type or
relation type that you actually want.  So to deal with this, the
aforementioned forms of taking types actually have
C<[.[|dh_][tuple|relation]_from]**0..1> in their syntax following the
...
C<[.[|dh_][set|maybe|single|array|bag]_of]**0..*>; this C<of> element

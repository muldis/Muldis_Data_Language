...
This document is Muldis::D::Basics version 0.57.0.
...
and C<Nothing> nonscalar types don't each come in 2 flavors.)

A I<scalar value> is, by definition, any quasi-scalar value that either has
no possreps (it is an C<Int> or a C<String>) or all of its possreps are
such that any quasi-relation-valued attributes it has, directly or
indirectly, are also just relation-valued.
...
system-defined root types (C<Int>, C<String>, C<Blob>, C<Text>, C<QTuple>,
C<QRelation>, etc) are infinite, as are the C<Universal>, C<QScalar>,
C<Reference>, C<Remnant> types.
...
of the depot C<mydb>, any code that references C<topic.dep.lib.myfunc> will

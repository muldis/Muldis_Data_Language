...
A C<type-constraint> is the same as an C<function>, but that it is
...
a C<Bool>.  The parameters of a C<type-constraint> carry information about
...
success.  A C<type-constraint> has exactly 1 parameter whose name is
...
=head2 state-constraint

A C<state-constraint> is the same as a C<type-constraint> except that it is
...
A C<named-value> is the same as an C<function>, but that it has only
zero parameters and unconditionally results in the same single value.  A
C<named-value> is often used when you want to declare a program constant
...
A C<possrep-map> is the same as an C<function>, but that it is part
of the definition of a scalar data type, and it is used to convert a value
from one of that type's possreps to another.  A C<possrep-map> has exactly
...
=head2 virtual-attr-map

A C<virtual-attr-map> is the same as a C<possrep-map> except that its range
...
probably a database).  A C<virtual-attr-map> has a single C<topic>
...
An C<order-determination> is the same as an C<function>, but that it
...
this file for more details.  An C<order-determination> has at least 3
parameters named [C<topic>, C<other>, C<is_reverse_order>] and
always results in an C<Order>.

=head2 value-filter

A C<value-filter> is the same as an C<function>, but that it has
...
collectively referred to as I<misc params>.  A C<value-filter> is similar
...
runtime by I<misc params>.  A C<value-filter> is what would be used in
operations like the general case of relational restriction.

=head2 value-map

A C<value-map> is the same as an C<function>, but that it has at
least 1 parameter, where that 1 is named C<topic>; C<topic> is usually a
C<Tuple> but doesn't have to be.  A C<value-map> is what would be used in
operations like the general case of relational extension or substitution.

=head2 value-reduction

A C<value-reduction> is the same as an C<function>, but that it has
at least 2 parameters, where those 2 are named [C<v1>, C<v2>]; C<v1> and
C<v2> must have the same declared type as each
other, and the C<value-reduction> function's declared result type must be
the same as that of C<v1> and C<v2>.  A C<value-reduction> is what would
...
=head2 system-service

A C<system-service> is an explicitly invokable system-defined procedure
...
beast, users can not define their own C<system-service> routines but by

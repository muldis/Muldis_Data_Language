...
This document is Muldis::D::Basics version 0.78.0.
...
function's third parameter, C<assuming>, is C<Tuple>-typed and carries
...
then the tuple argument would be of degree zero.  The function's fourth
...
A C<named_value> is the same as an C<inner_function>, but that it has zero
parameters and unconditionally results in the same single value.  A
...
of the definition of a scalar data type, and it is used to convert
a value from one of that type's possreps to another.  A C<possrep_map> has
a single C<topic> parameter whose declared type is a tuple whose
attributes match those of the possrep being converted from, and its result
type is a tuple whose attributes match those of the possrep being
...
this file for more details.  An C<order_determination> has exactly 4
parameters named [C<topic>, C<other>, C<assuming>, C<is_reverse_order>] and
always results in an C<Order>.

=head2 value_filter

A C<value_filter> is the same as an C<inner_function>, but that it has
exactly 2 parameters named [C<topic>, C<assuming>] and always results in a
C<Bool>; C<assuming> is always a C<Tuple>, and C<topic> is usually a
C<Tuple> but doesn't have to be.  A C<value_filter> is similar to a
C<type_constraint> in that it evaluates C<topic> for membership in a
...
runtime by C<assuming>.  A C<value_filter> is what would be used in
operations like the general case of relational restriction.

=head2 value_map

A C<value_map> is the same as an C<inner_function>, but that it has exactly
2 parameters named [C<topic>, C<assuming>]; C<assuming> is always a
C<Tuple>, and C<topic> is usually a C<Tuple> but doesn't have to be.  A
C<value_map> is what would be used in operations like the general case of
relational extension or substitution.

=head2 value_reduction

A C<value_reduction> is the same as an C<inner_function>, but that it has
exactly 3 parameters named [C<v1>, C<v2>, C<assuming>]; C<assuming> is
always a C<Tuple>; C<v1> and C<v2> must have the same declared type as each

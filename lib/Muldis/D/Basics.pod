...
This document is Muldis::D::Basics version 0.17.0.
...
L<Muldis::D::Dialect::HDMD_Perl_Tiny>, L<Muldis::D::Hierarchical>.  Other,
unofficial Muldis D dialects may be made by third parties in the future,
...
it does not live in any depot, it can not have any parameters or lexical
variables or inner routines, it can not directly see or update any global
variables, and it can only invoke procedures that have no subject-to-update
parameters.  Fundamentally, the only task that a 'main' should be doing is
loading or creating depots (by invoking system-defined procedures that
update the system catalog), which should contain all of the other
user-defined routines as if they were code libraries, and invoking them. In
a mixed-language application, where Muldis D code is invoked by another
host language, there is no Muldis D C<main> at all, since it would be
redundant with host langauge routines (and host language routines can do
anything that a C<function> or C<updater> or C<procedure> can do).
...
This is the hierarchy of invocation namespaces of DBMS entities:
    ...
    sys  # system-defined types and routines defined by standard Muldis D
        sys.Core
            sys.Core(.<namespace>){0,}
                sys.Core(.<namespace>){0,}.<type>
                sys.Core(.<namespace>){0,}.<routine>
        sys.<extension>
            sys.<extension>(.<namespace>){0,}
                sys.<extension>(.<namespace>){0,}.<type>
                sys.<extension>(.<namespace>){0,}.<routine>
...
besides the C<sys>.  The most fundamental standard types and routines,
...
extra name spaces.  For system-defined entities, package names are simply a
convenience, routines that work with particular types don't have to be
grouped with said types' declarations.

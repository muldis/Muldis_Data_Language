...
This document is Muldis::D::Basics version 0.18.1.
...
This is the hierarchy of invocation namespaces of DBMS entities:
    ...
    cat  # read-only sys cat db desc entities under cat|sys|imp|mnt, *.cat
    ...
    mnt  # updateable system catalog controlling what depot mounts exist
...
catalog variables exist partially as the C<cat|mnt> primary namespaces
(C<cat> is constant), and partially as the C<(fed|dep|sdp|pkg).cat>
secondary namespaces.  The global user data variables all live as the
C<(fed|dep|sdp|pkg).data> secondary namespaces.  All non-global variables
are just of the user data variety, can be of any types, and use the
C<pkg|lex> primary namespaces (yes, C<pkg> has both public and private
variables).

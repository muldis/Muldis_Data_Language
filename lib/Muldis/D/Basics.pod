...
semi-orthogonal to the above-mentioned ones:
...
Specifically, C<function> vs C<inner_function> is a distinction of where a
routine of that kind is declared, whereas all of the leaf nodes in the
second hierarchy above are distinguished by their API (parameters and
result) and intended use.  Each routine of the routine kinds in the second
hierarchy, such as C<possrep_map>, can be either a C<function> or
C<inner_function> depending on where it is declared; in practice, most of
the routine kinds in the second group tend to be C<inner_function> but they
don't need to be.

=head2 function, inner_function
...
A C<named_value> is the same as an C<inner_function>, but that it has only
...
A C<value_filter> is the same as an C<inner_function>, but that it has
...
A C<value_map> is the same as an C<inner_function>, but that it has at
...
A C<value_reduction> is the same as an C<inner_function>, but that it has
...
=head2 procedure, inner_procedure
...
Muldis D C<updater> (and C<inner_updater>) and C<system_service>, and by
...
By contrast, every C<procedure> (and C<inner_procedure>)
...
Similarly, the primary namespaces C<dep|sdp|inn|lex> (yes, C<dep|sdp> are
repeated) refer to entities within
the same private scope as the referencer; except for a subset of what is
seen under C<dep|sdp>, these items are not referenceable globally; a depot
or subdepot, on a case by case basis,
...
Each individual depot or subdepot
should be interpreted as an integrated collection
...
type to be C<sdp.foo."">; if it doesn't, then the parent's type must

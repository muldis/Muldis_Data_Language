...
This document is Muldis::D::Basics version 0.148.0.
...
rejected in favor of some other alternatives for now:
    ...
    ----+--------+-------------------------+-------------------------------
    ;   | 0x3B   | SEMICOLON               | possible stmt/expr separator
...
At the lowest, most primitive level of Muldis D, everything is generic
ordered lists; data types in general are just abstractions of this.

A C<Universal> is a generic ordered list of 0..N elements where every
element is also a C<Universal>; but you don't access elements using numeric
indexes but rather without any indexes, using operators like I<first> and
I<nonfirst>.  In the general case, a C<Universal> can be an arbitrarily
complex hierarchical structure of unlimited size, where the leaves of this
hierarchy are each the empty C<Universal>.  One can conceive of
implementing the general case of a C<Universal> either with an array or
singly-linked list, where either is copy-on-write; all of the most
primitive-level Muldis D operators are abstract enough to work either way.

A C<Boolean> is a C<Universal> of exactly 0 (false) or 1 (true) elements,
in the latter case whose sole element is the empty C<Universal>.

A C<LL_Natural> is a C<Universal> of 0..N elements where each element is an
empty C<Universal>; the (always non-negative) numeric value (0..N) is equal
to the count of elements.  The C<LL_Natural> type is just intended to be
used in the definitions of other primitive system-defined Muldis D types
and users should typically employ the wrapping type C<Integer> instead.

A C<LL_Structure>, loosely speaking, is a C<Universal> of exactly 2
elements where the first element is is the I<structure kind> and the second
element is the I<payload>.  To be more specific, a C<LL_Structure> is a
union type over 11 other types which all have that structure, and the
I<structure kind> element is a C<LL_Natural> with numeric value of 0..10
that specifies which of those other types the value is of; the allowed
I<payload> format is defined separately for each of the 11 types.

The 11 C<LL_Structure> types are: C<Integer>, C<String>, C<Array>,
C<SC_Heading>, C<SC_Renaming>, C<Tuple>, C<Relation>, C<SC_Identifier>,
C<Capsule>, C<Stream>, C<External>.  Generally speaking these 11 plus
C<Boolean> are what should be considered the fundamental Muldis D types
from a user's perspective, and typically all other system-defined types
are defined in terms of those, especially C<Capsule>.

This table says which I<structure kind> values indicate which of the 11
structure types, plus examples of literal syntax:

     0 | Integer       | 42 or 0 or -3 or 0xDEADBEEF or 0o644 or 0b11001001
     1 | String        | ideas are \+[expr,...] and \~[expr,...] and Blob-inspired versions like 0x[lit,...]/etc
     2 | Array         | [expr,...] or ([expr,...]) to disambiguate from procedure block
     3 | SC_Heading    | \~{lit,...} - T/R heading as userland value - arg for project/etc
                       |    and SC_Name is one with =1 elem, eg \~{lit}
     4 | SC_Renaming   | \<-{lit:lit,...} - use in routine predicate defs a lot to normalize params, arg for rename
     5 | Tuple         | \%{lit:expr,...}
     6 | Relation      | \@{{lit:expr,...},...} or \@[lit,...]:{[expr,...],...}
     7 | SC_Identifier | Several kinds:
                       |    - canon    : \$=pkg-canon-name.litpath  eg \$=Muldis_D:"http://muldis.com":0.Integer.sum
                       |    - local    : \$~loc-pkg-alias.litpath   eg \$~MD.Integer.sum
                       |    - relative : \$<nn-int-expr>litpath     eg \$<2>foo.bar or \$<0>quux
                       |    - search   : \$?litpath                 eg \$?Bag
                       |    also special syntax "resolved expr" will
                       |    produce canon/Reference from all kinds of Identifier expr eg "resolved \$?foo"
                       |    and $lit is shorthand for "resolved \$?lit" eg $Bag short for "resolved \$?Bag"
                       |    To mneumonics, = means identity, ? means where-is-it,
                       |    for rel read eg go-back-2-then-forward-this-path or "<" means relative-to.
                       |    For non-literals rtn call replace \$ with & eg "&~pkg.foo.bar()"
                       |    but non-prefixed forms like "foo.bar()" are assumed to be "search".
     8 | Capsule       | expr=>expr - that is Reference=>Tuple eg $Date=>\%{year:500,month:4,day:24}
                       |    and Singleton is one with nullary tuple eg $To_Zero=>\%{}
                       |    and $=>lit is shorthand for Singleton value eg $=>To_Zero
     9 | Stream        | stream expr - prefix keyword; foo is an Integer-payload
    10 | External      | external expr - prefix keyword; foo is a Universal

Literal syntax for C<Boolean> is just the keywords C<false> and C<true>.

A literal syntax for C<LL_Natural> also exists and looks like this:
C<\*42>; however, this syntax only exists for completeness and no one
should ever be using it in practice, as users should be using C<Integer>
instead for all normal situations.

A literal syntax for C<Universal> also exists and one can use it to select
a value of any type of the entire type system, and it looks like this:
C<\*[...]>; however, no one should ever be using it in practice, as better
syntax exists for all types that users should be using, and the general
C<Universal> literal syntax only exists for completeness, should one need
to serialize some strange C<Universal> value that isn't one of the other
system-defined types, such as a C<Universal> of 3 elements in isolation
from some wrapping structure value.  A variant literal syntax C<\*:[...]>
also exists which is specific to a C<Universal> with exactly 2 elements and
it exists just to help logically bootstrap the language's lowest levels.

Subtypes of Capsule:

    Bag      | \+{expr,...} or \+{expr:posint-expr,...}
    Blob     | 0x'27EO4' or 0o'350261' or 0b'101100110'
    Dict     | \~{expr:expr,...} or something
    Float    | 29*10**30 or 4.5209E19 or something?
    Interval | expr..expr etc
...
These are the I<payload> structures for the 11 structure types:

    Integer : Universal[2] :
        0 : is-neg : Boolean
        1 : magnitude : LL_Natural
    String : Universal[0..N] of Integer-payload
    Array  : Universal[0..N] of Universal
    Heading : Universal[M in 0..N] of String-payload - elems must be sorted
    Renaming : Universal[2] :
        0 : map-to : Heading-payload
        1 : map-from : Heading-payload
    Tuple  : Universal[2] :
        0 : heading : Heading-payload
        1 : body : Universal[M] of Universal - elems correspond to heading
    Relation : Universal[2] :
        0 : heading : Heading-payload
        1 : Universal[0..P] of same as Tuple body
    Identifier : Universal[5] :
        0 : identifier_kind : LL_Natural in 0..3
            0 : canon
            1 : local
            2 : relative
            3 : search
        1 : package name base : Universal[0..N] of String-payload - is nonempty iff kind in 0..1
        2 : package name extensions : Universal[0..E] of String-payload - may be nonempty iff kind=0
            0 : authority
            1 : version number
            2+ : available for something else?
        3 : path beneath package : Universal[0..P] of String-payload
        4 : relative starts K levels up : Integer-payload in 0..J - is nonzero iff kind=2
        ==> proper subtype Reference (aka "Global Identifier")
            - kind must = 0
            - package name base, extensions, path must all be fully-qualified
                - these are the full declared name of the package actually
                  linked in, not what was requested by users
                - extensions might be an empty list though, whatever was
                  declared (we might decide to disallow such declarations)
            - relative starts K must = 0
    Capsule : Universal[2] :
        0 : Reference-payload (subtype of Identifier-payload)
        1 : Tuple-payload
    Stream : Integer-payload
    External : Universal

The C<Identifier> type in general represents every variety of name-chain
identifier that can be present in Muldis D source code, from
fully-qualified to partially-qualified to unqualified, and thus matches
exactly what the user wrote, without context-based interpretation.

At program runtime, some contexts require the C<Reference> subtype,
the most fully-qualified possible version of an C<Identifier> in order to
work properly.  One main context is the type name portion of a C<Capsule>,
so that its values compare for equality properly in any package's routines
no matter what package's routines the values were selected in.  The other
main context is when for example a routine is declared in one package and
invoked in another, so that any types or entities that I<it> invokes are
still relative to its place of declaration.  Conceptually this could be
called "Global_Identifier" but it was expected that users would be
declaring signatures with this type frequently, much more often than the
more generic "Identifier", and so a shorter type name was chosen.

Typically in Muldis D code a special expression node must be wrapped around
any C<Identifier> selector so that it is resolved to a C<Reference>
in the appropriate context-based interpretation right where selected.

B<TODO: THE ABOVE DOESN'T YET HANDLE ANONYMOUSLY DECLARED TYPES OR ROUTINES
OR WHATEVER, IF WE WANT TO SUPPORT THAT.  IF SO, WE NEED TO ADD ANOTHER
Identifier COMPONENT OR FEW TO INDICATE NAMED VS ANON ETC.
OR IT MAY TURN OUT THAT SOURCE CODE METADATA WILL SATISFY NAMED VS ANON.
RELATED TO THAT, DO WE WANT TO SUPPORT TREATING AT RUNTIME SEVERAL
INDEPENDENTLY DEFINED Capsule TYPES AS BEING THE SAME TYPE AND THEIR VALUES
COMPARE EQUAL; THEN WE WOULD NEED TO REMAP ALL AT RUNTIME AS IF EVERYTHING
WAS THEN ANONYMOUS TYPES OR GENERATED NAMES ETC.
BUT CAPSULE TYPE DECLARATIONS ALREADY INCLUDE AN EXPLICIT DECLARATION OF
THEIR EFFECTIVE TYPE, IT ISN'T JUST READ FROM THE CONTEXT OF DEFINING.>

...

I<The following is out of date and needs updating.>

Rules for determining whether two C<Universal> are identical are defined
separately per each of the 5 C<Structure> subtypes; but two C<Universal> are
...
as a C<Universal>-hierarchy of C<Int> determines them to be identical, then this
...
An C<Integer> is a single exact
integral number of any magnitude; while it can be unbounded in size, by far
most of the C<Integer> values actually used are quite small, and would fit in a
single hardware CPU register in their entirety.  Two C<Integer> are considered
identical iff they both represent the same integer.

A C<Array> is a transparent dense
...
designated I<elements>.  The I<elements> is a C<Universal> of 0..N C<Int> where
...
each string element is interpreted as a single codepoint of the character
...
The I<heading> is a C<Universal> of 0..N mutually distinct elements where each
element is designated an I<attribute name>; each I<attribute name> is a
C<String>.  The I<tuple body> is a C<Universal> of 0..N elements where each
...
attributes> is a C<Universal> of 0..N elements where each element is designated
a I<tuple attribute>; a I<tuple attribute> is a C<Universal> of exactly 2
...
I<row-oriented tuple>.  The I<relation body> is a C<Universal> of 0..N mutually
...
attributes>.  The I<relation attributes> is a C<Universal> of 0..N elements
where each element is designated a I<relation attribute>; a I<relation
attribute> is a C<Universal> of exactly 2 elements, that are designated in
order: I<attribute name>, I<tuple attribute values>; a I<tuple attribute
values> is a C<Universal> of 0..N elements where each element is designated
...
I<possrep name>, I<possrep attributes>.  The I<type name> is a C<Universal> of
...
then interpretation is as per naive C<Universal> comparison semantics; iff there
...
designated I<payload>.  The I<payload> is a C<Universal> that the
...
identical if their I<payload> are the same C<Universal>; otherwise the
determination of identity is left to the other language.  Note that the
C<Universal> payload provision is provided so that it is possible to have some

...
Every C<Structure> is a C<List> having exactly 2 elements, the first being
the I<structure kind> and the second being the I<payload>.
...
types:
     ...
     3 | Boolean    | false or true - term keywords
     4 | Heading    | :~{lit,...} - T/R heading as userland value - arg for project/etc
     5 | Renaming   | :<{lit<-lit,...} - use in routine predicate defs a lot to normalize params, arg for rename
     6 | Tuple      | :%{lit:expr,...}
     7 | Relation   | :@{{lit:expr,...},...} or :@[lit,...]:{[expr,...],...}
     8 | Identifier | $lit etc - also special syntax :*expr will produce Reference from Identifier expr
     9 | Capsule    | expr=>expr - that is Reference=>Tuple eg :*$Date=>:%{year:500,month:4,day:24}
    10 | Stream     | stream expr - prefix keyword; foo is an Integer
    11 | External   | external expr - prefix keyword; foo is a Universal
...
These are the I<payload> structures for those:

    String : List[0..N] of Integer
    Array  : List[0..N] of Universal
    Boolean : Integer in 0..1
    Heading : List[M in 0..N] of String-payload - elems must be sorted
    Renaming : List[2] :
        0 : map-to : Heading-payload
        1 : map-from : Heading-payload
    Tuple  : List[2] :
        0 : heading : Heading-payload
        1 : body : List[M] of Universal - elems correspond to heading
    Relation : List[2] :
        0 : heading : Heading-payload
        1 : List[0..P] of same as Tuple body
    Identifier : List[5] :
        0 : kind : Integer in 1..3
            1 : absolute
            2 : relative
            3 : search
        1 : package name base : List[0..N] of String-payload - nonempty iff kind=1
        2 : package name extensions : List[0..2] of String-payload - nonempty iff kind=1
            0 : authority
            1 : version number
        3 : path beneath package : List[0..P] of String-payload
        4 : relative starts K levels up : Integer in 0..J - nonzero iff kind=2

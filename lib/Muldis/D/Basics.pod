...
structure types, plus examples of literal syntax:
     ...
     7 | SC_Identifier | $lit etc - also special syntax :*expr will produce Reference from Identifier expr
...
There is no literal syntax specific to C<LL_Natural>; instead use the
literal syntax for an C<Integer> and call an unwrap function on it.
...
from some wrapping structure value.  Mnemonic, C<|> as ultimate type union.

...
This document is Muldis::D::Ext::Blob version 0.53.0.
...
This is an enumeration data type.  A C<PInt1_4> is a proper subtype of
...
=item C<function sys.std.Blob.catenation result Blob params {
topic(array_of.Blob) }>
...
=item C<function sys.std.Blob.repeat result Blob params { topic(Blob),
count(NNInt) }>

This function results in the catenation of C<count> instances of C<topic>.

=item C<function sys.std.Blob.length_in_bits result NNInt params {
topic(Blob) }>

This function results in the length of its argument in bits.

=item C<function sys.std.Blob.length_in_octets result NNInt params {
topic(OctetBlob) }>

This function results in the length of its argument in octets.

=item C<function sys.std.Blob.is_substr_bits result Bool params {
...
=item C<function sys.std.Blob.is_not_substr_bits result Bool params {
look_in(Blob), look_for(Blob), fixed_start(Bool)?, fixed_end(Bool)? }>
...
=item C<function sys.std.Blob.is_substr_octets result Bool params {
...
=item C<function sys.std.Blob.is_not_substr_octets result Bool params {
look_in(OctetBlob), look_for(OctetBlob), fixed_start(Bool)?,
fixed_end(Bool)? }>
...
=item C<function sys.std.Blob.not result Blob params { topic(Blob) }>

This function results in the bitwise I<not> of its argument.

=item C<function sys.std.Blob.and result Blob params { topic(set_of.Blob)
}>
...
=item C<function sys.std.Blob.or result Blob params { topic(set_of.Blob) }>
...
=item C<function sys.std.Blob.xor result Blob params { topic(bag_of.Blob)
}>
...
=item C<function sys.std.Blob.Blob_from_Text result Blob params {
...
=item C<function sys.std.Blob.Text_from_Blob result Text params {

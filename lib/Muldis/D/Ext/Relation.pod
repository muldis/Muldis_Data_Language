...
This document is Muldis::D::Ext::Relation version 0.79.1.
...
ValMapFuncRef $func, Tuple $assuming?, Name $class_attr_name,
...
either of those tuples as its C<topic> argument, and C<assuming> as its
...
ValRedFuncRef $func, Tuple $assuming?, Tuple $identity) >>
...
argument, and that function must
...
C<Tuple>-resulting C<value_map> function named in its C<func> argument when
the latter
...
ValFiltFuncRef $restr_func, Tuple $restr_assuming?,
set_of.Name $subst_attr_names,
ValMapFuncRef $subst_func, Tuple $subst_assuming?) >>
...
Relation $topic, ValFiltFuncRef $restr_func, Tuple $restr_assuming?,
Tuple $subst) >>
...
ValFiltFuncRef $restr_func, Tuple $restr_assuming?, Tuple $subst)>
...
Relation $restr, set_of.Name $subst_attr_names,
ValMapFuncRef $subst_func, Tuple $subst_assuming?)>

...
This document is Muldis::D::Ext::Relation version 0.78.0.
...
in that they typically work with any relation types.  Each C<assuming>
...
FuncRef $func, Tuple $assuming?, Name $class_attr_name,
...
It classifies the tuples of C<topic> into N groups using the function
named by C<func>, such that any distinct tuples are in a common group if
the function named by C<func> results in the same value when given either
of those tuples as its C<topic> argument, and C<assuming> as its
...
C<func> and C<assuming>, and then grouping that result relation on all of
...
FuncRef $func, Tuple $assuming?, Tuple $identity) >>
...
pair until just one input tuple is left, which is the result.  The
...
parameters are the 2 input tuples for an invocation.  If C<topic> has
zero tuples, then C<reduction> results in the tuple given in
...
Name $name, FuncRef $ord_func, Tuple $ord_assuming?,
...
FuncRef $ord_func, Tuple $ord_assuming?, NNInt $min_rank, NNInt $max_rank,
Bool $is_reverse_order?) >>
...
FuncRef $restr_func, Tuple $restr_assuming?, set_of.Name $subst_attr_names,
FuncRef $subst_func, Tuple $subst_assuming?) >>
...
Relation $topic, FuncRef $restr_func, Tuple $restr_assuming?,
Tuple $subst) >>
...
Relation $primary, Relation $secondary, FuncRef $exten_func,
Tuple $exten_assuming?) >>
...
each said C<primary> tuple when the named function is curried using the
...
FuncRef $restr_func, Tuple $restr_assuming?, FuncRef $subst_func,
Tuple $subst_assuming?)>
...
FuncRef $restr_func, Tuple $restr_assuming?, Tuple $subst)>
...
Relation $restr, FuncRef $subst_func, Tuple $subst_assuming?)>

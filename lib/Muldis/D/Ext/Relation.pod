...
This document is Muldis::D::Ext::Relation version 0.51.0.
...
topic(Relation), func(Cat.FuncRef), assuming(QuasiTuple)?, identity(Tuple)
}>
...
topic(Relation), name(Cat.Name), ord_func(Cat.FuncRef),
...
topic(Relation), ord_func(Cat.FuncRef), ord_assuming(QuasiTuple)?,
...
Relation params { topic(Relation), restr_func(Cat.FuncRef),
restr_assuming(QuasiTuple)?, subst_func(Cat.FuncRef),
...
params { topic(Relation), restr(Relation), subst_func(Cat.FuncRef),
subst_assuming(QuasiTuple)? }>
...
params { primary(Relation), secondary(Relation), filler(Cat.TypeRef) }>
...
params { primary(Relation), secondary(Relation), exten_func(Cat.FuncRef),
...
{ topic(Relation) } read { restr_func(Cat.FuncRef),
restr_assuming(QuasiTuple)?, subst_func(Cat.FuncRef),
subst_assuming(QuasiTuple)? }>
...
update { topic(Relation) } read { restr_func(Cat.FuncRef),
restr_assuming(QuasiTuple)?, subst(Tuple) }>
...
topic(Relation) } read { restr(Relation), subst_func(Cat.FuncRef),

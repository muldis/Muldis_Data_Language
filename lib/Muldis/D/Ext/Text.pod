...
C<< function sys.std.Text.catenation (Text <-- topic(array_of.Text)) >>
...
C<< function sys.std.Text.repeat (Text <-- topic(Text), count(NNInt)) >>

This function results in the catenation of C<count> instances of C<topic>.

=head2 sys.std.Text.length_in_codepoints

C<< function sys.std.Text.length_in_codepoints (NNInt <-- topic(Text)) >>
...
C<< function sys.std.Text.length_in_graphemes (NNInt <-- topic(Text)) >>
...
C<< function sys.std.Text.is_substr (Bool <-- look_in(Text),
...
C<< function sys.std.Text.is_not_substr (Bool <-- look_in(Text),
look_for(Text), fixed_start(Bool)?, fixed_end(Bool)?) >>
...
C<< function sys.std.Text.folded_to_NF(C|D) (Text <-- topic(Text)) >>
...
C<< function sys.std.Text.case_folded_to_upper (Text <-- topic(Text)) >>
...
C<< function sys.std.Text.case_folded_to_lower (Text <-- topic(Text)) >>
...
C<< function sys.std.Text.accents_stripped (Text <-- topic(Text)) >>
...
C<< function sys.std.Text.ASCII (Text <-- topic(Text), mark(Text)?) >>
...
C<< function sys.std.Text.whitespace_trimmed (Text <-- topic(Text)) >>
...
C<< function sys.std.Text.is_match_using_like (Bool <-- look_in(Text),
...
C<< function sys.std.Text.is_not_match_using_like (Bool <-- look_in(Text),
look_for(Text), escape(Text)) >>

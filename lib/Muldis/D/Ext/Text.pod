...
This document is Muldis::D::Ext::Text version 0.21.0.
...
C<sys.Text.Type.(Set|Maybe|Seq|Bag)OfText>.

I<This documentation is pending.>

=head1 SYSTEM-DEFINED TEXT-CONCERNING DATA TYPES

=item C<sys.Text.Type.(Set|Maybe|Seq|Bag)OfText>
...
=item C<function sys.Text.catenation result Text params { topic(SeqOfText)
}>
...
=item C<function sys.Text.repeat result Text params { topic(Text),
count(UInt) }>

This function results in the catenation of C<count> instances of C<topic>.

=item C<function sys.Text.length_in_nfd_graphs result UInt params {
topic(Text) }>

This function results in the length of its argument in Unicode
Normalization Form D graphemes.

=item C<function sys.Text.length_in_nfc_graphs result UInt params {
topic(Text) }>

This function results in the length of its argument in Unicode
Normalization Form C graphemes.

=item C<function sys.Text.length_in_nfd_codes result UInt params {
topic(Text) }>

This function results in the length of its argument in Unicode
Normalization Form D code points.

=item C<function sys.Text.length_in_nfc_codes result UInt params {
topic(Text) }>

This function results in the length of its argument in Unicode
Normalization Form C code points.

=item C<function sys.Text.is_substr result Bool params { look_in(Text),
...
=item C<function sys.Text.is_not_substr result Bool params { look_in(Text),
look_for(Text), fixed_start(Bool), fixed_end(Bool) }>

This function is exactly the same as C<sys.Text.is_substr> except that it
results in the opposite boolean value when given the same arguments.

=item C<function sys.Text.case_folded_to_upper result Text { topic(Text) }>
...
=item C<function sys.Text.case_folded_to_lower result Text { topic(Text) }>

This function results in the transformation of its argument where any
letters considered to be (capital) uppercase are folded to (small)
lowercase.

=item C<function sys.Text.whitespace_trimmed result Text { topic(Text) }>

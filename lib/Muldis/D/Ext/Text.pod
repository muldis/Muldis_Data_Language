...
This document is Muldis::D::Ext::Text version 0.24.0.
...
constraint doesn't apply.  Note that C<is_substr> will handle the common
special cases of SQL's "LIKE" operator for patterns like ['foo', '%foo',
'foo%', '%foo%'], but see also the C<is_match_using_like> function which
provides the full generality of SQL's "LIKE", such as 'foo%bar%baz'.

...
This document is Muldis::D::Ext::Ordered version 0.61.0.
...
argument.  For any q/scalar root type's type-default order function, the
...
=head2 sys.std.Ordered.reverse_order

C<function sys.std.Ordered.reverse_order result Order params {
topic(Universal), other(Universal), func(OrdDetFuncRef)?, assuming(QTuple)?
}>
...
C<function sys.std.Ordered.is_before result Bool params { topic(Universal),
other(Universal), func(OrdDetFuncRef)?, assuming(QTuple)? }>

This function results in C<Bool:true> iff the wrapped function would result
in C<Order:increase> when given the same arguments, and C<Bool:false>
otherwise.

=head2 sys.std.Ordered.is_not_before

C<function sys.std.Ordered.is_not_before result Bool params {
topic(Universal), other(Universal), func(OrdDetFuncRef)?, assuming(QTuple)?
}>
...
C<function sys.std.Ordered.is_inside_range result Bool params {
topic(Universal), min(Universal), max(Universal), min_is_outside(Bool)?,
max_is_outside(Bool)?, func(OrdDetFuncRef)?, assuming(QTuple)? }>
...
C<function sys.std.Ordered.is_outside_range result Bool params {
topic(Universal), min(Universal), max(Universal), min_is_outside(Bool)?,
max_is_outside(Bool)?, func(OrdDetFuncRef)?, assuming(QTuple)? }>
...
C<function sys.std.Ordered.min result Universal params { topic(QSet),
func(OrdDetFuncRef)?, assuming(QTuple)? }>
...
C<function sys.std.Ordered.max result Universal params { topic(QSet),
func(OrdDetFuncRef)?, assuming(QTuple)? }>
...
C<function sys.std.Ordered.minmax result QTuple params { topic(QSet),
func(OrdDetFuncRef)?, assuming(QTuple)? }>
...
C<function sys.std.Ordered.maybe_min result QMaybe params { topic(QSet),
func(OrdDetFuncRef)?, assuming(QTuple)? }>
...
C<function sys.std.Ordered.maybe_max result QMaybe params { topic(QSet),
func(OrdDetFuncRef)?, assuming(QTuple)? }>
...
C<function sys.std.Ordered.maybe_minmax result QRelation params {
topic(QSet), func(OrdDetFuncRef)?, assuming(QTuple)? }>

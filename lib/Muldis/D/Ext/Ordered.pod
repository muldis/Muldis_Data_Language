...
This document is Muldis::D::Ext::Ordered version 0.64.0.
...
C<< function sys.std.Ordered.reverse_order (Order <-- Universal $topic,
Universal $other, OrdDetFuncRef $func?, QTuple $assuming?) >>

This C<order_determination> function is exactly the same as the
C<order_determination> function it wraps except that it results in the
reverse value when given the same non-C<func> arguments.  It is a
short-hand for applying C<sys.std.Core.Cat.Order_reverse> to the result of
the wrapped function with the same non-C<func> arguments.

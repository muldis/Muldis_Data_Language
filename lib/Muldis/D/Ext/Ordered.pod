...
This document is Muldis::D::Ext::Ordered version 0.54.0.
...
=item C<function sys.std.Ordered.reverse_order result Order params {
...
=item C<function sys.std.Ordered.min result Universal params { topic(QSet),
...
=item C<function sys.std.Ordered.max result Universal params { topic(QSet),
func(OrdDetFuncRef)?, assuming(QTuple)? }>
...
=item C<function sys.std.Ordered.maybe_min result QMaybe params {
topic(QSet), func(OrdDetFuncRef)?, assuming(QTuple)? }>

This function is exactly the same as C<sys.std.Ordered.min> except that it
results in a C<QMaybe> of what is otherwise the result type, and that result
has zero elements if the argument has zero elements, rather than the
function failing.

=item C<function sys.std.Ordered.maybe_max result QMaybe params {
topic(QSet), func(OrdDetFuncRef)?, assuming(QTuple)? }>

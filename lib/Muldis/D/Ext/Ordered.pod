...
This document is Muldis::D::Ext::Ordered version 0.21.0.
...
=item C<function sys.Ordered.is_before result Bool params {
...
=item C<function sys.Ordered.is_not_before result Bool params {
topic(Universal), other(Universal) }>

This function is exactly the same as C<sys.Ordered.is_before> except that
it results in the opposite boolean value when given the same arguments.
(It could alternately be called "is after or is identical".)

=item C<function sys.Ordered.is_inside_range result Bool params {
...
compatible declared types as per C<sys.Ordered.is_before>.  This function
will fail if C<max> is before C<min>.

=item C<function sys.Ordered.is_outside_range result Bool params {
topic(Universal), min(Universal), max(Universal), min_is_inside(Bool),
max_is_inside(Bool) }>

This function is exactly the same as C<sys.Ordered.is_inside_range> except
that it results in the opposite boolean value when given the same
arguments.

=item C<function sys.Ordered.min result Universal params { topic(Set) }>
...
=item C<function sys.Ordered.max result Universal params { topic(Set) }>

This function is exactly the same as C<sys.Ordered.min> except that it
...
=item C<function sys.Ordered.maybe_min result Maybe params { topic(Set) }>
...
=item C<function sys.Ordered.maybe_max result Maybe params { topic(Set) }>

This function is to C<sys.Ordered.max> as C<sys.Ordered.maybe_min> is to
C<sys.Ordered.min>.

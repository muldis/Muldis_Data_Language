...
=item C<function sys.std.QArray.value result ScaTupRel params {
topic(Array), index(NNInt) }>
...
=item C<function sys.std.QArray.update_value result Array params {
topic(Array), index(NNInt), value(ScaTupRel) }>
...
=item C<function sys.std.QArray.insertion result Array params {
topic(Array), index(NNInt), value(ScaTupRel) }>
...
=item C<function sys.std.QArray.deletion result Array params {
topic(Array), index(NNInt) }>
...
=item C<function sys.std.QArray.reduction result ScaTupRel params {
topic(Array), func(FuncRef), assuming(QuasiTuple)?, identity(ScaTupRel) }>
...
=item C<function sys.std.QArray.maybe_reduction result Maybe params {
topic(Array), func(FuncRef), assuming(QuasiTuple)? }>

This function is to C<sys.std.QSet.maybe_reduction> as
C<sys.std.QArray.reduction> is to C<sys.std.QSet.reduction>.

=item C<function sys.std.QArray.slice result Array params {
topic(Array), first_index(NNInt), last_index(NNInt) }>
...
=item C<function sys.std.QArray.catenation result Array params {
...
=item C<function sys.std.QArray.repeat result Array params { topic(Array),
count(NNInt) }>

This function results in the catenation of C<count> instances of C<topic>.

=item C<function sys.std.QArray.reverse result Array params { topic(Array)
}>
...
=item C<function sys.std.QArray.Array_from_wrap result array_of.Tuple
params { topic(Relation), ord_func(FuncRef), ord_assuming(QuasiTuple)? }>
...
which is the same as C<sys.std.QArray.Array_from_wrap> but that it just
adds an attribute to the source tuples and does not wrap them.

=item C<function sys.std.QArray.limit_of_Array_from_wrap result
array_of.Tuple params { topic(Relation), ord_func(FuncRef),
ord_assuming(QuasiTuple)?, first_index(NNInt), last_index(NNInt) }>

This function is a short-hand for invoking first
C<sys.std.QArray.Array_from_wrap> and then C<sys.std.QArray.slice> on its
result.  This function is to C<sys.std.QArray.Array_from_wrap> what the
C<sys.std.QRelation.limit> function is to C<sys.std.QRelation.rank>.

=item C<function sys.std.QArray.Array_from_attr result Array params {
topic(Relation), name(Name), ord_func(OrdDetFuncRef)?,
...
same as C<sys.std.QArray.Array_from_wrap>.  Each of the C<ord_func> and

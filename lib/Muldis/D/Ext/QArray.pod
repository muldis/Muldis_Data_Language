...
This document is Muldis::D::Ext::QArray version 0.54.0.
...
=item C<function sys.std.QArray.value result Universal params {
topic(QArray), index(NNInt) }>

This function results in the q/scalar or q/nonscalar C<value> attribute of the
q/tuple of C<topic> whose C<index> attribute is C<index>.  This function will
fail if no q/tuple exists in C<topic> with the specified index.

=item C<function sys.std.QArray.update_value result QArray params {
topic(QArray), index(NNInt), value(Universal) }>

This function results in its C<topic> argument but that the C<value>
attribute of the q/tuple of C<topic> whose C<index> attribute is C<index> has
been updated with a new q/scalar or q/nonscalar value given by C<value>.  This
function will fail if no q/tuple exists in C<topic> with the specified index,
or if the most specific types of the C<value> argument and the C<value>
attribute of C<topic> would be fatally incompatible in a common q/relation
...
=item C<function sys.std.QArray.insertion result QArray params {
topic(QArray), index(NNInt), value(Universal) }>
...
any existing q/tuples with C<index> values greater than or equal to C<index>
...
=item C<function sys.std.QArray.deletion result QArray params {
topic(QArray), index(NNInt) }>
...
=item C<function sys.std.QArray.is_element result Bool params {
topic(QArray), value(Universal) }>

This function results in C<Bool:true> iff its C<value> argument matches the
C<value> attribute of at least one q/tuple of its C<topic> argument (that is,
...
=item C<function sys.std.QArray.is_not_element result Bool params {
topic(QArray), value(Universal) }>
...
=item C<function sys.std.QArray.reduction result Universal params {
topic(QArray), func(FuncRef), assuming(QTuple)?, identity(Universal) }>
...
=item C<function sys.std.QArray.maybe_reduction result QMaybe params {
topic(QArray), func(FuncRef), assuming(QTuple)? }>

This function is to C<sys.std.QSet.maybe_reduction> as
C<sys.std.QArray.reduction> is to C<sys.std.QSet.reduction>.

=item C<function sys.std.QArray.slice result QArray params {
topic(QArray), first_index(NNInt), last_index(NNInt) }>
...
=item C<function sys.std.QArray.catenation result QArray params {
...
=item C<function sys.std.QArray.repeat result QArray params { topic(QArray),
count(NNInt) }>

This function results in the catenation of C<count> instances of C<topic>.

=item C<function sys.std.QArray.reverse result QArray params { topic(QArray)
}>
...
=item C<function sys.std.QArray.is_subarray result Bool params {
look_in(QArray), look_for(QArray) }>
...
=item C<function sys.std.QArray.is_not_subarray result Bool params {
look_in(QArray), look_for(QArray) }>
...
=item C<function sys.std.QArray.QArray_from_wrap result quasi_array_of.QTuple
params { topic(QRelation), ord_func(FuncRef), ord_assuming(QTuple)? }>

This function results in an C<QArray> whose C<value> attribute is
q/tuple-typed and that attribute's values are all the q/tuples of C<topic>; is
a short-hand for a relational wrap of all attributes of C<topic> such that
the new q/tuple-valued attribute is named C<value>, and then that result is
extended with an C<index> attribute whose values result from a rank of the
q/tuples, where the ranked-first q/tuple has an C<index> of zero, and so on.
...
q/tuples, with each invocation getting a C<topic> q/tuple as each its C<topic>
and C<other> arguments.  See also the C<sys.std.QRelation.rank> function,
which is the same as C<sys.std.QArray.QArray_from_wrap> but that it just
adds an attribute to the source q/tuples and does not wrap them.

=item C<function sys.std.QArray.limit_of_QArray_from_wrap result
quasi_array_of.QTuple params { topic(QRelation), ord_func(FuncRef),
...
=item C<function sys.std.QArray.QArray_from_attr result QArray params {
topic(QRelation), name(Name), ord_func(OrdDetFuncRef)?,
ord_assuming(QTuple)? }>

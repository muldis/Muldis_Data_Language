...
Muldis D extension for UTC date and time data types and operators

=head1 VERSION

This document is Muldis::D::Ext::DateTime version 0.104.0.
...
their data.  Right now all of the included data types are based on
year-month-day-hour-minute-second components in the Gregorian calendar,
either the UTC time zone or a "floating" time zone.  This C<DateTime>
document does I<not> deal with TAI temporal artifacts as the core does,
except for purposes of translation with UTC artifacts.
...
=head1 DATA TYPES FOR UTC-BASED DATES AND TIMES
...
C<< function sys.std.DateTime.UTCInstant.TAI_from_UTC (Instant <--
$utc : UTCDateTime) >>

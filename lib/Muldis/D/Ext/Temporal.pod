...
This document is Muldis::D::Ext::Temporal version 0.120.0.
...
infinity.  The default value of C<Instant> is the default value of
C<TAIInstant>.  The C<Instant> type is not itself ordered, but often a type
which composes C<Duration> is also ordered.  The C<Instant> type is
intended to have exactly the same meaning as the same-named type of Perl 6
(see L<http://perlcabal.org/syn/S02.html> for details).
...
cardinality of C<Duration> is infinity.  The default value of C<Duration>
is the default value of C<TAIDuration>.  The C<Duration> type is not itself
...
C<< function sys.std.Temporal.Duration.abs (Duration <--
$topic@ : Duration) {...} >>
...
C<< function sys.std.Temporal.Duration.abs_diff (Duration <--
...
the the type of the value that C<target> has at the moment that
C<fetch_curr_instant> is invoked; typically that value is the default value
of the declared type of the invoker variable that C<target>'s argument;

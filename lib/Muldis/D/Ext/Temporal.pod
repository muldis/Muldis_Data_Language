...
This document is Muldis::D::Ext::Temporal version 0.6.0.
...
and aren't re-described here):
    ...
    sys.Core.Type.Universal
        ...
        sys.Core.Type.Scalar
            ...
            sys.Temporal.Type.DateTimeWithTZ
            sys.Temporal.Type.DateWithTZ
            sys.Temporal.Type.TimeWithTZ
            sys.Temporal.Type.DurationWithTZ
            sys.Temporal.Type.DurationOfDaysWithTZ
            sys.Temporal.Type.DateTimeNoTZ
            sys.Temporal.Type.DateNoTZ
            sys.Temporal.Type.TimeNoTZ
            sys.Temporal.Type.DurationNoTZ
            sys.Temporal.Type.DurationOfDaysNoTZ
...
=item C<sys.Temporal.Type.DateTimeWithTZ>

A C<DateTimeWithTZ> is a single specific time on a specific date, with
...
C<DateTimeWithTZ> is the Perl 6 epoch, namely 2000-1-1T0:0:0 in the
...
type is infinity; to define a most-generalized finite C<DateTimeWithTZ>
subtype, you must specify the earliest and latest datetimes it includes,
and also its least magnitude fraction of a second.

=item C<sys.Temporal.Type.DateWithTZ>

A C<DateWithTZ> is the same as C<DateTimeWithTZ> in all respects but
...
infinity; to define a most-generalized finite C<DateWithTZ> subtype, you
must specify the earliest and latest dates it includes.

=item C<sys.Temporal.Type.TimeWithTZ>

A C<TimeWithTZ> is a single specific time that isn't on any day in
...
an explicit time-zone offset interval as per C<DateTimeWithTZ>.  Its
...
C<TimeWithTZ> subtype, you must specify its least magnitude fraction of
a second.

=item C<sys.Temporal.Type.DurationWithTZ>

A C<DurationWithTZ> is a single amount of time, with precision to
...
difference between two C<DateTimeWithTZ> values, and is likewise assumed
...
most-generalized finite C<DurationWithTZ> subtype, you must specify the
minimum and maximum amount of time that its values may be, plus the
granularity of the type in fractions of a second.

=item C<sys.Temporal.Type.DurationOfDaysWithTZ>

A C<DurationOfDaysWithTZ> is the same as C<DurationWithTZ> in all
respects but that its precision is only to the whole terrestrial day.  The
cardinality of this type is infinity; to define a most-generalized finite
C<DurationOfDaysWithTZ> subtype, you must specify the minimum and
maximum amount of time that its values may be.

=item C<sys.Temporal.Type.DateTimeNoTZ>

A C<DateTimeNoTZ> is the same as C<DateTimeWithTZ> in its
...
the same as for C<DateTimeWithTZ>.

=item C<sys.Temporal.Type.DateNoTZ>

A C<DateNoTZ> is the same as C<DateWithTZ> in all respects but
that its differences correspond to the differences between
C<DateTimeWithTZ> and C<DateTimeNoTZ>.  Matters of its
cardinality are the same as for C<DateWithTZ>.

=item C<sys.Temporal.Type.TimeNoTZ>

A C<TimeNoTZ> is the same as C<TimeWithTZ> in all respects but
that its differences correspond to the differences between
C<DateTimeWithTZ> and C<DateTimeNoTZ>.  Matters of its
cardinality are the same as for C<TimeWithTZ>.

=item C<sys.Temporal.Type.DurationNoTZ>

A C<DurationNoTZ> is the same as C<DurationWithTZ> in all
...
C<DateTimeNoTZ> values.  Matters of its cardinality are the same
as for C<DurationWithTZ>.

=item C<sys.Temporal.Type.DurationOfDaysNoTZ>

A C<DurationOfDaysNoTZ> is to C<DurationOfDaysWithTZ> what
C<DurationNoTZ> is to C<DurationWithTZ>.

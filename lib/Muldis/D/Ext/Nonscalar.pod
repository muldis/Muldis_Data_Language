...
Muldis::D::Ext::Nonscalar -
Muldis D extension adding more generic relational operators

=head1 VERSION

This document is Muldis::D::Ext::Nonscalar version 0.22.0.
...
=item C<function sys.std.Nonscalar.Tuple.degree result UInt params {
topic(Tuple) }>

This function results in the degree of its argument (that is, the count of
attributes it has).

=item C<function sys.std.Nonscalar.Tuple.attr_from_Tuple result ScaTupRel
params { topic(Tuple) }>
...
=item C<function sys.std.Nonscalar.Tuple.Tuple_from_attr result Tuple
params { name(Cat.Name), value(ScaTupRel) }>
...
=item C<function sys.std.Nonscalar.Tuple.substitution_in_default result
Tuple params { of(Cat.NameChain), subst(Tuple) }>
...
=item C<function sys.std.Nonscalar.Relation.degree result UInt params {
topic(Relation) }>

This function results in the degree of its argument (that is, the count of
attributes it has).

=item C<function sys.std.Nonscalar.Relation.is_empty result Bool params {
topic(Relation) }>
...
=item C<function sys.std.Nonscalar.Relation.is_not_empty result Bool params
{ topic(Relation) }>

This function is exactly the same as C<sys.std.Nonscalar.Relation.is_empty>
except that it results in the opposite boolean value when given the same
argument. And following the analogy with C<is_empty>, C<is_not_empty> is
analagous to SQL's C<is not null>.

=item C<function sys.std.Nonscalar.Relation.empty result Relation params {
topic(Relation) }>

This function results in the empty relation of the same heading of its
argument; it has zero tuples.

=item C<function sys.std.Nonscalar.Relation.universal result Relation
...
=item C<function sys.std.Nonscalar.Relation.power_set result
set_of.Relation params { topic(Relation) }>
...
=item C<function sys.std.Nonscalar.Relation.negation result Relation params
{ topic(Relation) }>
...
C<sys.std.Nonscalar.Relation.universal> would fail on.

=item C<function sys.std.Nonscalar.Relation.transitive_closure result
...
=item C<function sys.std.Nonscalar.Relation.reduction result Tuple params {
...
=item C<function sys.std.Nonscalar.Relation.maybe_reduction result
maybe_of.Tuple params { topic(Relation), func(Cat.NameChain),
assuming(Tuple) }>

This function is exactly the same as
C<sys.std.Nonscalar.Relation.reduction> except that it does not take an
C<assuming> argument, and it results in a C<Maybe> of what is otherwise the
result type, and that result has zero elements if the argument has zero
elements.

=item C<function sys.std.Nonscalar.Relation.map result Relation params {
...
=item C<function sys.std.Nonscalar.Relation.is_proper_subset result Bool
...
=item C<function sys.std.Nonscalar.Relation.is_not_proper_subset result
Bool params { look_in(Relation), look_for(Relation) }>

This function is exactly the same as
C<sys.std.Nonscalar.Relation.is_proper_subset> except that it results in
the opposite boolean value when given the same arguments.

=item C<function sys.std.Nonscalar.Relation.exclusion result Relation
params { topic(bag_of.Relation) }>
...
=item C<function sys.std.Nonscalar.Relation.composition result Relation
params { r1(Relation), r2(Relation) }>
...
=item C<function sys.std.Nonscalar.Relation.join_with_group result Relation
...
=item C<function sys.std.Nonscalar.Relation.substitution result Relation
...
=item C<function sys.std.Nonscalar.Relation.static_substitution result
Relation params { topic(Relation), attrs(Tuple) }>

This function is a simpler-syntax alternative to
C<sys.std.Nonscalar.Relation.substitution> in the typical scenario where
every tuple of a relation, given in the C<topic> argument, is updated with
identical values for the same attributes; the new attribute values are
given in the C<attrs> argument.

=item C<function sys.std.Nonscalar.Relation.substitution_in_restriction
...
=item C<function
sys.std.Nonscalar.Relation.static_substitution_in_restriction result
Relation params { topic(Relation), restr_func(Cat.NameChain),
restr_assuming(Tuple), subst(Tuple) }>

This function is to
C<sys.std.Nonscalar.Relation.substitution_in_restriction> what
C<sys.std.Nonscalar.Relation.static_substitution> is to
C<sys.std.Nonscalar.Relation.substitution>.  See also the
C<static_substitution_in_semijoin> function.

=item C<function sys.std.Nonscalar.Relation.substitution_in_semijoin result
...
=item C<function sys.std.Nonscalar.Relation.static_substitution_in_semijoin
result Relation params { topic(Relation), restr(Relation), subst(Tuple) }>

This function is to C<sys.std.Nonscalar.Relation.substitution_in_semijoin>
what C<sys.std.Nonscalar.Relation.static_substitution> is to
C<sys.std.Nonscalar.Relation.substitution>.
...
=item C<function sys.std.Nonscalar.Relation.outer_join_with_group result
Relation params { primary(Relation), secondary(Relation),
group_attr(Cat.Name) }>

This function is the same as C<sys.std.Nonscalar.Relation.join_with_group>
...
=item C<function sys.std.Nonscalar.Relation.outer_join_with_maybes result
Relation params { primary(Relation), secondary(Relation) }>
...
=item C<function sys.std.Nonscalar.Relation.outer_join_with_defaults result
Relation params { primary(Relation), secondary(Relation) }>

This function is the same as
C<sys.std.Nonscalar.Relation.outer_join_with_maybes> but that
...
=item C<function sys.std.Nonscalar.Relation.outer_join_with_product result
Relation params { primary(Relation), secondary(Relation), filler(Tuple) }>

This function is the same as
C<sys.std.Nonscalar.Relation.outer_join_with_defaults> but that missing
values are provided explicitly from the C<filler> argument, which is a
tuple whose heading matches the projection of C<secondary>'s attributes
that aren't in common with C<primary>, and whose body is the literal values
to use for those missing attribute values.  This function gets its name in
that conceptually the result tuples from non-matches are the result of
...
=item C<function sys.std.Nonscalar.Relation.outer_join_with_extension
result Relation params { primary(Relation), secondary(Relation),
exten_func(Cat.NameChain), exten_assuming(Tuple) }>

This function is the same as
C<sys.std.Nonscalar.Relation.outer_join_with_product> but that the result

...
This document is Muldis::D::Ext::Set version 0.21.0.
...
=item C<function sys.Set.is_member result Bool params { set(Set),
value(ScaTupRel) }>
...
=item C<function sys.Set.is_not_member result Bool params { set(Set),
value(ScaTupRel) }>

This function is exactly the same as C<sys.Set.is_member> except that it
results in the opposite boolean value when given the same arguments.

=item C<function sys.Set.insertion result Set params { set(Set),
value(ScaTupRel) }>
...
=item C<function sys.Set.disjoint_insertion result Set params { set(Set),
value(ScaTupRel) }>

This function is exactly the same as C<sys.Set.insertion> except that it
will fail if C<value> already exists in C<set>.

=item C<function sys.Set.deletion result Set params { set(Set),
value(ScaTupRel) }>
...
=item C<function sys.Set.reduction result ScaTupRel params { topic(Set),
...
=item C<function sys.Set.maybe_reduction result Maybe params { topic(Set),
func(Cat.NameChain), assuming(Tuple) }>

This function is exactly the same as C<sys.Set.reduction> except that it
does not take an C<assuming> argument, and it results in a C<Maybe> of what
is otherwise the result type, and that result has zero elements if the
argument has zero elements.

=item C<function sys.Set.Set_from_wrap result SetOfTuple params {
topic(Relation) }>
...
=item C<function sys.Set.Set_from_attr result Set params { topic(Relation),
name(Cat.Name) }>

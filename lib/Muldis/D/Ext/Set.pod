...
This document is Muldis::D::Ext::Set version 0.76.1.
...
C<< function sys.std.Set.has_member (Bool <-- QSet $set, Universal $value)
>>
...
C<< function sys.std.Set.has_not_member (Bool <-- QSet $set,
Universal $value) >>
...
C<< function sys.std.Set.insertion (QSet <-- QSet $set, Universal $value)
>>
...
C<< function sys.std.Set.disjoint_ins (QSet <-- QSet $set,
Universal $value) >>
...
C<< function sys.std.Set.deletion (QSet <-- QSet $set, Universal $value)
>>
...
C<< function sys.std.Set.reduction (Universal <-- QSet $topic,
...
C<< function sys.std.Set.maybe_reduction (QMaybe <-- QSet $topic,
...
=head2 sys.std.Set.QSet_from_wrap

C<< function sys.std.Set.QSet_from_wrap (set_of.QTuple <--
QRelation $topic) >>
...
=head2 sys.std.Set.QSet_from_attr

C<< function sys.std.Set.QSet_from_attr (QSet <-- QRelation $topic,
Name $name) >>
...
This selector function results in the only zero-tuple QMaybe value, which
is known by the special name C<Maybe:nothing>, aka C<nothing>, aka I<empty
set> aka C<∅>.

=head2 sys.std.Set.Maybe.single

C<< function sys.std.Set.Maybe.single (QSingle <-- Universal $value) >>
...
C<< function sys.std.Set.Maybe.attr (Universal <-- QSingle $topic) >>
...
argument is a C<QSingle>.  Note that this operation is also known as C<v>.

=head2 sys.std.Set.Maybe.attr_or_default

C<< function sys.std.Set.Maybe.attr_or_default (Universal <--
quasi_array_of.QMaybe $topic?, TypeRef $default) >>

This function results in the q/scalar or q/nonscalar value of the sole
attribute of the sole q/tuple of the lowest-indexed of its N C<topic> input
element values where said element isn't equal to C<nothing>, if there is
such an element; otherwise, it results in the default value of the q/scalar
or q/nonscalar data type whose name is given in the C<default> argument.
...
quasi_array_of.QMaybe $topic?, Universal $value) >>
...
situations where a C<QMaybe> is used analagously to a SQL nullable value,
this function is analagous to the N-adic SQL COALESCE function.

=head2 sys.std.Set.Maybe.order

C<< function sys.std.Set.Maybe.order (Order <-- QMaybe $topic,
...
curried by its C<ord_assuming> and C<reverse_order_of_singles> arguments.

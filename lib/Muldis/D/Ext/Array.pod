...
array_of.Tuple params { topic(Relation), ord_func(Cat.NameChain),
ord_assuming(Tuple)?, first_index(UInt), last_index(UInt) }>
...
topic(Relation), name(Cat.Name), ord_func(Cat.OrdDetFuncNameChain)?,
ord_assuming(Tuple)? }>

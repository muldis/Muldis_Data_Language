...
This document is Muldis::D::Ext::Array version 0.78.0.
...
C<< function sys.std.Array.value (Universal <-- Array $topic,
NNInt $index) >>
...
C<< function sys.std.Array.update_value (Array <-- Array $topic,
...
C<< function sys.std.Array.insertion (Array <-- Array $topic,
NNInt $index, Universal $value) >>
...
C<< function sys.std.Array.deletion (Array <-- Array $topic,
NNInt $index) >>
...
C<< function sys.std.Array.has_elem (Bool <-- Array $topic,
Universal $value) >>
...
C<< function sys.std.Array.has_not_elem (Bool <-- Array $topic,
Universal $value) >>
...
C<< function sys.std.Array.reduction (Universal <-- Array $topic,
...
Also, the C<value_reduction> function named in C<func> is only associative,
...
C<< function sys.std.Array.maybe_reduction (Maybe <-- Array $topic,
...
C<< function sys.std.Array.slice (Array <-- Array $topic,
...
C<< function sys.std.Array.catenation (Array <-- array_of.Array $topic?) >>
...
C<< function sys.std.Array.replication (Array <-- Array $topic,
NNInt $count) >>
...
C<< function sys.std.Array.reverse (Array <-- Array $topic) >>
...
C<< function sys.std.Array.has_subarray (Bool <-- Array $look_in,
Array $look_for) >>
...
C<< function sys.std.Array.has_not_subarray (Bool <-- Array $look_in,
Array $look_for) >>
...
=head2 sys.std.Array.Array_from_wrap

C<< function sys.std.Array.Array_from_wrap (array_of.Tuple <--
Relation $topic, OrdDetFuncRef $ord_func, Tuple $ord_assuming?,
...
arguments; this wrapped function is used to rank the tuples, with each
invocation getting a C<topic> tuple as each its C<topic> and C<other>
arguments.  See also the C<sys.std.Relation.rank> function, which is the
same as C<sys.std.Array.Array_from_wrap> but that it just adds an attribute
to the source tuples and does not wrap them.

=head2 sys.std.Array.Array_from_wrap_by_attr_names

C<< function sys.std.Array.Array_from_wrap_by_attr_names
(array_of.Tuple <-- Relation $topic,
...
=head2 sys.std.Array.limit_of_Array_from_wrap

C<< function sys.std.Array.limit_of_Array_from_wrap (array_of.Tuple <--
Relation $topic, OrdDetFuncRef $ord_func, Tuple $ord_assuming?,
Bool $is_reverse_order?, NNInt $first_index, NNInt $last_index) >>
...
=head2 sys.std.Array.limit_of_Array_from_wrap_by_attr_names

C<< function sys.std.Array.limit_of_Array_from_wrap_by_attr_names
(array_of.Tuple <-- Relation $topic, array_of.OrderByName $order_by,
Bool $is_reverse_order?, NNInt $first_index, NNInt $last_index) >>

This function is to C<limit_of_Array_from_wrap> what
C<Array_from_wrap_by_attr_names> is to C<Array_from_wrap>.

=head2 sys.std.Array.Array_from_attr

C<< function sys.std.Array.Array_from_attr (Array <-- Relation $topic,
Name $name, OrdDetFuncRef $ord_func?, Tuple $ord_assuming?,
Bool $is_reverse_order?) >>
...
same as C<sys.std.Array.Array_from_wrap>.  Each of the C<ord_func> and

...
This document is Muldis::D::Ext::Array version 0.24.0.
...
=item C<function sys.std.Array.update_value result Seq params {
...
=item C<function sys.std.Array.insertion result Seq params { topic(Seq),
...
=item C<function sys.std.Array.deletion result Seq params { topic(Seq),
index(UInt) }>
...
=item C<function sys.std.Array.slice result Seq params {
...
=item C<function sys.std.Array.catenation result Seq params {
topic(seq_of.Seq) }>
...
=item C<function sys.std.Array.repeat result Seq params { topic(Seq),
count(UInt) }>

This function results in the catenation of C<count> instances of C<topic>.

=item C<function sys.std.Array.reverse result Seq params { topic(Seq) }>
...
=item C<function sys.std.Array.is_subseq result Bool params {
...
=item C<function sys.std.Array.is_not_subseq result Bool params {
look_in(Seq), look_for(Seq) }>

This function is exactly the same as C<sys.std.Array.is_subseq> except
that it results in the opposite boolean value when given the same
arguments.

=item C<function sys.std.Array.Seq_from_wrap result seq_of.Tuple params
...
which is the same as C<sys.std.Array.Seq_from_wrap> but that it just
adds an attribute to the source tuples and does not wrap them.

=item C<function sys.std.Array.limit_of_Seq_from_wrap result
...
C<sys.std.Array.Seq_from_wrap> and then C<sys.std.Array.slice> on its
result.  This function is to C<sys.std.Array.Seq_from_wrap> what the
C<sys.std.Relation.limit> function is to C<sys.std.Relation.rank>.

=item C<function sys.std.Array.Seq_from_attr result Seq params {
...
same as C<sys.std.Array.Seq_from_wrap>.

...
This document is Muldis::D::Ext::Bool version 0.73.0.
...
This function results in C<Bool:false> iff its (mutually commutative) 2
...
C<< function sys.std.Bool.xnor (Bool <-- Bool $topic, Bool $other) >>

This function results in C<Bool:true> iff its (mutually commutative) 2
arguments are exactly the same value, and C<Bool:false> otherwise.  Note
that this operation is also known as I<not xor> or I<iff> (I<if and only
if>) or I<material equivalence> or I<biconditional> or I<equivalent> or
C<↔>.  Note that an invocation of C<xnor> is exactly the same operation as
a C<sys.std.Core.Universal.is_identical> invocation whose arguments are
both C<Bool>-typed.

=head2 sys.std.Bool.iff

C<< function sys.std.Bool.iff (Bool <-- Bool $topic, Bool $other) >>

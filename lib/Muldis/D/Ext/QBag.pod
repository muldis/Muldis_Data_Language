...
This document is Muldis::D::Ext::QBag version 0.53.0.
...
=item C<function sys.std.QBag.cardinality result NNInt params { topic(Bag)
}>
...
=item C<function sys.std.QBag.is_member result Bool params { bag(Bag),
value(ScaTupRel) }>
...
=item C<function sys.std.QBag.is_not_member result Bool params { bag(Bag),
value(ScaTupRel) }>
...
=item C<function sys.std.QBag.count result NNInt params { bag(Bag),
value(ScaTupRel) }>
...
=item C<function sys.std.QBag.insertion result Bag params { bag(Bag),
value(ScaTupRel) }>
...
=item C<function sys.std.QBag.deletion result Bag params { bag(Bag),
value(ScaTupRel) }>
...
=item C<function sys.std.QBag.reduction result ScaTupRel params {
topic(Bag), func(FuncRef), assuming(QuasiTuple)?, identity(ScaTupRel) }>
...
=item C<function sys.std.QBag.maybe_reduction result Maybe params {
topic(Bag), func(FuncRef), assuming(QuasiTuple)? }>

This function is to C<sys.std.QSet.maybe_reduction> as
C<sys.std.QBag.reduction> is to C<sys.std.QSet.reduction>.

=item C<function sys.std.QBag.Set_from_Bag result Set params { topic(Bag)
}>

This function results in the C<Set> that is the projection of the C<value>
attribute of its C<Bag> argument.

=item C<function sys.std.QBag.Bag_from_Set result Bag params { topic(Set)
}>
...
=item C<function sys.std.QBag.Bag_from_wrap result bag_of.Tuple params {
topic(Relation) }>
...
=item C<function sys.std.QBag.Bag_from_attr result Bag params {
topic(Relation), name(Name) }>
...
=item C<function sys.std.QBag.is_subset result Bool params { look_in(Bag),
look_for(Bag) }>
...
=item C<function sys.std.QBag.union result Bag params { topic(set_of.Bag)
}>
...
=item C<function sys.std.QBag.intersection result Bag params {
topic(set_of.Bag) }>
...
=item C<function sys.std.QBag.difference result Bag params { source(Bag),
filter(Bag) }>

This function is like C<sys.std.Core.QRelation.difference> as C<union> is
like C<sys.std.Core.QRelation.union>; for corresponding input tuples, the

...
This document is Muldis::D::Ext::QBag version 0.54.0.
...
C<QBag> parameterized q/relation type, and said operators are short-hands for
...
=item C<function sys.std.QBag.cardinality result NNInt params { topic(QBag)
}>
...
=item C<function sys.std.QBag.is_member result Bool params { bag(QBag),
value(Universal) }>

This function is the same as C<sys.std.QSet.is_member>, including that
matching of C<value> is done against the C<value> attribute, except that it
works with a C<QBag> rather than a C<QSet>.

=item C<function sys.std.QBag.is_not_member result Bool params { bag(QBag),
value(Universal) }>

This function is exactly the same as C<sys.std.QBag.is_member> except that
it results in the opposite boolean value when given the same arguments.

=item C<function sys.std.QBag.count result NNInt params { bag(QBag),
value(Universal) }>

This function results in the multiplicity / count of occurrances of
C<value> in C<bag>; if a q/tuple exists in C<bag> whose C<value> attribute is
C<value>, then the result is its C<count> attribute; otherwise the result
is zero.

=item C<function sys.std.QBag.insertion result QBag params { bag(QBag),
value(Universal) }>

This function is the same as C<sys.std.QSet.insertion> as per C<is_member>
but that its result differs depending on whether C<value> already exists in
C<bag>; if it does, then no new q/tuple is added, but the C<count> attribute
for the matching q/tuple is incremented by 1; if it does not, then a new
q/tuple is added where its C<value> is C<value> and its C<count> is 1.

=item C<function sys.std.QBag.deletion result QBag params { bag(QBag),
value(Universal) }>

This function is the same as C<sys.std.QSet.deletion> as per C<is_member>
...
q/tuple whose C<value> is C<value> is deleted.

=item C<function sys.std.QBag.reduction result Universal params {
topic(QBag), func(FuncRef), assuming(QTuple)?, identity(Universal) }>
...
C<topic>, except that it works with a C<QBag> rather than a C<QSet>; C<func>
...
=item C<function sys.std.QBag.maybe_reduction result QMaybe params {
topic(QBag), func(FuncRef), assuming(QTuple)? }>

This function is to C<sys.std.QSet.maybe_reduction> as
C<sys.std.QBag.reduction> is to C<sys.std.QSet.reduction>.

=item C<function sys.std.QBag.QSet_from_QBag result QSet params { topic(QBag)
}>

This function results in the C<QSet> that is the projection of the C<value>
attribute of its C<QBag> argument.

=item C<function sys.std.QBag.QBag_from_QSet result QBag params { topic(QSet)
}>
...
=item C<function sys.std.QBag.QBag_from_wrap result quasi_bag_of.QTuple params {
topic(QRelation) }>

This function results in a C<QBag> whose C<value> attribute is q/tuple-typed
and that attribute's values are all the q/tuples of C<topic>; is a short-hand
for a relational wrap of all attributes of C<topic> such that the new
q/tuple-valued attribute is named C<value>, and then that result is extended
with a C<count> attribute whose value for every q/tuple is 1.

=item C<function sys.std.QBag.QBag_from_attr result QBag params {
topic(QRelation), name(Name) }>
...
a new q/relation-typed attribute, and then extending the result of the group
with a new positive integer attribute whose values are the cardinality of
the q/relation-valued attribute's values, and then doing a binary projection
of the named attribute and the new integer attribute plus their renaming to
C<value> and C<count> respectively.

=item C<function sys.std.QBag.is_subset result Bool params { look_in(QBag),
look_for(QBag) }>
...
=item C<function sys.std.QBag.is_not_subset result Bool params {
look_in(QBag), look_for(QBag) }>

This function is like C<sys.std.Core.QRelation.is_not_subset> as per
C<is_subset>.

=item C<function sys.std.QBag.is_proper_subset result Bool params {
look_in(QBag), look_for(QBag) }>

This function is like C<sys.std.Core.QRelation.is_proper_subset> as per
C<is_subset>.  I<TODO: What is its definition?>

=item C<function sys.std.QBag.is_not_proper_subset result Bool params {
look_in(QBag), look_for(QBag) }>

This function is like C<sys.std.Core.QRelation.is_not_proper_subset> as per
C<is_subset>.  I<TODO: What is its definition?>

=item C<function sys.std.QBag.union result QBag params { topic(quasi_set_of.QBag)
}>
...
=item C<function sys.std.QBag.intersection result QBag params {
topic(quasi_set_of.QBag) }>
...
=item C<function sys.std.QBag.difference result QBag params { source(QBag),
filter(QBag) }>
...
C<source> q/tuple is greater than the C<count> of the C<filter> q/tuple, and
the C<count> of the result q/tuple is the difference of those two.

...
This document is Muldis::D::Ext::Rational version 0.62.1.
...
C<< function sys.std.Rational.sum (Rat <-- addends(bag_of.Rat)) >>
...
C<< function sys.std.Rational.difference (Rat <-- minuend(Rat),
subtrahend(Rat)) >>
...
C<< function sys.std.Rational.abs_difference (Rat <-- topic(Rat),
other(Rat)) >>
...
C<< function sys.std.Rational.product (Rat <-- factors(bag_of.Rat)) >>
...
C<system_service sys.std.Rational.fetch_random (&target(Rat),
radix(PInt2_N), max_denom(PInt), min(Rat), max(Rat), exclude_min(Bool)?,
exclude_max(Bool)?)>

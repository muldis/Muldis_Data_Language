...
This document is Muldis::D::Ext::Rational version 0.71.0.
...
always only result in the one that is positive.
...
argument.  The C<round_rule> parameter is as per C<power>.
...
The C<round_rule> parameter is as per C<power>.

...
This document is Muldis::D::Ext::Rational version 0.30.0.
...
This is an enumeration data type.  A C<PInt2_N> is a proper subtype of
...
This is an enumeration data type.  A C<URat> (unsigned / non-negative
rational) is a proper subtype of C<Rat> where all member values are greater
than or equal to zero (that is, the C<numerator>|C<mantissa> is greater
than or equal to zero).  Its minimum value is zero.

=head2 sys.std.Rational.Type.PRat

This is an enumeration data type.  A C<PRat> (positive rational) is a
proper subtype of C<URat> where all member values are greater than zero
...
This is an enumeration data type.  A C<BRat> (binary rational) is a proper
...
This is an enumeration data type.  A C<DRat> (decimal rational) is a proper

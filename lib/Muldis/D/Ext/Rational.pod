...
This document is Muldis::D::Ext::Rational version 0.22.0.
...
and aren't re-described here):
    ...
    sys.Core.Type.Universal
        ...
        sys.Core.Type.Scalar
            ...
            sys.Rational.Type.Rat
                sys.Rational.Type.URat
                    sys.Rational.Type.PRat
                sys.Rational.Type.BRat
                sys.Rational.Type.DRat
...
=head2 sys.Rational.Type.PInt2_36
...
=head2 sys.Rational.Type.RatRoundRule
...
=item C<sys.Rational.Type.(Set|Maybe|Seq|Bag)OfRat>
...
=item C<function sys.Rational.Rat_from_Int_pair result Rat params {
...
=item C<function sys.Rational.Rat_from_Int_triple result Rat params {
...
=item C<function sys.Rational.(numerator|mantissa|exponent) result Int
params { topic(Rat) }>

This function results in the canonical C<numerator|mantissa|exponent> of
its argument.

=item C<function sys.Rational.denominator result PInt params { topic(Rat)
}>

This function results in the canonical C<denominator> of its argument.

=item C<function sys.Rational.radix result Cat.PInt2_N params { topic(Rat)
}>
...
=item C<function sys.Rational.abs result URat params { topic(Rat) }>

This function results in the absolute value of its argument.

=item C<function sys.Rational.sum result Rat params { addends(BagOfRat) }>
...
=item C<function sys.Rational.difference result Rat params { minuend(Rat),
subtrahend(Rat) }>

This function results in the difference when its C<subtrahend> argument is
subtracted from its C<minuend> argument.

=item C<function sys.Rational.product result Rat params { factors(BagOfRat)
}>
...
=item C<function sys.Rational.quotient result Rat params { dividend(Rat),
divisor(Rat) }>
...
=item C<function sys.Rational.maybe_quotient result MaybeOfRat params {
dividend(Rat), divisor(Rat) }>

This function is exactly the same as C<sys.Rational.quotient> except that
it results in a C<Maybe> of what is otherwise the result, and that result
has zero elements if C<divisor> is zero.

=item C<function sys.Rational.range result Rat params { topic(SetOfRat) }>
...
=item C<function sys.Rational.mean result Rat params { topic(BagOfRat) }>
...
=item C<function sys.Rational.maybe_mean result MaybeOfRat params {
topic(BagOfRat) }>

This function is exactly the same as C<sys.Rational.mean> except that it
results in a C<Maybe> of what is otherwise the result, and that result has
zero elements if C<topic> has zero values.

=item C<function sys.Rational.median result SetOfRat params {
topic(BagOfRat) }>
...
=item C<function sys.Rational.mean_of_median result Rat params {
topic(BagOfRat) }>

This function is a wrapper over C<sys.Rational.median> that will result in
the mean of its result elements; it will fail if there are zero elements.

=item C<function sys.Rational.mode result SetOfRat params { topic(BagOfRat)
}>
...
=item C<function sys.Rational.round result Rat params { topic(Rat),
...
=item C<function sys.Rational.power result PRat params { radix(PRat),
exponent(Rat) }>
...
=item C<function sys.Rational.log result Rat params { topic(PRat),
...
=item C<function sys.Rational.natural_power result PRat params {
...
=item C<function sys.Rational.natural_log result Rat params { topic(PRat),
round_rule(Cat.RatRoundRule) }>
...
=item C<function sys.Rational.Rat_from_Text result Rat params { text(Text),
radix(Cat.PInt2_36) }>
...
=item C<function sys.Rational.Text_from_Rat result Text params { rat(Rat),
radix(Cat.PInt2_36) }>
...
=item C<function sys.Rational.Rat_from_Int result Rat params { int(Int) }>

This selector function results in the C<Rat> value that is conceptually
equal to its C<Int> argument.

=item C<function sys.Rational.Int_from_Rat result Int params { rat(Rat),
round_meth(Cat.E_RM) }>

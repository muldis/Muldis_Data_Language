...
topic(Relation), func(FuncRef), assuming(QTuple)?, identity(Tuple) }>
...
topic(Relation), ord_func(FuncRef), ord_assuming(QTuple)?,
...
restr_assuming(QTuple)?, subst(Tuple) }>
...
=item C<updater sys.std.QRelation.assign_empty update { topic(Relation) }
read {}>

...
This document is Muldis::D::Ext::QRelation version 0.53.0.
...
that they typically work with any relation types.  Each C<assuming>
...
=item C<function sys.std.QRelation.power_set result set_of.Relation params
{ topic(Relation) }>
...
=item C<function sys.std.QRelation.transitive_closure result Relation
params { topic(Relation) }>

This function results in the transitive closure of its argument.  The
argument is a binary relation whose attributes are both of the same type,
and the result is a relation having the same heading and a body which is a
superset of the argument's tuples.  Assuming that the argument represents
all of the node pairs in a directed graph that have an arc between them,
and so each argument tuple represents an arc, C<transitive_closure> will
determine all of the node pairs in that graph which have a path between
them (a recursive operation), so each tuple of the result represents a
...
=item C<function sys.std.QRelation.reduction result Tuple params {
topic(Relation), func(FuncRef), assuming(QuasiTuple)?, identity(Tuple) }>
...
=item C<function sys.std.QRelation.maybe_reduction result maybe_of.Tuple
...
=item C<function sys.std.QRelation.map result Relation params {
...
=item C<function sys.std.QRelation.exclusion result Relation params {
...
input relations.  Matters concerning a C<topic> with zero values are as per
...
=item C<function sys.std.QRelation.composition result Relation params {
...
tuples of the 2 arguments.  This function will fail|warn any time that
C<join> would fail|warn on the same 2 input relations.

=item C<function sys.std.QRelation.join_with_group result Relation params {
...
=item C<function sys.std.QRelation.rank result Relation params {
topic(Relation), name(Name), ord_func(FuncRef),
ord_assuming(QuasiTuple)? }>
...
C<sys.std.QArray.Array_from_wrap> function, which is the same as
C<sys.std.QRelation.rank> but that it wraps the source tuples rather than
just adding an attribute to them.

=item C<function sys.std.QRelation.limit result Relation params {
topic(Relation), ord_func(FuncRef), ord_assuming(QuasiTuple)?,
...
C<sys.std.QArray.limit_of_Array_from_wrap> for an alternative).
...
=item C<function sys.std.QRelation.substitution result Relation params {
topic(Relation), func(FuncRef), assuming(QuasiTuple)? }>
...
=item C<function sys.std.QRelation.static_substitution result Relation
params { topic(Relation), attrs(Tuple) }>

This function is a simpler-syntax alternative to
C<sys.std.QRelation.substitution> in the typical scenario where every tuple
...
Relation params { topic(Relation), restr_func(FuncRef),
restr_assuming(QuasiTuple)?, subst_func(FuncRef),
subst_assuming(QuasiTuple)? }>
...
result Relation params { topic(Relation), restr_func(FuncRef),
restr_assuming(QuasiTuple)?, subst(Tuple) }>
...
=item C<function sys.std.QRelation.substitution_in_semijoin result Relation
params { topic(Relation), restr(Relation), subst_func(FuncRef),
subst_assuming(QuasiTuple)? }>
...
=item C<function sys.std.QRelation.outer_join_with_group result Relation
params { primary(Relation), secondary(Relation), group_attr(Name) }>
...
=item C<function sys.std.QRelation.outer_join_with_maybes result Relation
...
=item C<function sys.std.QRelation.outer_join_with_defaults result Relation
params { primary(Relation), secondary(Relation), filler(TypeRef) }>

This function is the same as C<sys.std.QRelation.outer_join_with_product>
but that the filler tuple is the default value of the tuple data type whose
...
=item C<function sys.std.QRelation.outer_join_with_product result Relation
...
exten_func(FuncRef), exten_assuming(QuasiTuple)? }>
...
=item C<updater sys.std.QRelation.assign_exclusion update { topic(Relation)
} read { other(Relation) }>

This update operator is a short-hand for first invoking the
C<sys.std.QRelation.exclusion> function such that it has 2 input relations
...
{ topic(Relation) } read { restr_func(FuncRef),
restr_assuming(QuasiTuple)?, subst_func(FuncRef),
subst_assuming(QuasiTuple)? }>
...
update { topic(Relation) } read { restr_func(FuncRef),
restr_assuming(QuasiTuple)?, subst(Tuple) }>
...
topic(Relation) } read { restr(Relation), subst_func(FuncRef),
subst_assuming(QuasiTuple)? }>  This updater is analagous to the common
...
update { topic(Relation) } read { restr(Relation), subst(Tuple) }>

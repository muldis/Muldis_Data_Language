...
=item C<function sys.std.Relation.degree result NNInt params {
topic(Relation) }>
...
C<sys.std.Core.Relation.union>; this function will fail when given such,
...
C<sys.std.Array.Array_from_wrap> function, which is the same as
...
C<sys.std.Array.limit_of_Array_from_wrap> for an alternative).
...
explicitly from the C<filler> argument, which is a tuple whose heading
matches the projection of C<secondary>'s attributes that aren't in common
with C<primary>, and whose body is the literal values to use for those
missing attribute values.  This function gets its name in that conceptually
the result tuples from non-matches are the result of performing a
relational cross-product between the un-matched C<primary> tuples and the
single C<filler> tuple.  This function could alternately be named
I<outer_join_with_static_extension>.

=item C<function sys.std.Relation.outer_join_with_extension result Relation
params { primary(Relation), secondary(Relation), exten_func(FuncRef),
exten_assuming(QuasiTuple)? }>
...
=item C<updater sys.std.Relation.assign_exclusion update { topic(Relation)
} read { other(Relation) }>

This update operator is a short-hand for first invoking the
C<sys.std.Relation.exclusion> function such that it has 2 input relations
from C<assign_exclusion>'s 2 arguments, and then assigning the result of
that function to C<topic>.

=item C<updater sys.std.Relation.assign_substitution update {
topic(Relation) } read { func(FuncRef), assuming(QuasiTuple)? }>

This update operator is a short-hand for first invoking the
C<sys.std.Core.Relation.substitution> function with the same arguments, and
then assigning the result of that function to C<topic>.  This updater is
analagous to the general case of the unconditional SQL "UPDATE" statement.

=item C<updater sys.std.Relation.assign_static_substitution update {
topic(Relation) } read { attrs(Tuple) }>

This update operator is a short-hand for first invoking the
C<sys.std.Core.Relation.static_substitution> function with the same
arguments, and then assigning the result of that function to C<topic>.

=item C<updater sys.std.Relation.assign_substitution_in_restriction update
{ topic(Relation) } read { restr_func(FuncRef),
restr_assuming(QuasiTuple)?, subst_func(FuncRef),
subst_assuming(QuasiTuple)? }>

This update operator is a short-hand for first invoking the
C<sys.std.Core.Relation.substitution_in_restriction> function with the same
arguments, and then assigning the result of that function to C<topic>.
This updater is analagous to the general case of the conditional SQL
"UPDATE" statement.

=item C<updater sys.std.Relation.assign_static_substitution_in_restriction
update { topic(Relation) } read { restr_func(FuncRef),
restr_assuming(QuasiTuple)?, subst(Tuple) }>

This update operator is a short-hand for first invoking the
C<sys.std.Core.Relation.static_substitution_in_restriction> function with
the same arguments, and then assigning the result of that function to
C<topic>.

=item C<updater sys.std.Relation.assign_substitution_in_semijoin update {
...
This update operator is a short-hand for first invoking the
C<sys.std.Core.Relation.substitution_in_semijoin> function with the same
arguments, and then assigning the result of that function to C<topic>.

=item C<updater sys.std.Relation.assign_static_substitution_in_semijoin
update { topic(Relation) } read { restr(Relation), subst(Tuple) }>

This update operator is a short-hand for first invoking the
C<sys.std.Core.Relation.static_substitution_in_semijoin> function with the

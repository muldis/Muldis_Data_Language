...
restr_assuming(QTuple)?, subst_attrs(set_of.Name), subst_func(FuncRef),
...
C<substitution_in_semijoin> function, which is a simpler-syntax alternative
for C<substitution_in_restriction> in its typical usage where restrictions
...
QRelation params { topic(QRelation), restr(QRelation),

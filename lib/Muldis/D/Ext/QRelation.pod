...
This document is Muldis::D::Ext::QRelation version 0.57.0.
...
=item C<function sys.std.QRelation.has_attrs result Bool params {
...
=item C<function sys.std.QRelation.attr_names result set_of.Name params {
topic(QRelation) }>
...
This function results in C<Bool:true> iff its argument has zero q/tuples,
and C<Bool:false> otherwise.  Note that if you are using a C<QMaybe> to
...
=item C<function sys.std.QRelation.power_set result quasi_set_of.QRelation
...
Note that the N-ary relational union of the power set of some q/relation is
that q/relation; the N-ary intersection of any power set is the empty
relation.
...
and the result is a q/relation having the same heading and a body which is
a superset of the argument's q/tuples.  Assuming that the argument
...
=item C<function sys.std.QRelation.maybe_reduction result
quasi_maybe_of.QTuple params { topic(QRelation), func(FuncRef),
assuming(QTuple)? }>
...
=item C<function sys.std.QRelation.is_disjoint result Bool params {
...
=item C<function sys.std.QRelation.is_not_disjoint result Bool params {
topic(QRelation), other(QRelation) }>
...
restr_assuming(QTuple)?, subst_func(FuncRef), subst_assuming(QTuple)? }>
...
and C<restr_assuming>) are then transformed (defined by C<subst_func> and
...
=item C<function sys.std.QRelation.substitution_in_semijoin result
QRelation params { topic(QRelation), restr(QRelation), subst_func(FuncRef),
subst_assuming(QTuple)? }>

This function is like C<substitution_in_restriction> except that the subset
of the q/tuples of C<topic> to be transformed is determined by those
matched by a semijoin with C<restr> rather than those that pass a generic
...
=item C<function sys.std.QRelation.outer_join_with_defaults result
...
but that the filler q/tuple is the default value of the q/tuple data type
whose name is given in the C<filler> argument.  This function is a
short-hand for invoking C<outer_join_with_product> with the result from
invoking C<sys.std.Core.Universal.default>.
...
use for those missing attribute values.  This function gets its name in
that conceptually the result q/tuples from non-matches are the result of
performing a relational cross-product between the un-matched C<primary>
q/tuples and the single C<filler> q/tuple.  This function could alternately
be named I<outer_join_with_static_extension>.
...
This function is the same as C<sys.std.QRelation.outer_join_with_product>
but that the result q/tuples from non-matches are the result of performing
a relational extension on the un-matched C<primary> q/tuples such that each
...
=item C<updater sys.std.QRelation.assign_exclusion update {

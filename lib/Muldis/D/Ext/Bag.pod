...
This document is Muldis::D::Ext::Bag version 0.22.0.
...
=item C<function sys.Bag.cardinality result UInt params { topic(Bag) }>
...
=item C<function sys.Bag.is_member result Bool params { bag(Bag),
value(ScaTupRel) }>

This function is the same as C<sys.Set.is_member>, including that matching
of C<value> is done against the C<value> attribute, except that it works
with a C<Bag> rather than a C<Set>.

=item C<function sys.Bag.is_not_member result Bool params { bag(Bag),
value(ScaTupRel) }>

This function is exactly the same as C<sys.Bag.is_member> except that it
results in the opposite boolean value when given the same arguments.

=item C<function sys.Bag.count result UInt params { bag(Bag),
value(ScaTupRel) }>
...
=item C<function sys.Bag.insertion result Bag params { bag(Bag),
value(ScaTupRel) }>

This function is the same as C<sys.Set.insertion> as per C<is_member> but
that its result differs depending on whether C<value> already exists in
...
=item C<function sys.Bag.deletion result Bag params { bag(Bag),
value(ScaTupRel) }>

This function is the same as C<sys.Set.deletion> as per C<is_member> but
that its result differs depending on what the C<count> for any tuple
...
=item C<function sys.Bag.reduction result ScaTupRel params { topic(Bag),
func(Cat.NameChain), assuming(Tuple), identity(ScaTupRel) }>
...
=item C<function sys.Bag.maybe_reduction result Maybe params { topic(Bag),
func(Cat.NameChain), assuming(Tuple) }>

This function is to C<sys.Set.maybe_reduction> as C<sys.Bag.reduction> is
to C<sys.Set.reduction>.

=item C<function sys.Bag.Set_from_Bag result Set params { topic(Bag) }>

This function results in the C<Set> that is the projection of the C<value>
attribute of its C<Bag> argument.

=item C<function sys.Bag.Bag_from_Set result Bag params { topic(Set) }>
...
=item C<function sys.Bag.Bag_from_wrap result BagOfTuple params {
topic(Relation) }>
...
=item C<function sys.Bag.Bag_from_attr result Bag params { topic(Relation),
name(Cat.Name) }>
...
=item C<function sys.Bag.is_subset result Bool params { look_in(Bag),
look_for(Bag) }>
...
=item C<function sys.Bag.is_not_subset result Bool params { look_in(Bag),
look_for(Bag) }>

This function is like C<sys.Core.Relation.is_not_subset> as per
C<is_subset>.

=item C<function sys.Bag.is_proper_subset result Bool params {
...
=item C<function sys.Bag.is_not_proper_subset result Bool params {
look_in(Bag), look_for(Bag) }>

This function is like C<sys.Core.Relation.is_not_proper_subset> as per
C<is_subset>.  I<TODO: What is its definition?>

=item C<function sys.Bag.union result Bag params { topic(SetOfBag) }>
...
=item C<function sys.Bag.intersection result Bag params { topic(SetOfBag)
}>
...
=item C<function sys.Bag.difference result Bag params { source(Bag),
filter(Bag) }>

...
This document is Muldis::D::Ext::QTuple version 0.53.0.
...
=item C<function sys.std.QTuple.degree result NNInt params { topic(Tuple)
}>

This function results in the degree of its argument (that is, the count of
attributes it has).

=item C<function sys.std.QTuple.attr_from_Tuple result ScaTupRel params {
topic(Tuple) }>
...
=item C<function sys.std.QTuple.Tuple_from_attr result Tuple params {
name(Name), value(ScaTupRel) }>
...
=item C<function sys.std.QTuple.substitution_in_default result Tuple params
{ of(TypeRef), subst(Tuple) }>

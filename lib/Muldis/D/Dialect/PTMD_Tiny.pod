...
This document is Muldis::D::Dialect::PTMD_Tiny version 0.17.0.
...
routine, which is simply a series of procedure calls (specifically
procedures with no subject-to-update parameters), where earlier procedure
calls are to system-defined data-definition routines (their arguments are
values to put in the system catalog), and later ones are then to
user-defined routines that the earlier statements either loaded or defined.
The current grammar assumes that the C<main> does not have any lexical
variables, and that all parameters of routines it calls are read-only
parameters.
...
execution.  Any remaining errors should be corrected as soon as possible.
    ...
    # grammar Muldis_D_PTMD_Tiny:auth<http://muldis.com>:ver<0.17.0>;
    grammar Muldis_D_PTMD_Tiny-0.17.0-http://muldis.com;
    ...
    token cat_e_psak {
        (Cat.E_PSAK)
        <spec_sep>
        (upd_param|ro_param|inner_var|outer_var)
    }

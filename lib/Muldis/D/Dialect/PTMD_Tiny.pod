...
This document is Muldis::D::Dialect::PTMD_Tiny version 0.29.0.
...
at least.>  Any remaining errors should be corrected as soon as possible.
    ...
    # grammar Muldis_D_PTMD_Tiny:auth<http://muldis.com>:ver<0.29.0>;
    grammar Muldis_D_PTMD_Tiny-0.29.0-http://muldis.com;
    ...
    token imperative_routine_upd_args {
        <list_open>
            (((<name> <pair_sep> <name_chain_body>) <list_sep>)*)
        <list_close>
    }
    ...
    token int_body { (0|\-?<[1-9A-Z]><[0-9A-Z_]>*) }

    token uint_body { (0|<[1-9A-Z]><[0-9A-Z_]>*) }

    token pint_body { (<[1-9A-Z]><[0-9A-Z_]>*) }

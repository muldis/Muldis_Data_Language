...
This document is Muldis::D::Dialect::PTMD_Tiny version 0.52.0.
...
it.  This dialect similarly matches the dialect
L<HDMD_Perl_Tiny|Muldis::D::Dialect::HDMD_Perl_Tiny>.
...
See also L<HDMD_Perl_Tiny|Muldis::D::Dialect::HDMD_Perl_Tiny> and look up
its syntax for representing Muldis D literal values, each syntax item
typically corresponds to a syntax item in the current document; the other
document helps explain the meaning of and further constraints on value
...
by the dialect L<HDMD_Perl_Tiny|Muldis::D::Dialect::HDMD_Perl_Tiny> too.
...
capturing parens for simplicity to fulfil its purpose as an illustration.
    ...
    token imperative_routine_upd_args {
        <list_open>
            [[<name_body> <pair_elem_sep> <name_chain_body>]
                ** <list_elem_sep>]?
        <list_close>
    }
    ...
    token scalar_or_qv {
        [Quasi]? Scalar <val_node_elem_sep>
    ...
    token generic_relation_empty_body_or_qv {
        [Quasi]? Relation <val_node_elem_sep>
        [<type_name> <val_node_elem_sep>]?
        <list_open>
            [<name_body> ** <list_elem_sep>]?
        <list_close>
    }
    ...
    token tuple_body {
        <list_open>
            [[<name_body> <pair_elem_sep> <literal>] ** <list_elem_sep>]?
        <list_close>
    }

    token set_or_qv {
        [Quasi]? Set <val_node_elem_sep>
        [<type_name> <val_node_elem_sep>]?
        <list_open>
            [<literal> ** <list_elem_sep>]?
        <list_close>
    ...
    token single_or_qv {
        [Quasi]? Single <val_node_elem_sep>
        [<type_name> <val_node_elem_sep>]?
        <list_open>
            <literal>
        <list_close>

...
This document is Muldis::D::Dialect::PTMD_Tiny version 0.61.0.
...
A I<generic context value literal> (or I<GCVL>) is a value literal that
includes explicit I<value kind> meta-data (such as, "this is an C<Int>" or
"this is a C<Name>") so that the literal can be properly interpreted in a
context that is expecting I<a> value but has no expectation that said value
belongs to a specific data type.  For example, each element of a generic
Muldis D collection value, such as a member of an array or tuple, could
...
value kind meta-data, because the context of its use supplies said
...
only external meta-data of C<payload> necessary to interpret the latter;
what grammars are valid for C<payload> depend just on C<value_kind>.
...
Grammar:
    ...
    token text_payload {
        <quoted_char_str_seg> ** <segment_sep>
    }
...
Examples:
    ...
    Text:'\c<LATIN SMALL LETTER OU>\c<F;263A>\c<65>'
...
Grammar:
    ...
    token name_payload {
        [<quoted_char_str_seg> | <nonquoted_char_str_seg>] ** <segment_sep>
    }

...
Under Perl 6, the payload must be a Perl 6 C<Bool>, and so C<Bool::False>
and C<Bool::True> are mapped directly.

=item *

Under Perl 5, the canonical payload is the specific result of a Perl 5
logical expression, such as C<(1 == 0)> for 'false' or C<(1 == 1)> for
'true'; said values are probably the empty string and number 1,
respectively.

=item *

Under Perl 5, a few alternative payload formats are supported:  The Perl 5
value literals [C<'false'>, C<'0'>, C<0>, C<''>] all map to 'false', and
the Perl 5 value literals [C<'true'>, C<'1'>, C<1>] all map to 'true'.  The
4 character strings [C<false>, C<0>, C<true>, C<1>] specifically are what
...
Under Perl 6, the payload must be a Perl 6 C<Order>, and so
...
Under Perl 5, the canonical payload is the specific result of a Perl 5
order-determining expression, such as C<< (1 <=> 2) >> for 'increase' or
C<< (1 <=> 1) >> for 'same' or C<< (2 <=> 1) >> for 'decrease'; said values
are probably the numbers [-1,0,1], respectively.

=item *

Under Perl 5, a few alternative payload formats are supported:  The Perl 5
value literals [C<'increase'>, C<'-1'>, C<-1>] all map to 'increase', the
Perl 5 value literals [C<'same'>, C<'0'>, C<0>] all map to 'same, and the
Perl 5 value literals [C<'decrease'>, C<'1'>, C<1>] all map to 'decrease'.
The 6 character strings [C<increase>, C<-1>, C<same>, C<0>, C<decrease>,
C<1>] specifically are what the Plain Text Muldis D grammar uses, and are
the results of parsing it in Perl 5.
...
the payload.  The payload must be a Perl 6 C<Str> or Perl 5 character
string having one of the 7 values C<half_down>, C<half_up>, C<half_even>,

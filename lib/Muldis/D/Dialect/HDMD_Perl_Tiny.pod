...
C<Relation> or C<Array>, then often no information can be gleaned for what
attributes should exist, and so C<auto_complete> won't help you and you
must fully define relation values with the same attributes per tuple.  But
fortunately, a lot of the places where C<auto_complete> would help you the
most is when the bootloader is invoking system-defined data-defining
procedures, and their parameters are all of attribute-specifying types, and
it is in such data definition that you may be most likely to face a large
number of default-valued attributes, such as C<comment>.

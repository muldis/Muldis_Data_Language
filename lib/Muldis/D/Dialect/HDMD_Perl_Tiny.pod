...
This document is Muldis::D::Dialect::HDMD_Perl_Tiny version 0.27.0.
...
concrete syntax tree, comprised of native Perl scalar and collection typed
values, resulting from parsing code written in the Muldis D C<PTMD_Tiny>
dialect (L<Muldis::D::Dialect::PTMD_Tiny>) using Perl.  This dialect exists
as a convenience to Perl programmers that want to generate or introspect
Muldis D code by saving them the difficulty and overhead of escaping and
stitching plain text code; it is expected that a Muldis D implementation
written in Perl will natively accept input in both the C<PTMD_Tiny> and
C<HDMD_Perl_Tiny> dialects.  Furthermore, the C<HDMD_Perl_Tiny> dialect
provides additional Muldis D syntax options to Perl programmers besides
what C<PTMD_Tiny> would canonically parse into, such as the direct use of
some Perl-only features.  Note that most of the details that the 2 dialects
have in common are described just in the C<PTMD_Tiny> file, for both
dialects; this current file will mainly focus on the differences; you
should read the L<Muldis::D::Dialect::PTMD_Tiny> file before the current
one, so to provide a context for better understanding it.
...
A C<HDMD_Perl_Tiny> Muldis D code file is a Perl 5 code file that defines a
Perl value that is a tree of Perl Array, such that each Array is a tree
I<node>; or such a Perl value definition may be embedded or constructed in
one or more files of a larger Perl program that does more than this.  The
elements of each node/Array include typically a native Perl payload value,
which may be a node itself, plus meta-data for that payload, that meta-data
typically including the analogy of a class name, were nodes instead
represented by a tree of node-specific objects.
...
=head1 HERE AND BELOW WE RERITE HAVE TO STILL.
...
This node type represents a bag value.  It has 2 elements:
...
Examples:
    ...
    [ 'Bag', [
        [ [ 'Text', 'Apple'  ], [ 'md_int', '9', '500' ] ],
        [ [ 'Text', 'Orange' ], [ 'perl_int', 300 ] ],
        [ [ 'Text', 'Banana' ], [ 'perl_int', 400 ] ],
    ] ]

...
This document is Muldis::D::Dialect::HDMD_Perl_Tiny version 0.26.0.
...
Examples:
    ...
    [ 'Scalar', 'sys.std.Temporal.Type.UTCDateTime', 'datetime', {
        'year'   => [ 'Int', 'perl_int', 2003 ],
        'month'  => [ 'Int', 'perl_int', 10 ],
        'day'    => [ 'Int', 'perl_int', 26 ],
        'hour'   => [ 'Int', 'perl_int', 1 ],
        'minute' => [ 'Int', 'perl_int', 30 ],
        'second' => [ 'Rat', 'perl_rat', 0 ],
    } ]
...
If the format is C<any_perl>, then the payload may be any Perl value, and
it is simply coerced into an integer context as per Perl's own semantics
(resulting from adding zero to it), meaning base-10 where applicable.  If
something doesn't look numeric, it becomes zero; if something looks like a
fractional number, it is truncated.
...
Only when the node has 3 elements; format; one of: C<aoa_counted>,
C<array_repeated>.

=item *

The payload; only when the node has 2 elements, a Perl C<Bag|KeyBag> value;
only when the node has 3 elements, a Perl C<Array|Seq> or C<Array|Seq> of
C<Array|Seq>.
...
If the node has 2 elements, then the payload must be a Perl 6 (there is no
Perl 5 analogy) C<Bag|KeyBag> value; the payload elements are PHMD nodes
corresponding to the C<value> attribute of the new bag's tuples, and the
mapping is as you should expect.

=item *

If the node has 3 elements, and the format is C<aoa_counted>, then the
payload must be a Perl C<Array|Seq>, and each element of the payload
defines a binary tuple of the new bag; the element is a 2-element
C<Array|Seq>, and those 2 elements, by index order, are a PHMD node that
defines the C<value> attribute of the tuple, and a 2-3-element C<Array|Seq>
(which is the same as an 'Int' PHMD node minus the first constant element)
that defines the C<count> attribute of the tuple; the count must be a
positive integer.

=item *

If the node has 3 elements, and the format is C<array_repeated>, then the
payload must be a Perl C<Array|Seq>, and each element of the payload
contributes to a binary tuple of the new bag; the element value is a PHMD
node that defines the C<value> attribute of the tuple.  The bag has 1 tuple
for every distinct (after format normalization) element value in the
payload, and the C<count> attribute of that tuple says how many instances
of said element were in the payload.
...
Node type: the Perl C<Str> value C<NC>.
...
Examples:
    ...
    [ 'NC', ['fed','data','the_db','gene','sorted_person_name'] ]

    [ 'NC', 'fed.data.the_db.stats.samples_by_order' ]
...
Node type: the Perl C<Str> value C<DNC>.
...
Examples:
    ...
    [ 'DNC', ['gene','sorted_person_name'] ]

    [ 'DNC', '.stats.samples_by_order.' ]
...
If the format is C<md_rat> and the payload is not an C<Array|Seq>, then the
...
0\.?<[0-9A-Z]>* >> or C<< \-?<[1-9A-Z]><[0-9A-Z]>*\.?<[0-9A-Z]>* >>.  This
format specifically is what the Plain Text Muldis D grammar uses ('radix'
option), and is the result of parsing it.  The payload is interpreted as a
base-I<N> rational where I<N> might be between 2 and 36, and the given
...
If the format is C<md_rat> and the payload is a 2-element C<Array|Seq>,
...
format C<0> or C<< \-?<[1-9A-Z]><[0-9A-Z]>* >> (an integer) and the second
element is a Perl C<Str> of the format C<< <[1-9A-Z]><[0-9A-Z]>* >> (a
...
If the format is C<md_rat> and the payload is a 3-element C<Array|Seq>,
then the max-col-val must be as per the previous bullet, and the payload
C<Array|Seq> must be such that the first element is a Perl C<Str> of the
format C<0> or C<< \-?<[1-9A-Z]><[0-9A-Z]>* >> (an integer) and the second
element is a Perl C<Str> of the format C<< <[2-9A-Z]><[0-9A-Z]>* >> (an
...
If the format is C<perl_rat> and the payload is not an C<Array|Seq>, then:
...
If the format is C<perl_rat> and the payload is a 2-element C<Array|Seq>,
...
If the format is C<perl_rat> and the payload is a 3-element C<Array|Seq>,
then the payload C<Array|Seq> must be such that each element is a canonical
...
If the format is C<any_perl> and the payload is not an C<Array|Seq>, then
the payload may be any Perl value, and it is simply coerced into a numeric
context as per Perl's own semantics (resulting from adding zero to it),
meaning base-10 where applicable.  If something doesn't look numeric, it
becomes zero.

=item *

If the format is C<any_perl> and the payload is a 2-element C<Array|Seq>,
then each payload array element is simply coerced into an integer context,
and the payload is then interpreted as per C<perl_rat>.  If Perl's coercion
of the denominator produces an integer less than 1, it becomes a 1.  I<This
may be revised to fail instead.>

=item *

If the format is C<any_perl> and the payload is a 3-element C<Array|Seq>,
then each payload array element is simply coerced into an integer context,
and the payload is then interpreted as per C<perl_rat>.  If Perl's coercion
of the radix produces an integer less than 2, it becomes a 2.  I<This may
be revised to fail instead.>
...
Examples:
    ...
    [ 'Rat', 'md_rat', '6', [ '500001', '1000' ] ]

    [ 'Rat', 'md_rat', 'B', [ 'A09B', 'A' ] ]

    [ 'Rat', 'md_rat', '1', [ '1011101101', '10', '-11011' ] ]

    [ 'Rat', 'md_rat', '9', [ '45207196', '10', '37' ] ]

    [ 'Rat', 'perl_rat', 21.003 ]

    [ 'Rat', 'perl_rat', 5.428**295 ]

    [ 'Rat', 'perl_rat', [ 1, 43 ] ]

    [ 'Rat', 'perl_rat', [ 314159, 10, -5 ] ]

    [ 'Rat', 'any_perl', ' 54.67 ' ]

    [ 'Rat', 'any_perl', [ ' 57 ', ' 71 ' ] ]

    [ 'Rat', 'any_perl', [ ' 656573456 ', ' 8 ', ' 243323 ' ] ]
...
with the UTC time-zone.  It has 3-4 elements:
...
Format; one of: C<md_num>, C<perl_num>, C<any_perl>.

=item *

Only when format is C<md_num>; the max-col-val.
...
when defined, is further interpreted according to the format and
max-col-val elements of this node, in the same manner as how an C<Int> or
C<Rat> node payload is interpreted; the format value C<md_num> corresponds
directly to C<md_(int|rat)>, C<perl_num> corresponds to C<perl_(int|rat)>,
and C<any_perl> corresponds to C<any_perl>.

A defined C<year> may be any integer, each of [C<month>, C<day>] must be a
positive integer, each of [C<hour>, C<minute>] must be an
unsigned/non-negative integer, and C<second> must be an
unsigned/non-negative rational number.  If all 6 attributes are defined,
then the new C<UTCInstant> value is also a C<UTCDateTime>; if just the
first 3 or last 3 are defined, then the value is not a C<UTCDateTime> but
rather a C<UTCDate> or C<UTCTime>, respectively; if any other combination
of attributes are defined, then the value is just a C<UTCInstant> and not
of any of the other 3 subtypes.

Examples:

    [ 'UTCInstant', 'perl_num', [1964,10,16,16,12,47.5] ] # a UTCDateTime

    [ 'UTCInstant', 'perl_num', [2002,12,6] ] # a UTCDate

    [ 'UTCInstant', 'perl_num', [undef,undef,undef,14,2,29] ] # a UTCTime
...
Examples:
    ...
    [ 'FloatInstant', 'perl_num', [2003,4,5,2] ] # min,sec unknown or N/A

    [ 'FloatInstant', 'perl_num', [1407] ] # just know its sometime in 1407

=head2 sys.std.Temporal.Type.Duration

This node type represents a duration value, an amount of time, which is not
fixed to any instant in time.  It has 3-4 elements:
...
Examples:
    ...
    [ 'Duration', 'perl_num', [3,5,1,6,15,45.000012] ]

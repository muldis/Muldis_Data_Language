...
array ref having at least 2 elements; each element is a C<expr> node that
...
Perl array ref having at least 2 elements; each element is a C<expr> node
...
An C<comm_dyadic_infix_op> node has exactly 2 op args, each of which is a
C<expr> node that defines an input value for the operator; which function
arguments get which op args isn't significant.
...
An C<noncomm_dyadic_infix_op> node has exactly 2 op args, each of which is
a C<expr> node that defines an input value for the operator; the first and
second arg are C<lhs> and C<rhs>, respectively.

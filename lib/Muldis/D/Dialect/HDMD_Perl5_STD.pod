...
This document is Muldis::D::Dialect::HDMD_Perl5_STD version 0.104.0.
...
standard dialect is C<[ 'Muldis_D', 'http://muldis.com', '0.104.0',
'HDMD_Perl5_STD' ]>.
...
Examples:
    ...
    [ 'Muldis_D', 'http://muldis.com', '0.104.0', 'HDMD_Perl5_STD', {
...
C<Blob>, C<Text>, C<Name>, C<NameChain>, C<PNSQNameChain>, C<Comment>,
C<Instant>, C<Duration>,
...
C<PNSQNameChain>, C<Comment>, C<Instant>, C<Duration>,
C<RatRoundRule>, C<[|B|O|UCP]String>, C<[|DH]Scalar>,
...
except for the extra restrictions and shorthands.  If the first element of
a C<PNSQNameChain> payload is the empty string then the payload is treated
as if the first element were instead C<topic>.
...
An C<Interval> node represents a literal or selector invocation for a
possibly-discontinuous-or-empty interval value.  It is interpreted as a
Muldis D C<sys.std.Core.Type.Interval> value whose elements are defined by
the payload.  The payload must be just a Perl array ref; it is interpreted
as follows:
...
Iff the payload has zero elements, then it defines the only interval value
having zero elements.

=item *

Iff the payload has at least one element, and the payload's first element
I<is> a Perl array ref, then every one of the payload elements must be
itself a Perl array ref, and each payload element is designated a
I<sub-interval>.  Each I<sub-interval> of the payload defines a 4-ary tuple
of the new interval, which defines a continuous sub-interval of the
interval; each I<sub-interval> defines the 4 attributes of the tuple.  Each
I<sub-interval> must have exactly 3 elements, which
...
attribute value, respectively, of the new sub-interval.  The I<interval
...
sub-interval can have, which in order are: C<[false,false]>,
C<[false,true]>, C<[true,false]>, C<[true,true]>.

=item *

Iff the payload has at least one element, and the payload's first element
I<is not> a Perl array ref, then every one of the payload elements must
I<not> be itself a Perl array ref, and the payload as a whole is designated
a I<sub-interval>.  The single I<sub-interval> is interpreted as per one of
those in the previous bullet point.

=back

Examples:

    [ 'Interval', [] ]  # zero elements #

    [ 'Interval', ['abc','..','abc'] ]  # 1 element #

    [ 'Interval', [1,'..',10] ]  # 10 elements #
    ...
    [ 'Interval', [[1,'..',3],[6,'..',6],[8,'..',9]] ]  # 6 elements #
...
C<scalar_type>, C<nonscalar_type>, C<domain_type>, C<subset_type>.
...
C<acc_via_named> is a C<PNSQNameChain_payload>, which is by itself the
...
When this extension is active, there exist 3 additional varieties of
C<value> node:  C<UTCInstant>, C<FloatInstant>, C<UTCDuration>.  Also,
between these 3 varieties, these 9 additional values are allowed for
C<value>'s C<value_kind> attribute:  C<UTC[Instant|DateTime|Date|Time]>,

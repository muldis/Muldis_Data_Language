...
This document is Muldis::D::Dialect::HDMD_Perl5_STD version 0.75.0.
...
C<RatRoundRule>, C<String>, C<QScalar>, C<QTuple>, C<Database>,
C<QRelation>, C<QSet>, C<QMaybe>, C<QArray>, C<QBag>.
...
C<UTCDuration>, C<RatRoundRule>, C<[|B|O|UCP]String>, C<[Q|]Scalar>,
C<[Q|]Tuple>, C<Database>, C<[Q|]Relation>, C<[Q|]Set>,
C<[Q|][Maybe|Single]>, C<[Q|]Array>, C<[Q|]Bag>.
...
As a special case, the first element of a 2+ element C<NameChain> payload
may be undefined; if it is, then that element is treated as if it was the
C<Name> payload value C<topic>; no other element may be undefined, and an
undefined first element is only permitted when there are 2+ elements.
...
either of the 2 Perl character strings C<if_else_expr> and C<??!!>.  The

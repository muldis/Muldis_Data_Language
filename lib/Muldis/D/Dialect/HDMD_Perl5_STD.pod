...
This document is Muldis::D::Dialect::HDMD_Perl5_STD version 0.72.0.
...
C<QMaybe>, C<QArray>, C<QBag>, C<RatRoundRule>.
...
C<[Q|]Array>, C<[Q|]Bag>, C<RatRoundRule>.
...
=head2 Rational Rounding Rule Selectors

A C<RatRoundRule> node represents a literal or selector invocation for a
rational rounding rule.  It is interpreted as a Muldis D
C<sys.std.Core.Type.Cat.RatRoundRule> value whose attributes are defined by
the C<RatRoundRule_payload>.  A C<RatRoundRule_payload> must be a Perl
array ref with 3 elements, which correspond in order to the 3 attributes:
C<radix> (a C<PInt2_N>), C<min_exp> (an C<Int>), and C<round_meth> (a
C<RatRoundMeth>).  Each of the 3 C<RatRoundRule_payload> elements is an
C<expr> node.
...
character string C<named_expr>.  The second element is a C<Name_payload>
and the third element is an C<expr> node (that isn't a C<named_expr> node);
the second element declares an explicit expression node name for the third
element.
...
Examples:
    ...
    [ 'if_else_expr',
        ...
        [ 'op', 'QArray.value', ['expr_name','ary'], 0 ]
    ]
...
A C<func_invo_alt_syntax> node has 3-N ordered elements:  The first element
is the Perl character string C<op>.  The second element is a Perl character
string, hereafter referred to as I<op>, which determines the function to
invoke.  The last 1-N elements provide arguments to the function
invocation; the number and format of these last 1-N elements, hereafter
referred to as I<op args>, varies depending on the second
C<func_invo_alt_syntax> node element.

=head2 Simple Commutative N-adic Infix Reduction Operators

A C<comm_infix_reduce_op_invo> node has exactly 1 op arg, which is a Perl
array ref having at least 2 elements; each element is an C<expr> node that
defines an input value for the operator.
...
A C<noncomm_infix_reduce_op_invo> node has exactly 1 op arg, which is a
Perl array ref having at least 2 elements; each element is an C<expr> node
that defines an input value for the operator.
...
=head2 Simple Commutative Dyadic Infix Operators

A C<comm_dyadic_infix_op_invo> node has exactly 2 op args, each of which is
an C<expr> node that defines an input value for the operator; which
function arguments get which op args isn't significant.
...
=head2 Simple Non-commutative Dyadic Infix Operators

A C<noncomm_dyadic_infix_op_invo> node has exactly 2 op args, each of which
is an C<expr> node that defines an input value for the operator; the first
and second arg are C<lhs> and C<rhs>, respectively.
...
Examples:
    ...
    [ 'op', 'R-', [ 'Set', [ 8, 4, 6, 7 ] ], [ 'Set', [ 9, 0, 7 ] ] ]

    [ 'op', 'R/', [ 'Relation', [ ['x', 'y'] => [ [5, 6], [3, 6] ] ] ],
        [ 'Relation', [ { y => 6 } ] ] ]

=head2 Simple Monadic Prefix Operators

A C<monadic_prefix_op_invo> node has exactly 1 op arg, which is an C<expr>
node that defines the input value for the operator.
...
A C<monadic_postfix_op_invo> node has exactly 1 op arg, which is an C<expr>
node that defines the input value for the operator.
...
A C<rat_op_invo_with_round> node has exactly 2-3 op args, each of which is
an C<expr> node that defines an input value for the operator.  When there
are 2 op args, the first and second args are C<expr> and C<round_rule>,
respectively.  When there are 3 op args, the first, second and third args
are C<lhs>, C<rhs> and C<round_rule>, respectively.

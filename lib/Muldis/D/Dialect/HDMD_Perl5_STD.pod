...
This document is Muldis::D::Dialect::HDMD_Perl5_STD version 0.134.0.
...
max-col-val must be a Perl string composed of a single C<[1..9 A..Z bodx]>
...
base 10.  Note that the C<0bN>,C<0N>,C<0xN> formats for Perl 5 numeric
literals only work for Perl 5 integer literals, not non-integer literals.
...
specify, eg, bases [2,8,10,16], use max-col-val of [1,7,9,F].  Alternately
(recommended), use [b,o,d,x] for the common bases [2,8,10,16].  If the main
...
If the payload is a Perl hash ref, then it must have 1 element, whose key
and value are designated, in order, I<max-col-val> and I<main payload>; the
max-col-val must be a Perl string composed of a single C<[137Fbox]>
character,
...
one of [1,2,3,4] bits, depending on whether max-col-val is [1|b,3,7|o,F|x].
...
character string C<d>.  The second element is a C<Name_payload>.

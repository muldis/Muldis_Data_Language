...
This document is Muldis::D::Dialect::HDMD_Perl5_STD version 0.124.0.
...
Examples:
    ...
    [ 'Muldis_D', 'http://muldis.com', '0.124.0', 'HDMD_Perl5_STD', {
...
is one of these 9 Perl character strings: C<function>, C<named-value>,
...
is the Perl character string C<updater>.  The second element is a
...
character string C<recipe>.  The second element is a C<Name_payload>, which
is the recipe's declared name.  The third element is a C<recipe_payload>.
A C<recipe_payload> is a 2-element Perl array ref whose elements are
designated, in order, C<recipe_heading> and C<recipe_body>.

Iff the C<recipe_heading> is a Perl array ref, then it has 1-2 ordered
...
C<recipe_heading> is a Perl hash ref, then it is designated
C<rcp_params> and there is no C<implements>.

An C<rcp_params> is a Perl hash ref; it must have at least one element,
meaning the recipe has one or more parameters; each hash element specifies
one parameter, and for each hash element, the hash element's key and value,
respectively, are designated C<param_name> and C<param_details>.  Iff
C<param_details> is a Perl array ref with at least two elements, and its
first element is not a non-empty Perl character string consisting of just
...
A C<recipe_body> must be either the Perl character string C<...>, in which
case it is an C<empty_routine_body>, or it must be a Perl array ref, having
...
is one of these 3 Perl character strings: C<procedure>, C<system-service>,
...
Perl character string C<curried-func>.  The second element is a
...
the Perl character string C<imus-imp-invo>.  The second element is a
...
Perl character string C<atomic-stmt>.  The second element is a Perl
array ref; it is interpreted as per a nonempty updater body, which has
...
the Perl character string C<iproc-imp-invo>.  The second element is a
...
2 Perl character string C<if-else-stmt>.  The optional second element is
I<if_then>, a Perl array ref with 0..N elements, each of those being a
2-element Perl array ref; the first element is an
I<if> condition variable (a C<proc_expr> node), and the second element
is the associated I<then> result statement (a C<proc_stmt> node).
...
element is a I<when> comparand (a C<proc_expr> node), and the second

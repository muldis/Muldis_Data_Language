...
=head2 Infix N-adic Reduction Operators

An C<infix_reduce_op_invo> node has exactly 1 op arg, which is a Perl array
ref having at least 2 elements; each element is a C<expr> node that defines
an input value for the operator.
...
=head2 Infix Homogenous Dyadic Operators

An C<infix_h_dyadic_op_invo> node has exactly 2 op args, each of which is a
C<expr> node that defines an input value for the operator; the first and

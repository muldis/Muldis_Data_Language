...
This document is Muldis::D::Dialect::HDMD_Perl5_STD version 0.105.0.
...
standard dialect is C<[ 'Muldis_D', 'http://muldis.com', '0.105.0',
'HDMD_Perl5_STD' ]>.
...
C<ln_base_authority>; it is typically a character string like C<0.105.0>.
...
Examples:
    ...
    [ 'Muldis_D', 'http://muldis.com', '0.105.0', 'HDMD_Perl5_STD', {
...
An C<Array> node represents a literal or selector invocation for a
...
interval-union value.  It is interpreted as a Muldis D
C<sys.std.Core.Type.IntervalUnion> value whose elements are defined by the
payload.  The payload must be just a Perl array ref; it is
...
Iff the payload has zero elements, then it defines the only interval-union
value canonically having zero elements.
...
I<nu-interval>.  Each I<nu-interval> of the payload defines a 4-ary tuple
of the new interval-union, which defines a (non-union) interval
of the interval-union; each I<nu-interval> defines the 4 attributes of the
tuple.  Each I<nu-interval> must have exactly 3 elements, which
...
attribute value, respectively, of the new (non-union) interval.  The
...
the new (non-union) interval can have, which in order are:
...
a I<nu-interval>.  The single I<nu-interval> is interpreted as per one of
...
There are 8 main varieties of C<material> node, each of which is a named

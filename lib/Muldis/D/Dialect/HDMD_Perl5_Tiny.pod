...
This document is Muldis::D::Dialect::HDMD_Perl5_Tiny version 0.53.0.
...
explicitly stores; so code in the C<HDMD_Perl5_Tiny> dialect should be
...
represented by their equivalent Perl 5 native scalar and collection
types.  But since Muldis D is more strongly typed, or at least differently
typed, than Perl 5, each Muldis D literal is represented by a Perl 5 Array,
whose elements include both the payload Perl 5 literal plus explicit
meta-data for how to interpret that Perl 5 literal for mapping to Muldis D.

This document mainly just specifies a way to represent Muldis D values as
Perl 5 values.  Since the fundamental way to do data definition in Muldis D
...
For the present, this document will use the term I<PHMD> (I<Perl 5 Hosted
Muldis D>) for brevity when referring to aspects of the Perl 5-Tiny
dialect.
...
Dialect: the Perl C<Str> value C<HDMD_Perl5_Tiny>.
...
and whose attributes are defined by the payload.  Under Perl 6, the payload
must be a Perl 6 C<Pair>, whose key and value, respectively, are designated
I<possrep name> and I<possrep attrs>.  Under Perl 5, the payload must be
just a Perl 5 array ref having exactly 2 elements, that are designated the
same 2 labels in that order.  The possrep name and possrep attrs must be as
...
Under Perl 6, the payload must be a Perl 6 C<Int>, which is mapped
directly.

=item *

Under Perl 5, if the payload is a Perl 5 scalar, then it must be just a
canonical integer value according to Perl, and it is mapped directly; since
native Perl 5 integers are limited precision, larger integers can be
represented by a Perl 5 character string of the format C<0> or C<<
\-?<[1..9]><[ 0..9 _ ]>* >> that is interpreted as base 10.

=item *

Under Perl 5, an alternative payload format is a C<bigint> object, which is
conceptually the closest thing Perl 5 has in core to a Perl 6 C<Int>.

=item *

Under Perl 5, if the payload is a Perl 5 hash ref, then it must have 1
element, whose key and value are designated, in order, I<max-col-val> and
I<main payload>; the max-col-val must be a Perl 5 string composed of a
single C<[ 1..9 A..Z ]> character, and the main payload must be a Perl 5
character string of the format C<0> or C<< \-?<[ 1..9 A..Z ]><[ 0..9 A..Z _
]>* >>.  This format specifically is what the Plain Text Muldis D grammar
uses as its general form for integer literals, and is the result of parsing
it.  The main payload is interpreted as a base-I<N> integer where I<N>
...
Under Perl 6, the payload must be a Perl 6 C<Seq|Array>, whose elements are
all Perl 6 C<Int>, which is mapped directly; each payload element is mapped
directly as per the payload of an C<Int> PHMD node.

=item *

Under Perl 5, if the payload is a Perl 5 array ref, then every one of its
elements must constitute a valid payload for an C<Int> PHMD node.

=item *

Under Perl 5, if the payload is a Perl 5 hash ref, then it must have 1
element, whose key and value are designated, in order, I<max-col-val> and
I<main payload>; the max-col-val must be a Perl 5 string and the main
payload must be a Perl 5 array ref; every pairwise combination of the
max-col-val with the elements of the main payload must, when appropriately
wrapped in a Perl 5 hash ref, must constitute a valid hash ref payload for
an C<Int> PHMD node.
...
Under Perl 6, the payload must be a Perl 6 C<Blob>, which is mapped
directly.

=item *

Under Perl 5, if the payload is a Perl 5 scalar, then it must be a
canonical Perl 5 bit string, which is a scalar whose utf-8 flag is false,
and it is mapped directly.
...
element, whose key and value are designated, in order, I<max-col-val> and
I<main payload>; the max-col-val must be a Perl 5 string composed of a
single C<[137F]> character, and the main payload must be a Perl 5 character
string of the format C<< <[ 0..9 A..F ]>* >>.  This format specifically is
what the Plain Text Muldis D grammar uses, and is the result of parsing it
in Perl 5.  Each column of the main payload specifies a sequence of one of
[1,2,3,4] bits, depending on whether max-col-val is [1,3,7,F].
...
node.  Under Perl 6, the payload must be a Perl 6 C<Str>.  Under Perl 5,
the payload must be just a canonical Perl 5 character string, which is any
Perl 5 scalar value (a Muldis D implementation in Perl 5 can ignore the
...
Perl 5 Examples:
    ...
    [ 'Text', 'サンプル' ] # note: needs "use utf8;" pragma to work
...
the payload.  Under Perl 6, the payload must be a Perl 6 C<Mapping|Hash>.
Under Perl 5, the payload must be just a Perl 5 hash ref.  Each key+value
...
name, and a PHMD node that specifies the attribute value.
...
Under Perl 6, iff the payload is a Perl 6 C<Set|KeySet> with zero elements,
then it defines the only relation value having zero attributes and zero
tuples.  For the Perl 5 equivalent, change the C<Set|KeySet> to a Perl 5
array ref.

=item *

Under Perl 6, iff the payload is a Perl 6 C<Set|KeySet> with at least one
element, and every element is a Perl 6 C<Str> (as per a valid payload for a
C<Name> PHMD node), then it defines the attribute names of a relation
having zero tuples.  For the Perl 5 equivalent, change the C<Set|KeySet> to
a Perl 5 array ref, and each C<Str> to a Perl 5 character string.

=item *

Under Perl 6, iff the payload is a Perl 6 C<Set|KeySet> with at least one
element, and every element is a Perl 6 C<Mapping|Hash> (as per a valid
payload for a C<Tuple> PHMD node), then each element of the payload defines
a tuple of the new relation; every tuple-defining element of the payload
...
whole, which is its heading for the current purposes.  For the Perl 5
equivalent, change the C<Set|KeySet> to a Perl 5 array ref, and each
C<Mapping|Hash> to a Perl 5 hash ref.

=item *

Under Perl 6, iff the payload is a Perl 6 C<Pair>, then:  The new relation
value's attribute names are defined by the payload's key, which is a Perl 6
...
value, which is a Perl 6 C<Set|KeySet> of C<Seq|Array> of tuple attribute
...
matched up by ordinal position here.  For the Perl 5 equivalent, change the
C<Pair> to a Perl 5 array ref with exactly 2 elements, each of which is a
Perl 5 array ref, and:  The new relation value's attribute names are
defined by the payload's first element, which is a Perl 5 array ref of
character string (each as per a C<Name> node payload), and the relation
body's tuples' attribute values are defined by the payload's value, which
is a Perl 5 array ref of Perl 5 array ref of tuple attribute value defining
PHMD nodes.
...
payload.  Under Perl 6, the payload must be a Perl 6 C<Set|KeySet>.  Under
Perl 5, the payload must be just a Perl 5 array ref.  Each element of the
...
payload.  Under Perl 6, the payload must be a Perl 6 C<Seq|Array>.  Under
Perl 5, the payload must be just a Perl 5 array ref.  Each element of the
...
payload.  The payload is interpreted as follows:

=over

=item *

Under Perl 6, the payload must be a Perl 6 C<Bag|KeyBag|Set|KeySet> value;
...
Under Perl 5, iff the payload is a Perl 5 array ref with zero elements,
then it defines the only bag value having zero elements.  Iff the payload
is an array ref with at least one element, then every one of the payload
elements must be itself a Perl 5 array ref.

=item *

Under Perl 5, iff the payload is an array ref with at least one (array ref)
element, and the first element of that element I<is> itself an array ref,
then the payload is interpreted as being of the I<array counted values> bag
format.  Each element of the payload defines a binary tuple of the new bag;
the element is a 2-element array ref, and those 2 elements, by index order,
are a PHMD node that defines the C<value> attribute of the tuple, and a
valid C<Int> PHMD node payload that defines the C<count> attribute of the
tuple; the count must be a positive integer.

=item *

Under Perl 5, iff the payload is an array ref with at least one (array ref)
element, and the first element of that element I<is not> itself an array
ref, then the payload is interpreted as being of the I<array repeated
values> bag format.  Each element of the payload contributes to a binary
tuple of the new bag; the element value is a PHMD node that defines the
C<value> attribute of the tuple.  The bag has 1 tuple for every distinct
...
Muldis D C<sys.std.Rational.Type.Rat> value as follows:
...
Under Perl 5, if the payload is a Perl 5 scalar, then it must be just a
canonical numeric value according to Perl, and it is mapped directly; since
native Perl 5 numerics are limited precision or are inexact (IEEE float),
larger numerics can be represented by a Perl 5 character string of the
format C<< 0\.?<[ 0..9 _ ]>* >> or C<< \-?<[1..9]><[ 0..9 _ ]>*\.?<[ 0..9 _
]>* >> that is interpreted as base 10.

=item *

Under Perl 5, an alternative payload format is a C<bigrat|bignum|bigint>
object, which is conceptually the closest thing Perl 5 has in core to a
Perl 6 C<Rat|Num|Int>.

=item *

Under Perl 5, if the payload is a Perl 5 array ref, then the payload must
have exactly 2 or 3 elements, each of which constitutes a valid payload of
an C<Int> PHMD node.  If the payload has 2 elements, then the rational's
value is interpreted as the first element (a numerator) divided by the
second (a denominator).  If the payload has 3 elements, then the rational's
value is interpreted as the first element (a mantissa) multiplied by the
result of the second element (a radix) taken to the power of the third (an
exponent).
...
element, whose key and value are designated, in order, I<max-col-val> and
I<main payload>; the max-col-val must be a Perl 5 string composed of a
single C<[ 1..9 A..Z ]> character.  If the main payload is a Perl 5 scalar,
then the main payload must be a Perl 5 character string of the format C<<
0\.?<[ 0..9 A..Z _ ]>* >> or C<< \-?<[ 1..9 A..Z ]><[ 0..9 A..Z _ ]>*\.?<[
0..9 A..Z _ ]>* >>.  This format specifically is what the Plain Text Muldis
D grammar uses as one general form for rational literals, and is the result
...
payload is a Perl 5 array ref, then the main payload must have exactly 2 or
3 elements, and every pairwise combination of the max-col-val with the
elements of the main payload must, when appropriately wrapped in a Perl 5
...
Under Perl 6, the payload must be a Perl 6 C<Seq|Array> with 6 elements,
where each element may be either undefined or defined; or if fewer than 6
elements are provided, the C<Seq|Array> will be implicitly extended to 6,
filling with undefs.  The 6 payload elements correspond in
order, from the lowest to the highest indexed, to the 6 attributes:
C<year>, C<month> C<day>, C<hour>, C<minute>, C<second>.  For each
...
it must qualify as a valid payload for a C<Rat> PHMD node.
...
Under Perl 5, if the payload is a Perl 5 array ref, then the interpretation
of this payload is the same as for the Perl 6 payload.
...
element, whose key and value are designated, in order, I<max-col-val> and
I<main payload>; the max-col-val must be a Perl 5 string and the main
payload must be a Perl 5 array ref as per the payload of the previous
bullet point but that none of the 6 elements is a Perl 5 hash ref.  Each of
the 6 main payload elements, when defined, is further interpreted according
to the max-col-val, in the same manner as how an C<Int> or C<Rat> node's
hash ref payload's element's value is interpreted.  The interpretation of
this payload is the same as for the other Perl 5 payload.
...
values are defined by the payload.  The payload is interpreted as follows:
...
C<HDMD_Perl5_Tiny> dialects, and have the same semantics with both.
...
C<HDMD_Perl5_Tiny> dialect is comprised almost entirely of value literals,

...
This document is Muldis::D::Dialect::HDMD_Perl5_Tiny version 0.52.0.
...
A PHMD value is composed mainly of a tree of Perl 5 Array, such that each
Array is a tree node.  The elements of each node/Array include typically a
native Perl 5 payload value, which may be a PHMD value itself, plus
meta-data for that payload, that meta-data typically including the analogy
of a class name, were PHMD nodes instead represented by a tree of
PHMD-specific objects.

It should be emphasized that no Perl 5 undefined values are allowed
anywhere in a PHMD value; you must use only defined values instead.  This
documentation also assumes that only defined values are used, and that
supplying a Perl 5 undef will result in an error.  If you genuinely want to
represent that a value is unknown or inapplicable, then the C<Nothing> node
kind is provided as one way you can explicitly say so.  I<This policy may
be reconsidered.>
...
This node kind represents a scalar subtype value.  It has 4 elements:

=over

=item *

Node kind: the Perl C<Str> value C<Scalar>.
...
PHMD node kind can only be used for scalar types that have at least 1
...
This node kind represents a logical boolean value.  It has 2 elements:

=over

=item *

Node kind: the Perl C<Str> value C<Bool>.
...
This node kind represents an integer value.  It has 2-3 elements:

=over

=item *

Node kind: the Perl C<Str> value C<Int>.
...
composed of a single C<[ 1..9 A..Z ]> character, and the payload must be a
Perl C<Str> of the format C<0> or C<< \-?<[ 1..9 A..Z ]><[ 0..9 A..Z _ ]>*
>>.  This format specifically is what the Plain Text Muldis D grammar uses,
and is the result of parsing it.  The payload is interpreted as a base-I<N>
integer where I<N> might be between 2 and 36, and the given max-col-val
...
This node kind represents an integer string value.  It has 2-3 elements:

=over

=item *

Node kind: the Perl C<Str> value C<String>.
...
This node kind represents a bit string.  It has 2-3 elements:

=over

=item *

Node kind: the Perl C<Str> value C<Blob>.
...
If the node has 3 elements, then the max-col-val must be a Perl C<Str>
composed of a single C<[137F]> character, and the payload must be a Perl
C<Str> of the format C<< <[ 0..9 A..F ]>* >>.  This format specifically is
...
This node kind represents a character string.  It has 2 elements:

=over

=item *

Node kind: the Perl C<Str> value C<Text>.
...
This node kind represents a tuple value.  It has 2-3 elements:

=over

=item *

Node kind: the Perl C<Str> value C<Tuple>.
...
This node kind represents a relation value.  It has 2-4 elements:

=over

=item *

Node kind: the Perl C<Str> value C<Relation>.
...
This node kind represents a set value.  It has 2-3 elements:

=over

=item *

Node kind: the Perl C<Str> value C<Set>.
...
This node kind represents a 'single' value.  It has 2-3 elements:

=over

=item *

Node kind: the Perl C<Str> value C<Single>.
...
This node kind represents a sequence value.  It has 2-3 elements:

=over

=item *

Node kind: the Perl C<Str> value C<Array>.
...
This node kind represents a bag value.  It has 2-3 elements:

=over

=item *

Node kind: the Perl C<Str> value C<Bag>.
...
Node kind: the Perl C<Str> value C<Name>.
...
This node kind represents a canonical long name for invoking a DBMS
...
Node kind: the Perl C<Str> value C<NameChain>.
...
Node kind: the Perl C<Str> value C<Comment>.
...
This node kind represents an order-determination.  It has 2 elements:

=over

=item *

Node kind: the Perl C<Str> value C<Order>.
...
(note that each of the 3 C<Order> values has 2 possreps that are
...
This node kind represents a rational value.  It has 2-3 elements:

=over

=item *

Node kind: the Perl C<Str> value C<Rat>.
...
max-col-val must be a Perl C<Str> composed of a single C<[ 1..9 A..Z ]>
character, and the payload must be a Perl C<Str> of the format C<< 0\.?<[
0..9 A..Z _ ]>* >> or C<< \-?<[ 1..9 A..Z ]><[ 0..9 A..Z _ ]>*\.?<[ 0..9
A..Z _ ]>* >>.  This format specifically is what the Plain Text Muldis D
grammar uses ('radix' option), and is the result of parsing it.  The
payload is interpreted as a base-I<N> rational where I<N> might be between
2 and 36, and the given max-col-val says which possible value of I<N> to
use.  Assuming all column values are between zero and I<N>-minus-one, the
...
format C<0> or C<< \-?<[ 1..9 A..Z ]><[ 0..9 A..Z _ ]>* >> (an integer) and
the second element is a Perl C<Str> of the format C<< <[ 1..9 A..Z ]><[
0..9 A..Z _ ]>* >> (a positive integer).  The payload is interpreted as per
C<md_radix> but that its value comes from the first element (a numerator)
divided by the second (a denominator).  Note that while the C<md_radix>
format is limited to representing rationals whose denominator is a power of
some I<N> between 2 and 36, the C<md_ratio> format can represent those with
...
C<Array|Seq> must be such that the first element is a Perl C<Str> of the
format C<0> or C<< \-?<[ 1..9 A..Z ]><[ 0..9 A..Z _ ]>* >> (an integer) and
the second element is a Perl C<Str> of the format C<< <[ 2..9 A..Z ]><[
0..9 A..Z _ ]>* >> (an integer greater than 1) and the third element is as
per the first (an integer).  The payload is interpreted as per C<md_radix>
but that its value comes from the first element (a mantissa) multiplied by
the result of the second element (a radix) taken to the power of the third
(an exponent).  Note that while the C<md_ratio> format can represent all
rationals, the very large or very small ones would be quite verbose written
in that format, and so the C<md_float> format allows writing those values
in a more terse fashion, such as 1.56*10^37.
...
bigrat|bignum object or a character string of the format C<< 0\.?<[ 0..9 _
]>* >> or C<< \-?<[1..9]><[ 0..9 _ ]>*\.?<[ 0..9 _ ]>* >> that is
...
This node kind represents a rounding method.  It has 2 elements:

=over

=item *

Node kind: the Perl C<Str> value C<RatRoundMeth>.
...
Node kind: the Perl C<Str> value C<UTCInstant>.
...
Node kind: the Perl C<Str> value C<Duration>.

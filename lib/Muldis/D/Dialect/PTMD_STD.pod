...
involve rounding a number to one with less precision.  Such a function
...
in a declarative way.  A single C<rat_op_invo_with_round> node is
equivalent to a single C<func_invo> node whose C<func_arg_list> element
defines 2-3 arguments, and the C<expr|lhs|rhs|round_rule> elements of the

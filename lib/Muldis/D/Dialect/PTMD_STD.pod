...
This document is Muldis::D::Dialect::PTMD_STD version 0.96.1.
...
Examples:
    ...
    depot-catalog {
        public function cube (Int <-- $topic : Int) {
...
Grammar:
    ...
    <self_local_dbvar_type> ::=
        'self-local-dbvar-type' \s+ <NameChain_payload>
...
For example, the outer material will have the name C<dep.lib.bar.foo>.
...
C<dep.lib.bar.foo.""> and the inner will be named C<dep.lib.bar.foo.baz>.
...
used inline with its declaration, or by way of a C<MaterialRef> value which
...
Every material has 2-4 elements, illustrated by this grammar:
    ...
    <material_kind> ::=
          function
            | [type | state | transition] '-constraint'
            | 'named-value'
            | 'possrep-map'
            | 'virtual-attr-map'
            | 'order-determination'
            | 'value-' [filter | map | reduction]
        | updater
        | procedure
        | 'scalar-type'
        | 'nonscalar-type'
...
So a C<x_material>|C<material> node has 2-4 elements in general:

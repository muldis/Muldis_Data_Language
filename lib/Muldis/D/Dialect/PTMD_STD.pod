...
L</GENERIC VALUE EXPRESSIONS>, L</GENERIC RECIPE STATEMENTS>,
...
of a function or recipe or type definition, a C<value>
...
exactly the same manner as for a C<recipe> node, as a C<Function> is to
a C<Recipe>: C<ro_reg_param> (but that the C<Function> attribute is named
...
Grammar:
    ...
    <recipe_body> ::=
        <nonempty_recipe_body> | <empty_atomic_body>
...
The C<recipe_kind> has no impact at all on the interpretation of a
C<recipe>.  However, it can serve to apply additional constraints on the
allowed values of the resulting C<Recipe>, in the manner of simple
subset-type constraints, and similarly it can serve to add
self-documentation to the intended purpose or use of the recipe.  Iff
C<recipe_kind> is C<recipe> then there are no such subset-type
constraints applied, as the node is simply denoting a generic recipe; any
other value of C<recipe_kind> means that the node is denoting a value of
a proper subtype of C<Recipe>, and so that subtype's respective
constraints are applied to the new C<Recipe>.  The various
C<recipe_kind> map to C<Recipe> subtypes as follows:

    recipe kind | catalog data type
    ------------+------------------
    recipe      | Recipe
    updater     | Updater
...
A C<nonempty_recipe_body> must have at least one C<update_stmt>,
...
exactly the same manner as for a C<recipe> node, as a C<Procedure> is to
a C<Recipe>: C<upd_reg_param>, C<ro_reg_param>, C<implements_clause>,
...
beneath a C<depot>, or specifically into a function or recipe or
...
=head1 GENERIC RECIPE STATEMENTS
...
a recipe definition, because in the general case a
...
An C<imus_imp_invo> node represents the invocation of a named
recipe, with specific subject-to-update or read-only arguments, as a
single member statement of a multi-update statement of a
...
the name of the recipe being invoked, and the
...
In the general case of a recipe invocation, all of the
...
system-defined recipe where none of the corresponding parameters
are named C<topic>.
...
procedure as for imperatives invoked by a recipe.  The only
...
contexts where instances of the main nodes are being composed beneath
C<recipe> nodes, and just as C<var_name> or C<proc_expr>
...
expression (including a parameter) when that invocation is in a
...
Note that the imperative routines with alternate syntax include

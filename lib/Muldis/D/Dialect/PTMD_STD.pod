...
Muldis D has 22 precedence levels when the C<catalog_abstraction_level>
pragma is C<rtn_inv_alt_syn>; if it is C<plain_rtn_inv> instead, then 18 of
the levels can be eliminated, so then PTMD_STD has just 4; if it is
C<code_as_data> instead, then 3 more can be eliminated, leaving just 1.
...
I<The large block of 17 levels above is the least rigorous and will likely
need many changes.  The 4 small blocks are rigorous and should be okay.>

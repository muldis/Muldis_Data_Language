...
Examples:
    ...
    Rational.frac_quotient( dividend => 43.7, divisor => 16.9 )

    # same as previous #
    Rational.frac_quotient( divisor => 16.9, dividend => 43.7 )
...
This table indicates which function is invoked by each keyword:
    ...
    /    -> Core.Rational.frac_quotient( dividend => $lhs, divisor => $rhs )
...
This table indicates which function is invoked by each keyword:
    ...
    div -> Core.Integer.whole_quotient( dividend => $lhs, divisor => $rhs,
                $>round_meth )

...
This document is Muldis::D::Dialect::PTMD_STD version 0.135.0.
...
standard dialect is C<Muldis_D:"http://muldis.com":0.135.0:PTMD_STD>.
...
C<ln_base_authority>; it is typically a character string like C<0.135.0>.
...
The meanings of the simple escape sequences are:
    ...
    \`  | x;60      | GRAVE ACCENT    | `   | delim Comment literals
...
meaning also that literal backticks instead of apostrophes must be escaped.
...
backticks/grave-accents, such that the latter is nested inside the former,
and optional insignificant whitespace is allowed between the two.

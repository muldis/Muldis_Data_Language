...
Grammar:
    ...
    <func_invo_alt_syntax> ::=
          <infix_reduce_op_invo>
        | <infix_h_dyadic_op_invo>
        | ...
...
An C<infix_reduce_op_invo> node is for using infix notation to invoke a
(homogenous) N-adic reduction operator function.  Such a function takes
exactly 1 actual argument, which is collection typed (q/set or q/array or
q/bag), and the elements of that collection are the inputs of the
operation; the inputs are all of the same type as each other and of the
result.  A single C<infix_reduce_op_invo> node is equivalent to a single
C<func_invo> node whose C<func_arg_list> element defines a single argument,
whose value is a C<QSet> or C<QArray> or C<QBag> node, which has a payload
C<expr> element for each C<expr> element of the C<infix_reduce_op_invo>,
and the C<expr> elements have the same sequence (important for C<QArray>).
A C<infix_reduce_op_invo> node requires at least 2 input value providing
child nodes (C<expr> must match at least twice); if you already have your
inputs in a single collection-valued node then use C<func_invo> to invoke
the function instead.  If C<infix_reduce_op> matches more than once in the
same C<infix_reduce_op_invo>, then all of the C<infix_reduce_op> matches
must be identical / the same operator.

This table indicates which function is invoked by each infix keyword:

    and   -> Core.Bool.and( Set:{ ... } )
    or    -> Core.Bool.or( Set:{ ... } )
    xor   -> Core.Bool.xor( Bag:{ ... } )
    [<=>] -> Core.Cat.Order.reduction( Array:{ ... } )
    I+    -> Integer.sum( Bag:{ ... } )
    I*    -> Integer.product( Bag:{ ... } )
    N+    -> Rational.sum( Bag:{ ... } )
    N*    -> Rational.product( Bag:{ ... } )
    B~    -> Blob.catenation( Array:{ ... } )
    T~    -> Text.catenation( Array:{ ... } )
    R+    -> Core.QRelation.union( QSet:{ ... } )
    R*    -> Core.QRelation.join( QSet:{ ... } )
    A~    -> QArray.catenation( QArray:{ ... } )
...
An C<infix_h_dyadic_op_invo> node is for using infix notation to invoke a
homogenous dyadic operator function.  Such a function takes exactly 2
arguments, which are the inputs of the operation; the inputs are all of the
same type as each other and of the result.  A single
C<infix_h_dyadic_op_invo> node is equivalent to a single C<func_invo> node
whose C<func_arg_list> element defines 2 arguments, and the 2 C<expr>
elements of the C<infix_h_dyadic_op_invo> supply the values of those
arguments, which are associated as one would normally expect.

This table indicates which function is invoked by each infix keyword:

    I-   -> Integer.difference( minuend => $lhs, subtrahend => $rhs )
    I|-| -> Integer.abs_difference( $lhs, $rhs )
    ...
    N|-| -> Rational.abs_difference( $lhs, $rhs )

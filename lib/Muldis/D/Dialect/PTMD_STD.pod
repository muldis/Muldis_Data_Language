...
This document is Muldis::D::Dialect::PTMD_STD version 0.136.0.
...
standard dialect is C<Muldis_D:"http://muldis.com":0.136.0:PTMD_STD>.
...
built-in to Perl 6 but that are explicitly defined in this grammar may have
...
C<ln_base_authority>; it is typically a character string like C<0.136.0>.
...
is [1|b,3,7|o,F|x].  The I<body> may also contain underscore or whitespace
characters between the delimiters, to aid formatting; these are
...
have a leading backslash.  All literal characters that are whitespace or
control characters, except for the C<SPACE> char (C<x;20>), are
...
The main reason for this stripping feature is to ensure that the actual
values being selected by string literals are not variable per the kind of
linebreaks used to format the Muldis D source code itself.  The feature is
provided to support code that contains long value literals while still
being well formatted (no extra-long lines).  If you want to have actual
non-C<SPACE> whitespace or control characters in your strings, then they
must be formatted as escape sequences such as C<\n>.

...
This document is Muldis::D::Dialect::PTMD_STD version 0.133.0.
...
standard dialect is C<Muldis_D:"http://muldis.com":0.133.0:PTMD_STD>.
...
explicitly such as with C<< <ws>? >> (optional whitespace) and C<< <ws> >>
(mandatory whitespace) and C<< <unspace> >> (nothing at all may go here
except a pair of backslashes (C<\>) surrounding an optional run of
whitespace, which would be stripped) and C<< <splitter> >> (same as C<<
<unspace> >> but that surrounded whitespace is mandatory.  4. The meanings
of C<< <ws> >> and C<< <unspace> >> are explicitly defined in this file's
grammar and do not match the C<< <ws> >> and C<< <unspace> >> that are
defined by Perl 6; this file's versions are simpler for one thing.
...
C<ln_base_authority>; it is typically a character string like C<0.133.0>.
...
Examples:
    ...
    Muldis_D:"http://muldis.com":0.133.0:PTMD_STD:{
        ...
        List:[
            List:[
                List:[4,
                    ...
                    List:[2,
                        List:[List:[1,-4,List:[]]],
                        List:[List:[1,-4,List:[67,97,114,114,111,116,115]]]
                    ]
                ],
                100
            ],
            List:[
                List:[4,
                    ...
                    List:[2,
                        List:[List:[1,-4,List:[]]],
                        List:[List:[1,-4,List:[75,105,119,105,115]]]
                    ]
                ],
                30
            ]
...
L</GENERIC VALUE EXPRESSIONS>, L</GENERIC UPDATER OR RECIPE STATEMENTS>,
L</GENERIC PROCEDURE STATEMENTS>.
...
EXPRESSIONS>, L</IMPERATIVE INVOCATION ALTERNATE SYNTAX STATEMENTS>.
...
of a function or updater or recipe or type definition, a C<value>
...
For all values of just the 11 data types [C<Singleton>, C<Bool>, C<Order>,
C<RoundMeth>, C<Int>, C<Rat>, C<Blob>, C<Text>, C<Comment>, C<Set>,
...
Alternately (recommended), use [b,o,d,x] for the common bases [2,8,10,16].
...
(C<[137Fbox]>), similarly to how an C<int_body> is qualified by a
...
is [1|b,3,7|o,F|x].  If the C<value_kind> of a C<value> node is
C<OctetBlob>
...
pair of consecutive segments is separated by a I<splitter> token, which is
a pair of backslashes (C<\>) surrounding a run of whitespace; this
segmenting ability is provided to support code that contains long value
literals while still being well formatted (no extra long lines); the
I<splitter> tokens are
ignored/stripped by the parser, and the C<Text_payload> is interpreted as
if it just consisted of the rest of the delimited string contiguously.
...
The meanings of the simple escape sequences are:
    ...
    ----+-----------+-----------------+-----+------------------------------
    \\  | x;5C      | REVERSE SOLIDUS | \   | esc seq lead (aka backslash)
    \'  | x;27      | APOSTROPHE      | '   | delim Text literals
    \"  | x;22      | QUOTATION MARK  | "   | delim quoted Name literals
    \`  | x;60      | GRAVE ACCENT    | `   | delim as-val Comment literals
    \#  | x;23      | NUMBER SIGN     | #   | delim no-val Comment literals
    \t  | x;9       | CHAR... TAB...  |     | control char horizontal tab
    \n  | x;A       | LINE FEED (LF)  |     | ctrl char line feed / newline
    \f  | x;C       | FORM FEED (FF)  |     | control char form feed
    \r  | x;D       | CARR. RET. (CR) |     | control char carriage return

A design decision of PTMD_STD that is distinct at least from Perl is
that non-"space" whitespace characters in character string literals must
never appear literally (except within a
I<splitter> token), but must instead be denoted with escape sequences.
...
Examples:
    ...
    '\c<LATIN SMALL LETTER OU>\c<x;263A>\c<65>'
...
C<quoted_name_str> equivalent if you want a segmented string.  The
definitions of alphabetic and numeric in this context are expressly limited
to the ASCII repertoire, for various reasons including simplicity and a
better degree of security; if you want to include any other Unicode
alphanumeric characters, you will have to use a C<quoted_name_str>.
...
C<sys.std.Core.Type.Array."">, C<is_identical> rather than
C<sys.std.Core.Universal.is_identical>, C<Tuple.attr> rather than
C<sys.std.Core.Tuple.attr>, C<fetch_curr_instant> rather than
C<sys.std.Temporal.Instant.fetch_curr_instant>,
...
differs from C<Text_payload> only in that the string is delimited
...
Note that any leading or trailing space (x;20) characters inside the C<#>
...
space chars in an escaped form such as with C<< \c<x;20> >>.
...
exactly the same manner as for an C<updater> node, as a C<Function> is to
an C<Updater>: C<ro_reg_param> (but that the C<Function> attribute is named
C<params> rather than C<ro_params>), C<implements_clause>,
C<empty_atomic_body>, C<with_clause>, C<named_expr>.
...
An C<updater> node specifies a new updater that lives in a depot or
subdepot.  An C<updater> node in the PTMD_STD grammar corresponds directly
to a tuple of the C<updaters> attribute of a value of the catalog data type
C<sys.std.Core.Type.Cat.Depot>, which is how an updater specification is
actually represented in Muldis D's nonsugared form, which is as a component
of the system catalog.  The C<updaters> tuple has 2 primary attributes,
C<name> and C<material>, which are valued from the C<updater> node's
C<material_declared_name> and C<updater_payload> elements, respectively.

An C<updater_payload> specifies an entire updater besides its name.  It is
interpreted as a Muldis D C<sys.std.Core.Type.Cat.Updater> value.  The
C<updater_heading> element specifies the updater's public interface, which
is these 5 attributes of the new C<Updater>: C<upd_params>, C<ro_params>,
C<opt_params>, C<dispatch_params>, C<implements>.  The C<updater_body>
element specifies the updater's implementation, which is the 2 attributes
C<exprs> and C<stmt> of the new C<Updater>.

Any of these kinds of components of an C<updater> node are interpreted in
exactly the same manner as for a C<recipe> node, as an C<Updater> is to a
C<Recipe>: C<upd_reg_param>, C<ro_reg_param>, C<implements_clause>,
C<empty_atomic_body>, C<with_clause>, C<update_stmt>, C<named_expr>,
C<expr>.  In fact, the only structural difference between an updater and a
recipe is that a recipe has global parameters and an updater doesn't.
...
Grammar:
    ...
    <recipe_payload> ::=
        <recipe_heading> <ws> <recipe_body>

    <recipe_heading> ::=
        <rcp_signature> [<ws> <implements_clause>]*
...
A C<recipe> node specifies a new recipe that lives in a depot or subdepot.
A C<recipe> node in the PTMD_STD grammar corresponds directly to a tuple of
the C<recipes> attribute of a value of the catalog data type
C<sys.std.Core.Type.Cat.Depot>, which is how a recipe specification is
actually represented in Muldis D's nonsugared form, which is as a component
of the system catalog.  The C<recipes> tuple has 2 primary attributes,
C<name> and C<material>, which are valued from the C<recipe> node's
C<material_declared_name> and C<recipe_payload> elements, respectively.

A C<recipe_payload> specifies an entire recipe besides its name.  It is
interpreted as a Muldis D C<sys.std.Core.Type.Cat.Recipe> value.  The
C<recipe_heading> element specifies the recipe's public interface, which is
these 7 attributes of the new C<Recipe>: C<upd_params>, C<ro_params>,
C<opt_params>, C<upd_global_params>, C<ro_global_params>,
C<dispatch_params>, C<implements>.  The C<recipe_body> element specifies
the recipe's implementation, which is the 2 attributes C<exprs> and C<stmt>
of the new C<Recipe>.

Iff the C<recipe_heading> has at least one C<upd_reg_param> or
C<ro_reg_param>, then the recipe has one or more regular parameters, which
are what another routine can explicitly supply arguments for in an
invocation of the recipe; each regular parameter is either
subject-to-update or read-only.  Each C<upd_reg_param> is primarily
interpreted as a tuple of the C<Recipe>'s C<upd_params> attribute, and each
C<ro_reg_param> is primarily interpreted as a tuple of the C<Recipe>'s
C<ro_params> attribute; for each tuple, the C<param_name> and C<type_name>,
respectively, of the C<upd_reg_param> or C<ro_reg_param> provide the
tuple's C<name> and C<type> attribute.  Iff any of the parameters have an
C<opt_param_flag>, then those parameters are optional to supply arguments
for; for each parameter with an C<opt_param_flag>, the C<Recipe>'s
C<opt_params> attribute has a tuple with the parameter's C<param_name>.
Iff any of the parameters have a C<dispatch_param_flag>, then the recipe is
being explicitly declared to be a virtual recipe, and so the C<recipe_body>
must be C<empty_atomic_body>; for each parameter with a
C<dispatch_param_flag>, the C<Recipe>'s C<dispatch_params> attribute has a
tuple with the parameter's C<param_name>.

Iff the C<recipe_heading> has at least one C<upd_global_param> or
C<ro_global_param>, then the recipe has one or more global parameters,
...
primarily interpreted as a tuple of the C<Recipe>'s C<upd_global_params>
attribute, and each C<ro_global_param> is primarily interpreted as a tuple
of the C<Recipe>'s C<ro_global_params> attribute; for each tuple, the
...
Iff the C<recipe_heading> has at least one C<implements_clause>, then the
recipe is explicitly declaring that it implements one or more virtual
recipe, one being named by each C<implements_clause>.  Each
C<implements_clause> is interpreted as a tuple of the C<Recipe>'s
C<implements> attribute.

Iff the C<recipe_body> is an C<empty_atomic_body>, then the C<Recipe>'s
C<exprs> and C<stmt> attributes are both empty.

Iff the C<recipe_body> has at least one C<with_clause>, then the recipe is
explicitly declaring that it has one or more inner materials, such that the
other materials are conceptually part of the implementation of the recipe;
each C<with_clause> specifies one inner material in its C<named_material>
element.  A C<with_clause> is not interpreted as any part of the C<Recipe>
but rather results in other additions to its parent C<Depot>, in a manner
similar to as if the C<named_material> were specified externally of the
C<recipe> node; but see the L</MATERIAL SPECIFICATION> main description for
details on the complete effects of specifying an inner material.

A C<nonempty_updater_or_recipe_body> must have at least one C<update_stmt>,
because a recipe must by definition update at least one of its (regular or
...
as a tuple of the C<Recipe>'s C<stmt> attribute.

Iff the C<recipe_body> directly has at least one C<named_expr>, then each
such C<named_expr> is interpreted as a tuple of an attribute of the
C<Recipe>'s C<exprs> attribute such that said tuple's C<name> is explicitly
user-defined rather than generated by the parser.  Any C<expr> contained in
a C<recipe_body> by way of one of its direct C<update_stmt> or
C<named_expr> will similarly be interpreted as a tuple of an attribute of
the C<Recipe>'s C<exprs> attribute, where said tuple's C<name> is either
user-defined or generated as appropriate for the kind of C<expr>.
...
which is these 7 attributes of the new C<Procedure>: C<upd_params>,
C<ro_params>, C<opt_params>, C<dispatch_params>, C<implements>,
C<is_system_service>, C<is_transaction>.  The C<procedure_body> element
specifies the procedure's implementation, which is the 2 attributes
C<vars> and C<stmt> of the new C<Procedure>.

The C<procedure_kind> element of a C<procedure> node is the sole
determinant of the values of the C<is_system_service> and C<is_transaction>
attributes of the resulting C<Procedure>; for each valid combination there
also exists a C<Procedure> subtype.  It also can serve to add
self-documentation to the intended purpose or use of the procedure.  The
...
Any of these kinds of components of a C<procedure> node are interpreted in
exactly the same manner as for an C<updater> node, as a C<Procedure> is to
an C<Updater>: C<upd_reg_param>, C<ro_reg_param>, C<implements_clause>,
C<empty_procedure_body> (as per C<empty_atomic_body>), C<with_clause>.

Iff the C<procedure_body> has at least one C<proc_var>, then the procedure
has one or more regular lexical variables, whose names are explicitly
user-defined rather than generated by the parser, which are the exclusive
means (besides its own subject-to-update parameters) for a procedure to
directly shuttle transient data between its statements for use as their
arguments or logic branch conditions.  Each C<proc_var> is interpreted as a
tuple of the C<Procedure>'s C<vars> attribute; for each tuple, the
...
the name of the recipe or procedure being invoked in response.  Currently,
...
beneath a C<depot>, or specifically into a function or updater or recipe or
...
sequential 1..N C<if_expr> + C<then_expr> elements specifies the C<if_then>
...
empty sequence or all of its conditionals evaluate to C<Bool:False>.  (Note
that while an C<IfElseExprNodeSet> tuple can represent zero C<if_expr> +
C<then_expr> elements, that isn't an option in this concrete grammar
because such an C<if_else_expr> is indistinguishable from the generic
C<expr> that C<else_expr> is.)
...
=head1 GENERIC UPDATER OR RECIPE STATEMENTS
...
An C<update_stmt> node is a single member statement of a multi-update
statement, which must be composed beneath a C<depot>, or specifically into
an updater or recipe definition, because in the general case a
C<update_stmt> can I<not> be completely evaluated at compile time.  It is
interpreted as a tuple of a Muldis D C<sys.std.Core.Type.Cat.MultiUpdStmt>
value.

=head2 Generic In-Multi-Update-Statement Imperative Invocation Statements
...
An C<imus_imp_invo> node represents the invocation of a named updater
or recipe, with specific subject-to-update or read-only arguments, as a
single member statement of a multi-update statement of an updater or
recipe.  It is interpreted as a tuple of a Muldis D
C<sys.std.Core.Type.Cat.MultiUpdStmt> value.  The C<routine_name> element
specifies the C<imperative> attribute of the new C<MultiUpdStmt>, which is
the name of the updater or recipe being invoked, and the
C<imus_imp_arg_list> element specifies the C<upd_args> plus C<ro_args>
attributes, one tuple thereof per C<imus_imp_arg>; each C<imus_imp_arg>
having an C<upd_sigil> yields an C<upd_args> tuple, and each
C<imus_imp_arg> without one yields a C<ro_args> tuple.

In the general case of an updater or recipe invocation, all of the
arguments are named, as per C<named_[upd|ro]_arg>, and formatting an
C<imus_imp_invo> node that way is always allowed.  In some (common) special
cases, some (which might be all) arguments may be anonymous, as per
C<anon_[upd|ro]_arg>.  For further details on this, see the C<func_invo>
node kind, under L</Generic Function Invocation Expressions>, because the
rules regarding when arguments may be anonymous or must be named are
the same for all 4 main routine kinds.
...
system-defined updater or recipe where none of the corresponding parameters
are named C<topic>.

The C<imus_imp_invo> node kind also has the same special shorthand for
named arguments, in the form of C<same_named_[upd|ro]_arg>, as the
C<func_invo> node kind does with its C<same_named_ro_arg>, but that
C<imus_imp_invo>'s version also works with subject-to-update arguments.
...
execute in this given sequence; it lies at the heart of what makes a
procedure nonatomic, because each statement of the sequence conceptually
...
=head2 Procedure Atomic Statements
...
An C<atomic_stmt> node is a special shorthand syntax that is equivalent to
the combination of a C<with_clause> node that defines an inner updater of a
procedure plus an C<iproc_imp_invo> node that is a procedure statement
invoking said updater.  It entirely takes the form of an updater body
specification, which is delimited by a pair of curly braces; conceptually
an C<atomic_stmt> is analogous to a C<compound_stmt>, the latter entirely
taking the form of a procedure body; these are distinguished from each
other primarily by the different delimiter characters they use.  The inner
updater derived from a C<atomic_stmt> has an entirely parser-generated
updater heading, which is derived from information gleaned from both the
C<atomic_stmt> node and from other parts of the procedure specification in
which the latter is embedded.  The inner updater has a regular parameter
for every distinct C<expr_name> node in the C<atomic_stmt> which matches
the unqualified name of either a parameter or a lexical variable of the
procedure, meaning that the C<expr_name> refers to an updater parameter
rather than some updater body expression node, and the updater invocation
is passing an argument to each parameter from a same-named C<var_name>.
For each updater parameter, the declared type of that parameter is
identical to the declared type of the corresponding procedure parameter or
lexical variable.  For each updater parameter, that parameter is declared
subject-to-update iff the updater uses it as an argument to a
subject-to-update parameter in one of its C<update_stmt> nodes; otherwise
that parameter is declared read-only.  No parameters are optional nor
dispatch, and the updater doesn't implement a virtual.  The unqualified
name of the updater is parser-generated; see L</MATERIAL SPECIFICATION>.
...
=head2 Procedure Value Expressions
...
A C<proc_expr> node is the general case of a Muldis D value expression
which may be composed within the body of a procedure.  A C<proc_expr> node
is a proper subset of an C<expr> node, specifically the subset of the
language grammar constructs of C<expr> that are valid for use within the
body of a procedure, which is very restricted in that regard relative to
the body of an atomic routine in which any C<expr> is valid.

A C<var_name> node has exactly the same format as an C<expr_name> node, and
it has the same interpretation as well except that it references a lexical
variable or parameter of the containing procedure, rather than an
expression node or parameter.  A C<var_name> node corresponds directly to
the native system catalog entry that would be used where it appears in
code; a routine invocation argument value in the system catalog is natively
the name of a lexical variable or parameter, as are all other places in a
procedure body that take an input value or store an output value.

A C<proc_expr> that isn't a C<var_name> is treated as a triple of distinct
lexical elements of its containing procedure.  The first lexical element is
the declaration of a lexical variable whose name is generated by the
parser; the second lexical element is a procedure statement which assigns a
statically determined value to that variable; the third lexical element is
a C<var_name> that is used where the C<proc_expr> had appeared, which names
the lexical variable so that the previously assigned value can be used
there.  The lexical variable is effectively a temporary constant since it
is assigned to only once with a value that is hard-coded and is used
exactly once, and it has no user-visible name in the original code.  Note
that a sufficiently intelligent parser can notice if multiple such
temporary variables are given the same value and optimize the code so in
fact just one variable is declared, assigned to once, read multiple times.

Iff a C<proc_expr> is an C<opaque_value_literal>, then the second lexical
element it becomes is a tuple of a Muldis D
C<sys.std.Core.Type.Cat.OVLScaValStmtNodeSet> value, and the declared type
of the variable is the most-specific system-defined type of the value
denoted by the C<opaque_value_literal>.

Iff a C<proc_expr> is an C<material_ref>, then:  Iff its C<material_name>
element is I<not> already an C<AbsPathMaterialNC>, then the second lexical
element the C<proc_expr> becomes is a tuple of a Muldis D
C<sys.std.Core.Type.Cat.APMaterialNCSelStmtNodeSet> value; otherwise, the
second lexical element is a tuple of a C<OVLScaValStmtNodeSet>; in either
case, the declared type of the variable is C<AbsPathMaterialNC>.

A C<nil_func_invo> node has exactly the same format as a C<func_invo> node
whose C<func_arg_list> is empty but that it also disallows any
insignificant whitespace.  Iff a C<proc_expr> is a C<nil_func_invo>, then
the second lexical element it becomes is a tuple of a Muldis D
C<sys.std.Core.Type.Cat.NilFuncInvoStmtNodeSet> value, and the declared
type of the variable is C<Universal>.

=head2 Generic In-Procedure Imperative Invocation Statements
...
Grammar:
    ...
    <iproc_named_upd_arg> ::=
        <upd_sigil> <invo_param_name> <ws>? '=>' <ws>? <var_name>
    ...
    <iproc_same_named_upd_arg> ::=
        <upd_sigil> '=>' <invo_param_name>

    <iproc_same_named_ro_arg> ::=
        '=>' <invo_param_name>

An C<iproc_imp_invo> node represents the invocation of a named imperative
routine, with specific subject-to-update or read-only arguments, as a
statement of a procedure.  It is interpreted as a tuple of a Muldis D
C<sys.std.Core.Type.Cat.ImpInvoStmtNodeSet> value.  The C<routine_name>
element specifies the C<imperative> attribute of the new
C<ImpInvoStmtNodeSet>, which is the name of the imperative routine being
invoked, and the C<iproc_imp_arg_list> element specifies the C<upd_args>
plus C<ro_args> attributes, one tuple thereof per C<iproc_imp_arg>; each
C<iproc_imp_arg> having an C<upd_sigil> yields an C<upd_args> tuple, and
each C<iproc_imp_arg> without one yields a C<ro_args> tuple.

For details regarding the variation of formats that an C<iproc_imp_arg> can
have, including when arguments may be anonymous or be written out in
shorthand, see the C<imus_imp_invo> node kind, under L</Generic
In-Multi-Update-Statement Imperative Invocation Statements>, because the
rules regarding these things are the same for imperatives invoked by a
procedure as for imperatives invoked by an updater or recipe.  The only
main difference between an C<iproc_imp_invo> and a C<imus_imp_invo> is that
the former may only name its host procedure's variables (or parameters) as
arguments while the latter can use arbitrary value expressions for that.

As a common special situation, iff an C<iproc_imp_invo> node specifies an
invocation of the updater C<sys.std.Core.Universal.assign> I<and> that
updater's C<v> argument is specified by a C<proc_expr> node that isn't a
C<var_name> node, then the C<iproc_imp_invo> node is I<not> actually
interpreted as a tuple of a Muldis D C<ImpInvoStmtNodeSet> value as in the
general case.  Rather, the process described in L</Procedure Value
Expressions>, where the C<proc_expr> node is treated as a triple of
distinct lexical elements, is altered such that the lexical variable it
declares just takes the name from the C<var_name> given to the updater's
C<target> argument, rather than the variable's name being generated by the
parser, and the C<iproc_imp_invo> node simply disappears without creating a
catalog statement node that reads from this variable; so ultimately the
C<iproc_imp_invo> node just creates a catalog statement that sets the
variable, as a tuple of a C<OVLScaValStmtNodeSet> or
C<APMaterialNCSelStmtNodeSet> or C<NilFuncInvoStmtNodeSet> value.  This
means that the single concrete syntax of the C<iproc_imp_invo> grammar node
is used to define tuples of 4 different C<StmtNodeSet> attributes.
...
definitions of statements that represent the invocation of named imperative
routines.  The C<try> routine is unconditionally invoked first and then iff
...
mainly the valid options are: C<iproc_imp_invo>, C<atomic_stmt>,
C<imp_invo_alt_syntax>, and C<stmt_name> or C<named_stmt> for the first 3.
...
An C<if_else_stmt> node represents an N-way if-else control flow statement.
It is interpreted as a tuple of a Muldis D
C<sys.std.Core.Type.Cat.IfElseStmtNodeSet> value.  The whole collection of
sequential 1..N C<if_proc_expr> + C<then_stmt> elements specifies the
C<if_then> attribute of the new C<IfElseStmtNodeSet>, which is a sequence
of arbitrary but C<Bool>-valued I<if> lexical variables (possibly
sugar-coated with static-value expressions), and for just the first one of
those in the sequence that at runtime is valued with C<Bool:True>, its
associated I<then> statement is executed as if it were the whole
C<if_else_stmt>.  The C<else_stmt> element, if it is provided, specifies
the C<else> attribute, which determines the statement to execute as if it
were the whole C<if_else_stmt> at runtime if either C<if_then> is an empty
sequence or all of its conditionals are valued with C<Bool:False>.  (Note
that while an C<IfElseStmtNodeSet> tuple can represent zero C<if_proc_expr>
+ C<then_stmt> elements, that isn't an option in this concrete grammar
because such an C<if_else_stmt> is indistinguishable from the generic
C<proc_stmt> that C<else_stmt> is.)
...
C<given_proc_expr> element specifies the C<given> attribute of the new
C<GivenWhenDefStmtNodeSet>, which is the control value for the statement.
The whole collection of nonordered 0..N C<when_proc_expr> + C<then_stmt>
elements specifies the C<when_then> attribute, which is a set of I<when>
comparands; if any of these I<when> values matches the value of C<given>,
...
nodes is useful for a more ad-hoc means of performing nonatomic iteration
...
however; Muldis D currently doesn't provide a nonatomic "foreach", but
typically any tasks for such can effectively be performed in atomic code
using various list-processing relational routines; if a case can be made
for nonatomic "foreach" then Muldis D may gain this feature in the future.
...
Grammar:
    ...
    <pcf_rename_map> ::=
        [<atnm_after> <ws>? '<-' <ws>? <atnm_before>] ** [<ws>? ',' <ws>?]
...
the C<expr|lhs|rhs|func|assuming|reversed> elements of the
...
order of the main op args isn't significant.  I<Details on the extra op
args are pending.>

B<TODO: Update the above paragraph because the referenced func-assuming
pair isn't actually in grammar yet, as it would be within "using_clause",
and "ordered_by_clause" isn't defined yet,  may have parts.>
...
Some of the keywords are aliases for each other:
    ...
    >=      | ≥
    in-i    | ∈i | between
    !in-i   | ∉i | not-in-i | !between | not-between
    i-has   | i∋
    i-!has  | i∌ | i-not-has
...
This table indicates which function is invoked by each keyword:
    ...
    i-!has -> Core.Interval.has_not_member( interval => lhs, value => rhs )
...
=head1 IMPERATIVE INVOCATION ALTERNATE SYNTAX STATEMENTS
...
An C<imp_invo_alt_syntax> node represents the invocation of a named
system-defined imperative routine with specific arguments.  It is
interpreted as a tuple of a Muldis D C<sys.std.Core.Type.Cat.MultiUpdStmt>
or C<sys.std.Core.Type.Cat.ImpInvoExprNodeSet> value.  An
C<imp_invo_alt_syntax> node is a lot like an C<imus_imp_invo> or
C<iproc_imp_invo> node in purpose and interpretation but it differs in
several significant ways.

While an C<imus_imp_invo> or C<iproc_imp_invo> node can be used to invoke
any imperative routine at all (except that an C<imus_imp_invo> can't invoke
a procedure), an C<imp_invo_alt_syntax> node can only invoke a fraction of
them, and only standard system-defined imperative routines.  While an
C<imus_imp_invo> or C<iproc_imp_invo> node uses a simple common format with
all imperative routines, written in prefix notation with generally named
arguments, an C<imp_invo_alt_syntax> node uses potentially unique syntax
for each imperative routine, often written in infix notation, although
inter-imperative routine format consistency is still applied as much as is
reasonably possible.

Broadly speaking, an C<imp_invo_alt_syntax> node has 2-3 kinds of payload
elements:  The first is the determinant of what imperative routine to
invoke, hereafter referred to as an I<op> or I<keyword>.  The second is an
ordered list of 1-N mandatory imperative routine inputs, hereafter referred
to as I<main op args>, whose elements typically have generic names like
C<var_name> or C<lhs_var> or C<rhs_expr>.  The (optional) third is a named
list of optional imperative routine inputs, hereafter referred to as
...
Note that, with each of the main imperative invocation alternate syntax
statement nodes documented in this main POD section (members of
C<imp_invo_alt_syntax> etc), any occurrences of child C<var_name> or
C<proc_expr> nodes should be read as being C<expr> nodes instead in
contexts where instances of the main nodes are being composed beneath
C<updater> or C<recipe> nodes, and just as C<var_name> or C<proc_expr>
nodes when composed beneath C<procedure> nodes.  This is because any
argument to an imperative invocation may be any value or pseudo-variable
expression (including a parameter) when that invocation is in an updater or
recipe, but said argument can only be a lexical variable or parameter name
when in a procedure, except that arguments to read-only parameters in a
procedure may alternately be an opaque value literal or niladic function
invocation, those being an abstraction over the assignment to and use of a
temporary lexical variable for the argument.

Note that the imperative routines with alternate syntax include updaters,
recipes, and procedures, and they are all shown in one list for simplicity.
But only alternate syntax for updaters is valid in an updater, and only
alternate syntax for updaters and recipes is valid in a recipe; all of
these alternate syntaxes are valid in a procedure.

=head2 Imperative Simple Monadic Postfix Operators
...
An C<imp_monadic_postfix_op_invo> node is for using prefix notation to
invoke a monadic operator imperative routine.  Such a imperative routine
takes exactly 1 argument, which is the input of the operation.  A single
C<imp_monadic_postfix_op_invo> node is equivalent to a single
C<imus_imp_invo> or C<iproc_imp_invo> node whose C<imus_imp_arg_list> or
C<iproc_imp_arg_list> element defines 1 argument, and the 1 C<var_name>
element of the C<imp_monadic_postfix_op_invo> supplies the value of that
...
=head2 Imperative Simple Non-symmetric Dyadic Infix Operators
...
An C<imp_nonsym_dyadic_infix_op_invo> node is for using infix notation to
invoke a non-symmetric dyadic operator imperative routine.  Such an
imperative routine takes exactly 2 arguments.  A single
C<imp_nonsym_dyadic_infix_op_invo> node is equivalent to a single
C<imus_imp_invo> or C<iproc_imp_invo> node whose C<imus_imp_arg_list> or
C<iproc_imp_arg_list> element defines 2 arguments, and the C<var_name> plus
C<proc_expr> elements of the C<imp_nonsym_dyadic_infix_op_invo> supply the
...
as Perl (about 24) or C, PTMD_STD only has about 7 precedence levels in the
interest of simplicity, and so you will likely have to use more explicit
expression delimiters to force the nesting precedence you want.  That 7
...
>> and C<< <material_ref_sel_expr> >>.  In general these are very simple
...
C<< <infix_num_op_invo> >> and every C<< <ord_compare_op_invo> >> that is
not also a C<< <compare_reduce_op_invo> >>.  These are left-associative.

=item B<Reducing Infix>
...
and C<< <compare_reduce_op_invo> >>.  These are left-associative.

...
This document is Muldis::D::Dialect::PTMD_STD version 0.123.0.
...
standard dialect is C<Muldis_D:"http://muldis.com":0.123.0:PTMD_STD>.
...
C<ln_base_authority>; it is typically a character string like C<0.123.0>.
...
Examples:
    ...
    Muldis_D:"http://muldis.com":0.123.0:PTMD_STD:{
...
Grammar:
    ...
    <function_body> ::=
        <nonempty_function_body> | <empty_routine_body>
...
C<empty_routine_body>, C<with_clause>, C<named_expr>.
...
Grammar:
    ...
    <updater_body> ::=
        <nonempty_updater_or_recipe_body> | <empty_routine_body>
...
C<empty_routine_body>, C<with_clause>, C<update_stmt>, C<named_expr>,
...
must be C<empty_routine_body>; for each parameter with a
...
Iff the C<recipe_body> is an C<empty_routine_body>, then the C<Recipe>'s
...
Grammar:
    ...
    <procedure_body> ::=
        <nonempty_procedure_body> | <empty_routine_body>
...
C<empty_routine_body>, C<with_clause>.
...
A C<nonempty_procedure_body> must have at least one C<proc_stmt>, although
that C<proc_stmt> can simply cause the procedure to exit, making the
procedure a no-op.  Each C<proc_stmt> is interpreted as a tuple of an
attribute of the the C<Procedure>'s C<stmt> attribute.  A C<proc_stmt> may
also, and typically does, also have nested C<proc_stmt>, thereby forming a
tree, and that tree is flattened with each nested C<proc_stmt> becoming its
own tuple under C<stmt> like with the first.

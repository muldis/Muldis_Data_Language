...
This document is Muldis::D::Dialect::PTMD_STD version 0.69.0.
...
C<value> is treated as if it had an extra parent C<func_invo_expr> node
...
the C<value> ends up simply as itself with no new C<func_invo_expr> parent.
...
(Actually, C<type> is a 10th option, but that will be largely ignored in
...
Grammar:
    ...
    token func_invo_expr {
        (<routine_name> <func_arg_list>)
    }

    token routine_name { <NameChain_payload> }
    ...
    token param_list_open { \s* '(' \s* }

    token param_list_close { \s* ')' \s* }

    token named_ro_arg {
        (<param_name> <pair_elem_sep> <expr>)
    }

    token param_name { C<Name_payload> }

    token anon_ro_arg { <expr> }

A C<func_invo_expr> node represents the result of invoking a named
...
named, as per C<named_ro_arg>, and formatting a C<func_invo_expr> node that
...
With just functions in all top-level namespaces except C<sys.std> and
...
Grammar:
    ...
    token then_expr { <expr> }

    token else_expr { <expr> }
...
Grammar:
    ...
    token given_expr { <expr> }

    token when_expr { <expr> }

    token then_expr { <expr> }

    token default_expr { <expr> }

An C<given_when_def_expr> node represents an N-way given-when-default
...
Grammar:
    ...
    token lib_entity_ref_selector {
          <func_ref_selector>
        | <proc_ref_selector>
        | <type_ref_selector>
        | <ord_det_func_ref_selector>
    }

    token func_ref_selector {
        (('F->') <routine_name>)
    }

    token proc_ref_selector {
        (('P->') <routine_name>)
    }

    token type_ref_selector {
        (('T->') <type_name>)
    }

    token ord_det_func_ref_selector {
        (('ODF->') <routine_name>)
    }

A C<lib_entity_ref_selector> node represents a value of one of the 4
special C<sys.std.Core.Type.Cat.[Func|Proc|Type|OrdDetFunc]Ref> types, each
corresponding to a C<[func|proc|type|ord_det_func]_ref_selector> node,
respectively.  It is interpreted as a tuple of a Muldis D

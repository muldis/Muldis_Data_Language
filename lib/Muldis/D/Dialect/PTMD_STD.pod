...
standard dialect is C<Muldis_D:"http://muldis.com":0.117.0:PTMD_STD>.
...
C<ln_base_authority>; it is typically a character string like C<0.117.0>.
...
Examples:
    ...
    if $x = ∅ or $y = ∅ then ∅
        else s ((v $x) i+ ((v $y) exp 3)

    if $val isa T->Int then $val exp 3
...
This table indicates which function is invoked by each keyword:
    ...
    div  -> Core.Integer.quotient( dividend => $lhs, divisor => $rhs )
    mod  -> Core.Integer.remainder( dividend => $lhs, divisor => $rhs )
    exp  -> Core.Integer.power( radix => $lhs, exponent => $rhs )
    r-   -> Core.Rational.diff( minuend => $lhs, subtrahend => $rhs )
    /    -> Core.Rational.quotient( dividend => $lhs, divisor => $rhs )
...
Grammar:
    ...
    <num_op_invo_with_round> ::=
        <num_op_invo> <ws> <rounded_with_rule_clause>
...
C<expr|lhs|rhs|round_rule> elements of the C<num_op_invo_with_round> supply

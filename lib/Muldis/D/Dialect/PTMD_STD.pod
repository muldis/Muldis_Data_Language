...
This document is Muldis::D::Dialect::PTMD_STD version 0.112.0.
...
standard dialect is C<Muldis_D:"http://muldis.com":0.112.0:PTMD_STD>.
...
Examples:
    ...
    Muldis_D:"http://muldis.com":0.112.0:PTMD_STD:{
...
Grammar:
    ...
    <nonsym_dyadic_infix_op__op_cr_basic> ::=
        ...
        | 'N-' | 'N/'
        | Bx | Tx | Ax
        | in-r | '!in-r' | 'not-in-r' | has-t | '!has-t' | 'not-has-t'
        | Sin | 'S!in' | 'Snot-in' | Shas | 'S!has' | 'Snot-has'
        | Bin | 'B!in' | 'Bnot-in' | Bhas | 'B!has' | 'Bnot-has'
...
This table indicates which function is invoked by each keyword:
    ...
    in-r   -> Core.Tuple.is_member( t => $lhs, r => $rhs )
    !in-r  -> Core.Tuple.is_not_member( t => $lhs, r => $rhs )
    has-t  -> Core.Relation.has_member( r => $lhs, t => $rhs )
    !has-t -> Core.Relation.has_not_member( r => $lhs, t => $rhs )
    Sin    -> Core.Set.value_is_member( value => $lhs, set => $rhs )
    S!in   -> Core.Set.value_is_not_member( value => $lhs, set => $rhs )
    Shas   -> Core.Set.has_member( set => $lhs, value => $rhs )
    S!has  -> Core.Set.has_not_member( set => $lhs, value => $rhs )
    Bin    -> Core.Bag.value_is_member( value => $lhs, bag => $rhs )
    B!in   -> Core.Bag.value_is_not_member( value => $lhs, bag => $rhs )
    Bhas   -> Core.Bag.has_member( bag => $lhs, value => $rhs )
...
Grammar:
    ...
    <interval_membership_op__op_cr_basic> ::=
          Iin | 'I!in' | 'Inot-in' | Ihas | 'I!has' | 'Inot-has'
...
Some of the keywords are aliases for each other:
    ...
    >=      | ≥
    Iin     | I∈ | between
    I!in    | I∉ | Inot-in | !between | not-between
    Ihas    | I∋
    I!has   | I∌ | Inot-has
    <=<=    | ≤≤
    <=<     | ≤<
...
This table indicates which function is invoked by each keyword:
    ...
    Iin    -> Core.Interval.value_is_member( value => $lhs,
                interval => $rhs )
    I!in   -> Core.Interval.value_is_not_member( value => $lhs,
                interval => $rhs )
    Ihas   -> Core.Interval.has_member( interval => $lhs, value => $rhs )

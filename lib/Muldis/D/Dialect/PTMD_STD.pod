...
C<[|dh_]tuple_from> or
C<[|dh_][set|maybe|single|array|bag|sp_interval]_of>, programmers may omit
the single prefix-leading C<type> chain element.  So for example, one can
just write C<array_of.Rat> rather than C<type.array_of.Rat>, or
...
See also the definition of the catalog data type
C<sys.std.Core.Type.Cat.SPIvlSelExprNodeSet>, a tuple of which is what
an C<SPInterval> node distills to when it is beneath the context of a
C<depot> node, as it describes some semantics.

...
This document is Muldis::D::Dialect::PTMD_STD version 0.105.0.
...
standard dialect is C<Muldis_D:"http://muldis.com":"0.105.0":PTMD_STD>.
...
Examples:
    ...
    Muldis_D:"http://muldis.com":"0.105.0":PTMD_STD:{
...
elements, where each element must qualify as a valid I<body> part of an
...
Grammar:
    ...
    <nu_interval> ::=
        <nu_interval_range> | <nu_interval_single>
...
C<Interval_payload>.  Each C<nu_interval> of the C<Interval_payload>
defines a 4-ary tuple of the new interval-union, which defines a
(non-union) interval of the interval-union; each C<nu_interval> defines
the 4 attributes of the tuple.

The normal case of a C<nu_interval> is a C<nu_interval_range>:  Each of
C<min> and C<max> is an C<expr> node that defines the C<min> and C<max>
attribute value, respectively, of the new (non-union) interval.  Each of
the 4 C<interval_boundary_kind> values C<..>, C<..^>, C<^..>, C<^..^>
corresponds to one of the 4 possible combinations of C<excludes_min> and
C<excludes_max> values that the new (non-union) interval can have, which in
order are: C<[false,false]>, C<[false,true]>, C<[true,false]>,
C<[true,true]>.

A C<nu_interval_single> is a degenerate case of a C<nu_interval> that
represents exactly 1 value; it is simply a shorthand for a
C<nu_interval_range> whose C<min> and C<max> are identical and whose
C<interval_boundary_kind> is C<..>.

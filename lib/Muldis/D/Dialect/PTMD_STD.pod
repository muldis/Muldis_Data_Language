...
This document is Muldis::D::Dialect::PTMD_STD version 0.100.0.
...
C<sys.std.Core.Tuple.attr>, C<min> rather than C<sys.std.Core.Universal.min>,
...
This table indicates which function is invoked by each format-keyword:
    ...
    .[] -> Core.Array.value( $expr, index => $index )
...
This table indicates which function is invoked by each keyword:
          ...
          -> Core.Rational.round( $expr, round_rule => $round_rule )
    N^    -> Core.Rational.power( radix => $lhs, exponent => $rhs,
                round_rule => $round_rule )
    log   -> Core.Rational.log( $lhs, radix => $rhs, round_rule => $round_rule )
    e^    -> Core.Rational.natural_power( $expr, round_rule => $round_rule )
    log-e -> Core.Rational.natural_log( $expr, round_rule => $round_rule )
...
This table indicates which function is invoked by each keyword:
    ...
    I∉  -> Core.Interval.value_is_not_member( value => $lhs, interval => $rhs )

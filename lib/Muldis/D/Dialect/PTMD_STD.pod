...
This document is Muldis::D::Dialect::PTMD_STD version 0.106.0.
...
standard dialect is C<Muldis_D:"http://muldis.com":"0.106.0":PTMD_STD>.
...
Examples:
    ...
    Muldis_D:"http://muldis.com":"0.106.0":PTMD_STD:{
...
which may be used only if the C<min> and C<max> are identical and the
C<interval_boundary_kind> is C<..>.  In this situation, the identical
endpoint value can be specified just once, which is the shorthand; for
example, the interval C<6..6> may alternately be written out as C<6>.  This
shorthand is to help with the possibly common situation where an interval
is a singleton.

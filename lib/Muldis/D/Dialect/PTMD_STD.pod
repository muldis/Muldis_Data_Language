...
This document is Muldis::D::Dialect::PTMD_STD version 0.129.1.
...
standard dialect is C<Muldis_D:"http://muldis.com":0.129.1:PTMD_STD>.
...
whitespace, which would be stripped).  4. The meanings
...
C<ln_base_authority>; it is typically a character string like C<0.129.1>.
...
For all values of just the 9 data types [C<Singleton>, C<Bool>, C<Order>,
C<RoundMeth>, C<Int>, C<Rat>, C<Blob>, C<Text>, C<Comment>], the
...
characters of the C<value_payload>, which for each of said 9 data types has
...
segments is separated by an I<unspace> token, which is a
pair of backslashes (C<\>) surrounding
an optional run of whitespace; this segmenting ability is provided to
support code that contains very long numeric literals while still being
well formatted (no extra long lines); the I<unspace> tokens are also
...
pair of consecutive segments is separated by an I<unspace> token, which is
a pair of backslashes (C<\>) surrounding an optional run of whitespace;
this segmenting
ability is provided to support code that contains long value literals while
still being well formatted (no extra long lines); the I<unspace> tokens are
...
never appear literally (except within an
I<unspace> token), but must instead be denoted with escape sequences.
...
C<[|dh_]tuple_from> or
C<[|dh_][set|maybe|single|array|bag|[s|m]p_interval]_of>, programmers may
    ...
    relation-type Locations {
        ...
        with tuple-type Location {
            ...
            with value-map-unary uc_loc_name (Tuple <-- $topic : Tuple) {
                Tuple:{ loc_name_uc => upper( $.loc_name ) }
            }
        }
...
having an C<upd_cosigil> yields an C<upd_args> tuple, and each
...
or absence of an C<upd_cosigil> can always be counted on to distinguish the
...
Grammar:
    ...
    <iproc_same_named_upd_arg> ::=
        <upd_cosigil> <data_sigil> '>' <invo_param_name>
...
C<iproc_imp_arg> having an C<upd_cosigil> yields an C<upd_args> tuple, and
...
sequence.  When using this infix syntax, the C<&> cosigil isn't used to
...
Similarly, a user-defined entity name in C<nonquoted_name_str> format is

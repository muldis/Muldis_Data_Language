...
This document is Muldis::D::Dialect::PTMD_STD version 0.111.0.
...
standard dialect is C<Muldis_D:"http://muldis.com":0.111.0:PTMD_STD>.
...
(mandatory whitespace).  4. The meaning of C<< <ws> >> is explicitly
defined in this file's grammar and does not match the C<< <ws> >> that is
defined by Perl 6; this file's version is simpler for one thing.
...
Grammar:
    ...
    <language_name> ::=
        <ln_base_name>
        <ws>? ':' <ln_base_authority>
        <ws>? ':' <ln_base_version_number>
        <ws>? ':' <ln_dialect>
        <ws>? ':' <ln_extensions>
...
C<ln_base_authority>; it is typically a character string like C<0.111.0>.
...
Examples:
    ...
    Muldis_D:"http://muldis.com":0.111.0:PTMD_STD:{
...
following grammar definitions are in effect:
    ...
    <is_before_op> ::=
        <is_before_op__op_cr_basic>

    <is_after_op> ::=
        <is_after_op__op_cr_basic>

    <interval_membership_op> ::=
        <interval_membership_op__op_cr_basic>
...
following grammar definitions are in effect:
    ...
    <is_before_op> ::=
        <is_before_op__op_cr_extended>

    <is_after_op> ::=
        <is_after_op__op_cr_extended>

    <interval_membership_op> ::=
        <interval_membership_op__op_cr_extended>
...
unqualified declared name of one of the C<RatRoundMeth.*>
singleton types that the C<RatRoundMeth> type is defined as a union over.
...
segments is separated by an I<unspace> token, which is a run of whitespace
delimited by backslashes (C<\>); this segmenting ability is provided to
...
Grammar:
    ...
    <Rat_payload> ::=
          <num_max_col_val> ';' <ws>? <rat_body>
        | <d_rat_body>

    <rat_body> ::=
          <int_body>'.'<pint_tail>
        | <int_body> <ws>? '/' <ws>? <pint_body>
        | <int_body> <ws>? '*' <ws>? <pint_body> <ws>? '^' <ws>? <int_body>

    <d_rat_body> ::=
          <d_int_body>'.'<d_pint_tail>
        | <d_int_body> <ws>? '/' <ws>? <d_pint_body>
        | <d_int_body> <ws>? '*' <ws>? <d_pint_body>
            <ws>? '^' <ws>? <d_int_body>
...
a run of whitespace delimited by backslashes (C<\>); this segmenting
...
that 1 is unambiguously the entity that is resolved to).  For any
...
Examples:
    ...
    Scalar:Text:nfd_codes;{ "" => Array:[80,101,114,109] }  # 'Perl' #
...
an C<expr_name> and that C<expr_name> is identical to the C<param_name>.
...
Grammar:
    ...
    <comm_infix_reduce_op__op_cr_basic> ::=
        ...
        | 'I+' | 'I*'
        | 'N+' | 'N*'
        | union | 'R+' | intersect | 'R*' | exclude | symdiff | 'R%'
...
Grammar:
    ...
    <nonsym_dyadic_infix_op__op_cr_basic> ::=
        ...
        | 'N-' | 'N/'
        | Bx | Tx | Ax
        | imem | '!imem' | 'not-imem' | hmem | '!hmem' | 'not-hmem'
        | Simem | 'S!imem' | 'Snot-imem' | Shmem | 'S!hmem' | 'Snot-hmem'
        | Bimem | 'B!imem' | 'Bnot-imem' | Bhmem | 'B!hmem' | 'Bnot-hmem'
...
This table indicates which function is invoked by each keyword:
    ...
    imem   -> Core.Tuple.is_member( t => $lhs, r => $rhs )
    !imem  -> Core.Tuple.is_not_member( t => $lhs, r => $rhs )
    hmem   -> Core.Relation.has_member( r => $lhs, t => $rhs )
    !hmem  -> Core.Relation.has_not_member( r => $lhs, t => $rhs )
    Simem  -> Core.Set.value_is_member( value => $lhs, set => $rhs )
    S!imem -> Core.Set.value_is_not_member( value => $lhs, set => $rhs )
    Shmem  -> Core.Set.has_member( set => $lhs, value => $rhs )
    S!hmem -> Core.Set.has_not_member( set => $lhs, value => $rhs )
    Bimem  -> Core.Bag.value_is_member( value => $lhs, bag => $rhs )
    B!imem -> Core.Bag.value_is_not_member( value => $lhs, bag => $rhs )
    Bhmem  -> Core.Bag.has_member( bag => $lhs, value => $rhs )
    B!hmem -> Core.Bag.has_not_member( bag => $lhs, value => $rhs )
...
This table indicates which function is invoked by each keyword:
    ...
    ++ -> Core.Integer.succ( $expr )
    -- -> Core.Integer.pred( $expr )
...
Grammar:
    ...
    <pcf_s_acc_op_invo> ::=
        <expr> <ws>? '.${' [<ws>? <possrep_name> ';']? <ws>?
            <attr_name>
        <ws>? '}'

    <pcf_t_acc_op_invo> ::=
        <expr> <ws>? '.%{' <ws>? <attr_name> <ws>? '}'
...
Examples:
    ...
    2.0 N^ 0.5 round RatRoundRule:[2,-7,ToZero]

    309.1 log 5.4 round RatRoundRule:[10,-4,HalfUp]

    e^ 6.3 round RatRoundRule:[10,-6,ToCeiling]

    17.0 log-e round RatRoundRule:[3,-5,ToFloor]
...
Grammar:
    ...
    <interval_membership_op__op_cr_basic> ::=
          Iimem | 'I!imem' | 'Inot-imem' | Ihmem | 'I!hmem' | 'Inot-hmem'
    ...
    <triadic_compare_op_invo> ::=
       [[not | '!'] <ws>]?
       <min> <ws> <is_before_op> <ws> <expr> <ws> <is_before_op> <ws> <max>
...
Some of the keywords are aliases for each other:
    ...
    >=      | ≥
    Iimem   | I∈ | between
    I!imem  | I∉ | Inot-imem | !between | not-between
    Ihmem   | I∋
    I!hmem  | I∌ | Inot-hmem
    <=<=    | ≤≤
    <=<     | ≤<
...
This table indicates which function is invoked by each keyword:
    ...
    Iimem  -> Core.Interval.value_is_member( value => $lhs,
                interval => $rhs )
    I!imem -> Core.Interval.value_is_not_member( value => $lhs,
                interval => $rhs )
    Ihmem  -> Core.Interval.has_member( interval => $lhs, value => $rhs )
    I!hmem -> Core.Interval.has_not_member( interval => $lhs,
                value => $rhs )

    <=<=  -> Core.Interval.value_is_member( value => $expr,
                interval => SPInterval:{ $min .. $max } )
    <=<   -> Core.Interval.value_is_member( value => $expr,
                interval => SPInterval:{ $min ..^ $max } )
    <<=   -> Core.Interval.value_is_member( value => $expr,
                interval => SPInterval:{ $min ^.. $max } )
...
Grammar:
    ...
    <UTCInstant> ::=
        UTC [Instant | DateTime | Date | Time] ':' <ws>?
        [<type_name> ':' <ws>?]?
        <UTCInstant_payload>
    ...
    <FloatInstant> ::=
        Float [Instant | DateTime | Date | Time] ':' <ws>?
        [<type_name> ':' <ws>?]?
        <FloatInstant_payload>
    ...
    <UTCDuration_payload> ::=
          <num_max_col_val> ';' <ws>? <utc_duration_body>
        | <d_utc_duration_body>
...
D C<sys.std.DateTime.Type.TAIInstant> value as follows:  A
...
C<sys.std.DateTime.Type.TAIDuration> value as follows:  A
...
<postfix_rat_op_invo> >> and C<< <rat_op_invo> <ws>

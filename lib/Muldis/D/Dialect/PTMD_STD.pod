...
characters, that is not composed just of the C<SPACE> char (C<x;20>), is
...
This table indicates which function is invoked by each keyword:
    ...
    in-s   -> Core.Set.value_is_member( value => lhs, coll => rhs )
    !in-s  -> Core.Set.value_is_not_member( value => lhs, coll => rhs )
    s-has  -> Core.Set.has_member( coll => lhs, value => rhs )
    s-!has -> Core.Set.has_not_member( coll => lhs, value => rhs )
    in-b   -> Core.Bag.value_is_member( value => lhs, coll => rhs )
    !in-b  -> Core.Bag.value_is_not_member( value => lhs, coll => rhs )
    b-has  -> Core.Bag.has_member( coll => lhs, value => rhs )
    b-!has -> Core.Bag.has_not_member( coll => lhs, value => rhs )
    in-i   -> Core.Interval.value_is_member( value => lhs,
                coll => rhs )
    !in-i  -> Core.Interval.value_is_not_member( value => lhs,
                coll => rhs )
    i-has  -> Core.Interval.has_member( coll => lhs, value => rhs )
    i-!has -> Core.Interval.has_not_member( coll => lhs, value => rhs )

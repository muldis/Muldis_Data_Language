...
Grammar:
    ...
    <sub_interval> ::=
        <sub_interval_range> | <sub_interval_single>
...
the C<Interval_payload>.  Each C<sub_interval> of the C<Interval_payload>
defines a 4-ary tuple of the new interval, which defines a continuous
sub-interval of the interval; each C<sub_interval> defines the 4 attributes
of the tuple.

The normal case of a C<sub_interval> is a C<sub_interval_range>:  Each of
C<min> and C<max> is an C<expr> node that defines the C<min> and C<max>
attribute value, respectively, of the new sub-interval.  Each of the 4
C<interval_boundary_kind> values C<..>, C<..^>, C<^..>, C<^..^> corresponds
to one of the 4 possible combinations of C<excludes_min> and
C<excludes_max> values that the new sub-interval can have, which in order
are: C<[false,false]>, C<[false,true]>, C<[true,false]>, C<[true,true]>.

A C<sub_interval_single> is a degenerate case of a C<sub_interval> that
represents exactly 1 value; it is simply a shorthand for a
C<sub_interval_range> whose C<min> and C<max> are identical and whose

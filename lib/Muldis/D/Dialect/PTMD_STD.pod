...
This document is Muldis::D::Dialect::PTMD_STD version 0.76.0.
...
C<$topic> for that.  Note that if C<$topic> is a q/scalar value, you still
...
This table indicates which function is invoked by each keyword:
    ...
    ∪    -> Core.QRelation.union( QSet:{ $expr[0], ..., $expr[n] } )
    ∩    -> Core.QRelation.intersection( QSet:{ $expr[0], ..., $expr[n] } )
    ∆    -> QRelation.exclusion( QBag:{ $expr[0], ..., $expr[n] } )
    ⋈    -> Core.QRelation.join( QSet:{ $expr[0], ..., $expr[n] } )
    ×    -> Core.QRelation.product( QSet:{ $expr[0], ..., $expr[n] } )
...
This table indicates which function is invoked by each keyword:
    ...
    //    -> QSet.QMaybe.attr_or_value(
                QArray:{ $expr[0], ..., $expr[n-1] }, value => $expr[n] )
    //d   -> QSet.QMaybe.attr_or_default(
                QArray:{ $expr[0], ..., $expr[n-1] }, default => $expr[n] )
...
Grammar:
    ...
    <nonsym_dyadic_infix_op> ::=
        ...
        | '⊂' | '⊄' | '⊃' | '⊅'
        | '∖' | 'R-' | minus | except
        | '⊿' | 'not matching' | antijoin | semiminus
        | '⋉' | matching | semijoin
        | '÷' | 'R/' | divideby
        | like | 'not like'
...
Currently the alternate syntaxes for 12 functions, those testing set
...
This table indicates which function is invoked by each keyword:
    ...
    Ax   -> QArray.replication( $lhs, count => $rhs )
    ∈    -> Core.QTuple.is_member( t => $lhs, r => $rhs )
    ∉    -> Core.QTuple.is_not_member( t => $lhs, r => $rhs )
    ∋    -> Core.QRelation.has_member( r => $lhs, t => $rhs )
    ∌    -> Core.QRelation.has_not_member( r => $lhs, t => $rhs )
    ⊆    -> Core.QRelation.is_subset( $lhs, $rhs )
    ⊈    -> Core.QRelation.is_not_subset( $lhs, $rhs )
    ⊇    -> Core.QRelation.is_superset( $lhs, $rhs )
    ⊉    -> Core.QRelation.is_not_superset( $lhs, $rhs )
    ⊂    -> QRelation.is_proper_subset( $lhs, $rhs )
    ⊄    -> QRelation.is_not_proper_subset( $lhs, $rhs )
    ⊃    -> QRelation.is_proper_superset( $lhs, $rhs )
    ⊅    -> QRelation.is_not_proper_superset( $lhs, $rhs )
    ∖    -> Core.QRelation.diff( source => $lhs, filter => $rhs )
    ⊿    -> Core.QRelation.semidiff( source => $lhs, filter => $rhs )
    ⋉    -> Core.QRelation.semijoin( source => $lhs, filter => $rhs )
    ÷    -> Core.QRelation.quotient( dividend => $lhs, divisor => $rhs )

    like     -> sys.std.Text.is_like( look_in => $lhs, look_for => $rhs )
    not like -> sys.std.Text.is_not_like(look_in => $lhs, look_for => $rhs)
...
it; for simplicity, the infix C<like> and C<not like> don't support that
...
This table indicates which function is invoked by each keyword:
    ...
    I|| -> Integer.abs( $expr )
    N|| -> Rational.abs( $expr )
    R#  -> Core.QRelation.cardinality( $expr )
    t   -> Core.QRelation.QTuple_from_QRelation( $expr )
    r   -> Core.QRelation.QRelation_from_QTuple( $expr )
    s   -> QSet.QMaybe.single( value => $expr )
    v   -> QSet.QMaybe.attr( $expr )

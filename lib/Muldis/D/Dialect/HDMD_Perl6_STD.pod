...
This document is Muldis::D::Dialect::HDMD_Perl6_STD version 0.147.0.
...
standard dialect is C<[ 'Muldis_D', 'http://muldis.com', '0.147.0',
'HDMD_Perl6_STD' ]>.
...
C<ln_base_authority>; it is typically a Perl C<Str> value like C<0.147.0>.
...
Examples:
    ...
    [ 'Muldis_D', 'http://muldis.com', '0.147.0', 'HDMD_Perl6_STD', {
...
is one of these 5 Perl C<Str> values: C<procedure>, C<system-service>,
...
Iff the C<procedure_heading> is a Perl C<Seq|Array>, then it has 1-2
...
Iff the C<procedure_heading> is a Perl C<Mapping|Hash>, then it is
designated C<proc_params> and there is no C<implements>.

A C<proc_params> is a Perl C<Mapping|Hash>; it must have at least one
...
C<param_details>.  Iff C<param_details> is a Perl C<Seq|Array> with at
least two elements, and its first element is not a non-empty Perl C<Str>
...
Perl C<Str> value C<multi-upd-stmt>.  The second element is a Perl
...
the Perl C<Str> value C<proc-invo>.  The second element is a
...
I<if> condition statement (an C<expr> node), and the second element /
...
Examples:
    ...
    [ 'if-else-stmt',
        [
            :d<out_of_options> => :proc-invo<nlx.lib.give_up>,
        ],

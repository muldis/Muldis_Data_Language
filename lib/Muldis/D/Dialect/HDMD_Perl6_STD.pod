...
An C<infix_reduce_op_invo> node has exactly 1 op arg, which is a Perl
C<Seq|Array|Bag|KeyBag|Set|KeySet> (but for some ops it can only be a
C<Seq|Array>) having at least 2 elements; each element is a C<expr> node

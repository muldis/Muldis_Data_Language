...
There are 14 main varieties of C<material> node, each of which is a named
node kind of its own:  C<function>, C<recipe>, C<procedure>,
...
A C<recipe> node has 3 ordered elements:  The first element is
one of these 2 Perl C<Str> values:
C<recipe>, C<updater>.  The second element is a C<Name_payload>, which
...
C<PNSQNameChain_payload>, which names the recipe to invoke.  The
last 1-2 elements provide arguments to the recipe invocation;
...
composed beneath C<recipe> nodes, and just as C<var_name> or

...
This document is Muldis::D::Dialect::HDMD_Perl6_STD version 0.129.1.
...
dialect named C<HDMD_Perl6_STD>.  The fully-qualified name of this Muldis D
standard dialect is C<[ 'Muldis_D', 'http://muldis.com', '0.129.1',
'HDMD_Perl6_STD' ]>.
...
such as the direct use of some Perl 6-only features.
...
repertoire; it is typically the Perl C<Str> value C<http://muldis.com>.

=item C<ln_base_version_number>

This is the base version number; it is a Perl C<Str> formatted as per
C<ln_base_authority>; it is typically a Perl C<Str> value like C<0.129.1>.
...
C<catalog_abstraction_level> must be one of these 4 Perl C<Str> values:
...
each of the value's elements must be one of these 0 Perl C<Str> values.
...
be one of these 2 special named Perl values: C<-Inf>, C<Inf|+Inf>.
...
be a Perl C<FatRat|Rat|Num|Int>, which is mapped directly.
...
attributes are defined by the payload.  If the payload is a Perl C<Pair>,
then its key and value, respectively, are designated I<possrep name> and
I<possrep attrs>; if the payload is not a Perl C<Pair>, then it is
...
payload.  The node payload must be a Perl C<Range>, which is mapped
...
Perl C<Range>, then the payload is treated as if it was instead the usual
Perl C<Range>, whose C<min> and C<max> are both identical to the actual
...
a Perl C<Seq|Array>.  Each element of the payload defines an element of the
...
element specifies the normal-user-data database; it is a Perl C<Pair> whose
key is the Perl C<Str> value C<depot-data> and whose value is a C<Database>
node.  The second element specifies the system catalog database; it is a
Perl C<Pair> whose key is the Perl C<Str> value C<depot-catalog> and whose
value is either a C<Database> node or a Perl C<Seq|Array> which is
hereafter referred to as C<depot_catalog_payload>.  A
C<depot_catalog_payload> either has zero elements, designating an empty
catalog, or all of its elements are Perl C<Seq|Array> (in particular, none
of its elements is the Perl C<Str> value 'Database'), each of which is one
...
is the Perl C<Str> value C<self-local-dbvar-type>.  The second element
...
There are 15 main varieties of C<material> node, each of which is a named
...
This is a Perl C<Str> value of the format C<< [<[ a..z ]>+] ** '-' >>; it
...
is one of these 9 Perl C<Str> values: C<function>, C<named-value>,
C<value-map>, C<value-map-unary>, C<value-filter>, C<value-constraint>,
C<transition-constraint>, C<value-reduction>, C<order-determination>.  The
...
Perl C<Pair> whose key and value are designated, in order,
C<function_heading> and C<function_body>.

A C<function_heading> is a Perl C<Seq|Array> with 2-3 ordered elements.
...
A C<func_params> is structurally a proper subset of an C<rcp_params>; every
valid C<rcp_params> is also a structurally valid C<func_params> except for
any C<recipe_payload> that has either a C<::=> element or a C<&> element;
in other words, a function has neither global nor subject-to-update
parameters; it just has regular read-only parameters; another exception is
that a function may have zero parameters while a recipe must have more.

A C<function_body> must be either the Perl character string C<...>, in
which case it is an C<empty_routine_body>, or it must be a Perl
C<Seq|Array>, having at least one element which is an C<expr>, and each
other element of said Perl C<Seq|Array> must be either a C<with_clause> or
a C<named_expr>.

Each of C<implements> and C<with_clause> of a C<function_payload> is
structurally identical to one of a C<recipe_payload>.
...
subdepot.  An C<updater> node has 3 ordered elements:  The first element
is the Perl C<Str> value C<updater>.  The second element is a
C<Name_payload>, which is the updater's declared name.  The third element
is an C<updater_payload>.  An C<updater_payload> is structurally a proper
subset of a C<recipe_payload>; every valid C<recipe_payload> is also a
structurally valid C<updater_payload> except for any C<recipe_payload> that
has a C<::=> element; the only structural difference between an updater and
a recipe is that a recipe has global parameters and an updater doesn't.
...
Examples:
    ...
    [ 'updater', 'make_coprime', {a=>['&','NNInt'],b=>['&','NNInt']} => [
        ...
        [ '::=', 'gcd', [ 'func-invo', 'nlx.lib.gcd',
            { a => ['$','a'], b => ['$','b'] } ] ],
        [ 'op', ':=', [ ['$','a'], [ 'op', 'div',
            [ ['$','a'], ['$','gcd'], :RoundMeth<Down> ] ] ] ],
        [ 'op', ':=', [ ['$','b'], [ 'op', 'div',
            [ ['$','b'], ['$','gcd'], :RoundMeth<Down> ] ] ] ],
    ] ]

=head2 Recipe Specification

A C<recipe> node specifies a new recipe that lives in a depot or subdepot.
A C<recipe> node has 3 ordered elements:  The first element is the Perl
C<Str> value C<recipe>.  The second element is a C<Name_payload>, which
is the recipe's declared name.  The third element is a C<recipe_payload>.
A C<recipe_payload> is a Perl C<Pair> whose key and value are
designated, in order, C<recipe_heading> and C<recipe_body>.

Iff the C<recipe_heading> is a Perl C<Seq|Array>, then it has 1-2 ordered
elements.  The first element is designated C<rcp_params> and is mandatory.
The second element is designated C<implements> and is optional.  Iff the
C<recipe_heading> is a Perl C<Mapping|Hash>, then it is designated
C<rcp_params> and there is no C<implements>.

An C<rcp_params> is a Perl C<Mapping|Hash>; it must have at least one
element, meaning the recipe has one or more parameters; each hash element
specifies one parameter, and for each hash element, the hash element's key
and value, respectively, are designated C<param_name> and C<param_details>.
Iff C<param_details> is a Perl C<Seq|Array> with at least two elements, and
its first element is not a non-empty Perl C<Str> value consisting of just
the characters <[ a..z A..Z 0..9 _ - ]>, then the C<param_details> is a
C<param_multi_meta>; otherwise, C<param_details> is a C<param_single_meta>.
...
recipe which the current recipe is declaring that it implements.

A C<recipe_body> must be either the Perl C<Str> value C<...>, in which case
it is an C<empty_routine_body>, or it must be a Perl C<Seq|Array>, having
at least one element which is an C<update_stmt>, and each other element of
said Perl C<Seq|Array> must be either a C<with_clause> or a C<named_expr>
or an C<update_stmt>.

A C<with_clause> is a Perl C<Pair> whose key is the Perl C<Str> value
...
Examples:
    ...
    [ 'recipe', 'count_heads', { count => ['&','NNInt'], search => 'Text',
        ...
        [ 'op', ':=', [ ['$','count'], [ 'op', 'r#', [
            [ 'op', 'where', [ ['$','people'],
                ['curried-func', 'nlx.lib.filt', {search=>['$','search']}]
            ] ]
        ] ] ] ],
    ] ]
...
is one of these 3 Perl C<Str> values: C<procedure>, C<system-service>,
C<transaction>.  The second element is a C<Name_payload>, which is the
procedure's declared name.  The third element is a C<procedure_payload>.  A
C<procedure_payload> is a Perl C<Pair> whose key and value are
designated, in order, C<procedure_heading> and C<procedure_body>.

A C<procedure_heading> is structurally a proper subset of a
C<recipe_heading>; every valid C<recipe_heading> is also a structurally
valid C<procedure_heading> except for any C<recipe_heading> that has a
C<::=> element; the only structural difference between a
C<procedure_heading> and a C<recipe_heading> is that a recipe has global
parameters and an procedure doesn't; another exception is that a procedure
may have zero parameters while a recipe must have more.

A C<procedure_body> must be either the Perl C<Str> value C<...>, in which
case it is an C<empty_routine_body>, or it must be a Perl C<Seq|Array>
having zero or more elements where each element must be either a
C<with_clause> or a C<proc_var> or a C<proc_stmt>; zero elements means that
the procedure is an unconditional no-op.

A C<with_clause> of a C<procedure_body> is structurally identical to one of
a C<recipe_body>.

A C<proc_var> is a 3-element Perl C<Seq|Array> whose first element is the
Perl C<Str> value C<var>, whose second element is a C<Name_payload>, and
...
Examples:
    ...
    [ 'procedure', 'print_curr_time', {} => [
        [ 'var', 'now', 'Instant' ],
        [ 'iproc-imp-invo', 'fetch_curr_instant', [ ['&',['$','now']] ] ],
        [ 'var', 'message', 'Text' ],
        :atomic-stmt[
            [ 'op', ':=', [ ['$','message'], [ 'op', '~', [
                'The current time is: ',
        ...
        [ 'iproc-imp-invo', 'write_Text_line', [ ['$','message'] ] ],
    ] ]
...
element is the Perl C<Str> value C<scalar-type>.  The second element
...
is one of these 2 Perl C<Str> values: C<tuple-type>, C<database-type>.
    ...
    [ 'relation-type', 'Locations', [
        ...
        :with[ 'tuple-type', 'Location', [
            ...
            :with[ 'value-map-unary', 'uc_loc_name',
                [ 'Tuple', { topic => 'Tuple' } ]
                => [
                    [ 'Tuple', { loc_name_uc => [ 'func-invo', 'upper',
                        [ ['$.','topic.loc_name'] ] ] } ]
                ]
            ],
        ] ],
...
element is the Perl C<Str> value C<relation-type>.  The second element
...
element is the Perl C<Str> value C<domain-type>.  The second element
...
element is the Perl C<Str> value C<subset-type>.  The second element
...
element is the Perl C<Str> value C<mixin-type>.  The second element
...
these 2 Perl C<Str> values: C<key-constraint>, C<primary-key>.  The
...
elements:  The first element is the Perl C<Str> value C<subset-constraint>.
...
The first element is the Perl C<Str> value C<stimulus-response-rule>.
...
A C<stim_resp_rule_payload> is a Perl C<Pair> whose key and value
are designated, in order, C<stimulus> and C<response>; C<stimulus> is the
Perl C<Str> value C<after-mount> (the kind of stimulus), and
...
C<Str> value C<$>.  The second element is a C<Name_payload>.
...
C<Str> value C<::=>.  The second element is a C<Name_payload> and the third
element is an C<expr> node; the second element declares an explicit
...
element is the Perl C<Str> value C<$.>.  The last element of an
...
Perl C<Str> value C<func-invo>.  The second element is a
...
either of the 2 Perl C<Str> values C<if-else-expr> and C<??!!>.  The
...
Examples:
    ...
    [ 'if-else-expr',
        [
            ['func-invo','is_empty',[['$','ary']]] => ['$','empty_result'],
        ],
        [ 'op', '.[]', [['$','ary'], 0] ]
    ]
...
is the Perl C<Str> value C<given-when-def-expr>.  The second element is an
...
is the Perl C<Str> value C<material-ref>.  The second element is a
C<PNSQNameChain_payload>, which names the routine or type to reference.

A C<curried_func> node has 2-4 ordered elements:  The first element is the
Perl C<Str> value C<curried-func>.  The second element is a
...
A C<material_ref> node, or a C<curried_func> node with zero curried
...
An C<imus_imp_invo> node has 2-4 ordered elements:  The first element is
the Perl C<Str> value C<imus-imp-invo>.  The second element is a
C<PNSQNameChain_payload>, which names the updater or recipe to invoke.  The
last 1-2 elements provide arguments to the updater or recipe invocation;
either or both or none of an C<Array_payload> element and a
...
is for a read-only parameter, then the PTAV is just an C<expr> node which
is the argument value; if the argument is for a subject-to-update
parameter, then the PTAV is a Perl C<Seq|Array> with exactly 2 elements,
where the first element is the Perl C<Str> value C<&>, and the second
...
Examples:
    ...
    [ 'imus-imp-invo', 'assign', [ ['&',['$','foo']], 3 ] ]

    # same as previous #
    [ 'imus-imp-invo', 'assign', [ 3, ['&',['$','foo']] ] ]

    # still same as previous but with all-named syntax #
    [ 'imus-imp-invo', 'assign', { target => ['&',['$','foo']], v => 3 } ]

    # three mandatory params #
    [ 'imus-imp-invo', 'nlx.lib.lookup', { addr => ['&',['$','addr']],
...
A C<stmt_name> node may alternately be formatted as a Perl Pair whose key
and value are what would otherwise be the first and second node elements,
respectively.

A C<named_stmt> node has 3 ordered elements:  The first element is the Perl
C<Str> value C<s::=>.  The second element is a C<Name_payload> and the
third element is a C<proc_stmt> node; the second element declares an
explicit statement node name for the third element.
...
Examples:
    ...
    :compound-stmt[
        [ 'var', 'message', 'Text' ],
        [ 'iproc-imp-invo', 'read_Text_line', [ ['&',['$','message']] ] ],
        [ 'iproc-imp-invo', 'write_Text_line', [ ['$','message'] ] ],
    ]

=head2 Procedure Atomic Statements

An C<atomic_stmt> node has 2 ordered elements:  The first element is the
Perl C<Str> value C<atomic-stmt>.  The second element is a Perl
C<Seq|Array>; it is interpreted as per a nonempty updater body, which has
...
A C<var_name> node has 2 ordered elements:  The first element is the Perl
C<Str> value C<$>.  The second element is a C<Name_payload>.

A C<nil_func_invo> node has 2 ordered elements:  The first element is the
Perl C<Str> value C<func-invo>.  The second element is a
C<PNSQNameChain_payload>, which names the function to invoke.

A C<var_name> or C<nil_func_invo> node may alternately be formatted as a
Perl Pair whose key and value are what would otherwise be the first and
second node elements, respectively.

=head2 Generic In-Procedure Imperative Invocation Statements

An C<iproc_imp_invo> node has 2-4 ordered elements:  The first element is
the Perl C<Str> value C<iproc-imp-invo>.  The second element is a
C<PNSQNameChain_payload>, which names the imperative routine to invoke.
The last 1-2 elements provide arguments to the imperative routine
...
C<Array_payload> element or C<Tuple_payload> element value is the
I<possibly tagged argument value> (PTAV).  For each PTAV, if the argument
is for a read-only parameter, then the PTAV is just a C<proc_expr> node
which is the argument value; if the argument is for a subject-to-update
parameter, then the PTAV is a Perl C<Seq|Array> with exactly 2 elements,
where the first element is the Perl C<Str> value C<&>, and the second
element is a C<var_name> node, which is the same node that the PTAV would
have been were this for a read-only parameter with a C<var_name> argument.
...
the Perl C<Str> value C<try-catch>.  The second element is a
...
An C<if_else_stmt> node has 2-3 ordered elements:  The first element is the
2 Perl C<Str> value C<if-else-stmt>.  The optional second element is
I<if_then>, a Perl C<Seq|Array> with 0..N elements, each of those being a
2-element Perl C<Seq|Array> or a Perl Pair; the first element / key is an
I<if> condition statement (a C<proc_expr> node), and the second element /
value is the associated I<then> result statement (a C<proc_stmt> node).
The optional 3rd/last element of an C<if_else_stmt> node is I<else>
statement, which is a C<proc_stmt> node.

Examples:

    [ 'if-else-stmt',
        [
            ['$','out_of_options'] => :iproc-imp-invo<nlx.lib.give_up>,
        ],
...
is the Perl C<Str> value C<given-when-def-stmt>.  The second element is
a C<proc_expr> node which is the I<given> common comparand.  The optional
third element is I<proc_when_then>, a Perl C<Seq|Array> with 0..N elements,
each of those being a 2-element Perl C<Seq|Array> or a Perl Pair; the first
element / key is a I<when> comparand (a C<proc_expr> node), and the second
...
Perl C<Str> value C<leave>.  The optional second element is a
...
Perl C<Str> value C<iterate>.  The optional second element is a
...
Examples:
    ...
    [ 's::=', 'lookup_person', loop => :compound-stmt[
        [ 'iproc-imp-invo', 'prompt_Text_line',
            [ ['&',['$','name']], 'Enter a name to search for: ' ] ],
        [ 'given-when-def-stmt', ['$','name'],
            [ '' => :leave<lookup_person> ] ],
        ...
        [ 'if-else-stmt', [ ['$','not_found'] => :compound-stmt[
            [ 'iproc-imp-invo', 'write_Text_line', ['No person matched'] ],
            :iterate<lookup_person>
        ] ] ],
        [ 'iproc-imp-invo', 'write_Text_line', [ ['$','report_text'] ] ],
    ] ]
...
Examples:
    ...
    [ 'op', '~', [ :Array[24, 52], :Array[-9], :Array[0, 11, 24, 7] ] ]

    [ 'op', '//', [ ['$','a'], ['$','b'], 42 ] ]

    [ 'op', '//d', [['$','a'],['$','b'],:material-ref<nlx.lib.foo_type>] ]
...
Examples:
    ...
    [ 'op', 'isa', [ ['$','bar'], :material-ref<nlx.lib.foo_type> ] ]

    [ 'op', '!isa', [ ['$','bar'], :material-ref<nlx.lib.foo_type> ] ]

    [ 'op', 'as', [ ['$','scalar'], :material-ref<Int> ] ]
...
Examples:
    ...
    [ 'op', 'd', :material-ref<nlx.lib.foo_type> ]
...
Examples:
    ...
    [ 'op', 'round', [['$','foo'], :RatRoundRule[10,-2,'HalfEven']] ]
...
C<Seq|Array>.  I<Details on the extra op args are pending.>
...
An C<imp_invo_alt_syntax> node has 3-4 ordered elements:  The first element
is the Perl C<Str> value C<op>.  The second element is a Perl C<Str> value,
hereafter referred to as I<op> or I<keyword>, which determines the
imperative routine to invoke.  The third element is (usually) a Perl
C<Seq|Array>, hereafter referred to as I<main op args>, which is an ordered
list of 1-N mandatory inputs to the imperative routine invocation.  The
(optional) fourth element is a Perl C<Mapping|Hash>, hereafter referred to
as I<extra op args>, which is a named list of optional imperative routine
...
sole (node) element iff that node is not formatted as a Perl C<Seq|Array>.

Note that, with each of the main imperative invocation alternate syntax
statement nodes documented in this main POD section, any occurrences of
child C<var_name> or C<proc_expr> nodes should be read as being C<expr>
nodes instead in contexts where instances of the main nodes are being
composed beneath C<updater> or C<recipe> nodes, and just as C<var_name> or
C<proc_expr> nodes when composed beneath C<procedure> nodes.

=head2 Imperative Simple Monadic Postfix Operators

An C<imp_monadic_postfix_op_invo> node has exactly 1 main op arg, which is
a C<var_name> node.
...
An C<imp_nonsym_dyadic_infix_op_invo> node has exactly 2 main op args; the
first and second main op args are C<lhs_var> (a C<var_name> node) and
C<rhs_expr> (a C<proc_expr> node), respectively.
...
Examples:
    ...
    [ 'op', ':=!matching', [ ['$','people'],
        :Relation( Set.new( { age => 10 }, { age => 20 } ) ) ] ]

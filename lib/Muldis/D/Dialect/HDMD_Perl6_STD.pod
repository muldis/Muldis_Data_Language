...
This document is Muldis::D::Dialect::HDMD_Perl6_STD version 0.104.0.
...
C<ln_base_authority>; it is typically a Perl C<Str> value like C<0.104.0>.
...
each of the value's elements must be one of these 1 Perl C<Str> values:
C<date_time>.
...
Examples:
    ...
    [ 'Muldis_D', 'http://muldis.com', '0.104.0', 'HDMD_Perl6_STD', {
        catalog_abstraction_level => 'rtn_inv_alt_syn'
    } ]

    [ 'Muldis_D', 'http://muldis.com', '0.104.0', 'HDMD_Perl6_STD', {
        catalog_abstraction_level  => 'rtn_inv_alt_syn',
        standard_syntax_extensions => Set.new('date_time')
    } ]
...
If the payload is an C<Seq|Array>, then
...
terms of atomic seconds.  This node is interpreted as a Muldis D
...
between two instants) which is specified in terms of atomic seconds.
...
the payload.  The payload must be either a Perl C<Set|KeySet> or a Perl
C<Range>; it is interpreted as follows:

=over

=item *

Iff the payload is a Perl C<Set|KeySet>, then each payload element is
designated a I<sub-interval>.  Each I<sub-interval> of the payload defines
a 4-ary tuple of the new interval, which defines a continuous sub-interval
of the interval; each I<sub-interval> defines the 4 attributes of the
tuple.  Each I<sub-interval> must be a Perl C<Range>, which is mapped
directly.  Each of the C<min> and C<max> attributes/method-return-values of
the Range is an C<expr> node that defines the C<min> and C<max> attribute
value, respectively, of the new interval.  Each of the C<excludes_min> and
C<excludes_max> of the Range maps directly with the interval.

=item *

Iff the payload is a Perl C<Range>, then the payload as a whole is
designated a I<sub-interval>.  The single I<sub-interval> is interpreted as
per one of those in the previous bullet point.

=back

Examples I<TODO: What is the actual syntax for the P6 Set type?>:

    :Interval( Set.new() )  # zero elements #
    ...
    :Interval( Set.new(1..3,6..6,8..9) )  # 6 elements #
...
The first element is the Perl C<Str> value
...
A C<[func|proc|type|ord-det-func]-ref> node may alternately be formatted as

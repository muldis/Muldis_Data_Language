...
This document is Muldis::D::Dialect::HDMD_Perl6_STD version 0.105.0.
...
C<ln_base_authority>; it is typically a Perl C<Str> value like C<0.105.0>.
...
Examples:
    ...
    [ 'Muldis_D', 'http://muldis.com', '0.105.0', 'HDMD_Perl6_STD', {
...
payload.  The payload must be either a Perl C<Set|KeySet> or
a Perl C<Range>; it is interpreted as follows:
...
designated a I<nu-interval>.  Each I<nu-interval> of the payload defines a
4-ary tuple of the new interval-union, which defines a (non-union) interval
of the interval-union; each I<nu-interval> defines the 4 attributes of the
tuple.  Each I<nu-interval> must be a Perl C<Range>, which is mapped
...
value, respectively, of the new (non-union) interval.  Each of the
C<excludes_min> and
C<excludes_max> of the Range maps directly with the (non-union) interval.

=item *

Iff the payload is a Perl C<Range>, then the payload as a whole is
designated a I<nu-interval>.  The single I<nu-interval> is interpreted as

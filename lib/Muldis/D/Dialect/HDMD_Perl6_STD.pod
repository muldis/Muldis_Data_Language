...
This document is Muldis::D::Dialect::HDMD_Perl6_STD version 0.122.0.
...
standard dialect is C<[ 'Muldis_D', 'http://muldis.com', '0.122.0',
'HDMD_Perl6_STD' ]>.
...
C<ln_base_authority>; it is typically a Perl C<Str> value like C<0.122.0>.
...
Examples:
    ...
    [ 'Muldis_D', 'http://muldis.com', '0.122.0', 'HDMD_Perl6_STD', {
...
C<Array_payload> element or C<Tuple_payload> element value is the
I<possibly tagged argument value> (PTAV).  For each PTAV, if the argument
is for a read-only parameter, then the PTAV is just a C<var_name> node
which is the argument value; if the argument is for a subject-to-update
parameter, then the PTAV is a Perl C<Seq|Array> with exactly 2 elements,
where the first element is the Perl C<Str> value C<&>, and the second
element is the same C<var_name> node that the PTAV would have been were
this for a read-only parameter.
...
An C<imp_nonsym_dyadic_infix_op_invo> node has exactly 2 main op args, each
of which is a C<var_name> node; the first and second main op args are
C<lhs_var> and C<rhs_var>, respectively.

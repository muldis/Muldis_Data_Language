...
This document is Muldis::D::Dialect::HDMD_Perl6_Tiny version 0.61.0.
...
>>.  All of the PHMD node kinds for Muldis D value literals are these 32:
C<Bool>, C<Int>, C<String>, C<Blob>, C<Text>, C<Rat>, C<Instant>,
...
Examples:
    ...
    :Text("\c[LATIN SMALL LETTER OU]\x[263A]\c[65]")
...
This node kind represents a single point in time which is specified in
...
Examples I<TODO: What is the actual syntax for the P6 Instant type?>:

    :Instant( Instant.new(1235556432) )

=head2 sys.std.Core.Type.Duration

This node kind represents a single amount of time (the difference between
...
Examples I<TODO: What is the actual syntax for the P6 Duration type?>:

    :Duration( Duration.new(3600) )

    :Duration( Duration.new(-50) )

    :Duration( Duration.new(3.14159) )

    :Duration( Duration.new(:2<1011101101> * :2<10> ** :2<-11011>) )

    :Duration( Duration.new(1 div 43) )
...
respectively.  The C<Scalar> PHMD node kind can only be used for scalar
types that have at least 1 possrep, and the I<possrep attrs> is interpreted
specifically as attributes of the declared type's possrep which is
specified by the I<possrep name>.  Each key+value pair of the second
payload element defines a named possrep attribute of the new scalar; the
...
attribute value.  (Note that most scalar types have at least 1 possrep; the
only ones that don't are either union types of types having possreps, or
they are core system-defined types, and those all have their own dedicated
syntax for literals.)
...
value is just a C<(UTC|Float)Instant> and not of any of the other 3
subtypes.

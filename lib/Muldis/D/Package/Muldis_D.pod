...
Composed by: Positional (Array, String,...), Tuple, Dict, etc.
...
for collections of key-value pairs where all keys are distinct;
for collections you can enumerate with no canonical order.
Currently Associative is just Accessible plus a semantic, not adding its own API.
Composed by: Setty (Set, Relation), Baggy (Bag, Tuple_Bag), Dict, etc.>

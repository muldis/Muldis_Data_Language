...
=head1 SYNOPSIS
    ...
    package
    {
        ...
        foundation "http://muldis.com":"0.200.1";
        uses MD ::= Muldis_D:"http://muldis.com":"0.200.1";
        floating {::My_App, ::MD};
...
dividing integers.  All operators defined by the C<Muldis_D> core package
...
L<Muldis::D::Package::Muldis_D::Math>; these include types for irrational
...
(non-negative) distance between that argument and zero.
...
arguments; as such, C<fractional_divided_by> is the canonical way to select
...
either is an integer exact numeric of some kind or is something that can
...
C<0>.  C<Integral> is both C<Orderable> and C<Bicessable>.  For all types
composing C<Integral>, the definition of I<predecessor> and I<successor> is
exactly equal to subtracting or adding an integer positive-one
respectively, while other C<Bicessable> don't generally mean that.  In the
general case, C<Integral> has no minimum or maximum value, but often a type
that is C<Integral> will have them.  C<Integral> is composed by C<Integer>.
...
=head2 Integer_Inf Integer Int
    ...
    ::MD::Integer_Inf ::= selection where (topic -->^ FDN__is_a_Integer_Inf)
        default (0)
...
value is a general purpose integer exact numeric having no explicit limit
to its possible size, which explicitly does not represent any kind of thing
in particular, neither cardinal nor ordinal nor nominal.  Its default value
is C<0>.  C<Integer> is both C<Orderable> and C<Bicessable>; it has no
minimum or maximum value.  Other programming languages may name their
corresponding types I<BigInt>.
...
=head2 reciprocal (Integer)
    ...
    ::MD::reciprocal::Integer ::= function
        ...
        implements 2::""
        : (if \.0 = 0 then Excuse::Div_By_Zero() else ...);
...
=head2 fractional_divided_by (Integer)
    ...
    ::MD::fractional_divided_by::Integer ::= function
        ...
        implements 2::""
        : (if \.1 = 0 then Excuse::Div_By_Zero() else ...);
...
=head2 integral_divided_by (Integer)
    ...
    : (
        ...
        e2 ::= (
            ...
            rtz_quotient  ::= (Integer_divided_by_rtz ^<-- \%(dividend,divisor));
        ...
        e4 ::= (
            round_meth ??? {
                ...
                Half_Even()    :
                  (
                    q ::= (Integer_divided_by_rtz ^<-- \%(abs(rtz_quotient),2))
...
=head2 integral_power (Integer)
    ...
    ::MD::integral_power::Integer ::= function
        ...
        implements 2::""
        : (if \.0 = 0 and \.1 = 0 then Excuse::Zero_To_The_Zero() else ...);
...
=head2 Array_Inf Array
    ...
    ::MD::Array_Inf ::= selection where (topic -->^ FDN__is_a_Array_Inf)
        default ([])
...
=head2 Bag_Inf Bag
    ...
    ::MD::Bag_Inf ::= selection where (topic -->^ FDN__is_a_Bag_Inf)
        default (\+{})
        composes Homogeneous;
...
=head2 Tuple_Inf Tuple
    ...
    ::MD::Tuple_Inf ::= selection where (topic -->^ FDN__is_a_Tuple_Inf)
        default (\%())

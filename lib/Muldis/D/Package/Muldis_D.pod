...
=head2 integral_divide_by div

    ::MD::integral_divide_by::"" ::= function virtual
        --> {Numerical, Excuse::Div_By_Zero}
        <-- (Numerical, Numerical, Round_Meth);

    ::MD::div ::= alias of integral_divide_by;

The virtual function C<integral_divide_by> aka C<div> results in the
...
C<integral_divide_by> does with all of the same arguments; to be specific,
...
argument is a nonzero number; it is an C<Excuse> otherwise.

=head2 divide_by_and_modulo

    ::MD::divide_by_and_modulo::"" ::= function
        --> ({Numerical, Excuse::Div_By_Zero}, {Numerical, Excuse::Div_By_Zero})
        <-- (Numerical, Numerical, Round_Meth)
        : (\%(topic --> \@div, topic --> \@mod));

The function C<divide_by_and_modulo> results in a binary tuple whose C<0>
and C<1> attributes have the exact same values that C<integral_divide_by>
...
=head2 asset (Integral)

    ::MD::asset::Integral ::= function --> Integral <-- (Integral)
        implements 2::""
        : (\.0);

The function C<asset::Integral> simply results in its C<0> argument.
This function implements the C<Successable> virtual function C<asset> for
the composing type C<Integral>.
...
C<Bicessable>; it has no minimum or maximum value.  Other programming

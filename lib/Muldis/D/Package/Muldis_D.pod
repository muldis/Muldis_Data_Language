...
=head2 Boolable
    ...
    ::MD::Boolable ::= type abstract requires_implements to_Boolean;
...
=head2 to_Boolean
    ...
    ::MD::"?" ::= synonym of to_Boolean;

I<TODO.>

=head1 BOOLISH DATA TYPES

=head2 Boolish

    ::MD::Boolish ::= type abstract
        composes {Enumerable and_provides_its_default,
            Boolable and_provides_its_default}
        requires_implements {to_Boolean, not};

I<TODO.>

=head2 so

    ::MD::so ::= function --> Boolish <-- (Boolish)
        : to_Boolean \\0 ?? \\0 !! not \\0;

    ::MD::proposition ::= synonym of so;

I<TODO.>

=head2 not

    ::MD::not::"" ::= function virtual --> Boolish <-- (Boolish);

    ::MD::"!"      ::= synonym of not;
    ::MD::negation ::= synonym of not;
    ::MDS::"¬"     ::= synonym of not;
...
=head2 and
    ...
    ::MD::and ::= function --> Boolish <-- (Boolish, Boolish)
        is {commutative,associative,idempotent}
        : to_Boolean \\0 ?? \\1 !! \\0;

I<TODO. Note that while this operation conceptually has an identity value,
in practice you can only exploit that automatically by directly invoking a
variant specific to the composing type.>
...
=head2 not
    ...
    ::MD::not::Boolean ::= function --> Boolean <-- (Boolean)
        implements 1::""
        : topic ?? false !! true;
...
=head2 and
    ...
    ::MD::Boolean::and ::= function --> Boolean <-- (Boolean, Boolean)
        is {commutative,associative,idempotent} identity true
        : (.0 ?? .1 !! false);

I<TODO. Variant for direct invoking when you want to use the identity value
automatically.>
...
=head2 to_Boolean
    ...
    ::MD::to_Boolean::Integral ::= function --> Boolean <-- (Integral)
        implements 1::""
        : \\0->is_zero->not;

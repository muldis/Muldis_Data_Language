...
=head2 plus (Fraction)
    ...
    ::MD::plus::Fraction ::= function --> Fraction <-- (Fraction, Fraction)
        ...
        repeater times::Integer_Fraction
    : (
        <-- (\.0->denominator) = (\.1->denominator)
...
=head2 times (Integer, Fraction)

    ::MD::times::Integer_Fraction ::= function
        --> Fraction <-- (Integer_NN, Fraction)
        implements 2::""
        : ((\.0 * (\.1->numerator)) / (\.1->denominator));

The function C<times::Integer_Fraction> implements the C<Numerical> virtual
function C<times> aka C<*> aka C<×> for the composing type C<Fraction>,
specifically for adding an integral number of the latter.

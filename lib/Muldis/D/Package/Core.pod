...
The function C<superheading_of> aka C<has_subheading> aka C<@?> results in
C<True> iff the heading of its C<0> argument is a superset of the heading
of its C<1> argument; otherwise it results in C<False>.  Note that using
this function with a C<Heading> as its C<1> argument is the idiomatic way
to test if an C<Attributive> has all of a specific set of attribute names;
whereas the C<.?> function is idiomatic for testing for exactly 1 name.

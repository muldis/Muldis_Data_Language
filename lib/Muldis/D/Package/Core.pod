...
A C<Capsule> is more constrained than it conceptually needs to be such that
its I<asset> may only be a value of the C<Tuple> type and not some other
value.  This is for the purpose of avoiding unnecessary complication for
the design and users of the language.  The idiomatic way to have an
I<asset> I<AV> that is conceptually of some non-C<Tuple> type is to have a
single-attribute I<asset> whose attribute value is I<AV>.  The idiomatic
default attribute name for a single-attribute I<asset> is C<0> when there

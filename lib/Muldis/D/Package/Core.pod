...
The function C<empty::Relational> results in the only zero-tuple
C<Relational> value.  This function implements the C<Emptyable> virtual
function C<empty> aka C<∅> for the composing type C<Relational>.
...
argument's collection type that has zero attributes.  For types like
...
a value of the function's C<0> argument's collection type that has all of
the attributes of the function's C<0> argument but that, for each attribute
...
the function's C<0> argument's collection type that has all of the
...
of the function's C<0> argument's collection type that has all of the
...
=head2 has_attrs (Relational)

    ::MD::has_attrs::Relational ::= function
...
=head2 has_just_attrs (Relational)

    ::MD::has_just_attrs::Relational ::= function
        --> Boolean <-- (Relational, Signature)
        implements 2::""
        : (...);
...
of its C<body> argument.  Note that the identity
C<r = select_Relational::( like : r, heading : @r, body : |r )> should hold
for any C<Relational> type.
...
=head2 Relation
    ...
    ::MD::Relation ::= binder
    {
        ...
        _Asset ::= selection of Tuple where
        (
            if \<0 has_just_attrs ( heading : \Heading::(), body : \Set::() ) then
                if \<<body all \Tuple::() then
                    \<<body all (function : (@\<0 = \<1)) \<-- \<<heading
                else
                    False

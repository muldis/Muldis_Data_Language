...
This document I<also> defines the Muldis D package
...
normalization, encoding to and decoding from binary values, and so on.

Muldis D is designed expressly to avoid mandatory external dependencies of
large complexity, such as Unicode, in contrast with a lot of the more
modern languages of its time.  The language is designed to seamlessly scale
to other character repertoires and encodings than 7-bit ASCII, but all such
support is canonically provided by non-C<Core> Muldis D packages, and a
...
=head2 Text::ASCII
    ...
    ::MD::Text::ASCII ::= selection of Text
        where (topic->Text_from_Maximal_Chars is_a \@String::ASCII_Chars);
...
=head2 String::ASCII_Chars
    ...
    ::MD::String::ASCII_Chars ::= selection of String
...
=head2 Homogeneous
    ...
    ::MD::Homogeneous ::= interface
        composes Emptyable
        requires_implements {so,empty,has,nest,unnest,any,where,map,reduce,...};
...
If a C<Homogeneous> value is also C<Setty>, all of its members are
guaranteed to be distinct values; otherwise, duplication of values may
occur amongst members.  If a C<Homogeneous> value is also C<Positional>,
all of its members are arranged in an explicit total order and can both be
enumerated in that order as well as be looked up by integral indexed-based
position against that order; otherwise, that may not be possible.
...
=head2 Discrete
    ...
    ::MD::Discrete ::= interface
        ...
        requires_implements {so,empty,...,count,...};
...
=head2 Positional
    ...
    ::MD::Positional ::= interface
        ...
        requires_implements {so,empty,catenate,replicate
            ,count,elem,has_index,has,nest,unnest,any,where,map,reduce,...
            ,count
            ,first_index,last_index,slice,index_succ_all_matches,...};
...
I<TODO.  True if given member value exists at any index.>
...
=head2 Set
    ...
    ::MD::Set ::= binder
    {
        "" ::= selection of Capsule
            ...
            default ({})
            composes {Homogeneous, Setty and_provides_its_default};
...
MDSL is Copyright © 2002-2015, Muldis Data Systems, Inc.

...
orderable, and idiomatically that is done by some kind of pairwise
...
in C<False>.  Other programming languages may name their corresponding
operators I<in_array>.
...
defined by its C<2> argument; otherwise it results in C<False>.  Note that
...
I<TODO.  For collections that make sure each member can only appear once.
...
=head2 Set
    ...
    ::MD::Set ::= selection where (topic -->^ FDN__is_a_Set)
        default ({})
        composes {Discrete, Setty and_provides_its_default};

The selection contract C<Set> represents the infinite Muldis
D Foundation type I<FDN__Set>.  A C<Set> value is ...
...
=head2 has_n (Set)
    ...
    ::MD::has_n::Set ::= function --> Boolean <-- (Set, Any, Integer_NN)
        implements 2::""
        : (topic -->^ FDN__Set_has_n);

The function C<has_n::Set> results in C<True> iff, either its C<0> argument
has a member equal to its C<1> argument and its C<2> argument is 1, or C<0>
lacks that member and C<2> is zero; otherwise it results in C<False>.  This
...
=head2 Relation
    ...
    ::MD::Relation ::= binder
    {
        ...
        _Asset ::= selection of Tuple where
        (
            if topic ^?= \%( heading : \@Attr_Names, body : \@Set ) then

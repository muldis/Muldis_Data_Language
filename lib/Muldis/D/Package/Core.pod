...
=head2 is_a
    ...
    : (
        if Boolean \<1 then
            \<1
        else if Function_Call_But_0 \<1 then
            (\<0,) --> \<1
        else if Signature::Set(\<1) then
            \<1 any (function : (\<1 is_a \<0)) \<-- (1 : \<0)
        else if Signature::Tuple(\<1) then
            if Tuple \<0 then
                if \<0 =@ \<1 then
                    \<0 all_attrs
...
=head2 all_attr_assets
    ...
    ::MD::all_attr_assets ::= function --> Boolean <-- (Tuple, Function_Call_But_0)
        : (\<0 all_attrs ((function : (\<1 <-- (\<<asset,))) \<-- (:<1,)));
...
=head2 any (Relational)
    ...
    ::MD::any::Relational ::= function
        --> Boolean <-- (Relational, Function_Call_But_0)
        implements 2::""
        : (|\<0 any \<1);

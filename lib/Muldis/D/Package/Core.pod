...
=head2 Accessible
    ...
    ::MD::Accessible ::= interface
        requires_implements {has_any_at
...
=head2 Attributive
    ...
    ::MD::Attributive ::= interface
        requires_implements {has_any_attrs,nullary
            ,is_unary,only_attr_name,degree,attr_names
            ,on,but,extend,rename,has_attrs,has_just_attrs};
...
=head2 only_attr_name

    ::MD::only_attr_name::"" ::= function virtual
        --> Attr_Name <-- (Attributive)
        requires (is_unary \<0);

The virtual function C<only_attr_name> results in its C<0> argument's only
attribute's name.  This function will fail if the argument doesn't have
exactly 1 attribute.
...
=head2 attr_names heading @

    ::MD::attr_names::"" ::= function virtual --> Attr_Names <-- (Attributive);

    ::MD::heading ::= alias of attr_names;
    ::MD::"@"     ::= alias of attr_names;

The virtual function C<attr_names> aka C<heading> aka C<@> results in the
...
The purpose of C<attr_names> is to canonicalize C<Attributive> values such
that their headings can be reasoned about in isolation from their bodies.

I<TODO: Consider making "heading" the canonical name of this function etc.>
...
I<TODO.  Also known as "over" in some languages.>
...
I<TODO.  Also known as "remove" in some languages.>

=head2 extend +^

    ::MD::extend::"" ::= function virtual
        --> Attributive <-- (Attributive, Attributive)
        requires (\<*args --> \attr_names_disjoint::())
        is {associative,commutative};
...
=head2 only_attr_asset

    ::MD::only_attr_asset::"" ::= function virtual --> Any <-- (Structural)
        requires (is_unary \<0);

The virtual function C<only_attr_asset> results in its C<0> argument's only
attribute's asset.  This function will fail if the argument doesn't have
exactly 1 attribute.
...
=head2 only_attr_name (Tuple)

    ::MD::only_attr_name::Tuple ::= function
        --> Attr_Name <-- (Tuple)
        implements 2::""
        requires (is_unary \<0)
        : (\<*args -->^ FDN__Tuple_only_attr_name);

The function C<only_attr_name::Tuple> results in its C<0> argument's only
attribute's name.  This function will fail if the argument doesn't have
exactly 1 attribute.  This function implements the C<Attributive> virtual
function C<only_attr_name> for the composing type C<Tuple>.
...
=head2 attr_names (Tuple)

    ::MD::attr_names::Tuple ::= function --> Attr_Names <-- (Tuple)
        implements 2::""
        : (\<*args -->^ FDN__Tuple_attr_names);

The function C<attr_names> results in the relational I<heading> of its C<0>
argument, that is its set of distinct attribute names.  This function
implements the C<Attributive> virtual function C<attr_names> aka C<heading>
aka C<@> for the composing type C<Tuple>.
...
=head2 has_any_at (Tuple)

    ::MD::has_any_at::Tuple ::= function
        --> Boolean <-- (Tuple, Attr_Name)
        implements 2::""
        : (\<*args -->^ FDN__Tuple_has_any_at);

The function C<has_any_at::Tuple> results in C<True> iff its C<0> argument
has an attribute whose attribute name is equal to its C<1> argument;
otherwise it results in C<False>.  This function implements the
C<Accessible> virtual function C<has_any_at> aka C<.?> for the composing
type C<Tuple>.
...
=head2 replace_at (Tuple)

    ::MD::replace_at::Tuple ::= function
        --> Tuple <-- (Tuple, (Attr_Name, Any))
        implements 2::""
        requires (\<0 .? \>>0)
    : (\<*args -->^ FDN__Tuple_replace_at);

The function C<replace_at::Tuple> results in the C<Tuple> value that has
all of the attributes of the function's C<0> argument but that, for the 1
attribute of the C<0> argument whose attribute name is equal to the
function's C<1> argument's C<0> attribute asset, the result's attribute
instead has an asset equal to the C<1> argument's C<1> attribute asset.
This function will fail if the C<0> argument doesn't have an attribute with
that name.  This function implements the C<Accessible> virtual function
C<replace_at> aka C<.=> for the composing type C<Tuple>.

=head2 shiftless_insert_at (Tuple)

    ::MD::shiftless_insert_at::Tuple ::= function
        --> Tuple <-- (Tuple, (Attr_Name, Any))
        implements 2::""
        requires (not \<0 .? \>>0)
        : (\<*args -->^ FDN__Tuple_shiftless_insert_at);

The function C<shiftless_insert_at::Tuple> results in the C<Tuple> value
that has all of the attributes of the function's C<0> argument plus 1
additional attribute whose name and asset are equal to the function's C<1>
argument's C<0> and C<1> attribute assets, respectively.  This function
will fail if the C<0> argument already has an attribute with that name.
This function implements the C<Accessible> virtual function
C<shiftless_insert_at> aka C<.+> for the composing type C<Tuple>.

=head2 shiftless_remove_at (Tuple)

    ::MD::shiftless_remove_at::Tuple ::= function
        --> Tuple <-- (Tuple, Attr_Name)
        implements 2::""
        requires (\<0 .? \<1)
        : (\<*args -->^ FDN__Tuple_shiftless_remove_at);

The function C<shiftless_remove_at::Tuple> results in the C<Tuple> value
that has all of the attributes of the function's C<0> argument minus 1
existing attribute whose name is equal to its C<1> argument.  This function
will fail if the C<0> argument doesn't have such an attribute.  This
function implements the C<Accessible> virtual function
C<shiftless_remove_at> aka C<.-> for the composing type C<Tuple>.
...
=head2 only_attr_asset (Tuple)

    ::MD::only_attr_asset::Tuple ::= function --> Any <-- (Tuple)
        implements 2::""
        requires (is_unary \<0)
        : (\<0 . only_attr_name::(\<0));

The function C<only_attr_asset::Tuple> results in its C<0> argument's only
attribute's asset.  This function will fail if the argument doesn't have
exactly 1 attribute.  This function implements the C<Structural> virtual
function C<only_attr_asset> for the composing type C<Tuple>.
...
=head2 Tuple_Array
    ...
    ::MD::Tuple_Array ::= binder
    {
        ...
        _Asset ::= selection of Tuple where
        (
            if \<0 ^?= ( heading : \Attr_Names::(), body : \Array::() ) then
                if \<<body all \Tuple::() then
                    \<<body all (function : (\"^?="::() \<-- \<<heading))
                else
                    False
            else
                False
...
=head2 Tuple_Bag
    ...
    ::MD::Tuple_Bag ::= binder
    {
        ...
        _Asset ::= selection of Tuple where
        (
            if \<0 ^?= ( heading : \Attr_Names::(), body : \Bag::() ) then
                if \<<body all \Tuple::() then
                    \<<body all (function : (\"^?="::() \<-- \<<heading))
                else
                    False
...
=head2 Source_Code::Package
    ...
    ::MD::Source_Code::Package ::= binder
    {
        ...
        (
            ...
            then
                \<<uses attr_names_disjoint \<<identity
            else
                False
        );
    };
...
=head2 Source_Code::Attr_Names

    ::MD::Source_Code::Attr_Names ::= selection of Tuple
...
I<TODO: Consider making "Heading" the canonical name of this type etc.>

=head2 Source_Code::Attr_Name

    ::MD::Source_Code::Attr_Name ::= selection of Attr_Names
        where (is_unary \<0);
...
=head2 Source_Code::Key_Asset_Pair
    ...
    ::MD::Source_Code::Key_Asset_Pair ::= selection of Tuple
        where (\<0 ^?= (key : \Attr_Name::(), asset : True));

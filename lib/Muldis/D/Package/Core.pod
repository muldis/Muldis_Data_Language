...
=head2 Stringy
    ...
    ::MD::Stringy ::= interface
        ...
        requires_implements {in_order,so,empty,catenate,replicate};
...
I<TODO:  Add other operators such as substr_of/superstr_of which are like
subset/superset but only match in order.>
...
C<0> and ends with the members of C<1>.  This operation has an identity
...
=head2 in_order (Bits)
    ...
    ::MD::in_order::Bits ::= function --> Boolean <-- (Bits, Bits)
        implements 2::""
        : ((\.0->Bits_to_Bits) in_order (\.1->Bits_to_Bits));
...
=head2 catenate (Bits)
    ...
    ::MD::catenate::Bits ::= function --> Bits <-- (Bits, Bits)
        ...
        : (Bits_from_Bits((\.0->Bits_to_Bits) ~ (\.1->Bits_to_Bits)));
...
=head2 replicate (Bits)
    ...
    ::MD::replicate::Bits ::= function --> Bits <-- (Bits, Integral_NN)
        implements 2::""
        : (Bits_from_Bits((\.0->Bits_to_Bits) ~# \.1));

The function C<replicate::Bits> implements the C<Stringy> virtual function
C<replicate> aka C<~#> for the composing type C<Bits>.

=head2 Bits_from_Bits

    ::MD::Bits_from_Bits ::= function --> Bits <-- (String::Bits)
        : (\Bits wrap \.0);

The function C<Bits_from_Bits> results in the C<Bits> value selected in
terms of the integer sequence of its C<0> argument.

=head2 Bits_to_Bits

    ::MD::Bits_to_Bits ::= function --> String::Bits <-- (Bits)
        : (\.0->unwrap);

The function C<Bits_to_Bits> results in an integer sequence defining the
bits of its C<Bits>-typed C<0> argument.
...
=head2 replicate (Blob)
    ...
    ::MD::replicate::Blob ::= function --> Blob <-- (Blob, Integral_NN)
        implements 2::""
...
=head2 Textual
    ...
    ::MD::Textual ::= interface
        composes Stringy
        requires_implements {in_order,so,empty,catenate,replicate,to_Text};
...
=head2 replicate (Text)
    ...
    ::MD::replicate::Text ::= function --> Text <-- (Text, Integral_NN)
        implements 2::""
...
=head2 proper_subset ⊂
    ...
    ::MD::proper_subset ::= function
        --> Boolean <-- (Homogeneous, Homogeneous)
        : (topic --> \@not_same and topic --> \@subset);

    ::MD::Unicode_Aliases::"⊂" ::= alias of proper_subset;

The function C<proper_subset> aka C<⊂> results in C<True> iff the
...
=head2 not_proper_subset ⊄

    ::MD::not_proper_subset ::= function negates proper_subset;

    ::MD::Unicode_Aliases::"⊄" ::= alias of not_proper_subset;

The function C<not_proper_subset> aka C<⊄> results in C<True> iff the
multiset of members of its C<0> argument is not a proper subset of the
multiset of members of its C<1> argument; otherwise it results in C<False>.

=head2 proper_superset ⊃

    ::MD::proper_superset ::= function commutes proper_subset;

    ::MD::Unicode_Aliases::"⊃" ::= alias of proper_superset;

The function C<proper_superset> aka C<⊃> results in C<True> iff the
multiset of members of its C<0> argument is a proper superset of the
multiset of members of its C<1> argument; otherwise it results in C<False>.

=head2 not_proper_superset ⊅

    ::MD::not_proper_superset ::= function negates proper_superset;

    ::MD::Unicode_Aliases::"⊅" ::= alias of not_proper_superset;

The function C<not_proper_superset> aka C<⊅> results in C<True> iff the
multiset of members of its C<0> argument is not a proper superset of the
multiset of members of its C<1> argument; otherwise it results in C<False>.

=head2 subset ⊆

    ::MD::subset::"" ::= function virtual
        --> Boolean <-- (Homogeneous, Homogeneous);

    ::MD::Unicode_Aliases::"⊆" ::= alias of subset;

The function C<subset> aka C<⊆> results in C<True> iff the multiset of
...
=head2 not_subset ⊈
    ...
    ::MD::not_subset ::= function negates subset;

    ::MD::Unicode_Aliases::"⊈" ::= alias of not_subset;

The function C<not_subset> aka C<⊈> results in C<True> iff the multiset
...
=head2 superset ⊇
    ...
    ::MD::superset ::= function commutes subset;

    ::MD::Unicode_Aliases::"⊇" ::= alias of superset;

The function C<superset> aka C<⊇> results in C<True> iff the multiset of
members of its C<0> argument is a superset of the multiset of members of
its C<1> argument; otherwise it results in C<False>.

=head2 not_superset ⊉

    ::MD::not_superset ::= function negates superset;

    ::MD::Unicode_Aliases::"⊉" ::= alias of not_superset;

The function C<not_superset> aka C<⊉> results in C<True> iff the multiset
of members of its C<0> argument is not a superset of the multiset of
members of its C<1> argument; otherwise it results in C<False>.
...
=head2 Positional
    ...
    ::MD::Positional ::= interface
        ...
        requires_implements {so,empty,catenate,replicate
            ,has_n,insert_n,remove_n
            ,all_unique,unique
            ,subset
            ,nest,unnest,any,where,map,reduce,...
...
simply the sum of its members.  An C<Array> in the general case may have
...
=head2 replicate (Array)
    ...
    ::MD::replicate::Array ::= function --> Array <-- (Array, Integral_NN)
        implements 2::""
...
=head2 subset (Array)
    ...
    ::MD::subset::Array ::= function --> Boolean <-- (Array, Array)
        implements 2::""
        : (topic -->^ FDN__Array_subset);

The function C<subset::Array> results in C<True> iff the multiset of
...
the C<Homogeneous> virtual function C<subset> aka C<⊆> for the composing
type C<Array>.
...
The function C<insert_n::Set> results in the value of its C<0> argument's
collection type that has all of the members of the function's C<0>
argument; but, if the C<2> argument is nonzero then the result has a member
equal to the C<1> argument, whether or not C<0> had it.  This function
implements the C<Homogeneous> virtual function C<insert_n> for the
composing type C<Set>.
...
The function C<remove_n::Set> results in the value of its C<0> argument's
collection type that has all of the members of the function's C<0>
argument; but, if the C<2> argument is nonzero and C<0> had a member equal
to the C<1> argument, the result lacks that member.  This function
implements the C<Homogeneous> virtual function C<remove_n> for the
composing type C<Set>.
...
=head2 subset (Set)
    ...
    ::MD::subset::Set ::= function --> Boolean <-- (Set, Set)
        implements 2::""
        : (subset(\.0->unwrap, \.1->unwrap));

The function C<subset::Set> results in C<True> iff the set of members of
...
C<Homogeneous> virtual function C<subset> aka C<⊆> for the composing type
C<Set>.
...
=head2 subset (Bag)
    ...
    ::MD::subset::Bag ::= function --> Boolean <-- (Bag, Bag)
        implements 2::""
        : (topic -->^ FDN__Bag_subset);

The function C<subset::Bag> results in C<True> iff the multiset of members
of its C<0> argument is a subset of the multiset of members of its C<1>

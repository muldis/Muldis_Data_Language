...
This document is Muldis::D::Core::Types version 0.143.0.
...
representing an abstract character codepoint of an infinite-size
...
numeric by codepoint integer) all correspond directly.  But C<Text> is
...
officially compatible with the Unicode standard version 6.0.0, and so all
...
standard version 6.0.0; the integer codepoint space that Unicode reserves
for itself is 0..x;10FFFF, of which it currently has about 10% allocated.
...
version 6.0.0 character abstract codepoint number.  C<Text.Unicode> values
...
name is the empty string; C<unicode_utf8_octets> represents each codepoint
as a sequence of 1..4 octets in the UTF-8 encoding; the number of octets
used varies by codepoint as follows: 1 for x;0..x;7F, 2 for x;80..x;7FF,
3 for x;800..x;FFFF, 4 for x;10000..x;10FFFF.
...
is the empty string and each of whose elements is a codepoint in the range
0..255 inclusive, and also doubles as the octet format of said codepoint in

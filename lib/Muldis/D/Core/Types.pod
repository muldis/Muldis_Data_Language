...
conceptual implementation of the types.
...
To be clear, C<DHScalarWP> is the intersection type of C<ScalarWP> and
C<DHScalar>; or, C<Scalar> is the union type of just C<Int> and
C<ScalarWP>, and C<DHScalar> is the union type of just C<Int> and
C<DHScalarWP>.
...
neither scalar nor nonscalar nor external types.  They are all
...
The C<Scalar> type is the maximal type
...
A C<Text> is a string of Unicode abstract
...
An C<SPInterval> (single-piece interval)
is a C<Tuple>.  It
...
C<Single> is a proper subtype of
...
C<DHSingle> is the intersection type of
...
C<MPInterval> (multi-piece interval) is
a proper subtype of

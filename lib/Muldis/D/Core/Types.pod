    ...
    sys.std.Core.Type.Universal
        ...
        sys.std.Core.Type.Scalar
            sys.std.Core.Type.DHScalar
                ...
                sys.std.Core.Type.Core_Text
                sys.std.Core.Type.Unicode_Text
...
types, including: C<Bool>, C<Int>, C<Rat>, C<Blob>, C<Core_Text>.  The
...
the C<Core_Text> value empty string.  The C<Stringy> type is not itself
...
the C<Core_Text> value empty string.  The C<Text> type is not itself
...
=head2 sys.std.Core.Type.Core_Text

A C<Core_Text> is a string of abstract characters which is formatted as a
sequence of abstract codepoints of a conceptually Muldis D proprietary
minimal character repertoire.  The C<Core_Text> type explicitly composes
the C<Text> mixin type, and by extension also implicitly composes the
C<Stringy> mixin type.  The purpose of C<Core_Text> is to provide a
...
The character repertoire of C<Core_Text> is presently defined to be the
same 128 7-bit ASCII characters, with codepoints in 0..127, which both
ISO8859-1/Latin1 and all versions of Unicode are proper supersets of.
The detail of this might change in the future, but the spirit should stay.
C<Core_Text> is the only explicit composing type of C<Text> that
...
A C<Core_Text> has 1 system-defined possrep named C<core_codes> which
...
C<Core_Text> type explicitly composes the C<Ordered> mixin type.  A
C<Core_Text> is a simple wrapper for a C<CCPString> and all of its other
...
number) all correspond directly.  But C<Core_Text> is explicitly disjoint
from C<CCPString> due to having a different intended interpretation.
C<Core_Text> is mainly intended for use in the system catalog and should
only be applied to user data on less capable Muldis D implementations that
don't support C<Unicode_Text> and the latter's proper subtype
C<ASCII_Text>; on more capable implementations that support C<ASCII_Text>,
you should use that by default when you would otherwise use C<Core_Text>.

=head2 sys.std.Core.Type.Unicode_Text

A C<Unicode_Text> is a string of Unicode abstract
characters which is formatted as a sequence of Unicode abstract codepoints
in canonical decomposed normal form (NFD).  The C<Unicode_Text> type
explicitly composes the C<Text> mixin type, and by extension also
implicitly composes the C<Stringy> mixin type.  Two C<Unicode_Text> will
...
default matching semantics need to be as if it did.  A C<Unicode_Text> has
...
abstract codepoint number.  The C<Unicode_Text> type explicitly composes
the C<Ordered> mixin type.  A C<Unicode_Text> is a simple wrapper for a
...
default text sorting, or make C<Unicode_Text> into a mixin type composing
a multiplicity of other nationality-specific types perhaps.>

=head2 sys.std.Core.Type.ASCII_Text

C<ASCII_Text> is a proper subtype of C<Unicode_Text> where all member
values have just characters whose codepoints are in the range 0..127.
C<ASCII_Text> has the same character repertoire as C<Core_Text> but the
former is comparable with C<Unicode_Text> while the latter isn't.
C<ASCII_Text> is what you should be using by default on all Muldis D
implementations that support it, when you would otherwise use C<Core_Text>.
...
totally-ordered type (typically one of C<Int>, C<Rat>, C<Unicode_Text>,

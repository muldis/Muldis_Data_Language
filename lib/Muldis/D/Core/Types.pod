...
This document is Muldis::D::Core::Types version 0.55.0.
...
=item C<sys.std.Core.Type.QScalar>
...
These are only called q/nonscalar data types in a loose sense, because by
themselves they are incomplete type definitions.  Actual q/nonscalar data
...
actual q/nonscalar types derived from them; such are called I<generic
(|quasi-)nonscalar operators>.  In the Muldis D type system, these
incomplete q/nonscalar types are defined as union types over all actual
types derived from them, and are proper supertypes of said.
...
document), and some other q/nonscalar types are system-defined for
convenience since they are the types of many core operators (see further
below in the current document), but all other actual q/nonscalar data types
are user-defined.  Users can also define their own incomplete q/nonscalar
data types that are q/tuple or q/relation types I<(or eventually so?)>.
...
attribute types) is equal to the product of the N-ary multiplication where
...
is also the default value of C<Tuple> and C<QTuple>.
...
value is the same as that of C<Relation> and C<QRelation>.
...
system-defined N-ary operators where the order of their argument elements
...
given I<complete> C<QSet> subtype, C<Foo>, where its C<value> attribute has
a declared type of C<Bar>, the type C<Foo> can be considered the I<power
set> of the type C<Bar>.

=head2 sys.std.Core.Type.Set

This is an enumeration data type.  C<Set> is the intersection type of
C<QSet> and C<Relation>.  Its cardinality is infinite.

=head2 sys.std.Core.Type.QMaybe

This is an enumeration data type.  C<QMaybe> is a proper subtype of C<QSet>
that may have at most one element; that is, it is a unary C<QRelation> with
a nullary key.  Operators that work specifically with C<QMaybe> subtypes
can provide a syntactic shorthand for working with sparse data; so Muldis D
has something which is conceptually close to SQL's nullable types without
actually having 3-valued logic; it would probably be convenient for code
that round-trips SQL by way of Muldis D to use the C<QMaybe> type.  Its
...
C<QMaybe> and C<Set>.  Its cardinality is infinite.
...
C<QMaybe> that has exactly 1 element.  Its default value's only q/tuple's
only attribute has the value C<Bool:false>.  The C<QSingle> type is
complementary to the C<Nothing> type under C<QMaybe>; C<Nothing> consists
of exactly 1 of C<QMaybe>'s values, and C<QSingle> consists of all of
C<QMaybe>'s other values.
...
typically be one.  Its cardinality is infinite.
...
C<QArray> and C<Relation>.  Its cardinality is infinite.
...
C<QBag> and C<Relation>.  Its cardinality is infinite.
...
way of its C<TypeRef> subtype.  The cardinality of this type is 4.

...
This document is Muldis::D::Core::Types version 0.142.0.
...
this type is infinity.  Considering the low-level type system, C<Universal>
is conceptually the union type of just the 2 types C<Int> and C<List>.
...
zero.  Considering the low-level type system, C<Empty> is
conceptually the intersection type of just the 2 types C<Int> and C<List>.
...
types, including: C<Bool>, C<Int>, C<Rat>, C<Blob>, C<CoreText>.  The
...
the C<CoreText> value empty string.  The C<Stringy> type is not itself
ordered, but often a type which is stringy is also ordered.

=head2 sys.std.Core.Type.Text

The C<Text> type is a mixin (union) type that is intended to be explicitly
composed by all other types that are considered character string types.  A
C<Text> is a string of abstract characters which is formatted as a sequence
of abstract codepoints of some character repertoire.  In the general case,
two distinct scalar root types that compose C<Text> would have at least
partially disjoint repertoires, where even when they represent the same
conceptual characters, these are represented with different codepoint
numbers.  In some cases a C<Text>-composing root type would have a proper
superset of another's repertoire; for these, they would still be disjoint
types, but directly mappable.  The C<Text> type explicitly composes the
C<Stringy> mixin type, and so every type which explicitly composes C<Text>
also implicitly composes C<Stringy>.  Most existing or likely
system-defined stringy types are also text types, including
C<CoreText>, C<Text.Unicode>.
The cardinality of C<Text> is infinity.  The default value of C<Text> is
the C<CoreText> value empty string.  The C<Text> type is not itself
ordered, but often a type which is a text type is also ordered.
...
=head2 sys.std.Core.Type.CoreText

A C<CoreText> is a string of abstract characters which is formatted as a
sequence of abstract codepoints of a conceptually Muldis D proprietary
minimal character repertoire.  The C<CoreText> type explicitly composes
the C<Text> mixin type, and by extension also implicitly composes the
C<Stringy> mixin type.  The purpose of C<CoreText> is to provide a
reasonable minimum of support for character strings in the Muldis D
language core, such that if this was the only character string support,
users could still do the same things as are supported by least common
denominator text data support in general purpose languages and DBMSs.
The character repertoire of C<CoreText> is presently defined to be the
same 128 7-bit ASCII characters, with codepoints in 0..127, which both
ISO-8859-1/Latin1 and all versions of Unicode are proper supersets of.
The detail of this might change in the future, but the spirit should stay.
C<CoreText> is the only explicit composing type of C<Text> that
is in the Muldis D language core; all others are provided by extensions.
All system-defined entity names in the core use only this repertoire.
A C<CoreText> has 1 system-defined possrep named C<core_codes> which
consists of 1 C<CCPString>-typed attribute whose name is the empty string;
each element of C<core_codes> represents a codepoint number.  The
C<CoreText> type explicitly composes the C<Ordered> mixin type.  A
C<CoreText> is a simple wrapper for a C<CCPString> and all of its other
details such as default and minimum and maximum values and cardinality and
default ordering algorithm (sorting is numeric by abstract codepoint
number) all correspond directly.  But C<CoreText> is explicitly disjoint
from C<CCPString> due to having a different intended interpretation.
C<CoreText> is mainly intended for use in the system catalog and should
only be applied to user data on less capable Muldis D implementations that
don't support C<Text.Unicode> and the latter's proper subtype
C<Text.ASCII>; on more capable implementations that support C<Text.ASCII>,
you should use that by default when you would otherwise use C<CoreText>.

=head2 sys.std.Core.Type.Text.Unicode

A C<Text.Unicode> is a string of Unicode abstract
characters which is formatted as a sequence of Unicode abstract codepoints
in canonical decomposed normal form (NFD).  The C<Text.Unicode> type
explicitly composes the C<Text> mixin type, and by extension also
implicitly composes the C<Stringy> mixin type.  Two C<Text.Unicode> will
generally match at the grapheme abstraction level.  Of course, a Muldis D
implementation doesn't actually have to store character data in NFD; but
default matching semantics need to be as if it did.  A C<Text.Unicode> has
1 system-defined possrep named C<nfd_codes> which consists of 1
C<UCPString>-typed attribute whose name is the empty string; each element
of C<nfd_codes> represents a Unicode standard version 5.2.0 character
abstract codepoint number.  The C<Text.Unicode> type explicitly composes
the C<Ordered> mixin type.  A C<Text.Unicode> is a simple wrapper for a
...
numeric by abstract codepoint number) all correspond directly.  But
C<Text.Unicode>
...
default text sorting, or make C<Text.Unicode> into a mixin type composing
a multiplicity of other nationality-specific types perhaps.>

=head2 sys.std.Core.Type.Text.ASCII

C<Text.ASCII> is a proper subtype of C<Text.Unicode> where all member
values have just the abstract characters in the 128-character repertoire of
7-bit ASCII.  C<Text.ASCII> adds 1 system-defined
possrep named C<ascii_codes> which consists of 1 C<String>-typed attribute
whose name is the empty string and each of whose elements is a codepoint in
the range 0..127 inclusive.  The C<ascii_codes> and C<nfd_codes> possreps
correspond as you might expect, such that both represent the same abstract
characters using the appropriate codepoints of their repertoires.
C<Text.ASCII> has the same character repertoire as C<CoreText> but the
former is comparable with C<Text.Unicode> while the latter isn't.
C<Text.ASCII> is what you should be using by default on all Muldis D
implementations that support it, when you would otherwise use C<CoreText>.

=head2 sys.std.Core.Type.Text.Latin1

C<Text.Latin1> is a proper subtype of C<Text.Unicode> where all member
values have just the abstract characters in the 256-character repertoire of
8-bit ISO Latin 1 / ISO-8859-1.  C<Text.Latin1> adds 1 system-defined
possrep named C<latin1_codes> which consists of 1 C<String>-typed attribute
whose name is the empty string and each of whose elements is a codepoint in
the range 0..255 inclusive.  The C<latin1_codes> and C<nfd_codes> possreps
correspond as you might expect, such that both represent the same abstract
characters using the appropriate codepoints of their repertoires.
...
totally-ordered type (typically one of C<Int>, C<Rat>, C<Text.Unicode>,

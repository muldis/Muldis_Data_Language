...
This document is Muldis::D::Core::Types version 0.129.0.
...
array operators like C<~> or C<x>.  Just a few primary system-defined types
are stringy types, including C<Blob>, C<Text>, and C<Array>.  The
cardinality of C<Stringy> is infinity.  The default value of C<Stringy> is
the C<Text> value empty string.  The C<Stringy> type is not itself ordered,
but often a type which is stringy is also ordered.
...
in canonical decomposed normal form (NFD).  The C<Text> type explicitly
composes the C<Stringy> mixin type.  Two C<Text> will generally
...
abstract codepoint number.  The C<Text> type explicitly composes the
C<Ordered> mixin type.  A C<Text> is a simple wrapper for a
...
An C<SPInterval> (single-piece interval) is a C<Tuple>.  It typically

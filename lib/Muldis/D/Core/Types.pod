...
This document is Muldis::D::Core::Types version 0.59.1.
...
C<Maybe> that has exactly zero elements; the cardinality of this type is 1
and its only value is its default.  The single C<Nothing> value, which is a
relation with zero tuples and a single attribute named C<value>, is Muldis
D's answer to the SQL NULL and is intended to be used for the same
purposes; that is, a special marker for missing or inapplicable
information, that does not typically equal any normal/scalar value;
however, in Muldis D, C<Nothing> I<is a value>, and it I<is> equal to
itself.

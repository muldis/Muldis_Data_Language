    ...
    sys.std.Core.Type.Universal
        ...
        sys.std.Core.Type.Scalar
            sys.std.Core.Type.DHScalar
                ...
                sys.std.Core.Type.Minimal_Text
...
types, including: C<Bool>, C<Int>, C<Rat>, C<Blob>, C<Minimal_Text>.  The
...
the C<Minimal_Text> value empty string.  The C<Stringy> type is not itself
...
the C<Minimal_Text> value empty string.  The C<Text> type is not itself
...
=head2 sys.std.Core.Type.Minimal_Text

A C<Minimal_Text> is a string of abstract characters which is formatted as
a sequence of abstract codepoints of a conceptually Muldis D proprietary
I<minimal> character repertoire, which consists of exactly 64 characters.

This table says what those 64 characters are and what codepoint numbers
they have (the codepoints are expressed in base-10):

     0..25 | 'A'..'Z' | LATIN CAPITAL LETTER A .. LATIN CAPITAL LETTER Z
    26..51 | 'a'..'z' | LATIN SMALL LETTER A .. LATIN SMALL LETTER Z
    52..61 | '0'..'9' | DIGIT ZERO .. DIGIT NINE
        62 | '-'      | HYPHEN-MINUS
        63 | '_'      | LOW LINE (underscore)

The purpose of C<Minimal_Text> is to provide the absolute minimum of
support for character strings such that one can write the least common
denominator of identifier strings with them such as the names of types or
routines or attributes or variables, these usually being comprised of Latin
alphabetic and numeric characters plus underscore and hyphen for word
separation.  C<Minimal_Text> could also be used for some user data, such as
some basic alphanumeric codes, but use by the system catalog is its primary
intended purpose.  The C<Minimal_Text> type explicitly composes the C<Text>
mixin type, and by extension also implicitly composes the C<Stringy> mixin
type.  C<Minimal_Text> is the only explicit composing type of C<Text> that
...
The C<Minimal_Text> repertoire and codepoints also happen to exactly
correspond to a Base64 flavor called "base64url encoding" or "Modified
Base64 for URL applications".  I<TODO: Cite some official reference.>

A C<Minimal_Text> has 1 system-defined possrep named C<mini_codes> which
consists of 1 C<MCPString>-typed attribute whose name is the empty string;
each element of C<mini_codes> represents a codepoint number.  The
C<Minimal_Text> type explicitly composes the C<Ordered> mixin type.  A
C<Minimal_Text> is a simple wrapper for a C<MCPString> and all of its other
...
number) all correspond directly.  But C<Minimal_Text> is explicitly
disjoint from C<MCPString> due to having a different intended
interpretation.

...
This document is Muldis::D::Core::Types version 0.104.0.
...
This is an enumeration data type.  C<Interval> is a proper subtype of
C<Relation> that has 4 attributes, and their names are C<min|max>
(C<Universal>) and C<excludes_[min|max]> (C<Bool>).  An C<Interval> defines
a single possibly-discontinuous-or-empty interval in terms of a set of 0..N
continuous sub-intervals, such that each of its tuples defines a single
sub-interval and an C<Interval> with zero tuples defines an empty interval;
each sub-interval is a I<bounded interval> or I<finite interval>, defined
in terms of 2 I<endpoint> values plus an indicator of whether either, both,
or none of the endpoint values are included in the interval.
I<COMING SOON: Adding support for unbounded/infinite intervals.>

Details on an C<Interval>'s 4 attributes:

=over

=item C<min|max> - C<Universal>

These are the sub-interval endpoint values; C<min> defines the
...
C<max> I<is not> considered to be included within the sub-interval,
respectively; otherwise, it I<is> considered to be included within the
sub-interval.  If both endpoints are within the sub-interval (the use case
which Muldis D optimizes its syntax for), the sub-interval is I<closed>;
otherwise if both endpoints are not in the sub-interval, the sub-interval
is I<open>.

=back

The C<Interval> type, at least as a matter of simplicity, doesn't place any
restrictions on the combination of attribute values a sub-interval
...
also determines whether or not a given sub-interval is considered
empty (which is degenerate) or not.
With respect to each compatible C<order-determination> function, a
sub-interval is considered empty iff at least one of the following is true:
...
The default value of C<Interval> represents the sole I<canonical> empty
interval, which is the only C<Interval> with zero tuples.

The C<Interval> type has (yet) no support for I<unbounded>/I<infinite> or
...
This is an enumeration data type.  C<DHInterval> is the intersection type
of C<Interval> and C<DHRelation>.  The cardinality of this type is
infinite.

...
This document is Muldis::D::Core::Types version 0.60.0.
        ...
        sys.std.Core.Type.QScalar
            sys.std.Core.Type.Scalar
                ...
                sys.std.Core.Type.String
                    sys.std.Core.Type.String.*
                    sys.std.Core.Type.SEString
                    sys.std.Core.Type.BString
                    sys.std.Core.Type.OString
...
L<Muldis::D::Dialect::HDMD_Perl6_Tiny> or
L<Muldis::D::Dialect::HDMD_Perl5_Tiny> for a Perl 6|5-based example.  These
...
the infinite number of types having C<sys.std.Core.Type.String.*>-format
names (see also their collective entry in the type list).  A C<String> is a
string of integers, or more specifically it is a dense sequence of 0..N
elements (I<not> defined over C<Relation>) where each element is an
integer.  Its default and minimum value is the empty sequence; its maximum
value is an infinite-length sequence and practically impossible.  The
...
subtype's values can have, and you must specify the 2 integer end-points of
the inclusive range that all its values' integer elements are in.  The
C<String> type has a default ordering algorithm; for 2 distinct C<String>
values, their order is determined as follows:  First eliminate any
identical leading element sequences from both strings as those alone would
make the strings compare as same (if the remainder of both strings was the
empty string, then the strings are identical).  Then, iff the remainder of
just one string is the empty string, then that string is ordered before the
non-empty one; otherwise, compare the first element of each of the string
remainders according to the following ordering algorithm to get the order
of their respective strings: the integer value closer to negative infinity
is ordered before the value closer to positive infinity.

=head2 sys.std.Core.Type.String.*

There are an infinite number of types having
C<sys.std.Core.Type.String.*>-format names and each one is a structure data
type; for the rest of this description, the type name C<String.Value> will
be used as a proxy for each and every one of them.  A C<String.Value> is a
string of integers, or more specifically it is a dense sequence of 0..N
elements (I<not> defined over C<Relation>) where each element is an
...
=head2 sys.std.Core.Type.SEString
...
=head2 sys.std.Core.Type.OString
...
data types they are composed of.  Almost all of these types are
quasi-nonscalar types; only the C<Nothing> type and the 2 C<D0> types are
non-quasi nonscalar types.
...
This is an enumeration data type.  C<TupleD0> is a proper subtype of
C<Tuple> that has zero attributes; the cardinality of this type is 1, and
its only value is also the default value of C<Tuple> and C<QTuple>.
...
This is an enumeration data type.  C<RelationD0> is a proper subtype of
C<Relation> that has 0 attributes; the cardinality of this type is 2, and
its default value is the same as that of C<Relation> and C<QRelation>.
...
This is an enumeration data type.  C<Nothing> is a proper subtype of
C<Maybe> where its only member value has exactly zero elements; the
cardinality of this type is 1 and its only value is its default.  The
...
C<QSingle> type is complementary to the C<Nothing> type under C<QMaybe>;
C<Nothing> consists of exactly 1 of C<QMaybe>'s values, and C<QSingle>
consists of all of C<QMaybe>'s other values.

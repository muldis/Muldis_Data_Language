...
This is an enumeration data type.  C<Bag> (or I<multiset>) is a proper
...
This is an enumeration data type.  The C<QScalar> type is the maximal
...
C<QScalarDVPT:default>.  The cardinality of this type is infinity.

=item C<sys.std.Core.Type.QScalarDVPT>

This is a structure data type.  C<QScalarDVPT> (quasi-scalar default
value providing type) consists of just the 1 value
C<QScalarDVPT:default>, which is also its default value.  The only
reason it exists is to provide a default value for the QScalar
enumeration type.  A C<QScalarDVPT> has 1 system-defined possrep whose
...
similarly-named (differing only by the C<Q>) nonscalar data types, and
...
particular that there is no "QDatabase" type, since all normal data or
...
This is a primitive data type.  A C<QTuple> is like a C<Tuple> but that
the declared types of its attributes can be anything at all.  Its
cardinality is infinite.

=head2 sys.std.Core.Type.QTupleD0

This is an enumeration data type.  C<QTupleD0> is a proper subtype of
C<QTuple> in the corresponding manner to C<TupleD0> being a proper
subtype of C<Tuple>.  Its cardinality is 1.

=head2 sys.std.Core.Type.QRelation

This is a primitive data type.  A C<QRelation> is like a C<Relation>
but that the declared types of its attributes can be anything at all.  Its
cardinality is infinite.

=head2 sys.std.Core.Type.QRelationD0

This is an enumeration data type.  C<QRelationD0> is a proper subtype
of C<QRelation> in the corresponding manner to C<RelationD0> being a
...
C<QRelation> in the corresponding manner to C<Set> being a proper
subtype of C<Relation>.  Its cardinality is infinite.

=head2 sys.std.Core.Type.QMaybe

This is an enumeration data type.  C<QMaybe> is a proper subtype of
C<QSet> in the corresponding manner to C<Maybe> being a proper subtype
of C<Set>.  Its cardinality is infinite.

=head2 sys.std.Core.Type.QNothing

This is an enumeration data type.  C<QNothing> is a proper subtype of
C<QMaybe> in the corresponding manner to C<Nothing> being a proper
...
C<QMaybe> in the corresponding manner to C<Single> being a proper
...
C<QRelation> in the corresponding manner to C<Array> being a proper
subtype of C<Relation>.  Its cardinality is infinite.

=head2 sys.std.Core.Type.QBag

This is an enumeration data type.  C<QBag> is a proper subtype of
C<QRelation> in the corresponding manner to C<Bag> being a proper
subtype of C<Relation>.  Its cardinality is infinite.

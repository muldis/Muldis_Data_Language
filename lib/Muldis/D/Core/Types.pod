...
This document is Muldis::D::Core::Types version 0.21.0.
...
L<Muldis::D::Dialect::PTMD_Tiny>.  Hosted Data Muldis D as hosted
...
L<Muldis::D::Dialect::HDMD_Perl_Tiny> for a Perl-based example.
...
=head2 sys.Core.Type.Name

A C<Name> (scalar) is a canonical short name for any kind of DBMS entity
(or named component) when declaring it; this short name is sufficient to
identify the entity within its immediate namespace.  Similarly, a DBMS
entity can often be invoked or referred to using just its C<Name>,
depending on the context; other times, a C<Cat.NameChain> must be used
instead to also qualify the reference with a namespace.  A C<Name> is
represented as a string of characters.  Its default and minimum value is
the empty string; its maximum value is an infinite-length string and
practically impossible.  Note that there is only one system-defined
character repertoire for character string types, which is the newest
Unicode repertoire (5.0.0).  The cardinality of this type is infinity; to
define a most-generalized finite character string subtype, you must specify
a maximum length in characters (that is, eg, in NFC graphemes) that the
subtype's strings are.  See also the C<sys.Text.Type.Text> type, which is
exactly the same as C<Name> in representation and cardinality though its
values are distinct; it is intended for general-purpose user data while
C<Name> is just intended for naming DBMS entities.

=head2 sys.Core.Type.Comment

A C<Comment> (scalar) is the text of a Muldis D code comment, which
programmers can populate as an attribute of several catalog data types,
such as whole routines or statements or expression nodes.  C<Comment> is
exactly the same as C<Name> in representation and cardinality though its
values are distinct; it is intended just for commenting Muldis D code.  One
main intended use of this type is to help preserve comments in code

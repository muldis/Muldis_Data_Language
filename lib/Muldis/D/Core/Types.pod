...
This document is Muldis::D::Core::Types version 0.109.0.
...
This is an enumeration data type.  The C<Bool> type is explicitly defined
...
and each one is a structure data type; for the rest of this description,
...
This is an enumeration data type.  The C<Int> type is a union type over
the infinite number of types having
C<sys.std.Core.Type.Int.*>-format names (see also their collective
entry in the type list).  An C<Int> is a single exact integral number of
any magnitude.  Its default value is zero; its minimum and maximum values
...
C<sys.std.Core.Type.Int.*>-format names and each one is a structure data
type; for the rest of this description, the type name C<Int.Value> will be
used as a proxy for each and every one of them.  An C<Int.Value> is a
single exact integral number of any magnitude.  An C<Int.Value> has 1
system-defined possrep whose name is the empty string and which has zero
attributes.  The cardinality of this type is 1, and its only value is its
default and minimum and maximum value.

The sole value of C<Int.Value> is the same as the decoded form of its
unqualified name; its unqualified name is a C<Name> value formatted simply
as a base-10 integer literal, and its value is the integer that this
literal denotes; for example, the C<Name> value C<42> defines the C<Int>
value C<42>.  A valid base-10 integer literal in this context matches the
(Muldis D PTMD_STD or Perl 6 grammar) pattern C<0|\-?[1..9][0..9]*>.

Now, one main rationale to have the system-defined C<Int> type as a
union type over an infinite number of C<Int.*> types, is to avoid any
...
fact that an implicitly defined infinity of types underlie C<Int>
...
may be very non-specific; see also the L<Muldis D DateTime
Extension|Muldis::D::Ext::DateTime> for a selection of other temporal data
...
artifacts; again see the I<Muldis D DateTime Extension> for alternatives.
...
This is an enumeration data type.  C<Set.T> is a proper subtype of
C<Tuple>, and it exists in order for the relation type
C<Set> (and C<Maybe> and C<Single>) to be defined partly in terms of it.  A
C<Set.T> has 1 attribute, C<value> (a C<Universal>).  Its default value a
C<value> of C<Bool:False>.

=head2 sys.std.Core.Type.DHSet.T

This is an enumeration data type.  C<DHSet.T> is the intersection type of
C<Set.T> and C<DHTuple>, and it exists in order for the dh-relation type
C<DHSet> (and C<DHMaybe>, C<DHSingle>) to be defined partly in terms of it.

=head2 sys.std.Core.Type.Array.T

This is an enumeration data type.  C<Array.T> is a proper subtype of
C<Tuple>, and it exists in order for the relation type
...
This is an enumeration data type.  C<DHArray.T> is the intersection type of
...
This is an enumeration data type.  C<Bag.T> is a proper subtype of
C<Tuple>, and it exists in order for the relation type
...
This is an enumeration data type.  C<DHBag.T> is the intersection type of
...
This is an enumeration data type.  An C<SPInterval> (single-piece interval)
is a C<Tuple>.  It
...
I<COMING SOON: Adding support for unbounded/infinite intervals.>
...
C<is_identical> function should not be used to test an C<SPInterval> for
being empty or not.
...
The C<SPInterval> type has no support for I<unbounded>/I<infinite> or
...
then those special values can be used for endpoints of intervals over those
types.  I<As of this writing, no system-defined types have values for
infinities, so only user-defined types would leverage this.  This will
change very soon, with system-defined generic infinity values coming.>
...
This is an enumeration data type.  C<DHSPInterval> is a proper subtype of
...
This is an enumeration data type.  C<MPInterval> (multi-piece interval) is
a proper subtype of
...
constraints of its own.  It defines a single I<multi-piece interval>, which
is conceptually either a set of 0..N intervals or a single larger interval
that had 0..N sub-intervals sliced out.  An C<MPInterval> is the
canonical means that Muldis D provides of representing the result of
set-unioning 2 C<SPInterval> where the latter do not touch or overlap.
...
This is an enumeration data type.  C<DHMPInterval> is the intersection
type of C<MPInterval> and C<DHRelation>.  The cardinality of this type
is infinite.

=head1 GENERIC REFERENCE TYPES
...
type of all Muldis D reference types.  The cardinality of this type is
infinity.  The default value of this type is the same as for C<External>,
meaning implementation-defined.

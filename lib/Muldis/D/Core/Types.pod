...
also known by the special name C<Tuple:d0>, which serves as the default
value of the 3 types C<[|DH]Tuple> and C<Database>.
...
is otherwise the same.  The 4 system-defined user-data variables named
...
which serves as the default value of the 2 types C<[|DH]Relation>.  The
only member value of C<DHRelation> that has exactly zero attributes and
exactly one tuple is also known by the special name C<Relation:d0c1>.

...
This document is Muldis::D::Ext::Relation version 0.100.0.
...
C<< function sys.std.Core.Relation.is_not_nullary (Bool <-- $topic : Relation)
>>

This function is exactly the same as C<sys.std.Core.Relation.is_nullary> except
...
C<< function sys.std.Core.Relation.is_not_empty (Bool <-- $topic : Relation) >>

This function is exactly the same as C<sys.std.Core.Relation.is_empty> except
that it results in the opposite boolean value when given the same argument.
...
$topic : Relation, $func : ValFiltFuncRef) >>
...
$topic : Relation, $func : ValMapFuncRef, $class_attr_name : Name,
...
$topic : Relation, $attr_names : set_of.Name, $func : ValMapFuncRef) >>
...
$result_attr_names : set_of.Name, $func : ValMapFuncRef) >>
...
$summ_attr_names : set_of.Name, $summ_func : ValMapFuncRef) >>
...
$func : ValRedFuncRef, $identity : Tuple) >>
...
$topic : Relation, $func : ValRedFuncRef) >>

This function is exactly the same as C<sys.std.Core.Relation.reduction> except
...
This function is exactly the same as C<sys.std.Core.Relation.is_proper_subset>
except that it results in the opposite boolean value when given the same
arguments.  Note that this operation is also known as C<⊄>.
...
This function is an alias for C<sys.std.Core.Relation.is_proper_subset> except
that it transposes the C<topic> and C<other> arguments.  This function
...
C<< function sys.std.Core.Relation.is_not_disjoint (Bool <-- $topic : Relation,
$other : Relation) >>
...
C<< function sys.std.Core.Relation.composition (Relation <-- $topic : Relation,
$other : Relation) >>
...
$name : Name, $ord_func : OrdDetFuncRef, $is_reverse_order? : Bool) >>
...
also the C<sys.std.Core.Array.Array_from_wrap> function, which is the same as
C<sys.std.Core.Relation.rank> but that it wraps the source tuples rather than
...
C<sys.std.Core.Tuple.order_by_attr_names> as its C<ord_func> argument after the
...
$ord_func : OrdDetFuncRef, $is_reverse_order? : Bool,
$rank_interval : interval_of.NNInt) >>
...
$is_reverse_order? : Bool, $rank_interval : interval_of.NNInt) >>
...
C<< function sys.std.Core.Relation.substitution (Relation <-- $topic : Relation,
$attr_names : set_of.Name, $func : ValMapFuncRef) >>
...
$topic : Relation, $restr_func : ValFiltFuncRef,
$subst_attr_names : set_of.Name, $subst_func : ValMapFuncRef) >>
...
$topic : Relation, $restr_func : ValFiltFuncRef, $subst : Tuple) >>
...
This function is the same as C<sys.std.Core.Relation.join_with_group> except
...
$primary : Relation, $secondary : Relation, $filler : TypeRef) >>
...
C<< function sys.std.Core.Relation.outer_join_with_static_exten (Relation <--
$primary : Relation, $secondary : Relation, $filler : Tuple) >>

This function is the same as C<sys.std.Core.Relation.outer_join_with_maybes>
...
$primary : Relation, $secondary : Relation, $exten_func : ValMapFuncRef) >>
...
$restr_func : ValFiltFuncRef, $subst_attr_names : set_of.Name,
$subst_func : ValMapFuncRef)>
...
C<updater sys.std.Core.Relation.assign_subst_in_semijoin (&$topic : Relation,
$restr : Relation, $subst_attr_names : set_of.Name,
$subst_func : ValMapFuncRef)>

...
topic(Scalar), possrep(Name)?, attrs(set_of.Cat.Name) }>
...
topic(Tuple), inner(set_of.Cat.Name), outer(Name) }>
...
topic(Tuple), cmpl_inner(set_of.Cat.Name), outer(Name) }>
...
topic(Relation), inner(set_of.Cat.Name), outer(Name) }>
...
topic(Relation), cmpl_inner(set_of.Cat.Name), outer(Name) }>
...
topic(Relation), outer(Name), inner(set_of.Cat.Name) }>
...
topic(Relation), group_per(set_of.Cat.Name), outer(Name) }>
...
topic(Relation), group_per(set_of.Cat.Name), summ_func(FuncRef),
...
read { length_in_codepoints(NNInt) }>

This system service routine will attempt to read C<length_in_codepoints>
...
C<topic> whose value is a C<Exception>; if the C<catch> procedure also

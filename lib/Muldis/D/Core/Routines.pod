...
This document is Muldis::D::Core::Routines version 0.79.1.
...
Relation $topic, ValFiltFuncRef $func, Tuple $assuming?) >>
...
set_of.Name $attr_names, ValMapFuncRef $func, Tuple $assuming?) >>
...
ValMapFuncRef $summ_func, Tuple $summ_assuming?) >>
...
C<Tuple>-resulting C<value_map> function named in its C<summ_func> argument
when the latter function is curried by its C<summ_assuming> argument
(passed to it as just C<assuming>); for each post-group main relation
...
ValFiltFuncRef $func, Tuple $assuming?)>

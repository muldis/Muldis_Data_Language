...
both subtypes of a common q/scalar root type; 2. they are both subtypes of a
common non-incomplete q/tuple or q/relation type, that is they essentially have
...
This function results in the C<QTuple> that is the projection of the possrep
...
=item C<function sys.std.Core.QScalar.QTuple_from_QScalar result QTuple params
...
This function results in the C<QScalar> value whose q/scalar root (|sub)type
...
of C<topic>.  This function can be used to select any q/scalar value at all
that has a possrep.
...
C<Order:same> iff its C<topic> and C<other> arguments are exactly the
same value, and otherwise it results in C<Order:increase> if the value
...
C<Order:decrease> as the reverse of the last condition would be true.
This function will fail if its C<topic> and C<other> arguments are not
values of a common q/scalar root type that declares a type-default
...
supporting that choice; if the algorithm is not customizable, then a q/tuple
...
This function results in the q/scalar or q/nonscalar value of the attribute of
...
name is C<name> has been updated with a new q/scalar or q/nonscalar value given
...
handles N q/tuple attributes at once rather than just 1.  The heading of the
...
C<QTuple>-typed attribute, which exists in place of the original attributes.
...
works with q/tuples rather than q/relations.  This function is mainly intended
for use in connecting q/tuples that have all disjoint headings, such as for
extending one q/tuple with additional attributes.

=back

=head1 SYSTEM-DEFINED GENERIC SINGLE INPUT QUASI-/RELATION FUNCTIONS

These functions are applicable to mainly q/relation types, but are generic in
that they typically work with any q/relation types.  Each C<\w*assuming>
...
This function results in C<Bool:true> iff its C<t> argument matches a q/tuple
of its C<r> argument (that is, iff conceptually C<t> is a member of C<r>),
and C<Bool:false> otherwise.  This function is like
C<sys.std.Core.QRelation.is_subset> except that the q/tuple being looked for
doesn't have to be wrapped in a q/relation.  This function will warn if its
2 arguments' common-named attributes have declared types that are
...
This function results in the C<QTuple> that is the sole member q/tuple of its
argument.  This function will fail if its argument does not have exactly
one q/tuple.

=item C<function sys.std.Core.QRelation.QRelation_from_QTuple result QRelation
params { topic(QTuple) }>
...
This function results in a C<QRelation> that is the relational union of C<r>
and a q/relation whose sole q/tuple is C<t>; that is, conceptually the result
...
from C<r> of a q/relation whose sole q/tuple is C<t>; that is, conceptually the
...
operates on and results in a C<QRelation> rather than a C<QTuple>.  But note
that the result q/relation will have fewer q/tuples than C<topic> if any
C<topic> q/tuples were non-distinct for just the projected attributes.
...
operates on and results in a C<QRelation> rather than a C<QTuple>, where each
of its member q/tuples was transformed as per C<sys.std.Core.QTuple.wrap>.
...
operates on and results in a C<QRelation> rather than a C<QTuple>, where each
of its member q/tuples was transformed as per
...
C<topic> attribute-wrapping transformations result in new C<QRelation>-typed
attributes rather than new C<QTuple>-typed attributes, and moreover multiple
C<topic> q/tuples may be combined into fewer q/tuples whose new
C<QRelation>-typed attributes have multiple q/tuples.  This function takes a
q/relation of N q/tuples and divides the q/tuples into M groups where all the
q/tuples in a group have the same values in the attributes which aren't being
grouped (and distinct values in the attributes that are being grouped); it
then results in a new q/relation of M q/tuples where the new q/relation-valued
attribute of the result has the q/tuples of the M groups.  A grouped q/relation
contains all of the information in the original q/relation, but it has less
redundancy due to redundant non-grouped attributes now just being
represented in one q/tuple per the multiple q/tuples whose grouped attributes
had them in common.  A q/relation having q/relation-valued attributes like this
is a common way to group so-called child q/tuples under their parents.  As a
trivial case, if C<inner> is empty, then the result has all the same q/tuples
...
attributes of C<topic>, then the result has a single q/tuple of a single
...
function named by C<func> results in C<Bool:true> when passed the q/tuple as
...
argument.  The result q/relation has a heading that is a superset of that of
C<topic>, and its body contains the same number of q/tuples, with all
...
q/tuple in the main q/relation, this function applies the C<QTuple>-resulting
...
which are valued with the q/relation-valued attribute and q/tuple-valued
...
input q/tuple and results in a whole q/tuple; the applied function would
...
not intended to be used to summarize an entire C<topic> q/relation at once
...
C<summary>, especially if you want a single-q/tuple result on an empty
...
These functions are applicable to mainly q/relation types, but are generic in
that they typically work with any q/relation types.
...
the result.  The result q/relation has the same heading as all of its inputs,
and its body contains every q/tuple that is in any of the input q/relations.
...
=item C<function sys.std.Core.QRelation.intersection result QRelation params
{ topic(quasi_set_of.QRelation) }>
...
commutative and associative) them together until just one is left, which is
the result.  The result q/relation has the same heading as all of its inputs,
and its body contains only the q/tuples that are in every one of the input
...
is the universal q/relation with the same heading (that is, the q/relation
having all the q/tuples that could ever exist in a q/relation with that
...
heading / attribute names for the result q/relation in question (and even if
they were, more information on attribute types would be needed to produce
said universal q/relation, and even then it might be infinite or impossibly
...
complementary subset of q/tuples of C<source> when given the same arguments.
...
C<source>, and its body contains the subset of C<source> q/tuples that match
...
The result q/relation has a heading that is a union of all of the headings of
its inputs, and its body is the result of first pairwise-matching every
q/tuple of each input q/relation with every q/tuple of each other input q/relation,
then where each member of a q/tuple pair has attribute names in common,
eliminating pairs where the values of those attributes differ and unioning
the remaining said q/tuple pairs, then eliminating any result q/tuples that
duplicate others.  If C<topic> has zero values, then C<join> results in the
nullary q/relation with one q/tuple, which is the identity value for relational
join.  As a trivial case, if any input q/relation has zero q/tuples, then the
function's result will too; or, if any input is the nullary q/relation with
one q/tuple, that input can be ignored (see identity value); or, if any 2
...
latter.  This function will warn if any input q/relations have attributes
with common names where their declared types are incompatible as
...
subsets of the C<dividend> heading); the result has all q/tuples C<{X}> such
that a q/tuple C<{X,Y}> appears in C<A> for all q/tuples C<{Y}> appearing in
...
C<Database>-typed; each q/relation-typed attribute of C<other> corresponds to
a created SQL table.
...
=item C<updater sys.std.Core.QTuple.assign_projection update { topic(QTuple)
} read { attrs(set_of.Name) }>
...
statement iff C<topic> is C<Database>-typed; each q/relation-typed attribute
named by C<attrs> corresponds to a dropped SQL table.
...
This update operator is a short-hand for first invoking the
C<sys.std.Core.QRelation.union> function such that it has 2 input q/relations
...
=item C<updater sys.std.Core.QRelation.assign_deletion update { r(QRelation)
} read { t(QTuple) }>

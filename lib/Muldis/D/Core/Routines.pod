...
Scalar $other, Tuple $assuming?, Bool $is_reverse_order?) >>
...
C<< function sys.std.Core.Tuple.wrap (Tuple <-- Tuple $topic,
...
in that they typically work with any relation types.  Each C<\w*assuming>
...
Relation $topic, set_of.Name $cmpl_inner, Name $outer) >>
...
This function will fail with a non-C<DHRelation> valued C<topic> unless, for
every tuple of C<topic>, the attribute specified by C<outer> is valued
...
Relation $topic, set_of.Name $group_per, Name $outer) >>
...
Relation $topic, Name $outer, set_of.Name $inner) >>
...
tuples.  A relation can be first grouped and then that result ungrouped
to produce the original relation, with no data loss.  However, the
ungroup of a relation on a relation-valued attribute will lose the
information in any outer relation tuples whose inner relation value
has zero tuples; a group on this result won't bring them back.  This
function will fail if C<outer> specifies any attribute name that C<topic>
doesn't have, or if an attribute of C<topic{outer}> has the same name as
another C<topic> attribute.  This function will fail with a non-C<DHRelation>
valued C<topic> unless, for every tuple of C<topic>, the attribute
specified by C<outer> is valued with a relation of the same degree and
...
would never happen by definition with a deeply homogeneous relation C<topic>).

=head2 sys.std.Core.Relation.restriction

C<< function sys.std.Core.Relation.restriction (Relation <--
Relation $topic, FuncRef $func, Tuple $assuming?) >>
...
Relation $topic, set_of.Name $attr_names, FuncRef $func,
Tuple $assuming?) >>
...
FuncRef $summ_func, Tuple $summ_assuming?) >>
...
C<Tuple>-resulting function named in its C<summ_func> argument when the
...
Relation $topic, set_of.Name $group_per, Name $count_attr_name) >>
...
complementary subset of tuples of C<source> when given the same
...
set_of.Name $attr_names, FuncRef $func, Tuple $assuming?)>
...
Tuple &$try_updating?, Tuple $try_assuming?, ProcRef $catch,
Tuple &$catch_updating?, Tuple $catch_assuming?)>

...
are defined over them.  This function also has a C<QuasiTuple>-typed third
...
{ topic(Relation), func(FuncRef), assuming(QuasiTuple)? }>
...
params { topic(Relation), func(FuncRef), assuming(QuasiTuple)? }>
...
function(FuncRef), args(QuasiTuple)? }>
...
topic(Relation) } read { func(FuncRef), assuming(QuasiTuple)? }>
...
} read { updater(ProcRef), ro_args(QuasiTuple)? }>
...
upd_args(QuasiTuple)? } read { procedure(ProcRef), ro_args(QuasiTuple)? }>
...
try(ProcRef), catch(ProcRef), try_assuming(QuasiTuple)?,

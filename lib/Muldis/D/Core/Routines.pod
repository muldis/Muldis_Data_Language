...
This document is Muldis::D::Core::Routines version 0.26.0.
...
=item C<function sys.std.Core.Universal.func_invo result Universal params {
function(Cat.NameChain), args(QuasiTuple)? }>
...
values that have possreps, and not with values of the 6 system-defined
scalar types lacking any possreps: C<Bool>, C<Int>, C<Blob>, C<Text>,
C<Cat.Name>, C<Cat.Comment>; other functions are not limited in that way,
but may be limited in other ways.  Note that the terminology used to
describe these functions is taking advantage of the fact that a scalar
possrep looks just like a tuple.  Each C<possrep> parameter is optional and
defaults to the empty string if no explicit argument is given to it.
...
params { topic(Tuple), type(Cat.NameChain), possrep(Cat.Name)? }>
...
topic(Scalar), other(Scalar), assuming(Tuple)? }>
...
=item C<function sys.std.Core.Bool.(false|true) result Bool params {}>
...
=item C<function sys.std.Core.Bool.if_then_else result Universal params {
if(Bool), then(Universal), else(Universal) }>

This function results in the value of its C<then> argument, when its C<if>
argument is C<Bool:true>; it results in the value of its C<else> argument,
when its C<if> argument is C<Bool:false>.  Note that the invocation of
C<if_then_else> denotes short-circuit evaluation of the value expressions
making up its arguments, so for example the expression producing C<then>
may be impossible to run in situations where the expression producing C<if>
is false, and one purpose of C<if> may be to test if the other expression
should be evaluated at all; by contrast, most other Muldis D functions
don't require short-circuit evaluation in order to work in the general case
(but short-circuiting and lazy evaluation should often be employed for
performance anyway).  Note that there is no if-elseif-elsif-...-else
function; you would chain several of this current triadic C<if_then_else>
function to get that effect; however, see also the
C<sys.std.Core.Relation.given_when_otherwise> function.
...
=item C<function sys.std.Core.Relation.cardinality result UInt params {
topic(Relation) }>
...
topic(Relation), group_per(set_of.Cat.Name), summ_func(Cat.NameChain),
summ_assuming(Tuple)? }>
...
=item C<function sys.std.Core.Relation.given_when_default result ScaTupRel
params { given(ScaTupRel), when_then(Relation), default(ScaTupRel) }>

This function implements the logical given-when-otherwise switch expression
that dispatches based on matching a single value with several options; it
will use its binary relation C<when_then> argument as a lookup table,
wherein it looks for a tuple whose C<when> attribute value matches the
value of the function's C<given> attribute; if a match is found, then this
function results in the value of the matched tuple's C<then> attribute; if
no match is found, then this function results in the value of its
C<default> argument.  This function is an abstraction over a relational
semijoin.  This function will fail if its C<when_then> argument isn't a
binary relation with attributes named C<when> and C<then>, or if the type
of C<given> isn't join-wise compatible with the type of the C<when>
attribute, or if the type of C<default> isn't join-wise compatible with the
type of the C<then> attribute.
...
=head1 SYSTEM-DEFINED GENERIC QUASI- FUNCTIONS

I<This documentation is pending.  But assume that any necessary quasi-
routines are like those of their non-quasi counterparts, and that mapping
functions exist between corresponding quasi- and non-quasi types as well.>
...
=item C<updater sys.std.Core.Control.upd_invo update {
upd_args(QuasiTuple)? } read { updater(Cat.NameChain), ro_args(QuasiTuple)?
}>

This update operator has the same purpose and features as
C<sys.std.Core.Universal.func_invo> but that it invokes an updater rather
...
C<upd_args> and C<ro_args> arguments, respectively.  The C<upd_args> and
C<ro_args> parameters are optional and default as per the C<args> parameter
of C<func_invo>.

=back

=head1 SYSTEM-DEFINED GENERIC SYSTEM SERVICES

These system services are applicable to just one or more specific
system-defined core scalar data type.

Note: Basic I/O procedures go here!
...
upd_args(QuasiTuple)? } read { procedure(Cat.NameChain),
ro_args(QuasiTuple)? }>

This procedure is the same as C<sys.std.Core.Control.upd_invo> but that it
invokes a procedure rather than an updater.
...
try_updating(Tuple)?, catch_updating(Tuple)? } read { try(Cat.NameChain),
catch(Cat.NameChain), try_assuming(Tuple)?, catch_assuming(Tuple)? }>

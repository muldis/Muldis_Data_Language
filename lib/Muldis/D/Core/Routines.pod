...
This document is Muldis::D::Core::Routines version 0.78.0.
...
deeply homogeneous relation C<topic>).
...
C<procedure sys.std.Core.Control.try_catch (ImpRef $try,
Tuple &$try_updating?, Tuple $try_assuming?, ImpRef $catch,
Tuple &$catch_updating?, Tuple $catch_assuming?)>

This procedure invokes the imperative routine named in its C<try> argument,
giving it the arguments C<try_updating> and C<try_assuming> as its
C<updating> and C<assuming> arguments, respectively.  If the C<try> routine
...
routine is not called.  Each of the C<[try|catch]_[updating|assuming]>

...
This document is Muldis::D::Core::Routines_Catalog version 0.117.0.
...
=head2 sys.std.Core.Cat.imp_invo

C<updater sys.std.Core.Cat.imp_invo ($imperative : APImperativeNC,
&$upd_args : Tuple, $ro_args? : Tuple)>

This imperative routine has the same purpose and features as
C<sys.std.Core.Cat.func_invo> but that it invokes an imperative routine
rather than a function; there is no result to deal with, and there are both
subject-to-update parameters and read-only parameters of the invoked
imperative routine to bind to; they are bound with the attributes of this
imperative routine's C<upd_args> and C<ro_args> arguments, respectively. 
The C<ro_args> parameter is optional and defaults as per the C<args>
parameter of C<func_invo>; the C<upd_args> parameter is also optional in
the general case, but it is non-optional when the imperative routine to
invoke is an updater, because an updater must always be invoked with at
least one subject-to-update argument.
...
=head2 sys.std.Core.Cat.create_[function|updater|recipe|procedure]

C<recipe sys.std.Core.Cat.create_[function|updater|recipe|procedure]
($depot : Name, $subdepot? : NameChain, $name : Name,
$comment? : Comment, $material : [Function|Updater|Recipe|Procedure])>

This recipe is an abstraction over inserting a tuple into the catalog
relvar
C<fed.cat.mounts{name=$depot}.depot.[function|updater|recipe|procedure]s>.
It will create a new function|updater|recipe|procedure, in the depot
mounted under the name given by the C<depot> argument, whose name and other
details match the other arguments.  This recipe is analogous to a
SQL CREATE FUNCTION|PROCEDURE statement.

=head2 sys.std.Core.Cat.drop_[function|updater|recipe|procedure]

C<recipe sys.std.Core.Cat.drop_[function|updater|recipe|procedure]
($depot : Name, $subdepot? : NameChain, $name : Name)>

This recipe is an abstraction over deleting a tuple from the catalog relvar
C<fed.cat.mounts{name=$depot}.depot.[function|updater|recipe|procedure]s>.
It will drop an existing depot function|updater|recipe|procedure.  This
recipe is analogous to a SQL DROP FUNCTION|PROCEDURE statement.

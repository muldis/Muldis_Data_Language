...
This document is Muldis::D::Core::Routines_Catalog version 0.88.0.
...
create|drop subdepots and packages, create|drop user-defined routines and
...
C<procedure sys.std.Core.Cat.create_package (Name $name,
Comment $comment?)>
...
argument, whose name and other details match the other arguments.  This
procedure is analagous to an Oracle CREATE PACKAGE statement, but that it
will just create an empty package.
...
statement, but that it will just drop an empty package.
...
This procedure is an abstraction over inserting a tuple into the catalog
relvar C<fed.cat.mounts{name=$depot}.depot.materials>.  It will
...
This procedure is an abstraction over deleting a tuple from the catalog
relvar C<fed.cat.mounts{name=$depot}.depot.materials>.  It will drop

...
This document is Muldis::D::Core::Routines_Catalog version 0.61.0.
...
C<function sys.std.Core.Cat.Order_reverse result Order params {
topic(Order) }>
...
C<function sys.std.Core.Cat.Order_reduction result Order params {
...
C<procedure sys.std.Core.Cat.create_depot_mount update {} read {
...
C<procedure sys.std.Core.Cat.drop_depot_mount update {} read {
name(Name) }>
...
C<procedure sys.std.Core.Cat.select_topic_namespaces update {} read {
...
C<procedure sys.std.Core.Cat.create_subdepot update {} read {
...
C<procedure sys.std.Core.Cat.drop_subdepot update {} read { name(Name) }>
...
C<procedure sys.std.Core.Cat.create_package update {} read { name(Name),
comment(Comment)?, package(Package)? }>
...
C<procedure sys.std.Core.Cat.drop_package update {} read { name(Name) }>
...
These 8 procedures conceptually have the extra 2 parameters C<depot(Name),
parent(DeclNameChain)?>, whose values are implicitly supplied from
C<tpc.cat.topic_(depot|subdepot)> respectively; those 1 or 2 (as
applicable) catalog variables must be non-empty as a precondition to
invoking the 4 procedures.
...
update {} read { name(Name), comment(Comment)?,
...
C<procedure sys.std.Core.Cat.create_depot_type update {} read {
...
C<procedure sys.std.Core.Cat.drop_depot_type update {} read { name(Name) }>
...
These 8 procedures conceptually have the extra 3 parameters C<depot(Name),
...
update {} read { name(Name), comment(Comment)?, is_public(Bool)?,
head(Cat.(Func|Upd|Proc)Head), body(Cat.(Func|Upd|Proc)Body) }>
...
C<procedure sys.std.Core.Cat.create_package_type update {} read {
...
C<procedure sys.std.Core.Cat.drop_package_type update {} read {
name(Name) }>

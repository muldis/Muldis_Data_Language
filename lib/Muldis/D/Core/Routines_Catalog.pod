...
This document is Muldis::D::Core::Routines_Catalog version 0.127.0.
...
C<< function sys.std.Core.Cat.curried_func_invo (Universal <--
...
C<< function sys.std.Core.Cat.curried_func_static_exten (CurriedFuncNC <--
...
C<updater sys.std.Core.Cat.upd_invo ($updater : APImperativeNC,
...
=head2 sys.std.Core.Cat.rcp_invo

C<recipe sys.std.Core.Cat.rcp_invo ($recipe : APImperativeNC,
&$upd_args? : Tuple, $ro_args? : Tuple) {...}>

This recipe is the same as C<sys.std.Core.Cat.upd_invo> but that it
invokes a recipe rather than an updater, and that C<upd_args> is optional.

=head2 sys.std.Core.Cat.proc_invo

C<procedure sys.std.Core.Cat.proc_invo ($procedure : APImperativeNC,
&$upd_args? : Tuple, $ro_args? : Tuple) {...}>

This procedure is the same as C<sys.std.Core.Cat.rcp_invo> but that it
invokes a procedure rather than an recipe.
...
$allow_auto_run? : Bool, $details? : OVLScaValExprNodeSet,
...
C<recipe sys.std.Core.Cat.create_function ($depot : Name,
...
C<recipe sys.std.Core.Cat.drop_function ($depot : Name,
$subdepot? : NameChain, $name : Name,
&$cat ::= $fed.cat, &$data ::= $fed.data) {...}>
...
=head2 sys.std.Core.Cat.create_[updater|recipe|procedure]

C<recipe sys.std.Core.Cat.create_[updater|recipe|procedure] ($depot : Name,
$subdepot? : NameChain, $name : Name,
$scm_comment? : Comment, $scm_vis_ord? : NNInt,
$material : [Updater|Recipe|Procedure], &$cat ::= $fed.cat) {...}>

This recipe is an abstraction over inserting a tuple into the catalog
relvar C<fed.cat.mounts{name=$depot}.depot.[updater|recipe|procedure]s>.
It will create a new updater|recipe|procedure, in the depot mounted under
...
=head2 sys.std.Core.Cat.drop_[updater|recipe|procedure]

C<recipe sys.std.Core.Cat.drop_[updater|recipe|procedure] ($depot : Name,
$subdepot? : NameChain, $name : Name, &$cat ::= $fed.cat) {...}>

This recipe is an abstraction over deleting a tuple from the catalog relvar
C<fed.cat.mounts{name=$depot}.depot.[updater|recipe|procedure]s>.  It will
drop an existing depot updater|recipe|procedure.  This recipe is analogous
to a SQL DROP PROCEDURE statement.

=head2 sys.std.Core.Cat.create_[scalar|tuple|relation|domain|subset|mixin]_type

C<recipe sys.std.Core.Cat.create_[scalar|tuple|relation|domain|subset|mixin]_type
...
C<recipe sys.std.Core.Cat.drop_[scalar|tuple|relation|domain|subset|mixin]_type
($depot : Name, $subdepot? : NameChain, $name : Name,
&$cat ::= $fed.cat, &$data ::= $fed.data) {...}>
...
=head2 sys.std.Core.Cat.create_[|distrib_][key|subset]_constraint

C<recipe sys.std.Core.Cat.create_[|distrib_][key|subset]_constraint
($depot : Name, $subdepot? : NameChain, $name : Name,
$scm_comment? : Comment, $scm_vis_ord? : NNInt,
...
This recipe is an abstraction over inserting a tuple into the catalog
relvar
C<fed.cat.mounts{name=$depot}.depot.[|distrib_][key|subset]_constraints>.
...
=head2 sys.std.Core.Cat.drop_[|distrib_][key|subset]_constraint

C<recipe sys.std.Core.Cat.drop_[|distrib_][key|subset]_constraint
($depot : Name, $subdepot? : NameChain, $name : Name,
&$cat ::= $fed.cat, &$data ::= $fed.data) {...}>
...
C<recipe sys.std.Core.Cat.create_stim_resp_rule ($depot : Name,
...
C<recipe sys.std.Core.Cat.drop_stim_resp_rule ($depot : Name,
$subdepot? : NameChain, $name : Name, &$cat ::= $fed.cat) {...}>

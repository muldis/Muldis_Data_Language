...
This document is Muldis::D::Core::Routines_Catalog version 0.78.0.
...
=head1 FUNCTIONS FOR SPECIAL ENTITY REFERENCE DEFAULT VALUES

These functions are defined primarily for use in the definitions of several
reference types that are references to functions; each one is an example
function of an appropriate structure such that the reference types can use
references to these functions as their default values.
...
maybe_of.Name $package?, Comment $comment?)>
...
=head2 sys.std.Core.Cat.create_depot_material

C<procedure sys.std.Core.Cat.create_depot_material (Name $name,
Comment $comment?, Material $material)>

This procedure is an abstraction over inserting a tuple into the catalog
relvar C<fed.cat.mounts{name=$depot}.depot.materials>.  It will create a
new routine or type, in the depot mounted under the name given by the
C<depot> argument, whose name and other details match the other arguments.
This procedure is analagous to a SQL CREATE FUNCTION|PROCEDURE|TYPE|DOMAIN
statement.

=head2 sys.std.Core.Cat.drop_depot_material

C<procedure sys.std.Core.Cat.drop_depot_material (Name $name)>

This procedure is an abstraction over deleting a tuple from the catalog
relvar C<fed.cat.mounts{name=$depot}.depot.materials>.  It will drop an
existing depot routine or type.  This procedure is analagous to a SQL DROP
FUNCTION|PROCEDURE|TYPE|DOMAIN statement.

=head1 Procedures For Defining Package Routines and Data Types

These 2 procedures conceptually have the extra 3 parameters C<Name $depot,
...
precondition to invoking the 2 procedures.

=head2 sys.std.Core.Cat.create_package_material

C<procedure sys.std.Core.Cat.create_package_material (Name $name,
Comment $comment?, Bool $is_public?, Material $material)>

This procedure is an abstraction over inserting a tuple into the catalog
relvar C<fed.cat.mounts{name=$depot}.depot
.packages{parent=$subdepot,name=$package}.package.materials>.  It will
create a new routine or type, in the package having the name given by the
C<package> argument, in the depot mounted under the name given by the
C<depot> argument, whose name and other details match the other arguments.

=head2 sys.std.Core.Cat.drop_package_material

C<procedure sys.std.Core.Cat.drop_package_material (Name $name)>

This procedure is an abstraction over deleting a tuple from the catalog
relvar C<fed.cat.mounts{name=$depot}.depot
.packages{parent=$subdepot,name=$package}.package.materials>.  It will drop
an existing package routine or type.

...
of this function are equivalent to taking the floor of the result of a real
number division; for example, C<-7 / 3 = -3>.  This function
...
has the same semantics as C<sys.std.Core.Integer.quotient> (taking the
floor of the result of a real number division), and so the sign of this
function's result always matches the sign of the divisor.  This function
will fail if C<divisor> is zero.  Note that this operation is also known as
I<modulus> or C<%>.
...
C<sys.std.Core.Integer.quotient>, meaning floor rounding.  If

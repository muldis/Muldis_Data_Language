...
C<< function sys.std.Core.Integer.abs_diff (Int <-- $topic : Int, $other : Int)
>>
...
C<sys.std.Core.Integer.remainder> would result in when given the same arguments.
...
This function is exactly the same as C<sys.std.Core.Integer.quotient> except
...
This function is exactly the same as C<sys.std.Core.Integer.remainder> except
that it results in a C<Maybe> of what is otherwise the result, and that
result has zero elements if C<divisor> is zero.
...
C<< function sys.std.Core.Integer.median (set_of.Int <-- $topic : bag_of.Int) >>

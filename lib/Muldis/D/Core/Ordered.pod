...
This document is Muldis::D::Ext::Ordered version 0.100.0.
...
C<< function sys.std.Ordered.is_before (Bool <-- $topic : Universal,
$other : Universal, $func? : OrdDetFuncRef) >>
...
C<< function sys.std.Ordered.is_after (Bool <-- $topic : Universal,
$other : Universal, $func? : OrdDetFuncRef) >>
...
C<< function sys.std.Ordered.is_before_or_same (Bool <--
$topic : Universal, $other : Universal, $func? : OrdDetFuncRef) >>
...
C<< function sys.std.Ordered.is_after_or_same (Bool <-- $topic : Universal,
$other : Universal, $func? : OrdDetFuncRef) >>
...
=head2 sys.std.Ordered.is_member

C<< function sys.std.Ordered.is_member (Bool <-- $value : Universal,
$interval : Interval, $func? : OrdDetFuncRef) >>

This function is an alias for C<sys.std.Interval.has_member>.  This
...
that this operation is also known as C<I∈> or I<between>, and depending on
the combination of C<excludes_[min|max]> attributes of its C<interval>
argument, it is alternately also known as one of [C<≤≤>, C<< ≤< >>, C<< <≤
>>, C<< << >>].

=head2 sys.std.Ordered.is_not_member

C<< function sys.std.Ordered.is_not_member (Bool <-- $value : Universal,
$interval : Interval, $func? : OrdDetFuncRef) >>

This function is an alias for C<sys.std.Interval.has_not_member>.  This
function is exactly the same as C<sys.std.Ordered.is_member> except that it
results in the opposite boolean value when given the same arguments.  Note
that this operation is also known as C<I∉> or I<not between>, and depending
on the combination of C<excludes_[min|max]> attributes of its C<interval>
argument, it is alternately also known as one of [C<!≤≤>, C<< !≤< >>, C<<
!<≤ >>, C<< !<< >>].

=head2 sys.std.Ordered.min

C<< function sys.std.Ordered.min (Universal <-- $topic : Set,
$func? : OrdDetFuncRef) >>
...
C<< function sys.std.Ordered.max (Universal <-- $topic : Set,
$func? : OrdDetFuncRef) >>
...
C<< function sys.std.Ordered.minmax (Tuple <-- $topic : Set,
$func? : OrdDetFuncRef) >>
...
C<< function sys.std.Ordered.maybe_min (Maybe <-- $topic : Set,
$func? : OrdDetFuncRef) >>
...
C<< function sys.std.Ordered.maybe_max (Maybe <-- $topic : Set,
$func? : OrdDetFuncRef) >>
...
C<< function sys.std.Ordered.maybe_minmax (Relation <-- $topic : Set,
$func? : OrdDetFuncRef) >>

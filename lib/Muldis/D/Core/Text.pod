...
C<< function sys.std.Core.Text.catenation (Text <-- $topic? : array_of.Text) >>
...
This function is exactly the same as C<sys.std.Core.Text.has_substr> except that
...
C<< function sys.std.Core.Text.ASCII (Text <-- $topic : Text, $mark? : Text) >>

...
This document is Muldis::D::Core::Set version 0.114.0.
...
arguments.  Note that this operation is also known as C<s∌> or C<s-!has> or
C<s-not-has>.
...
otherwise.  Note that this operation is also known as C<∈s> or C<in-s>.
...
that it results in the opposite boolean value when given the same
arguments.  Note that this operation is also known as C<∉s> or C<!in-s> or
C<not-in-s>.

...
Muldis::D::Core::Set -
Muldis D Set and Maybe specific operators

=head1 VERSION

This document is Muldis::D::Core::Set version 0.109.0.
...
C<< function sys.std.Core.Set.has_member (Bool <-- $set : Set,
$value : Universal) >>
...
C<< function sys.std.Core.Set.has_not_member (Bool <-- $set : Set,
$value : Universal) >>
...
arguments.  Note that this operation is also known as C<S∌> or C<S!hmem> or
C<Snot-hmem>.

=head2 sys.std.Core.Set.value_is_member

C<< function sys.std.Core.Set.value_is_member (Bool <--
$value : Universal, $set : Set) >>
...
otherwise.  Note that this operation is also known as C<S∈> or C<Simem>.

=head2 sys.std.Core.Set.value_is_not_member

C<< function sys.std.Core.Set.value_is_not_member (Bool <--
$value : Universal, $set : Set) >>
...
that it results in the opposite boolean value when given the same
arguments.  Note that this operation is also known as C<S∉> or C<S!imem> or
C<Snot-imem>.

=head2 sys.std.Core.Set.insertion

C<< function sys.std.Core.Set.insertion (Set <-- $set : Set,
$value : Universal) >>
...
C<< function sys.std.Core.Set.disjoint_ins (Set <-- $set : Set,
$value : Universal) >>
...
C<< function sys.std.Core.Set.deletion (Set <-- $set : Set,
$value : Universal) >>
...
C<< function sys.std.Core.Set.reduction (Universal <-- $topic : Set,
...
=head2 sys.std.Core.Set.maybe_reduction

C<< function sys.std.Core.Set.maybe_reduction (Maybe <-- $topic : Set,
$func : ValRedCFuncNC) >>

This function is exactly the same as C<sys.std.Core.Set.reduction> except
that it does not take an C<identity> argument, and it results in a C<Maybe>
...
C<< function sys.std.Core.Set.Set_from_wrap (set_of.Tuple <--
$topic : Relation) >>
...
C<< function sys.std.Core.Set.Set_from_attr (Set <-- $topic : Relation,
$name : Name) >>
...
C<< function sys.std.Core.Set.Maybe.Nothing (Maybe <--) >>
...
=head2 sys.std.Core.Set.Maybe.single

C<< function sys.std.Core.Set.Maybe.single (Single <-- $value : Universal)
>>
...
C<< function sys.std.Core.Set.Maybe.attr (Universal <-- $topic : Single) >>
...
C<< function sys.std.Core.Set.Maybe.attr_or_default (Universal <--
...
C<< function sys.std.Core.Set.Maybe.attr_or_value (Universal <--
...
C<< function sys.std.Core.Set.Maybe.order (Order <-- $topic : Maybe,
$other : Maybe, $S_ord_func? : OrdDetCFuncNC, $S_is_reverse_order? : Bool,
$N_is_after_all_S : Bool, $is_reverse_order? : Bool) >>
...
C<order-determination> function given in its C<S_ord_func> argument, as
curried by its C<S_is_reverse_order> argument; to be specific, the
C<order-determination> function takes the attribute values of C<topic> and
C<other>, not the whole C<Single> values.  Otherwise, iff C<topic> is
C<Nothing>, this function results in C<Order:Increase> or C<Order:Decrease>
respectively when C<N_is_after_all_S> is C<Bool:False> (the default) or
...
underlying a SQL "ORDER BY", then C<S_is_reverse_order> designates SQL's
"ASC|DESC" and C<N_is_after_all_S> designates SQL's "NULLS FIRST|LAST".

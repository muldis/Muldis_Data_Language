...
C<[[[Func|Upd|Proc][Head|Body]]|[Scalar|Nonscalar|Union|Restr]Type]>,
one of which it is a proxy-wrapper for along with other routines and
...
routines or types, exactly 1 of which must have the empty string as
its name; that routine or type defines the actual main routine or
type that the C<Material> is a proxy-wrapper for, and the other routines
and types, if any, support it.  A C<Material> is typically
...
=item C<funcs> - C<FuncSet>

These are all the function definitions that this material contains.

=item C<upds> - C<UpdSet>

These are all the updater definitions that this material contains.
This attribute must be empty unless the material is a procedure or updater.

=item C<procs> - C<ProcSet>

These are all the procedure definitions that this material contains.
...
These are all the scalar|nonscalar|union|restriction type definitions
...
The default value of C<Material> is as per C<RestrType>.

=head2 sys.std.Core.Type.Cat.[Func|Upd|Proc]Set

An C<[Func|Upd|Proc]Set> is a C<DHRelation> that specifies a set of
functions|updaters|procedures that a function|updater|procedure might
contain.

An C<[Func|Upd|Proc]Set> has these 4 attributes:
...
This defines the entire function|updater|procedure heading, that is
...
An C<[Func|Upd|Proc]Set> has a unary primary key on the C<name>
attribute.  Its default value is empty.

=head2 sys.std.Core.Type.Cat.[Sca|Nonsca|Uni|Restr]TypeSet

An C<[Sca|Nonsca|Uni|Restr]TypeSet> is a C<DHRelation> that specifies
a set of scalar|nonscalar|union|restriction types that a
function|updater|procedure or scalar|nonscalar|union|restriction type might
contain.

An C<[Sca|Nonsca|Uni|Restr]TypeSet> has these 3 attributes:
...
An C<[Sca|Nonsca|Uni|Restr]TypeSet> has a unary primary key on the

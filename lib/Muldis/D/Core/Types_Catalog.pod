...
This is an enumeration data type.  The C<RoundMeth> (rounding method)
...
C<ToCeiling>, C<ToZero>, C<ToInf>.  (Note: There are actually more rounding
methods in the world, so this list may be expanded later.)  When a value
...
infinity from it, respectively.  The default value of C<RoundMeth> is
C<HalfEven>, since in general that should be the most likely to minimize
the rounding error from a sequence of operations that each round, which is
especially useful in contexts where a rounding method is implicit.  The
C<RoundMeth> type does I<not> have a default ordering algorithm.

=head2 sys.std.Core.Type.Cat.RoundMeth.*

There are exactly 7 types having
C<sys.std.Core.Type.Cat.RoundMeth.*>-format names and each one is a

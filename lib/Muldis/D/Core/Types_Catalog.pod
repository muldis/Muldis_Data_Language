...
set of system-defined functions|updaters|procedures.  (Note that
C<SysProcedureSet> also specifies the system-defined system-services.)
...
A C<Procedure> has these 6 attributes, of which the 3 C<upd_params>,
C<ro_params>, C<opt_params> define the I<heading> and the 2 C<vars>,
C<stmt> define the I<body>:
...
(or system-service) with specific arguments.  The invoked procedure can be

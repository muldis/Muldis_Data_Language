...
=item C<body> - C<maybe_of.[Func|Upd|Proc]Body>

This defines the entire function|updater|procedure body, that is its
implementation, besides what C<head> defines.  Note that it is not
mandatory for a system-defined routine to have a specified body (just a
specified heading is mandatory), and often it won't; but often it will,
especially if it is a function used in the definition of a system-defined
data type.  Iff a body is specified, then C<body> is a C<Single> whose sole
element defines the entire body; otherwise a body is not specified, and
C<body> is C<nothing>.
...
=item C<type> - C<[Scalar|Nonscalar|Union|Subset]Type>
...
An C<[Func|Upd|Proc]Set> has a binary primary key on the C<parent> plus
C<name> attributes.  Its default value is empty.

...
=item C<[scalar|nonscalar|domain|subset]_types> -
C<Sys[Scalar|Nonscalar|Domain|Subset]TypeSet>
...
nonscalar types, and domain types, and subset types.
...
=head2 sys.std.Core.Type.Cat.Sys[Scalar|Nonscalar|Domain|Subset]TypeSet

A C<Sys[Scalar|Nonscalar|Domain|Subset]TypeSet> is a C<DHRelation> that
specifies a set of scalar|nonscalar|domain|subset system-defined types.

A C<Sys[Scalar|Nonscalar|Domain|Subset]TypeSet> has these 4 attributes:
...
This is the fully-qualified name, in the nameless global root namespace, of
the scalar|nonscalar|domain|subset type's parent system namespace.

=item C<name> - C<Name>

This is the declared name of the scalar|nonscalar|domain|subset type
...
scalar|nonscalar|domain|subset type as a whole.

=item C<material> - C<[Scalar|Nonscalar|Domain|Subset]Type>

This defines the entire scalar|nonscalar|domain|subset type sans its name.

=back

A C<Sys[Scalar|Nonscalar|Domain|Subset]TypeSet> has a binary primary key on
...
=item C<[scalar|nonscalar|domain|subset]_types> -
C<[Scalar|Nonscalar|Domain|Subset]TypeSet>

These are all the definitions that this [|sub]depot contains of scalar
types with possreps, complete nonscalar types, domain types, and subset
types.
...
=head2 sys.std.Core.Type.Cat.[Scalar|Nonscalar|Domain|Subset]TypeSet

An C<[Scalar|Nonscalar|Domain|Subset]TypeSet> is a C<DHRelation> that
specifies a set of scalar|nonscalar|domain|subset types that a
[|sub]depot might directly contain.  I<TODO: each type may be either public
for the DBMS as a whole or private to the subdepot.>

An C<[Scalar|Nonscalar|Domain|Subset]TypeSet> has these 4 attributes:
...
This is the fully-qualified name, in the C<nlx.[cat|lib|data]> namespace
of any hypothetical immediate child namespace of the depot,
of the scalar|nonscalar|domain|subset type's parent [|sub]depot.
...
An C<[Scalar|Nonscalar|Domain|Subset]TypeSet> has a binary primary key on
the C<parent> plus C<name> attributes.  Its default value is empty.
...
These represent the elements, each of which is a continuous sub-interval,
of the possibly-discontinuous-or-empty interval value being selected.
When this value expression is evaluated, if any child expression nodes are
such that any duplicate tuples might be input to this C<IvlSelExprNodeSet>
selector, the duplicates are silently eliminated and do not constitute a
failure condition.
...
An C<DomainType> has these 7 attributes:
...
This matches the declared name of a C<named-value> function that results in
the default value of the domain type; it has zero parameters and its result
type is the same as the domain type whose declaration it is part of.

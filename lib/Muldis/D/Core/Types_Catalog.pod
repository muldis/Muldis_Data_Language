...
This document is Muldis::D::Core::Types_Catalog version 0.106.0.
...
C<Rat>, C<Instant>, C<Duration>, C<String>, C<Name>, C<NameChain>,
...
C<sys.std.Core.Type.Cat> namespace, just C<String>.  These
...
A C<Procedure> has these 8 attributes, of which the 5 C<upd_params>,
C<ro_params>, C<opt_params>, C<is_recipe>, C<is_system_service> define the
...
be invoked first unconditionally and which constitutes its own child
transaction; the child transaction will commit if the procedure completes
its execution normally and it will roll back if the procedure completes
abnormally by throwing an exception; any thrown exception will be caught.

...
This document is Muldis::D::Core::Types_Catalog version 0.79.1.
...
defines the entire (main) body of a C<named_value> function that
...
implicit function heading that this goes with has at least 3 parameters;
the first two required parameters, named C<topic> and C<other>, each have
declared types that are the same as the scalar root type whose declaration
the function is part of, and this function is said root type's type-default
ordering function; the third required parameter, named C<is_reverse_order>,
is C<Bool>-typed; the function's implicit result type is C<Order>.  Iff
...
This defines the entire (main) body of a C<named_value> function that
...
C<ImpRef> that is a reference to an updater.  Its default value is the same
as that of C<ImpRef>.

...
element is an integer.  A C<String> subtype is normally composed into any
...
the 2 integer end-points of the inclusive range that all its values'
integer element values are in.  The C<String> type has a default ordering
...
C<Int>, C<String>, C<Tuple>, C<Relation>, C<External>.  These
constitute all 4 Muldis D primitive types, all 1 Muldis D external types,

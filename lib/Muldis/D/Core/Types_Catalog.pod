...
This document is Muldis::D::Core::Types_Catalog version 0.112.0.
    ...
    sys.std.Core.Type.Universal
        ...
        sys.std.Core.Type.Relation
            sys.std.Core.Type.DHRelation
                ...
                sys.std.Core.Type.Cat.Sys[Function|Updater|Procedure]Set
                ...
                sys.std.Core.Type.Cat.[Function|Updater|Procedure]Set
                ...
                sys.std.Core.Type.DHArray
                    ...
                    # The following are all array types.
...
These system-defined subtypes can not in general be used as components of a
system catalog, or any database, but they may be used temporarily at
runtime:
...
This is an enumeration data type.  The C<OVLScalar> type is explicitly
...
"opaque value literal" syntax for:  C<Bool>, C<Int>, C<Blob>, C<Text>,
C<Rat>, C<Name>, C<NameChain>, C<Comment>, C<Order>,
C<RatRoundMeth>, C<RatRoundRule>, C<TAIInstant>, C<TAIDuration>,
...
This is an enumeration data type.  C<PNSQNameChain> (primary namespace
...
This is an enumeration data type.  C<MaterialNC> is a proper subtype of
...
This is an enumeration data type.  C<AbsPathMaterialNC> is a proper subtype
...
This is an enumeration data type.  C<APNonTypeNC> is a proper subtype of
...
This is an enumeration data type.  C<APFunctionNC> is a non-proper subtype
of C<APNonTypeNC>.  Its default value is a reference to the
C<sys.std.Core.Universal.is_identical> function.

=head2 sys.std.Core.Type.Cat.APUpdaterNC

This is an enumeration data type.  C<APUpdaterNC> is a non-proper subtype
of C<APNonTypeNC>.  Its default value is a reference to the
C<sys.std.Core.Universal.assign> updater.

=head2 sys.std.Core.Type.Cat.APProcedureNC

This is an enumeration data type.  C<APProcedureNC> is a non-proper subtype
of C<APNonTypeNC>.  Its default value is a reference to the
C<sys.std.Core.Cat.noop> procedure.

=head2 sys.std.Core.Type.Cat.APTypeNC

This is an enumeration data type.  C<APTypeNC> is a proper subtype of
...
This is an enumeration data type.  C<RelPathMaterialNC> is a proper subtype
of C<MaterialNC> where every member value's chain starts with either C<sys>
...
I<Conjecture:  Subtypes like C<[Abs|Rel]PathCataNC> might also be defined
later if we have some situation where such a restriction might be useful.>

=head2 sys.std.Core.Type.Cat.LexDataNC

This is an enumeration data type.  C<LexDataNC> is a proper subtype of
C<DataNC> where every member value's chain starts with C<lex>.  One is used
for every data reference by a function or updater.
...
This is an enumeration data type.  The C<Order> (order determination) type
...
names and each one is a structure data type; for the rest of this
...
This is an enumeration data type.  The C<RatRoundMeth> (rounding method)
type is explicitly defined as a union type over just these 7 singleton
types having C<sys.std.Core.Type.Cat.RatRoundMeth.*>-format names:
C<HalfDown>, C<HalfUp>, C<HalfEven>, C<ToFloor>,
C<ToCeiling>, C<ToZero>, C<ToInf>.  When a value
...
that Muldis D operators can typically apply.  With C<HalfDown>,
C<HalfUp>, and C<HalfEven> (aka I<unbiased rounding>, I<convergent
...
infinity, C<HalfUp> will round towards positive infinity, and C<HalfEven>
will round towards the nearest "even" target.  With C<ToFloor>,
C<ToCeiling>, C<ToZero> (aka I<truncate>), C<ToInf>, the original value
...
C<HalfUp>, since that is the most commonly used method of rounding.  The
C<RatRoundMeth> type does I<not> have a default ordering algorithm.

=head2 sys.std.Core.Type.Cat.RatRoundMeth.*

There are exactly 7 types having
C<sys.std.Core.Type.Cat.RatRoundMeth.*>-format names and each one is a
structure data type; for the rest of this description, the type name
C<RatRoundMeth.Value> will be used as a proxy for each and every one of
them.  A C<RatRoundMeth.Value> has 1 system-defined possrep whose name is
...
C<HalfUp> rounding method (its radix is 2 and its min exp is 0).  The
C<RatRoundRule> type does I<not> have a default ordering algorithm.

=head1 SIMPLE GENERIC STRING TYPES

=head2 sys.std.Core.Type.Cat.String

This is an enumeration data type.  C<String> is a proper subtype of
C<DHArray> where the declared type of its C<value> attribute is C<Int>; it
is otherwise the same.  A C<String> is a string of integers, or more
specifically it is a sequence of 0..N elements where each element is an
integer.  A C<String> subtype is normally composed into any system-defined
type that is conceptually a string of integers or bits or characters, such
as C<Blob> or C<Text>.  The C<String> type's default and canonical minimum
value is the empty sequence; its canonical maximum value is an
infinite-length sequence and practically impossible.  The cardinality of
this type is infinity; to define a most-generalized finite C<String>
subtype, you must specify a maximum length in elements that the subtype's
values can have, and you must specify the 2 integer end-points of the
inclusive range that all its values' tuples' C<value> attribute values are
in.  The C<String> type has a canonical ordering algorithm; for 2 distinct
C<String> values, their order is determined as follows:  First eliminate
any identical leading element sequences from both strings as those alone
would make the strings compare as same (if the remainder of both strings
was the empty string, then the strings are identical).  Then, iff the
remainder of just one string is the empty string, then that string is
ordered before the non-empty one; otherwise, compare the first element of
each of the string remainders according to the following ordering algorithm
to get the order of their respective strings: the C<value> attribute
integer value closer to negative infinity is ordered before the value
...
of the Unicode standard version 5.2.0, all character abstract codepoints
...
A C<System> has these 13 attributes:
...
=item C<functions|updaters|procedures> -
C<Sys[Function|Updater|Procedure]Set>
...
user-defined types.  Specifically, it declares these 8 standard types: in
the C<sys.std.Core.Type> namespace, C<Universal>, C<Empty>, C<Scalar>,
C<Int>, C<Tuple>, C<Relation>, C<Reference>, C<External>.  These
constitute all 3 Muldis D primitive types, all 1 Muldis D reference types,
all 2 Muldis D type system minimal and maximal (enumeration) types, all 2
...
=head2 sys.std.Core.Type.Cat.Sys[Function|Updater|Procedure]Set

A C<Sys[Function|Updater|Procedure]Set> is a C<DHRelation> that specifies a
set of system-defined functions|updaters|procedures.

A C<Sys[Function|Updater|Procedure]Set> has these 4 attributes:
...
=item C<material> - C<Function|Updater|Procedure>

This defines the entire function|updater|procedure sans its name.  Note
that it is not mandatory for a system-defined routine to have a specified
I<body> (just a specified I<heading> is mandatory), and often it won't; but
often it will, especially if it is a function used in the definition of a
system-defined data type.

=back

A C<Sys[Function|Updater|Procedure]Set> has a binary primary key on the
...
A C<Depot> has these 12 attributes:
...
=item C<functions|updaters|procedures> - C<[Function|Updater|Procedure]Set>
...
=head2 sys.std.Core.Type.Cat.[Function|Updater|Procedure]Set

A C<[Function|Updater|Procedure]Set> is a C<DHRelation> that specifies a
set of functions|updaters|procedures that a [|sub]depot might directly
contain.  I<TODO: each routine may be either public for the DBMS as a whole
or private to the subdepot.>

A C<[Function|Updater|Procedure]Set> has these 4 attributes:
...
This is the declared name of the function|updater|procedure within the
namespace defined by C<parent>; other Muldis D code would reference it
...
This defines the entire function|updater|procedure sans its name.

=back

A C<[Function|Updater|Procedure]Set> has a binary primary key on the
C<parent> plus C<name> attributes.  Its default value is empty.
...
=item C<implements> - C<maybe_of.RPFunctionNC>

Iff C<implements> is nonempty then this function is explicitly declaring
that it implements the other, virtual function whose name is given in its
sole element; otherwise, empty means not implementing a virtual.  An
implementing function must the same parameter list as its virtual, save
that the implementer's parameters' or result's declared types must be
subtypes of the corresponding ones of the virtual.
...
subject-to-update parameter of the expression-containing update operator.

An C<ExprNodeSet> has these 14 attributes:
...
including all system-defined types, except for values of C<Int>, although
...
=item C<implements> - C<maybe_of.RPUpdaterNC>

Iff C<implements> is nonempty then this updater is explicitly declaring
that it implements the other, virtual updater whose name is given in its
sole element; otherwise, empty means not implementing a virtual.  An
implementing updater must the same parameter list as its virtual, save
that the implementer's parameters' or result's declared types must be
subtypes of the corresponding ones of the virtual.
...
invocation of an updater.  C<MultiUpdStmt> itself has no restriction on the
...
C<MultiUpdStmt> has a distributed primary key over the C<name> attributes
of C<upd_args> and C<ro_args>.  Its default value is empty.
...
A C<Procedure> has these 11 attributes, of which the 8 C<upd_params>,
C<ro_params>, C<opt_params>, C<dispatch_params>, C<implements>,
C<is_recipe>, C<is_system_service>, C<is_transaction> define the
...
=item C<implements> - C<maybe_of.RPProcedureNC>

Iff C<implements> is nonempty then this procedure is explicitly declaring
that it implements the other, virtual procedure whose name is given in its
sole element; otherwise, empty means not implementing a virtual.  An
implementing procedure must the same parameter list as its virtual, save
that the implementer's parameters' or result's declared types must be
subtypes of the corresponding ones of the virtual.

=item C<is_recipe> - C<Bool>

Iff this is C<Bool:True> then the procedure is explicitly declared to be a
C<recipe>, meaning it will be subject to tighter constraints on its allowed
actions (such as that it may not do anything nondeterministic) and its
execution will automatically be entirely contained within a single
transaction of the highest possible isolation level, "serializable", same
as an updater is; iff this is C<Bool:False> then the procedure is I<not>
explicitly declared to be a C<recipe>, and the other restrictions or
automatic wrapper transaction won't be present for supporting a C<recipe>.
...
highest possible isolation level, "serializable", same as an updater is;
...
transaction won't be present for supporting a C<system-service>.  Note that
a procedure is forbidden from having both its C<is_recipe> and
C<is_system_service> valued with C<Bool:True> at once.
...
procedure's C<is_transaction> must be C<Bool:True> if either of its
C<is_recipe> or C<is_system_service> is C<Bool:True>; otherwise,
...
A C<Recipe> defines a C<recipe>, which is a kind of procedure.  C<Recipe>
is a proper subtype of C<Procedure> where for every member value its
C<is_recipe> attribute is C<Bool:True>.
...
A C<StmtNodeSet> has these 12 attributes:
...
C<var> (a C<DataNC>); the C<arg> is the name of the routine argument,
...
C<if> (a C<DataNC>) and C<then> (a C<Name>); C<if> has the name of a
...
attributes, C<when> (a C<DataNC>) and C<then> (a C<Name>); C<when> has

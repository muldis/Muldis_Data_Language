...
This document is Muldis::D::Core::Types_Catalog version 0.33.1.
...
A C<Cat.DbVar> has these 2 attributes:
...
would live.  For example, if public function C<dep.mymain> had an inner
function C<inn.mytest> and C<dep.myfoo> invoked the public function
C<dep.myfilter> with an argument value of C<inn.mytest>, then if
C<dep.myfilter> tried to invoke the function it was passed, it would
actually be trying to invoke an inner function of C<dep.myfilter> named
...
conceptually like C<dep.mymain.mytest>, which then C<dep.myfilter> could
correctly invoke.  However, if that was made to work literally with a

    ...
    sys.std.Core.Type.Universal
        ...
        sys.std.Core.Type.Relation
            sys.std.Core.Type.DHRelation
                ...
                sys.std.Core.Type.Cat.MaterialRefSelStmtNodeSet
...
=item C<material_ref_sel_stmts> - C<MaterialRefSelStmtNodeSet>
...
=head2 sys.std.Core.Type.Cat.MaterialRefSelStmtNodeSet

A C<MaterialRefSelStmtNodeSet> is a C<DHRelation> that specifies a set of
statement nodes where each node represents the assignment to a variable (or
parameter) of a newly selected value of the special
C<sys.std.Core.Type.Cat.MaterialRef> type.  Said selected value would not
have any pre-bound routine parameters where applicable, and those can be
added afterwards with the system-defined currying routines.  Using one of
these statement nodes should be an effective alternative to declaring (or
extending) an updater that simply combines a C<MaterialRef> selecting
expression node with an C<assign> updater call.

A C<MaterialRefSelStmtNodeSet> has these 4 attributes:
...
This is name of the local parameter or variable that will have the newly
selected C<MaterialRef> value assigned to it.

=item C<referencing> - C<NameChain>

This is the name, from the point of view of the procedure embedding this
statement node, of the routine or type that the new special reference
value is supposed to facilitate portable invoking of.

=back

A C<MaterialRefSelStmtNodeSet> has a unary (unique) key on the C<name>
attribute.  Its default value is empty.
...
TODO: HEAVILY UPDATE THIS SECTION.  OR REMOVE IT / ITS TYPES OUTRIGHT.
...
would live.  For example, if public function C<fed.lib.mydb.mymain> had an
function C<nlx.lib.mytest> and C<fed.lib.mydb.mymain> invoked the public
function
C<fed.lib.mydb.myfilter> with an argument value of C<nlx.lib.mytest>,
then if
C<fed.lib.mydb.myfilter> tried to invoke the function it was passed,
it would
actually be trying to invoke an function of C<fed.lib.mydb.myfilter> named
C<nlx.lib.mytest>, which won't work.  Conceptually, a solution to this
problem
is to actually pass a routine name in a more fully-qualified form,
conceptually like C<fed.lib.mydb.mymain.mytest>, which then
C<fed.lib.mydb.myfilter>
...
reference to C<sys> or C<fed>, the context is global; for C<nlx>
...
to define imperative versions of C<sys.std.Core.Cat.curried_func>.>
...
C<FuncRef> that is a reference to an C<order-determination> function.  Its
...
C<FuncRef> that is a reference to a C<value-filter> function.  Its default
...
C<FuncRef> that is a reference to a C<value-map> function.  Its default
...
C<FuncRef> that is a reference to a C<value-reduction> function.  Its
...
updater name.  It is used to implement a deterministic imperative closure.
Its default value is a reference to the C<sys.std.Core.Universal.assign>
updater.

=head2 sys.std.Core.Type.Cat.ProcRef

This is an enumeration data type.  C<ProcRef> is a proper subtype of
C<MaterialRef> where every member value's conceptual C<NameChain> is a
procedure name.  It is used to implement a possibly-nondeterministic

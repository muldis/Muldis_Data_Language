...
it declares these 22 standard language namespaces:
C<[sys|mnt|fed|dep|sdp|lex]> (which have the nameless global
root namespace as their parent, spelled as the empty C<DeclNameChain>
value, that isn't also declared here), C<sys.[cat|std|imp]>,
C<mnt.cat>, C<[fed|dep|sdp].[cat|lib|data]>, C<sys.std.Core>,

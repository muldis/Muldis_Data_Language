...
This document is Muldis::D::Core::Types_Catalog version 0.115.0.
...
non-core catalog types for context:
                        ...
                        sys.std.Core.Type.Cat.PNSQNameChain
                            sys.std.Core.Type.Cat.MaterialNC
                                sys.std.Core.Type.Cat.AbsPathMaterialNC
                                    sys.std.Core.Type.Cat.APNonTypeNC
                                        sys.std.Core.Type.Cat.APFunctionNC
                                        sys.std.Core.Type.Cat.APUpdaterNC
                                        sys.std.Core.Type.Cat.APRecipeNC
                                ...
                                sys.std.Core.Type.Cat.RelPathMaterialNC
                                    ...
                                    sys.std.Core.Type.Cat.RPUpdaterNC
                                    sys.std.Core.Type.Cat.RPRecipeNC
                                    sys.std.Core.Type.Cat.RPProcedureNC
                                    sys.std.Core.Type.Cat.RPTypeNC
                            sys.std.Core.Type.Cat.DataNC
                                sys.std.Core.Type.Cat.LexDataNC
    ...
    sys.std.Core.Type.Universal
        ...
        sys.std.Core.Type.Tuple
            sys.std.Core.Type.DHTuple
                ...
                sys.std.Core.Type.Cat.Function
                    ...
                    sys.std.Core.Type.Cat.TransConstrFunc
        ...
        sys.std.Core.Type.Relation
            sys.std.Core.Type.DHRelation
                ...
                sys.std.Core.Type.Cat.Sys[Function|Updater|Recipe|Procedure]Set
                sys.std.Core.Type.Cat.SysSpecTypeSet
                sys.std.Core.Type.Cat.Sys[Scalar|Tuple|Relation]TypeSet
                sys.std.Core.Type.Cat.Sys[Domain|Subset|Mixin]TypeSet
                ...
                sys.std.Core.Type.Cat.[Function|Updater|Recipe|Procedure]Set
                ...
                sys.std.Core.Type.Cat.MultiUpdInvoStmt
                sys.std.Core.Type.Cat.RcpGlobalVarAliasMap
                sys.std.Core.Type.Cat.MultiRcpInvoStmt
                sys.std.Core.Type.Cat.OVLScaValStmtNodeSet
                sys.std.Core.Type.Cat.LeaveStmtNodeSet
                sys.std.Core.Type.Cat.CompoundStmtNodeSet
                sys.std.Core.Type.Cat.FuncInvoStmtNodeSet
                sys.std.Core.Type.Cat.UpdInvoStmtNodeSet
                sys.std.Core.Type.Cat.RcpInvoStmtNodeSet
                ...
                sys.std.Core.Type.Cat.APMaterialNCSelStmtNodeSet
...
used temporarily at runtime:
    ...
    sys.std.Core.Type.Universal
        sys.std.Core.Type.Tuple
            ...
            sys.std.Core.Type.Cat.CurriedFuncNC
                sys.std.Core.Type.Cat.ValMapCFuncNC
                    sys.std.Core.Type.Cat.ValFiltCFuncNC
                sys.std.Core.Type.Cat.ValRedCFuncNC
                sys.std.Core.Type.Cat.OrdDetCFuncNC
...
=head2 sys.std.Core.Type.Cat.OVLScalar

The C<OVLScalar> type is explicitly
defined as a union type over all system-defined C<DHScalar> types, and just
...
C<RoundMeth>, C<RatRoundRule>, C<Singleton>, C<TAIInstant>, C<TAIDuration>,
C<UTCInstant>, C<FloatInstant>, C<UTCDuration>.  The C<OVLScalar> type is
...
of possrep attributes.  The C<OVLScalar> type is I<not> intended to be used
...
element is an C<Int>.  A C<String> subtype is normally composed into any
...
element values are in.  The C<String> type has a default ordering
...
represent a string of bits.  Considering the low-level type system, for
every C<BString> value, its second C<Structure> element is the C<Int> value
C<-1> (bit string).
...
represent a string of octets.  Considering the low-level type system, for
every C<OString> value, its second C<Structure> element is the C<Int> value
C<-2> (octet string).

=head2 sys.std.Core.Type.Cat.UCPString

C<UCPString> (Unicode abstract codepoint string) is a proper subtype of
C<String> where all member value elements are between zero and F;D7FF
inclusive or F;E000 and F;10FFFF inclusive.  As of the Unicode standard
version 5.2.0, all character abstract codepoints exist within that range,
and so each element could be used to represent any Unicode character and
the whole string can be used to represent a string of Unicode characters.
Considering the low-level type system, for every C<UCPString> value, its
second C<Structure> element is the C<Int> value C<-4> (text with Unicode
5.2.0 canonical character repertoire).  I<Support in Muldis D for the
Unicode compatibility repertoire, other character repertoires is pending.>
...
such as depot entities grouped in a
subdepot hierarchy.  A C<NameChain> is conceptually a sequence of
...
C<nlx[.par]**0..*.[cat|lib|data]>, C<rtn>, C<lex>, C<type>.  I<The
...
=head2 sys.std.Core.Type.Cat.APNonTypeNC

C<APNonTypeNC> is a proper subtype of
...
C<APFunctionNC> is a non-proper subtype
of C<APNonTypeNC>.  Its default value is a reference to the
C<sys.std.Core.Universal.is_identical> function.

=head2 sys.std.Core.Type.Cat.APUpdaterNC

C<APUpdaterNC> is a non-proper subtype
of C<APNonTypeNC>.  Its default value is a reference to the
C<sys.std.Core.Universal.assign> updater.

=head2 sys.std.Core.Type.Cat.APRecipeNC

C<APRecipeNC> is a non-proper subtype of C<APNonTypeNC>.  Its default value
is a reference to the C<sys.std.Core.Cat.fail> recipe.

=head2 sys.std.Core.Type.Cat.APProcedureNC

C<APProcedureNC> is a non-proper subtype
of C<APNonTypeNC>.  Its default value is a reference to the
C<sys.std.Core.Cat.noop> procedure.

=head2 sys.std.Core.Type.Cat.APTypeNC

C<APTypeNC> is a proper subtype of
C<AbsPathMaterialNC> that excludes the C<rtn> value and a subset of the
C<sys>-prefix values.  Its default value is a reference to the
C<sys.std.Core.Type.Universal> data type, the same as its parent type and
therefore exists soley for use in better-self-documenting code that expects
a reference to a type specifically.
...
invoked directly when both the invoker and invoked are in the same depot.

=head2 sys.std.Core.Type.Cat.RPFunctionNC

C<RPFunctionNC> is a non-proper subtype
of C<RelPathMaterialNC> and is analogous to the C<APFunctionNC> type.

=head2 sys.std.Core.Type.Cat.RPUpdaterNC

C<RPUpdaterNC> is a non-proper subtype
of C<RelPathMaterialNC> and is analogous to the C<APUpdaterNC> type.

=head2 sys.std.Core.Type.Cat.RPRecipeNC

C<RPRecipeNC> is a non-proper subtype
of C<RelPathMaterialNC> and is analogous to the C<APRecipeNC> type.

=head2 sys.std.Core.Type.Cat.RPProcedureNC

C<RPProcedureNC> is a non-proper subtype
of C<RelPathMaterialNC> and is analogous to the C<APProcedureNC> type.

=head2 sys.std.Core.Type.Cat.RPTypeNC

C<RPTypeNC> is a non-proper subtype of
C<RelPathMaterialNC> and is analogous to the C<APTypeNC> type.
...
C<nlx[.par]**0..*.[cat|data]>, C<lex>.  One can be used to reference a
...
value is a reference to a lexical parameter or variable named C<lex.topic>.
...
=head2 sys.std.Core.Type.Cat.LexDataNC

C<LexDataNC> is a proper subtype of C<DataNC> where every member value's
chain starts with C<lex>.  One is used for every data reference by a
routine, except when defining a global-variable-alias in a recipe.
...
C<Increase>, C<Same>, C<Decrease>.  When some context
...
C<Increase> is ordered before C<Same>, and C<Same> before C<Decrease>.
...
A C<System> has these 14 attributes:

=over

=item C<scm_comment> - C<single_of.Comment>
...
it declares these 19 standard language namespaces:
C<[sys|mnt|fed|nlx|rtn|lex]> (which have the nameless global
root namespace as their parent, spelled as the empty C<NameChain>
value, that isn't also declared here), C<sys.[cat|std|imp]>,
C<mnt.cat>, C<[fed|nlx].[cat|lib|data]>, C<sys.std.Core>,
...
=item C<functions|updaters|recipes|procedures> -
C<Sys[Function|Updater|Recipe|Procedure]Set>

These are the interfaces, or definitions in some cases, of all the
system-defined functions|updaters|recipes|procedures.
...
user-defined types.  Specifically, it declares these 4 standard types: in
the C<sys.std.Core.Type> namespace: C<Universal>, C<Empty>, C<Int>; in the
C<sys.std.Core.Type.Cat> namespace: C<List>.  These constitute all 2 Muldis
D declaration types, all 2 Muldis D type system maximal and minimal
(enumeration) types.  Note that, while conceptually both C<Universal> and
C<Empty> I<can> be defined like user-defined types, as the union or
intersection of C<Int> and C<List>, respectively, in practice that wouldn't
work for C<Empty> at least because all user-defined types are required to
have a default value and it can't, and C<Universal> is also kept here at
least to balance C<Empty>; actually, the maximal and minimal types are used
in many places as default semantics in non-special type definitions.

=item C<[scalar|tuple|relation|domain|subset|mixin]_types> -
C<Sys[Scalar|Tuple|Relation|Domain|Subset|Mixin]TypeSet>

These are all the system-defined scalar types, except for C<Int> because it
can't be defined like user-defined types, and complete tuple and relation
types, and domain types, and subset types, and mixin types.
...
A C<SysNspSet> has these 4 attributes:
...
=head2 sys.std.Core.Type.Cat.Sys[Function|Updater|Recipe|Procedure]Set

A C<Sys[Function|Updater|Recipe|Procedure]Set> is a C<DHRelation> that
specifies a set of system-defined functions|updaters|recipes|procedures.

A C<Sys[Function|Updater|Recipe|Procedure]Set> has these 5 attributes:
...
This is the fully-qualified name, in the nameless global root namespace, of
the function|updater|recipe|procedure's parent system namespace.

=item C<name> - C<Name>

This is the declared name of the function|updater|recipe|procedure within
...
function|updater|recipe|procedure as a whole.
...
=item C<material> - C<Function|Updater|Recipe|Procedure>

This defines the entire function|updater|recipe|procedure sans its name.
...
A C<Sys[Function|Updater|Recipe|Procedure]Set> has a binary primary key on
...
A C<SysSpecTypeSet> has these 4 attributes:
...
=head2 sys.std.Core.Type.Cat.Sys[Scalar|Tuple|Relation|Domain|Subset|Mixin]TypeSet

A C<Sys[Scalar|Tuple|Relation|Domain|Subset|Mixin]TypeSet> is a
C<DHRelation> that specifies a set of
scalar|tuple|relation|domain|subset|mixin system-defined types.

A C<Sys[Scalar|Tuple|Relation|Domain|Subset|Mixin]TypeSet> has these 5
attributes:
...
This is the fully-qualified name, in the nameless global root namespace, of
the scalar|tuple|relation|domain|subset|mixin type's parent system
namespace.
...
A C<Sys[Scalar|Tuple|Relation|Domain|Subset|Mixin]TypeSet> has a binary
...
A C<SysCatSet> has these 5 attributes:
...
=item C<details> - C<OVLScaValExprNodeSet>
...
the root.  The system catalog variable named C<nlx.cat> is of the C<Depot>
type.

A C<Depot> has these 13 attributes:
...
attributes of all 8 of a C<Depot>'s main C<DHRelation>-typed attributes.
...
A C<SubdepotSet> has these 4 attributes:
...
of any hypothetical immediate child namespace of the depot,
...
This is the visible order of this subdepot's declaration relative to all of
the named entities directly within the namespace defined by C<parent>.
...
=head2 sys.std.Core.Type.Cat.[Function|Updater|Recipe|Procedure]Set

A C<[Function|Updater|Recipe|Procedure]Set> is a C<DHRelation> that
specifies a set of functions|updaters|recipes|procedures that a [|sub]depot
might directly contain.  I<TODO: each routine may be either public for the
DBMS as a whole or private to the subdepot.>

A C<[Function|Updater|Recipe|Procedure]Set> has these 5 attributes:

=over

=item C<parent> - C<NameChain>

This is the fully-qualified name, in the C<nlx.[cat|lib|data]> namespace of
any hypothetical immediate child namespace of the depot, of the
function|updater|recipe|procedure's parent [|sub]depot.
...
A C<[Function|Updater|Recipe|Procedure]Set> has a binary primary key on the
...
that a [|sub]depot might directly contain.  I<TODO: each type may be either
public for the DBMS as a whole or private to the subdepot.>
...
This is the fully-qualified name, in the C<nlx.[cat|lib|data]> namespace
of any hypothetical immediate child namespace of the depot,
of the scalar|tuple|relation|domain|subset|mixin type's parent [|sub]depot.
...
C<Function>, which is either a C<Depot> or C<System>.  Every C<Function>
...
must match a parameter name in C<params>.
...
=head2 sys.std.Core.Type.Cat.TransConstrFunc

A C<TransConstrFunc> defines a C<transition-constraint>, which is a kind of
function.  C<TransConstrFunc> is a proper subtype of C<Function> where all
member values declare a function that is binary / has exactly 2 parameters,
and those 2 are named C<before> and C<after>, and the declared types of
those 2 parameters are identical, and the declared type of the function's
result is C<Bool>.  Its default value is a function whose invocation
unconditionally results in C<Bool:True>.
...
function|updater|recipe.  Each tuple of an C<ExprNodeSet> attribute is a
...
Only functions and updaters and recipes may contain expression
...
named functions or updaters or recipes.  Note that, while the
...
happens in updaters or recipes where the expression is used as an argument
for a subject-to-update parameter of an updater or recipe call; in
that case the leaf nodes / only node of the expression must map to a
subject-to-update parameter of the expression-containing updater or recipe.

An C<ExprNodeSet> has these 15 attributes:

=over

=item C<ovl_sca_val_exprs> - C<OVLScaValExprNodeSet>
...
expression node is declared directly within its host atomic routine's body,
...
when generating concrete Muldis D code from a C<Function|Updater|Recipe>,
...
=head2 sys.std.Core.Type.Cat.OVLScaValExprNodeSet

An C<OVLScaValExprNodeSet> is a C<DHRelation> that specifies a set of value
...
An C<OVLScaValExprNodeSet> has these 4 attributes:
...
=item C<value> - C<OVLScalar>

This is the actual scalar value that the expression node represents.

=back

An C<OVLScaValExprNodeSet> has a unary primary key on the C<name>
...
optimized Muldis D code will likely use C<OVLScaValExprNodeSet> where it
...
expression node, defined in terms of a C<LexDataNC>.
...
An C<IfElseExprNodeSet> has these 5 attributes:
...
An C<Updater> is a C<DHTuple>.  It defines a new updater, which has 2
main parts, called I<heading> and I<body>:  The I<heading> defines the
updater's entire public interface, which is all the details of how to use
it, except for its name, and no more detail than
necessary about how it is implemented.  The I<body> defines the updater's
entire implementation (or the main body of an updater), besides its
name/etc and what the I<heading> defines.  The updater's name is
provided by the larger context that embeds the
C<Updater>, which is either a C<Depot> or C<System>.  Every C<Updater>
must have a specified I<heading>, but having a specified I<body> is
optional iff the C<Updater> is embedded in a C<System>, because often the
implementations of system-defined routines are not defined in terms of
other Muldis D routines.

An C<Updater> has these 8 attributes, of which the 5 C<upd_params>,
C<ro_params>, C<opt_params>, C<dispatch_params>, C<implements> define the
I<heading> and the 2 C<exprs>, C<stmt> define the I<body>:
...
Iff C<dispatch_params> is nonempty then this updater is a virtual
updater; otherwise, empty means not virtual.  A virtual updater must
have no I<body> specified.  This attribute indicates the subset of the
updater's parameters whose invocation arguments' types are consulted to
determine which other updater, that explicitly implements this virtual
one, is automatically dispatched to.  Each element of C<dispatch_params>
must match a parameter name in C<upd_params> or C<ro_params>.

=item C<implements> - C<set_of.RPUpdaterNC>

Iff C<implements> is nonempty then this updater is explicitly declaring
that it implements the other (typically just one), virtual updaters named
by its elements; otherwise, empty means not implementing any virtuals.  An
implementing updater must have the same parameter list as its virtuals,
...
=item C<stmt> - C<MultiUpdInvoStmt>
...
Iff an C<Updater> has no specified I<body>, then C<exprs> must have zero
member nodes and C<stmt> must have zero member tuples; otherwise, C<stmt>
must have at least 1 member tuple.

C<Updater> has a distributed primary key over the C<name> attributes of
C<upd_params> and C<ro_params> and the C<name> attributes of all the
attributes of C<exprs>.  Its default value has 1 subject-to-update,
non-optional parameter whose name is C<topic> and whose declared type is
C<Bool>; it has zero read-only parameters; it has no specified I<body>.

=head2 sys.std.Core.Type.Cat.MultiUpdInvoStmt

A C<MultiUpdInvoStmt> is a C<DHRelation> that specifies a multi-update
...
accessed via the host updater's or recipe's subject-to-update parameters
(or via the host updater's lexical-alias variables).  All
arbitrarily complex Muldis D value assignments, including relational
assignments, are composed of just C<MultiUpdInvoStmt>, either directly, or
indirectly by way of updater or recipe invocations, as each updater and
recipe body is itself composed entirely of 1 multi-update statement (plus
supporting value expressions).  Only updaters and recipes may contain
C<MultiUpdInvoStmt>, so for any procedures that would conceptually include
them, those portions of the procedures need to be separated out and
encapsulated by named updaters or recipes.  Each tuple of a multi-update
statement is a single update statement, which represents a single
invocation of an updater.  C<MultiUpdInvoStmt> itself has no restriction on
the
number of tuples it consists of, but:  At least 1 tuple is required iff the
multi-update statement is composed into a specified updater body; otherwise
there must be only be zero tuples for an unspecified updater body.

A C<MultiUpdInvoStmt> has these 5 attributes:
...
This is the visible order of this single update statement relative to all
of its sibling statements, or non-nested expression nodes, of its host
updater's or recipe's multi-update statement.

=item C<updater> - C<MaterialNC>
...
statement-containing-updater subject-to-update parameters; in the most
trivial (and common) case, the parameter is referenced directly.
...
C<MultiUpdInvoStmt> has a distributed primary key over the C<name>
attributes of C<upd_args> and C<ro_args>.  Its default value is empty.

=head2 sys.std.Core.Type.Cat.Recipe

A C<Recipe> is a C<DHTuple>.  It defines a new recipe, which has 2 main
parts, called I<heading> and I<body>:  The I<heading> defines the recipe's
entire public interface, which is all the details of how to use it, except
for its name, and no more detail than necessary about how it is
implemented.  The I<body> defines the recipe's entire implementation (or
the main body of an recipe), besides its name/etc and what the I<heading>
defines.  The recipe's name is provided by the larger context that embeds
the C<Recipe>, which is either a C<Depot> or C<System>.  Every C<Recipe>
must have a specified I<heading>, but having a specified I<body> is
optional iff the C<Recipe> is embedded in a C<System>, because often the
implementations of system-defined routines are not defined in terms of
other Muldis D routines.

A C<Recipe> has these 11 attributes, of which the 7 C<upd_params>,
C<ro_params>, C<opt_params>, C<dispatch_params>, C<implements>,
C<upd_globals>, C<ro_globals> define the I<heading> and the 3 C<exprs>,
C<stmt_part_upd>, C<stmd_part_rcp> define the I<body>:

=over

=item C<scm_comment> - C<Comment>

This is an optional programmer comment about the recipe as a whole.

=item C<upd_params> - C<NameTypeMap>

This is the declared subject-to-update parameter list of the recipe, which
has 0..N named and typed such parameters.

=item C<ro_params> - C<NameTypeMap>

This is the declared read-only parameter list of the recipe, which has
0..N named and typed such parameters.

=item C<opt_params> - C<set_of.Name>

This is as per the same-named attribute of C<Updater>.

=item C<dispatch_params> - C<set_of.Name>

Iff C<dispatch_params> is nonempty then this recipe is a virtual
recipe; otherwise, empty means not virtual.  A virtual recipe must
have no I<body> specified.  This attribute indicates the subset of the
recipe's parameters whose invocation arguments' types are consulted to
determine which other recipe, that explicitly implements this virtual
one, is automatically dispatched to.  Each element of C<dispatch_params>
must match a parameter name in C<upd_params> or C<ro_params>.

=item C<implements> - C<set_of.RPRecipeNC>

Iff C<implements> is nonempty then this recipe is explicitly declaring
that it implements the other (typically just one), virtual recipes named
by its elements; otherwise, empty means not implementing any virtuals.  An
implementing recipe must have the same parameter list as its virtuals,
save that the implementer's parameters' declared types must be
subtypes of the corresponding ones of the virtuals.

=item C<upd_globals> - C<RcpGlobalVarAliasMap>

This declares 0..N lexical aliases for global variables which will
serve as implicit subject-to-update parameters of the recipe.

=item C<ro_globals> - C<RcpGlobalVarAliasMap>

This declares 0..N lexical aliases for global variables which will
serve as implicit read-only parameters of the recipe.
...
=item C<stmt_part_upd> - C<MultiUpdInvoStmt>

This defines one of two parts of the entire body of the recipe, which is
composed of a single multi-update statement; C<stmt_part_upd> specifies any
invocations of updaters.  This multi-update statement must reference all of
the parameters that the recipe has.

=item C<stmt_part_rcp> - C<MultiRcpInvoStmt>

This defines the other of two parts of the entire body of the recipe,
specifically any invocations of recipes.

=back

Iff an C<Recipe> has no specified I<body>, then C<exprs> must have zero
member nodes and C<stmt> must have zero member tuples; otherwise, C<stmt>
must have at least 1 member tuple.

C<Recipe> must have at least one subject-to-update parameter or
subject-to-update lexical-alias variable.

C<Recipe> has a distributed primary key over the C<name> attributes of
C<upd_params> and C<ro_params> and C<upd_globals> and C<ro_globals> and the
C<name> attributes of all the attributes of C<exprs>.  Its default value
has zero parameters and has no specified I<body>.

=head2 sys.std.Core.Type.Cat.RcpGlobalVarAliasMap

A C<RcpGlobalVarAliasMap> is a C<DHRelation>.  It defines a set of lexical
variable names, with a declared global variable for each.  It is used to
define lexical variables of recipes that are aliases for global variables,
for reading or updating.  A C<RcpGlobalVarAliasMap> has 4 attributes,
C<name> (a C<Name>), C<global> (a C<DataNC>), C<scm_comment> (a
...
=head2 sys.std.Core.Type.Cat.MultiRcpInvoStmt

A C<MultiRcpInvoStmt> is a C<DHRelation> that specifies a multi-update
statement that is composed into a specified recipe body.  It is the same as
a C<MultiUpdInvoStmt> except that tuple of a multi-update statement
represents a single invocation of a recipe rather than an updater.

A C<MultiRcpInvoStmt> has these 5 attributes:
...
=item C<recipe> - C<MaterialNC>

This is the name of the recipe being invoked.

=item C<upd_args> - C<NameExprMap>

These are the 0..N subject-to-update arguments to the recipe invocation,
as-per C<ro_args>.  But since each expression tree in C<upd_args> is
binding to a subject-to-update parameter, the expression tree actually is
defining a pseudo-variable / virtual-variable over 1..N
statement-containing-recipe subject-to-update parameters or lexical
variable-aliases; in the most trivial (and common) case, the parameter or
lexical variable-alias is referenced directly.

=item C<ro_args> - C<NameExprMap>

These are the 0..N read-only arguments to the recipe invocation.  Each
element defines one argument value, with the element C<name> matching the
invoked routine's parameter name, and the element C<expr> naming a local
expression node (or parameter or lexical variable-alias) which defines the
value.

=back

C<MultiRcpInvoStmt> has a distributed primary key over the C<name>
attributes of C<upd_args> and C<ro_args>.  Its default value is empty.
...
provided by the larger context that embeds the
C<Procedure>, which is either a C<Depot> or C<System>.  Every C<Procedure>
must have a specified I<heading>, but having a specified I<body> is
optional iff the C<Procedure> is embedded in a C<System>, because often the
implementations of system-defined routines are not defined in terms of
other Muldis D routines.

A C<Procedure> has these 10 attributes, of which the 7 C<upd_params>,
C<ro_params>, C<opt_params>, C<dispatch_params>, C<implements>,
C<is_system_service>, C<is_transaction> define the
I<heading> and the 2 C<vars>, C<stmt> define the I<body>:
...
allowed actions (it may not see any global variables) and its execution
...
Iff a C<Procedure> has no specified I<body>, then C<stmt> must have zero
member nodes and C<vars> must have zero member tuples; otherwise, C<stmt>
must have at least 1 member node.  (If you actually want to have a
procedure with a specified body that is a no-op, there are a variety of
single statement bodies that could do it, such as using a "leave" statement
node as the root node, or such as by invoking the C<sys.std.Core.Cat.noop>
procedure; the former is simpler.)
...
C<upd_params> and C<ro_params> and C<vars> and the C<name> attributes of
...
A C<StmtNodeSet> has these 13 attributes:

=over

=item C<ovl_sca_val_stmts> - C<OVLScaValStmtNodeSet>

These are statement nodes that represent the assignment to local variables
of newly selected scalar values of types such that all of the standard
Muldis D dialects provide special "opaque value literal" syntax specific to
the type.
...
=item C<func_invo_stmts> - C<FuncInvoStmtNodeSet>

These are statement nodes that represent function invocations and the
assignment to local variables of the results of those invocations.

=item C<upd_invo_stmts> - C<UpdInvoStmtNodeSet>

These are statement nodes that represent updater invocations.

=item C<rcp_invo_stmts> - C<RcpInvoStmtNodeSet>

These are statement nodes that represent recipe invocations.
...
=item C<ap_material_nc_sel_stmts> - C<APMaterialNCSelStmtNodeSet>

These are statement nodes that represent the assignment to local variables
of newly selected routine or type reference values.
...
=head2 sys.std.Core.Type.Cat.OVLScaValStmtNodeSet

An C<OVLScaValStmtNodeSet> is a C<DHRelation> that specifies a set of
atomic statement nodes where each node represents the assignment to a
variable (or parameter) of a hard-coded scalar literal that is I<not> being
specified explicitly in terms of possrep attributes, but rather is
specified using special "opaque value literal" syntax that all of the
Muldis D standard dialects provide.  Using one of these statement nodes
should be an effective alternative to either declaring a niladic function
or declaring (or extending) an updater that simply combines a simple scalar
selecting expression node with an C<assign> updater call.

An C<OVLScaValStmtNodeSet> has these 4 attributes:
...
This is name of the local parameter or variable that will have the newly
selected scalar value assigned to it.

=item C<value> - C<OVLScalar>

This is the actual scalar value that the statement node is selecting.

=back

An C<OVLScaValStmtNodeSet> has a unary primary key on the C<name>
attribute.  Its default value is empty.
...
Alternately, a C<LeaveStmtNodeSet> node may name itself as the node to exit
as a form of a no-op.
...
=head2 sys.std.Core.Type.Cat.FuncInvoStmtNodeSet

A C<FuncInvoStmtNodeSet> is a C<DHRelation> that specifies a set of atomic
statement nodes where each node represents the assignment to a variable (or
parameter) of the result of an invocation of a function with specific
arguments.  The invoked function can be either user-defined or
system-defined.  Using one of these statement nodes should be an effective
alternative to declaring (or extending) an updater that simply combines a
single function-invoking expression node (whose arguments are all just the
updater's own arguments) with an C<assign> updater call.

A C<FuncInvoStmtNodeSet> has these 5 attributes:
...
This is name of the local parameter or variable that will have the result
value of the function invocation assigned to it.

=item C<function> - C<MaterialNC>

This is the name of the function being invoked by the statement node.

=item C<args> - C<IISArgVarMap>

These are the 0..N read-only arguments to the function invocation.  Each
element defines one argument value, with the element C<arg> matching the
invoked function's parameter name, and the element C<var> naming a local or
global variable or statement-containing-procedure parameter which defines
the value.

=back

A C<FuncInvoStmtNodeSet> has a unary primary key on the C<name>
attribute.  Its default value is empty.

=head2 sys.std.Core.Type.Cat.UpdInvoStmtNodeSet

An C<UpdInvoStmtNodeSet> is a C<DHRelation> that specifies a set of atomic
statement nodes where each node is an invocation of an updater with
specific arguments.  The invoked updater can be either user-defined or
system-defined.

An C<UpdInvoStmtNodeSet> has these 5 attributes:
...
These are the 1..N subject-to-update arguments to the updater invocation,
as-per C<ro_args> but that an updater must take at least 1
subject-to-update argument.

=item C<ro_args> - C<IISArgVarMap>

These are the 0..N read-only arguments to the updater invocation.  Each
element defines one argument value, with the element C<arg> matching the
invoked updater's parameter name, and the element C<var> naming a local or
global variable or statement-containing-procedure parameter which defines
the value.

=back

An C<UpdInvoStmtNodeSet> has a unary primary key on the C<name>
...
=head2 sys.std.Core.Type.Cat.RcpInvoStmtNodeSet

A C<RcpInvoStmtNodeSet> is a C<DHRelation> that specifies a set of atomic
statement nodes where each node is an invocation of a recipe with specific
arguments.  The invoked recipe can be either user-defined or
system-defined.

A C<RcpInvoStmtNodeSet> has these 5 attributes:
...
These are the 0..N read-only arguments to the recipe invocation.  Each
element defines one argument value, with the element C<arg> matching the
invoked recipe's parameter name, and the element C<var> naming a local
or global variable or statement-containing-recipe parameter which
defines the value.

=back

A C<RcpInvoStmtNodeSet> has a unary primary key on the C<name>
attribute.  Its default value is empty.  There is a distributed primary
key over the C<name> attributes of C<upd_args> and C<ro_args>.
...
statement nodes where each node is an invocation of some other procedure
with specific arguments.  The invoked procedure can be
...
These are the 0..N read-only arguments to the procedure invocation.  Each
element defines one argument value, with the element C<arg> matching the
invoked procedure's parameter name, and the element C<var> naming a local
or global variable or statement-containing-procedure parameter which
defines the value.
...
C<var> (a C<LexDataNC>); the C<arg> is the name of the routine argument,
...
statement.  A C<TryCatchStmtNodeSet> is conceptually a wrapper over a pair
...
This is the name of a local procedure invocation statement node that will
be invoked first unconditionally; any thrown exception will be caught.

=item C<catch> - C<maybe_of.Name>

Iff C<catch> is a C<Single>, it is the name of a local procedure invocation
statement node that will be invoked second iff the C<try> procedure throws
...
C<catch> is C<Nothing>, then there is no second procedure invocation,
...
C<if> (a C<LexDataNC>) and C<then> (a C<Name>); C<if> has the name of a
...
attributes, C<when> (a C<LexDataNC>) and C<then> (a C<Name>); C<when> has
...
=head2 sys.std.Core.Type.Cat.APMaterialNCSelStmtNodeSet

An C<APMaterialNCSelStmtNodeSet> is a C<DHRelation> that specifies a set of
atomic statement nodes where each node represents the assignment to a
variable (or parameter) of a newly selected value of the
C<sys.std.Core.Type.Cat.AbsPathMaterialNC> type, which is selected in terms
of a value of the C<sys.std.Core.Type.Cat.RelPathMaterialNC> type.  Using
one of these statement nodes should be an effective alternative to either
declaring a niladic function or declaring (or extending) an updater that
simply combines an C<AbsPathMaterialNC> selecting expression node with an
C<assign> updater call.

An C<APMaterialNCSelStmtNodeSet> has these 4 attributes:
...
This is name of the local parameter or variable that will have the newly
selected C<AbsPathMaterialNC> value assigned to it.

=item C<referencing> - C<RelPathMaterialNC>

This is the name, from the point of view of the procedure embedding this
statement node, of the routine or type that the new C<AbsPathMaterialNC>
value is supposed to facilitate portable invoking of.

=back

An C<APMaterialNCSelStmtNodeSet> has a unary (unique) key on the C<name>
attribute.  Its default value is empty.
...
A C<ScalarType> has these 8 attributes:
...
=item C<composed_mixins> - C<set_of.RPTypeNC>
...
C<Single> whose sole element matches the invocation name of a
...
=item C<order> - C<maybe_of.RPFunctionNC>
...
matches the invocation name of an C<order-determination> function.  The
...
that are the same as the scalar root type
being defined, and this function is said root type's type-default
...
A C<PossrepSet> has these 6 attributes:
...
=item C<constraint> - C<maybe_of.RPFunctionNC>

Iff it is a C<Single>, then C<constraint>
matches the invocation name of a C<type-constraint> function that
...
A C<TupleType> has these 10 attributes:
...
subtype of some other tuple type, then C<base_type> is a C<Single>
...
matches the invocation names of 0..N C<type-constraint> functions
...
form) for a particular kind of I<tuple constraint> of the type being
...
canonical simplest form) for a particular kind of I<tuple constraint> of
...
(the canonical simplest form) for a particular kind of I<tuple constraint>
...
C<default> is C<Nothing> and C<base_type> is a C<Single>, then the
...
C<constraints> and the C<name> attributes of
C<distrib_keys> and C<[|distrib_]subset_constraints>.
...
A C<RelationType> has these 7 attributes:
...
not applicable and is C<Nothing>.  Iff the type being defined is a
subtype of some other relation type, then C<base_type> is a C<Single>
whose sole element is the name of that other type.
...
This, I<relation constraints>, is like the I<tuple constraints> of a
...
moreover no key's attributes may be a subset of any other key's
...
I<relation constraint> of the type being defined, one that compares the
...
invalid and C<default> must be a C<Single>.

=back

There is a distributed primary key over the elements of C<constraints> and
the C<name> attributes of C<keys>, and the similar elements or C<name>
attributes of the C<TupleType> composed into this type.
...
A C<KeySet> has these 5 attributes:
...
This is the visible order of this declaration relative to all of
the other declarations beneath this relation type declaration.
...
A C<DistribKeySet> has these 6 attributes:
...
A C<SubsetConstrSet> has these 7 attributes:
...
A C<DistribSubsetConstrSet> has these 6 attributes:
...
=item C<parent_distrib_key> - C<NameChain>
...
A C<DomainType> has these 9 attributes:
...
=item C<default> - C<RPFunctionNC>

This matches the invocation name of a C<named-value> function that results
...
A domain type definition, where the domain type is a simple union over just
singleton scalar root types, may optionally have a type-default
total ordering function associated with it, iff the singleton types don't
have their own C<order> defined and no other domain type which declares an
C<order> ranges over any of those singleton types, in which case the domain
type is treated as if it were the scalar root type declaring the values of
the singletons in respect to order-determination semantics;
iff the domain type being defined qualifies and desires such an
associated function, then C<order> is a C<Single> whose sole element
matches the invocation name of an C<order-determination> function.  The
function that this names must have exactly 4 parameters; the
first two parameters, named C<topic> and C<other>, each have declared types
that are the same as the domain type
being defined, and this function is said domain type's type-default
ordering function; the third parameter, named C<misc_args>, is
C<Tuple>-typed; the fourth parameter, named C<is_reverse_order>, is
C<Bool>-typed; the function's result type must be C<Order>.  Iff
the type being defined is a subtype of some other type or does not desire
...
The default value of C<DomainType> defines an alias for C<Universal>,
...
C<DomainType> attribute values that would result in an alias for the
...
no valid combination of C<SubsetType> attribute values that would
...
are a union of those from the one or more other not-C<Empty> types that
...
Perl 6 uses the term) which is meant to be composed into other types, such
...
A C<MixinType> has these 3 attributes:
...
This matches the invocation name of a C<named-value> function that results
in the default value of the mixin type; it has zero parameters and its
result type is the same as one of the other types that, directly or
indirectly, composes the mixin type.

=back

The default value of C<MixinType> defines a mixin type that is identical to
C<Boolean>; its C<composed_mixins> is empty and its default value is
the default value of C<Bool>.  I<This paragraph is subject to change.>
...
argument lists for routine invocations by functions or updaters or recipes.
A C<NameExprMap> has 3 attributes, C<name> and C<expr>, each of those being
a C<Name>, and C<scm_vis_ord> (a
C<NNInt>); the C<name> is the name of the tuple/etc attribute or routine
...
=head1 TYPES FOR POSSIBLY CURRIED HIGHER-ORDER FUNCTIONS

=head2 sys.std.Core.Type.Cat.CurriedFuncNC

A C<CurriedFuncNC> is a (not necessarily deeply homogeneous) C<Tuple>.  It
is conceptually a I<curried higher-order function>, that is, an
...
is actually invoked.  A C<CurriedFuncNC> has 2 attributes, C<function> (an
...
deeply homogeneous is the sole determinant of whether a C<CurriedFuncNC> is
a C<DHTuple> rather than just a C<Tuple>.  Its default value is a reference
to the C<sys.std.Core.Universal.is_identical> function with no pre-bound
parameters.

=head2 sys.std.Core.Type.Cat.ValMapCFuncNC

C<ValMapCFuncNC> is a non-proper subtype of C<CurriedFuncNC> that is
...
=head2 sys.std.Core.Type.Cat.ValFiltCFuncNC

C<ValFiltCFuncNC> is a non-proper subtype of C<ValMapCFuncNC> that is
...
=head2 sys.std.Core.Type.Cat.ValRedCFuncNC

C<ValRedCFuncNC> is a non-proper subtype of C<CurriedFuncNC> that is
...
=head2 sys.std.Core.Type.Cat.OrdDetCFuncNC

C<OrdDetCFuncNC> is a non-proper subtype of C<CurriedFuncNC> that is

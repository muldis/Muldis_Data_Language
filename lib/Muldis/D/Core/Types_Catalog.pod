...
hard-coded meanings and are not defined like any other types. Specifically,
it declares these 9 standard types (all in the C<sys.std.Core.Type>
namespace): C<Universal>, C<Empty>, C<Scalar>, C<Tuple>, C<Relation>,
C<QuasiScalar>, C<QuasiTuple>, C<QuasiRelation>, C<Remnant>.
...
=head2 sys.Core.Type.Cat.DomainType

A C<Cat.DomainType> is a C<Tuple>.  It defines a new data (sub-)type simply
in terms of its values being an explicit subset of the values of a
...
A C<Cat.DomainType> has these N attributes:
...
=item C<main_type> - C<Cat.InnerDomainType>

This defines the actual main domain type, which a C<Cat.DomainType> is just
a wrapper for, namely what's left of the updater body after parts of it are
optionally factored out into inner routines or inner types.
...
The default value of C<Cat.DomainType> is as per C<Cat.InnerDomainType>.

=head1 TYPES FOR DEFINING INNER DATA TYPES

=head2 sys.Core.Type.Cat.InnerDomainType

A C<Cat.InnerDomainType> is a C<Tuple>.  Its purpose is as per
C<Cat.DomainType> but that it does the actual work for that type.
...
=item C<type_constraint> - C<Cat.InnerFuncBody>
...
=item C<default_value> - C<Cat.InnerFuncBody>
...
C<type_constraint> is unconditionally C<Bool:true>.
...
=head1 OLD TYPE DEFINITIONS TO REWRITE OR REPLACE

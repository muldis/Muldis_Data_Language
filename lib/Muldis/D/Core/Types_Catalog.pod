...
This document is Muldis::D::Core::Types_Catalog version 0.127.0.
    ...
    sys.std.Core.Type.Universal
        ...
        sys.std.Core.Type.Relation
            sys.std.Core.Type.DHRelation
                ...
                sys.std.Core.Type.Cat.Sys[|Distrib][Key|Subset]ConstrSet
...
default value is a reference to the C<sys.std.Core.Universal.is_identical>
function.

=head2 sys.std.Core.Type.Cat.RPImperativeNC

C<RPImperativeNC> is a proper subtype of C<RelPathMaterialNC> that excludes
the C<type>-prefix values and a subset of the C<sys>-prefix values.  Its
...
A C<System> has these 18 attributes:
...
=item C<[|distrib_][key|subset]_constraints> -
C<[|Distrib][Key|Subset]ConstrSet>

These are all the system-defined |distributed key|subset constraints.
...
system-defined scalar|tuple|relation|domain|subset|mixin types.
...
=head2 sys.std.Core.Type.Cat.Sys[|Distrib][Key|Subset]ConstrSet

A C<Sys[|Distrib][Key|Subset]ConstrSet> is a C<DHRelation> that specifies a
set of system-defined |distributed key|subset constraints.

A C<Sys[|Distrib][Key|Subset]ConstrSet> has these 5 attributes:
...
This is the fully-qualified name, in the nameless global root namespace, of
the |distributed key|subset constraint's parent system namespace.
...
A C<Sys[|Distrib][Key|Subset]ConstrSet> has a binary primary key on the
...
A C<Depot> has these 18 attributes:
...
set of |distributed key|subset constraints that a [|sub]depot might
directly contain.
...
This is the fully-qualified name, in the C<nlx.[cat|lib|data]> namespace of
any hypothetical immediate child namespace of the depot, of the
|distributed key|subset constraint's parent [|sub]depot.
...
=item C<distrib_key_constraints> - C<set_of.RelPathMaterialNC>

This matches the invocation names of
0..N explicit simple distributed (unique) key constraints that are
...
=item C<subset_constraints> - C<set_of.RelPathMaterialNC>

This matches the invocation names of
0..N explicit simple (non-distributed) subset constraints
...
=item C<distrib_subset_constraints> - C<set_of.RelPathMaterialNC>

This matches the invocation names of
0..N explicit simple distributed subset constraints (foreign
...
=item C<key_constraints> - C<set_of.RelPathMaterialNC>

This matches the invocation names of
0..N explicit (unique) key constraints that are applicable to relation

...
This document is Muldis::D::Core::Types_Catalog version 0.72.0.
    ...
    sys.std.Core.Type.Universal
        ...
        sys.std.Core.Type.QRelation
            sys.std.Core.Type.Relation
                ...
                sys.std.Core.Type.Cat.[Func|Proc|Type]RefSelExprNodeSet
                sys.std.Core.Type.Cat.OrdDetFuncRefSelExprNodeSet
...
is 1, and its only value is its default and minimum and maximum value.  The
sole value of C<String.Value> is the same as its unqualified name, or more
accurately, it is identical to the sole attribute value of the sole possrep
of the C<Name> value C<Value>.
...
"chicken and egg" scenario where at least one q/scalar root type needs to
be special and have no possreps in order to be the foundation for other
scalar root types.  Under the current system, I<every> q/scalar root type
has at least one possrep and the type system design does not require any
q/scalar root type to be special.  One can always avoid the need for some
...
subtype of C<String> where all member value elements are between zero and
255.  One can be used to represent a string of octets.

=head2 sys.std.Core.Type.Cat.UCPString

This is an enumeration data type.  C<UCPString> (Unicode codepoint
...
string which consists of 1 C<String>-typed attribute whose name is the
empty string.  A C<Name> is a simple wrapper for a C<String> and all of its
other details such as default and minimum and maximum values and
cardinality and default ordering algorithm all correspond directly.  But
C<Name> is explicitly disjoint from C<String> due to having a different
intended interpretation.  C<Name> is also very similar to C<Text> but that
while C<Text> is limited to string elements matching Unicode codepoints,
C<Name> elements can be any integer; besides that they are explicitly
disjoint due to having a different intended interpretation; it is
specifically intended for use in naming DBMS entities rather than being for
general-purpose user data.

All of the above being said, any use of a C<Name> value that can not be
exactly matched by a C<Text> value (because its component C<String> value
is not a valid C<UCPString> value) is strongly discouraged for various
reasons of practicality.  In fact it is so strongly discouraged that all of
the standard Muldis D dialects do not provide support in their C<Name>/etc
value literal syntax for selecting said discouraged values, and so the only
way you can select said values in those dialects is in terms of a generic
C<Scalar> value literal with a component that is a C<String> value literal.
The primary reason that the internal C<Name> flexibility exists for now is
so that some/most of the implicitly defined infinite number of
system-defined singleton scalar types comprising the quasi-foundational
system-defined C<String> type can be named after their values.
...
possrep, default value and ordering algorithm, etc) is the same as C<Text>
...
user-defined types.  Specifically, it declares these 13 standard types: in
the C<sys.std.Core.Type> namespace, C<Universal>, C<Empty>, C<QScalar>,
C<String>, C<QTuple>, C<QRelation>, C<Reference>, C<External>, C<Remnant>;
in the C<sys.std.Core.Type.Cat> namespace,
C<[Func|Proc|Type|OrdDetFunc]Ref>.  These constitute all 2 Muldis D
primitive types, all 5 Muldis D reference types, all 2 Muldis D type system
minimal and maximal (enumeration) types, all 3 Muldis D enumeration types
that are each implicit maximal types of 3 of 5 type categories (the other 2
such maximals are primitive types), and the 1 sole scalar enumeration type
that is defined in terms of an infinite number of singleton structure types
(and those structure types are not listed individually in the system
catalog at all, for what should be obvious reasons).

=item C<types> - C<SysTypeSet>

These are all the system-defined q/scalar types, except for
C<String> because it can't be defined like user-defined types, and complete
q/nonscalar types, and union types, and restriction types.
...
q/scalar types with possreps, and complete q/nonscalar types,
...
These are all the definitions that this package contains of q/scalar
types with possreps, and complete q/nonscalar types, and union
...
These are expression nodes that represent generic q/scalar value
selections specified simply in terms of possrep attributes.

=item C<tup_sel_exprs> - C<TupSelExprNodeSet>

These are expression nodes that represent q/tuple value selections.

=item C<rel_sel_exprs> - C<RelSelExprNodeSet>

These are expression nodes that represent generic q/relation value
selections.

=item C<set_sel_exprs> - C<SetSelExprNodeSet>

These are expression nodes that represent q/set value selections.

=item C<ary_sel_exprs> - C<ArySelExprNodeSet>

These are expression nodes that represent q/array value selections.

=item C<bag_sel_exprs> - C<BagSelExprNodeSet>

These are expression nodes that represent q/bag value selections.
...
=item C<[func|proc|type|ord_det_func]_ref_sel_exprs> -
C<Cat.[Func|Proc|Type|OrdDetFunc]RefSelExprNodeSet>
...
A C<ScaValExprNodeSet> is a C<Relation> that specifies a set of value
...
A C<ScaPRSelExprNodeSet> is a C<Relation> that specifies a set of value
expression nodes where each node represents a q/scalar value
...
A C<TupSelExprNodeSet> is a C<Relation> that specifies a set of value
expression nodes where each node represents a q/tuple value selection.
...
These represent the attributes (names and values) of the q/tuple
...
A C<RelSelExprNodeSet> is a C<Relation> that specifies a set of value
expression nodes where each node represents a q/relation value selection.
...
These are the names of all of this q/relation value's attributes.

=item C<body> - C<set_of.NameExprMap>

These represent the q/tuples of the q/relation value being
...
A C<SetSelExprNodeSet> is a C<Relation> that specifies a set of value
expression nodes where each node represents a q/set value selection.
...
These represent the elements of the q/set value being selected.
...
An C<ArySelExprNodeSet> is a C<Relation> that specifies a set of value
expression nodes where each node represents a q/array value selection.
...
These represent the elements of the q/array value being selected.
...
A C<BagSelExprNodeSet> is a C<Relation> that specifies a set of value
expression nodes where each node represents a q/bag value selection.
...
These represent the elements of the q/bag value being selected.
When this value expression is evaluated, if any child expression nodes are
...
expression node like with C<value>; if you actually want the q/bag value
...
=head2 sys.std.Core.Type.Cat.[Func|Proc|Type|OrdDetFunc]RefSelExprNodeSet

A C<Cat.[Func|Proc|Type|OrdDetFunc]RefSelExprNodeSet> is a C<Relation> that
...
A C<Cat.[Func|Proc|Type|OrdDetFunc]RefSelExprNodeSet> has these 3
attributes:
...
A C<Cat.[Func|Proc|Type|OrdDetFunc]RefSelExprNodeSet> has a unary (unique)
...
These are statement nodes that represent [|inner ]procedure invocations.
...
q/scalar root type with at least 1 possrep, or it defines a subtype
of some other q/scalar type which also adds at least one possrep to
...
or none are.  You can not declare a q/scalar root type at all except
...
This is an optional programmer comment about the q/scalar
[|sub]type as a whole.

=item C<base_type> - C<maybe_of.NameChain>

Iff the type being defined is a q/scalar root type, then
C<base_type> is not applicable and is C<nothing>.  Iff the type being
defined is a subtype of some other q/scalar type, then C<base_type>
is a C<Single> whose sole element is the name of that other type.  Note
that any type named by C<base_type> must itself be a q/scalar root
type or a subtype of one.

=item C<subtype_constraint> - C<maybe_of.InnerFuncBody>

Iff the type being defined is a q/scalar root type, or it is a
...
subtype of some other q/scalar type, then C<subtype_constraint> is a
C<Single> whose sole element defines the entire (main) body of a
C<type_constraint> function that determines what base type values are part
of the subtype.  The implicit function heading that this goes with has a
...
implicit result type is C<Bool>.  This constraint function may only
...
accepts, so it is possible to apply the new possreps' constraints.
...
type.  Note that if multiple q/scalar types are related to each
...
results in the default q/scalar value of the [|sub]type; it has
zero parameters and its result type is the same as the q/scalar type
whose declaration it is part of.

=item C<order> - C<maybe_of.InnerFuncBody>

A q/scalar root type definition may optionally have a type-default
...
implicit function heading that this goes with has 4 parameters; the first
two parameters, named C<topic> and C<other>, each have declared types that
are the same as the q/scalar root type whose declaration the
...
C<QTuple>-typed; the fourth parameter, named C<is_reverse_order>, is
C<Bool>-typed; the function's implicit result type is C<Order>.  Iff
...
a q/scalar [|sub]type might consist primarily of.
...
q/nonscalar heading (set of attributes) with associated constraints
for a q/tuple and q/relation type having that heading, or it defines a
subtype of some other q/nonscalar type.  That is, it defines 2 new
...
This is an optional programmer comment about the q/nonscalar
[|sub]type as a whole.

=item C<base_type> - C<maybe_of.NameChain>

Iff the type being defined is a q/nonscalar root type, then C<base_type> is
not applicable and is C<nothing>.  Iff the type being defined is a
subtype of some other q/nonscalar type, then C<base_type> is a C<Single>
...
Iff the type being defined is a q/nonscalar root type, then C<attrs>
...
is indeed valid for a q/nonscalar type to have zero attributes
...
This defines the entire (main) body of a C<type_default> function that
results in the default q/tuple value of the [|sub]type; it has zero
parameters and its result type is the same as the q/tuple type whose
declaration it is part of.

=item C<default_relation> - C<InnerFuncBody>

This defines the entire (main) body of a C<type_default> function that
results in the default q/relation value of the [|sub]type; it has
zero parameters and its result type is the same as the q/relation
...
dependencies between attributes of a q/nonscalar data type, such
...
q/nonscalar's attributes, which are I<determinant> and I<dependent>
...
that if you want to define a possrep-adding q/scalar subtype whose base is
...
An C<OrderByName> is a C<Tuple>.  It defines an element of an order-by
...
these 3 attributes: C<radix> (a C<PInt2_N>), C<min_exp> (an C<Int>), and
C<round_meth> (a C<RatRoundMeth>).  The rational resulting from the
operation is as close as possible to the conceptual result but that it is
an exact multiple of the rational value resulting from C<radix> taken to
the power of C<min_exp>; if rounding is needed, then C<round_meth> dictates
the rounding method.  The default value of C<RatRoundRule> specifies a
coersion to a whole number using the C<half_up> rounding method (its radix
is 2 and its min exp is 0).
...
data types (similar to quasi-scalar types but with no possreps) which will

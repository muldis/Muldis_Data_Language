...
This document is Muldis::D::Core::Types_Catalog version 0.51.0.
...
This is a structure data type.  A C<Cat.Name> (scalar) is a canonical short
...
using just its C<Cat.Name>, depending on the context; other times, a
C<Cat.NameChain> must be used instead to also qualify the reference with a
namespace.  Every detail of C<Cat.Name>'s representation (its 1 possrep,
...
This is a structure data type.  A C<Cat.NameChain> (scalar) is a canonical
long name for invoking or referring to a DBMS entity, when its name needs
to be qualified with a namespace.  A C<Cat.NameChain> is conceptually a
sequence of 2..N C<Cat.Name>, the 2..N elements being ordered from
parent-most to child-most component name.  A C<Cat.NameChain> has 2
system-defined possreps, named C<array> and C<flat>, both of which are
defined over C<Cat.Name>.

The C<array> possrep directly matches the conception of the type; it
consists of 1 attribute whose name is the empty string; the attribute is an
C<Array> whose C<value> attribute has a declared type of C<Cat.Name> and
that must have at least 2 elements.
...
The C<Cat.NameChain> type has a default ordering algorithm; for 2 distinct
C<Cat.NameChain> values, their order is determined as follows:  1.
Eliminate any identical parent-most sequence elements as those alone would
make the chains compare as same (if the remainder of both strings was the
empty chain, then the chains are identical).  2.  Iff the remainder of just
one chain is the empty chain, then that chain is ordered before the
non-empty one.  3.  Compare the first element of each of the chain
remainders; the chain with the first remaining element that is ordered
first as per its own algorithm (that of C<Cat.Name>) is ordered before the
other chain.

The C<Cat.NameChain> type has an implementation hint for less intelligent
Muldis D implementations, that suggests using the C<flat> possrep as the
basis for the physical representation.

=head2 sys.std.Core.Type.Cat.DeclNameChain

This is a structure data type.  A C<Cat.DeclNameChain> (scalar) is mostly
the same as a C<Cat.NameChain> but that it is a sequence of 0..N rather
than 2..N, and that its C<flat> possrep has N+1 periods rather than N-1;
the possrep starts with a period, and also has a period just following each
sequence element, and so an empty sequence means a C<flat> possrep
consisting of just a period.  A C<Cat.DeclNameChain> is used in declaring
system catalogs where DBMS entities live under a potentially N-depth
namespace, such as depot entities grouped in a subdepot hierarchy.  The
default and minimum value of C<Cat.DeclNameChain> is a zero element
sequence.  The C<Cat.DeclNameChain> type has a default ordering algorithm;
it is the same as for C<Cat.NameChain>.  Likewise it has a same
implementation hint.

=head2 sys.std.Core.Type.Cat.Comment

This is a structure data type.  A C<Cat.Comment> (scalar) is the text of a
...
expression nodes.  Every detail of C<Cat.Comment>'s representation (its 1
possrep, default value and ordering algorithm, etc) is the same as
C<Cat.Name> but it is explicitly disjoint due to having a different
intended interpretation; it is intended just for commenting Muldis D code.
...
This is a structure data type.  C<Cat.Order> (order determination) consists
of just the 3 values C<increase>, C<same>, C<decrease>.  When some context
...
main/only arguments, and that function results in a C<Cat.Order> value for
the context to make use of.  A C<Cat.Order> has 2 system-defined possreps,
named C<name> and C<int>.  The C<name> possrep directly matches the
conception of the type as consisting of 3 character string values; it
consists of 1 C<Cat.Name>-typed attribute whose name is the empty string.
The C<int> possrep consists of 1 C<Int>-typed attribute whose name is the
empty string and whose value must be one of [C<-1>, C<0>, C<1>]; the 3
values of each possrep correspond in the same order as they are documented
here.  The default value of C<Cat.Order> is C<same>; its minimum and
maximum values are, respectively, C<increase> and C<decrease>.  The
cardinality of this type is 3.  The C<Cat.Order> type has a default
ordering algorithm that corresponds directly to that of its C<int> possrep
attribute; C<increase> is ordered before C<same>, and C<same> before
C<decrease>.  The C<Cat.Order> type has an implementation hint for less
intelligent Muldis D implementations, that suggests using the C<int>
possrep as the basis for the physical representation.

=head2 sys.std.Core.Type.Cat.OrdDetFuncNameChain

This is an enumeration data type.  The C<Cat.OrdDetFuncNameChain> type is
defined as a non-proper / identical subtype of the C<Cat.NameChain> type
that has a different default value, which is C<sys.std.Core.Scalar.order>
...
A C<Cat.System> has these 9 attributes:
...
A C<Cat.SysNspSet> has a binary primary key on the C<parent> plus
...
A C<Cat.Sys(Func|Upd|Proc)Set> has a binary primary key on the C<parent>
...
A C<Cat.SysSpecTypeSet> has a binary primary key on the C<parent> plus
C<name> attributes.  Its default value is empty.
...
A C<Cat.SysScaNPRTypeSet> has a binary primary key on the C<parent> plus
C<name> attributes.  Its default value is empty.

=head2 sys.std.Core.Type.Cat.SysTypeSet

A C<Cat.SysTypeSet> is a C<Relation> that specifies a set of system-defined
types.

A C<Cat.SysTypeSet> has these 4 attributes:
...
=item C<type> - C<Cat.Type>

This defines the entire type sans the name.

=back

A C<Cat.SysTypeSet> has a binary primary key on the C<parent> plus C<name>
...
=item C<catalog> - C<Cat.NameChain>

This is the declared data type of the catalog dbvar.

=back

A C<Cat.SysCatSet> has a unary primary key on the C<name> attribute.  Its
...
A C<Cat.MountControlCat> is a C<Database>.  It specifies the control
...
catalog variable named C<mnt.cat> is of the C<Cat.MountControlCat> type.

A C<Cat.MountControlCat> has these 2 attributes:
...
=item C<mounts> - C<Cat.MountControlSet>

These are the controls for the current depot mounts.

=back

The default value of C<Cat.MountControlCat> has zero depot mount controls.

=head2 sys.std.Core.Type.Cat.MountControlSet

A C<Cat.MountControlSet> is a C<Relation> that specifies a set of controls
...
A C<Cat.MountControlSet> has these 7 attributes:
...
=item C<details> - C<Cat.ScaLitExprNodeSet>
...
A C<Cat.MountControlSet> has a unary primary key on the C<name> attribute.
...
A C<Cat.Federation> has these 3 attributes:
...
=item C<type_maps> - C<Cat.FedTypeMapSet>
...
A C<Cat.DepotMountSet> has a unary primary key on the C<name> attribute.
Its default value is empty.

=head2 sys.std.Core.Type.Cat.FedTypeMapSet

A C<Cat.FedTypeMapSet> is a C<Relation> such that each tuple in it
...
A C<Cat.FedTypeMapSet> has these 2 attributes:
...
A C<Cat.FedTypeMapSet> has a primary key on the C<map> attribute.  Its
default value is empty.

=head2 sys.std.Core.Type.Cat.FedTypeMap

A C<Cat.FedTypeMap> is a C<Relation>.  It lists the fully-qualified names
of 0..N data types that are all considered to be copies and of the same 1
type, and should be treated interchangeably by the DBMS.  A
C<Cat.FedTypeMap> has 2 attributes, C<depot> (a C<Cat.Name>) and C<type> (a
C<Cat.NameChain>); the C<depot> is the declared name of the depot mount
that the data type lives in; the C<type> is the C<dep.>-qualified name of
the data type living in it.  Its default value has zero tuples.
...
A C<Cat.Depot> has these 8 attributes:
...
The default value of C<Cat.Depot> defines an empty (|sub)depot that does
...
A C<Cat.SubdepotSet> has a binary primary key on the C<parent> plus
C<name> attributes.  Its default value is empty.
...
=item C<package> - C<Cat.Package>

This defines the entire system catalog of the package.

=back

A C<Cat.PackageSet> has a binary primary key on the C<parent> plus
C<name> attributes.  Its default value is empty.
...
A C<Cat.Dep(Func|Upd|Proc)Set> has a binary primary key on the C<parent>
plus C<name> attributes.  Its default value is empty.

=head2 sys.std.Core.Type.Cat.DepTypeSet

A C<Cat.DepTypeSet> is a C<Relation> that specifies a set of types that a
(|sub)depot might directly contain.

A C<Cat.DepTypeSet> has these 4 attributes:
...
A C<Cat.DepTypeSet> has a binary primary key on the C<parent> plus C<name>
attributes.  Its default value is empty.
...
A C<Cat.Package> has these 6 attributes:
...
The default value of C<Cat.Package> defines an empty package that does not
...
A C<Cat.PkgTypeSet> has a unary primary key on the C<name> attribute.  Its
default value is empty.
...
A C<Cat.FuncHead> has these 4 attributes:
...
A C<Cat.UpdHead> has these 4 attributes:
...
A C<Cat.ProcHead> has these 4 attributes:
...
This is as per the same-named attribute of C<Cat.UpdHead>.
...
The default value of C<Cat.ProcHead> has zero parameters; by default, its
...
A C<Cat.FuncBody> has these 4 attributes:
...
A C<Cat.UpdBody> has these 5 attributes:
...
A C<Cat.ProcBody> has these 6 attributes:
...
A C<Cat.Inner(Func|Upd|Proc)Set> has a unary primary key on the C<name>
attribute.  Its default value is empty.
...
A C<Cat.ExprNodeSet> has these 11 attributes:
...
=item C<if_else_exprs> - C<Cat.IfElseExprNodeSet>

These are expression nodes that represent if-else control flow expressions.

=item C<given_when_def_exprs> - C<Cat.GivenWhenDefExprNodeSet>
...
A C<Cat.ScaLitExprNodeSet> has a unary primary key on the C<name>
attribute.  Its default value is empty.
...
A C<Cat.ScaPRLitExprNodeSet> has a unary primary key on the C<name>
attribute.  Its default value is empty.
...
A C<Cat.TupLitExprNodeSet> has a unary primary key on the C<name>
attribute.  Its default value is empty.
...
A C<Cat.RelLitExprNodeSet> has a unary primary key on the C<name>
attribute.  Its default value is empty.
...
A C<Cat.FuncInvoExprNodeSet> has a unary primary key on the C<name>
attribute.  Its default value is empty.

=head2 sys.std.Core.Type.Cat.IfElseExprNodeSet

A C<Cat.IfElseExprNodeSet> is a C<Relation> that specifies a set of value
expression nodes where each node represents an N-way if-else control flow
expression.

The reason that the C<Cat.IfElseExprNodeSet> expression node type exists,
...
A C<Cat.IfElseExprNodeSet> has these 4 attributes:
...
A C<Cat.IfElseExprNodeSet> has a unary primary key on the C<name>
attribute.  Its default value is empty.

=head2 sys.std.Core.Type.Cat.GivenWhenDefExprNodeSet

A C<Cat.GivenWhenDefExprNodeSet> is a C<Relation> that specifies a set of
...
A C<Cat.GivenWhenDefExprNodeSet> is essentially a more specialized version
of a C<Cat.IfElseExprNodeSet> where every condition expression is a simple
...
A C<Cat.GivenWhenDefExprNodeSet> has these 5 attributes:
...
=item C<default> - C<Cat.NameChain>
...
A C<Cat.GivenWhenDefExprNodeSet> has a unary primary key on the C<name>
attribute.  Its default value is empty.
...
=item C<referencing> - C<Cat.NameChain>
...
C<Cat.InnerProcBody> is paired with a C<Cat.ProcHead>, then the distributed
key also extends over the head's parameter names.

A C<Cat.InnerProcBody> specifies a simple statement tree of named statement
...
default value of C<Cat.ProcHead> and is a no-op, having an empty statement
tree.

=head2 sys.std.Core.Type.Cat.StmtNodeSet

A C<Cat.StmtNodeSet> is a C<Database> that specifies a set of named
statement nodes.  It is typically composed into a (|inner )procedure.  Each
tuple of a C<Cat.StmtNodeSet> attribute is a named statement node, from
...
There is a distributed primary key over the C<name> attributes of all of a
C<Cat.StmtNodeSet>'s attributes.  Its default value is empty.

=head2 sys.std.Core.Type.Cat.LeaveStmtNodeSet

A C<Cat.LeaveStmtNodeSet> is a C<Relation> that specifies a set of
...
A C<Cat.LeaveStmtNodeSet> has these 3 attributes:
...
A C<Cat.CompoundStmtNodeSet> has a unary primary key on the C<name>
attribute.  Its default value is empty.

=head2 sys.std.Core.Type.Cat.ProcInvoStmtNodeSet

A C<Cat.ProcInvoStmtNodeSet> is a C<Relation> that specifies a set of
...
A C<Cat.ProcInvoStmtNodeSet> has these 5 attributes:
...
A C<Cat.ProcInvoStmtNodeSet> has a unary primary key on the C<name>
...
A C<Cat.IfElseStmtNodeSet> is a C<Relation> that specifies a set of
statement nodes where each node represents an N-way if-else control flow
statement.  A C<Cat.IfElseStmtNodeSet> is essentially the imperative
version of the functional C<Cat.IfElseExprNodeSet>, except that for design
...
A C<Cat.IfElseStmtNodeSet> has these 4 attributes:
...
A C<Cat.IfElseStmtNodeSet> has a unary primary key on the C<name>
attribute.  Its default value is empty.

=head2 sys.std.Core.Type.Cat.GivenWhenDefStmtNodeSet

A C<Cat.GivenWhenDefStmtNodeSet> is a C<Relation> that specifies a set of
...
value with several options.  A C<Cat.GivenWhenDefStmtNodeSet> is
essentially the imperative version of the functional
C<Cat.GivenWhenDefExprNodeSet>.

A C<Cat.GivenWhenDefStmtNodeSet> has these 5 attributes:
...
A C<Cat.GivenWhenDefStmtNodeSet> has a unary primary key on the C<name>
attribute.  Its default value is empty.

=head2 sys.std.Core.Type.Cat.IterateStmtNodeSet

A C<Cat.IterateStmtNodeSet> is a C<Relation> that specifies a set of
statement leaf nodes where each node represents an instruction to
...
A C<Cat.IterateStmtNodeSet> has these 3 attributes:
...
=item C<iterate> - C<Cat.NameChain>
...
A C<Cat.LoopStmtNodeSet> has a unary primary key on the C<name> attribute.
Its default value is empty.
...
A C<Cat.Type> is a C<Tuple>.  Its purpose is as per
C<Cat.Inner(Scalar|Nonscalar|Union|Restr)Type>, one of which it is a
wrapper for along with other inner types that are composed into it.

A C<Cat.Type> has these 3 attributes:
...
type that the C<Cat.Type> is a wrapper for, and the other inner types, if
any, support it.
...
The default value of C<Cat.Type> is as per C<Cat.InnerRestrType>.

=head2 sys.std.Core.Type.Cat.InnerTypeSet

A C<Cat.InnerTypeSet> is a C<Database> that specifies a set of named inner
types.  It is typically composed into a
function|updater|procedure|outer-type.

A C<Cat.InnerTypeSet> has these 4 attributes:

=over

=item C<inner_(sca|nonsca|uni|restr)_types> -
C<Cat.Inner(Sca|Nonsca|Uni|Restr)TypeSet>
...
There is a distributed primary key over the C<name> attributes of all of a
C<Cat.InnerTypeSet>'s attributes.  Its default value is empty.

=head2 sys.std.Core.Type.Cat.Inner(Sca|Nonsca|Uni|Restr)TypeSet

A C<Cat.Inner(Sca|Nonsca|Uni|Restr)TypeSet> is a C<Relation> that specifies
...
A C<Cat.Inner(Sca|Nonsca|Uni|Restr)TypeSet> has these 3 attributes:
...
=item C<type> - C<Cat.Inner(Scalar|Nonscalar|Union|Restr)Type>
...
A C<Cat.Inner(Sca|Nonsca|Uni|Restr)TypeSet> has a unary primary key on the
...
A C<Cat.InnerScalarType> has these 7 attributes:
...
C<Tuple>-typed; the function's implicit result type is C<Cat.Order>.  Iff
...
A C<Cat.PossrepSet> has these 5 attributes:
...
A C<Cat.PossrepSet> has a unary primary key on the C<name> attribute.  Its
default value is empty.  The default value of a tuple of C<Cat.PossrepSet>
...
A C<Cat.PossrepMapSet> has a binary primary key on the C<p1> plus C<p2>
attributes.  Its default value is empty.
...
A C<Cat.InnerNonscalarType> has these 14 attributes:
...
=item C<virtual_attr_maps> - C<Cat.VirtualAttrMapSet>
...
=item C<tuple_constraints> - C<Cat.ConstraintSet>
...
=item C<relation_constraints> - C<Cat.ConstraintSet>
...
=item C<distrib_keys> - C<Cat.DistribKeySet>
...
=item C<foreign_keys> - C<Cat.ForeignKeySet>
...
=item C<foreign_distrib_keys> - C<Cat.ForeignDistribKeySet>
...
A C<Cat.VirtualAttrMapSet> is a C<Relation> that defines special functional
...
tuple of a C<Cat.VirtualAttrMapSet> specifies 2 disjoint subsets of the
...
I<This all being said, for the moment the C<Cat.VirtualAttrMapSet> type
...
A C<Cat.VirtualAttrMapSet> has these 5 attributes:
...
=item C<determinant_attrs> - C<Cat.NameDNCMap>
...
=item C<dependent_attrs> - C<Cat.NameDNCMap>
...
=item C<virtual_attr_map> - C<Cat.InnerFuncBody>
...
A C<Cat.VirtualAttrMapSet> has a binary primary key on the
...
A C<Cat.ConstraintSet> is a C<Relation> that specifies a set of named
...
empty) C<Cat.ConstraintSet> is composed into the definition each nonscalar
type and union type and restriction type and dbvar.

A C<Cat.ConstraintSet> has these 3 attributes:
...
=item C<body> - C<Cat.InnerFuncBody>

This defines the entire (main) body of the constraint function.

=back

A C<Cat.ConstraintSet> has a unary primary key on the C<name> attribute.
Its default value is empty.  The default value of a tuple of
C<Cat.ConstraintSet> has a C<name> that is the empty string, and its
...
A C<Cat.KeySet> has a unary primary key on the C<name> attribute, and a
unary (unique) key on its C<attrs> attribute.  Its default value is empty.

=head2 sys.std.Core.Type.Cat.DistribKeySet

A C<Cat.DistribKeySet> is a C<Relation> that specifies a set of candidate
...
A C<Cat.DistribKeySet> has these 5 attributes:
...
=item C<relations> - C<Cat.DKMemRelAttrMap>
...
This has the same meaning as C<is_primary> of C<Cat.KeySet> but for being
distributed as if the relations distributed over were one relation.

=back

A C<Cat.DistribKeySet> has a unary primary key on the C<name> attribute,
and a unary (unique) key on its C<relations> attribute.  Its default value
is empty.

=head2 sys.std.Core.Type.Cat.DKMemRelAttrMap

A C<Cat.DKMemRelAttrMap> is a C<Relation> that names the 0..N
...
A C<Cat.DKMemRelAttrMap> has these 3 attributes:

=over

=item C<rel_name> - C<Cat.DeclNameChain>
...
=item C<attr_map> - C<Cat.DKRelAttrKeyAttrMap>
...
A C<Cat.DKMemRelAttrMap> has a unary primary key on the C<rel_name>
attribute.  Its default value is empty.

=head2 sys.std.Core.Type.Cat.DKRelAttrKeyAttrMap

A C<Cat.DKRelAttrKeyAttrMap> is a C<Relation>.  It maps 0..N attributes of
...
itself.  A C<Cat.DKRelAttrKeyAttrMap> has 2 attributes, C<rel_attr> and
C<key_attr>, each of those being a C<Cat.Name>, and each of those being a
unary key.  Its default value has zero tuples.

=head2 sys.std.Core.Type.Cat.ForeignKeySet

A C<Cat.ForeignKeySet> is a C<Relation> that specifies a set of foreign
...
A C<Cat.ForeignKeySet> has these 6 attributes:
...
=item C<child> - C<Cat.DeclNameChain>
...
=item C<attr_map> - C<Cat.FKChildAttrParentAttrMap>
...
A C<Cat.ForeignKeySet> has a unary primary key on the C<name> attribute.
Its default value is empty.

=head2 sys.std.Core.Type.Cat.ForeignDistribKeySet

A C<Cat.ForeignDistribKeySet> is a C<Relation> that specifies a set of
...
A C<Cat.ForeignDistribKeySet> has these 5 attributes:
...
=item C<parent_distrib_key> - C<Cat.DeclNameChain>
...
A C<Cat.ForeignDistribKeySet> has a unary primary key on the C<name>
attribute.  Its default value is empty.

=head2 sys.std.Core.Type.Cat.FKChildAttrParentAttrMap

A C<Cat.FKChildAttrParentAttrMap> is a C<Relation>.  It maps 0..N
...
of the host.  A C<Cat.FKChildAttrParentAttrMap> has 2 attributes,
C<child_attr> and C<parent_attr>, each of those being a C<Cat.Name>, and
each of those being a unary key.  Its default value has zero tuples.

=head2 sys.std.Core.Type.Cat.InnerUnionType

A C<Cat.InnerUnionType> is a C<Tuple>.  It defines a new data type whose
...
A C<Cat.InnerUnionType> has these 7 attributes:
...
=item C<constraints> - C<Cat.ConstraintSet>
...
The default value of C<Cat.InnerUnionType> defines an alias for
...
no valid combination of C<Cat.InnerUnionType> attribute values that would
result in an alias for the C<Empty> type, due to C<default> being
mandatory.

=head2 sys.std.Core.Type.Cat.InnerRestrType

A C<Cat.InnerRestrType> is a C<Tuple>.  It provides a relatively terse
...
A C<Cat.InnerRestrType> has these 4 attributes:
...
=item C<base_type> - C<Cat.NameChain>
...
The default value of C<Cat.InnerRestrType> defines an alias for
...
Note that there is no valid combination of C<Cat.InnerRestrType> attribute
...
A C<Cat.NameDNCMap> is a C<Relation>.  It defines an association of a short
name with a declaration name chain, to be used for aliasing the latter with
the former in a particular context.  A C<Cat.NameDNCMap> has 2 attributes,
C<name> (a C<Cat.Name>) and C<dnc> (a C<Cat.DeclNameChain>); each of those
...
A C<Cat.IfThenMap> is a C<Tuple>.  It defines an element of an if-else
sequential expression or statement.  A C<Cat.IfThenMap> has 2 attributes,
C<if> and C<then>, each of which is a C<Cat.NameChain>; C<if> has the name
...
default value has the default value of the C<Cat.NameChain> type in both
attributes.

=head2 sys.std.Core.Type.Cat.WhenThenMap

A C<Cat.WhenThenMap> is a C<Relation>.  It defines a set of dispatch
options for a given-when-default expression or statement.  A
C<Cat.WhenThenMap> has 2 attributes, C<when> and C<then>, each of which is
a C<Cat.NameChain>; C<when> has the name of a local expression node (or
...
a C<Cat.NameChain>.  But using an actual C<Cat.NameChain> frequently
...
C<Cat.NameChain> value for referencing it from outside that context
...
C<Cat.NameChain>, then this could cause problems where anything could
...
C<Cat.NameChain> values, so any routine may be correctly invoked from any
...
C<Cat.NameChain> argument, and the identity of the selected special value
...
system-defined routines, for which incidentally the C<Cat.NameChain>
...
This is a reference data type.  A C<Cat.FuncRef> is functionally like a
smart C<Cat.NameChain> that holds a function name but it remembers the
...
This is a reference data type.  A C<Cat.ProcRef> is functionally like a
smart C<Cat.NameChain> that holds an imperative routine name but it
...
This is a reference data type.  A C<Cat.TypeRef> is functionally like a
smart C<Cat.NameChain> that holds a data type name but it remembers the
...
This is a reference data type.  A C<Cat.OrdDetFuncRef> is to a
C<Cat.OrdDetFuncNameChain> what a C<Cat.FuncRef> is to a C<Cat.NameChain>;

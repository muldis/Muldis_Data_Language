...
C<LexDataNC> is a proper subtype of
...
C<String> is a proper subtype of
...
C<BString> (bit string) is a proper
...
user-defined types.  Specifically, it declares these 7 standard types: in
the C<sys.std.Core.Type> namespace, C<Universal>, C<Empty>, C<Scalar>,
C<Int>, C<Tuple>, C<Relation>, C<External>.  These
constitute all 3 Muldis D primitive types, all 1 Muldis D external types,
all 2 Muldis D type system minimal and maximal (enumeration) types, all 1
Muldis D enumeration types that are each implicit maximal types of 1 of 4
non-remnant type categories (the other 3 such maximals are primitive
types or external types).

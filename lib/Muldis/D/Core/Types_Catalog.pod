...
A C<Sys[Function|Updater|Recipe|Procedure]Set> has these 4 attributes:
...
A C<Sys[Scalar|Tuple|Relation|Domain|Subset|Mixin]TypeSet> has these 4
attributes:
...
A C<[Function|Updater|Recipe|Procedure]Set> has these 4 attributes:
...
A C<[Scalar|Tuple|Relation|Domain|Subset|Mixin]TypeSet> has these 4
attributes:
...
An C<OVLScaValExprNodeSet> has these 3 attributes:
...
A C<ScaSelExprNodeSet> has these 5 attributes:
...
An C<SPIvlSelExprNodeSet> has these 3 attributes:
...
An C<MPIvlSelExprNodeSet> has these 3 attributes:
...
An C<ListSelExprNodeSet> has these 3 attributes:
...
An C<AccExprNodeSet> has these 3 attributes:
...
An C<APMaterialNCSelExprNodeSet> has these 3 attributes:
...
A C<MultiUpdInvoStmt> has these 4 attributes:
...
An C<RcpGlobalVarAliasMap> is a C<DHRelation>.  It defines a set of lexical
...
for reading or updating.  An C<RcpGlobalVarAliasMap> has 2 attributes,
C<name> (a C<Name>) and C<global> (a C<DataNC>); the C<name> is the name of
...
A C<MultiRcpInvoStmt> has these 4 attributes:
...
C<name> (a C<Name>) and C<type> (a C<RPTypeNC>); the C<name> is the
...
A C<NameExprMap> has 2 attributes, C<name> and C<expr>, each of those being
a C<Name>; the C<name> is the name of the tuple/etc attribute or routine
...
name with a name chain, to be used for aliasing the latter with
the former in a particular context.  A C<NameNCMap> has 2 attributes,
C<name> (a C<Name>) and C<nc> (a C<NameChain>); each of those is a

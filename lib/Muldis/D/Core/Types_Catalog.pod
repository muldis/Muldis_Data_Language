...
This document is Muldis::D::Core::Types_Catalog version 0.101.0.
...
A C<StmtNodeSet> has these 10 attributes:
...
parameter) of a hard-coded scalar literal that is I<not> being specified
explicitly in terms of possrep attributes, but rather is specified using
special "opaque value literal" syntax that all of the Muldis D standard
dialects provide.  Using one of these statement nodes should be an
effective alternative to declaring (or extending) an updater that simply
combines a simple scalar selecting expression node with an C<assign>
updater call.
...
An C<UpdInvoStmtNodeSet> is a C<DHRelation> that specifies a set of
...
invoked updater's parameter name, and the element C<expr> naming a local or
...
element defines one argument value, with the element C<name> matching the
invoked procedure's parameter name, and the element C<expr> naming a local
...
statement nodes where each node represents the assignment to a variable (or
parameter) of a newly selected value of the
C<sys.std.Core.Type.Cat.AbsPathMaterialNC> type, which is selected in terms
of a value of the C<sys.std.Core.Type.Cat.RelPathMaterialNC> type.  Using
one of these statement nodes should be an effective alternative to
declaring (or extending) an updater that simply
combines an C<AbsPathMaterialNC> selecting
expression node with an C<assign> updater call.

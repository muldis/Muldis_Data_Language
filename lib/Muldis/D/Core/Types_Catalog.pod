...
These are all the system-defined [|quasi-]scalar types, except for
C<String> because it can't be defined like user-defined types, and complete
[|quasi-]nonscalar types, and union types, and restriction types.
...
These are expression nodes that represent generic [|quasi-]scalar value
selections specified simply in terms of possrep attributes.

=item C<tup_sel_exprs> - C<TupSelExprNodeSet>

These are expression nodes that represent [|quasi-]tuple value selections.

=item C<rel_sel_exprs> - C<RelSelExprNodeSet>

These are expression nodes that represent generic [|quasi-]relation value
selections.

=item C<set_sel_exprs> - C<SetSelExprNodeSet>

These are expression nodes that represent [|quasi-]set value selections.

=item C<ary_sel_exprs> - C<ArySelExprNodeSet>

These are expression nodes that represent [|quasi-]array value selections.

=item C<bag_sel_exprs> - C<BagSelExprNodeSet>

These are expression nodes that represent [|quasi-]bag value selections.
...
expression nodes where each node represents a [|quasi-]scalar value
...
possrep of the [|quasi-]scalar value being selected.
...
expression nodes where each node represents a [|quasi-]tuple value
selection.
...
These represent the attributes (names and values) of the [|quasi-]tuple
...
expression nodes where each node represents a [|quasi-]relation value
selection.
...
These are the names of all of this [|quasi-]relation value's attributes.

=item C<body> - C<set_of.NameExprMap>

These represent the [|quasi-]tuples of the [|quasi-]relation value being
...
expression nodes where each node represents a [|quasi-]set value selection.
...
These represent the elements of the [|quasi-]set value being selected.
...
expression nodes where each node represents a [|quasi-]array value
selection.
...
These represent the elements of the [|quasi-]array value being selected.
...
expression nodes where each node represents a [|quasi-]bag value selection.
...
These represent the elements of the [|quasi-]bag value being selected.
When this value expression is evaluated, if any child expression nodes are
...
that any type named by C<base_type> must itself be a [|quasi-]scalar root
type or a subtype of one.

=item C<subtype_constraint> - C<maybe_of.InnerFuncBody>

Iff the type being defined is a [|quasi-]scalar root type, or it is a
...
subtype of some other [|quasi-]scalar type, then C<subtype_constraint> is a
...
may actually be less restrictive than the total constraint of the sub-type
...
constraints of all the possreps of the sub-type; therefore, mainly the

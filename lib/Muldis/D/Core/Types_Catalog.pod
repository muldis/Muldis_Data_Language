...
This document is Muldis::D::Core::Types_Catalog version 0.88.0.
...
system-defined system services.)
...
Directly within every namespace defined by a depot or subdepot or package,
...
declared data type of the self-local dbvar that this [|sub]depot or package
contains; if said type doesn't exist, then this [|sub]depot or package does
not have a self-local dbvar.

A C<DepotOrPackage> has these 4 attributes:
...
There is a distributed primary key over the C<name> attributes of all 3 of
a C<DepotOrPackage>'s main C<DHRelation>-typed attributes.  Moreover, iff
any of those C<name> is the empty string, then the distributed primary key
also extends over all of the inner types and routines of that material,

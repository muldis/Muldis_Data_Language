...
A C<SysSpecTypeSet> is a C<DHRelation> that specifies a set of system-defined
...
A C<Cat.[Func|Proc|Type|OrdDetFunc]RefSelExprNodeSet> is a C<DHRelation> that
...
types of the attributes its possreps compose, whether they any are
deeply homogeneous
...
Iff the type being defined is a nonscalar root type, then C<attrs>
...
is indeed valid for a nonscalar type to have zero attributes
(which is then just a dh-nonscalar type by definition); in this case, a tuple
...
A C<ForeignDistribKeySet> is a C<DHRelation> that specifies a set of foreign
distributed keys over relation-valued attributes of a tuple/database;
each tuple specifies one foreign key, which is a kind of referential
constraint, that relates tuples of such attributes.  Each tuple of a
child attribute must have a corresponding tuple in one member of a
...
A C<FKChildAttrParentAttrMap> is a C<DHRelation>.  It maps 0..N attributes of
a child relation-valued attribute of a host tuple/database type
...
An C<InnerRestrType> is a C<DHTuple>.  It provides a relatively terse method
...
its values.  You can only declare a nonscalar type with an

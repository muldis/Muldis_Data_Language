...
This document is Muldis::D::Core::Types_Catalog version 0.141.0.
        ...
        sys.std.Core.Type.Scalar
            sys.std.Core.Type.DHScalar
                ...
                sys.std.Core.Type.Cat.String
                    ...
                    sys.std.Core.Type.Cat.CCPString
    ...
    sys.std.Core.Type.Universal
        ...
        sys.std.Core.Type.Relation
            sys.std.Core.Type.DHRelation
                ...
                sys.std.Core.Type.Cat.SysScaValStmtNodeSet
...
C<Core_Text>, C<Unicode_Text>,
C<Name>, C<NameChain>, C<Comment>, C<Order>,
...
=head2 sys.std.Core.Type.Cat.SysText

The C<SysText> type is explicitly defined as a domain-union type over all
system-defined character string root types, and just those types: 
C<Core_Text>, C<Unicode_Text>.
The C<SysText> type is mainly intended to be used as the declared type of
some attributes of some other system-defined catalog types.  The C<SysText>
type is I<not> intended to be used as the declared type of any user type
attributes, generally speaking; if they would even consider it, they should
be using C<Text> instead.  Its default value is the empty string value of
the C<Core_Text> type.  The cardinality of this type is infinity.
...
=head2 sys.std.Core.Type.Cat.CCPString

C<CCPString> (core abstract codepoint string) is a proper subtype of
C<String> where all member value elements are between zero and 127
inclusive.  One can be used to represent a string of characters of a
128-character repertoire.  Considering the low-level type system, for every
C<CCPString> value, its second C<Structure> element is the C<Int> value
C<-3> (text with 7-bit ASCII char repertoire).
...
string, which has 1 C<SysText>-typed attribute whose name is the empty
string.  The C<Name> type explicitly composes the C<Ordered> mixin type.
A C<Name> is a simple wrapper for a C<SysText> and all of its other
...
explicitly disjoint from C<SysText> due to having a different intended
...
C<SysText>-based possrep and its default value) is the same as C<Name>
...
A C<Package> has these 19 attributes:
...
functions, updaters, recipes, procedures.
...
the C<Core> module: in the C<Type> namespace: C<Universal>, C<Empty>,
C<Int>; in the C<Type.Cat> namespace: C<List>.  These constitute all 2
Muldis D declaration types, all 2 Muldis D type system maximal and minimal
...
must have possrep attribute values of the same C<Text>-composing scalar
root type (such as C<Core_Text> or C<Unicode_Text>), and hence are all
directly comparable.  Similarly, a C<Package> is constrained such that all
of its C<Comment>-typed components must have possrep attribute values of
the same C<Text>-composing scalar root type.
...
specifies a set of functions|updaters|recipes|procedures
...
This is the fully-qualified name, in the C<nlx.[cat|lib|data]> namespace,
of any hypothetical immediate child namespace of the package, of the
function|updater|recipe|procedure's parent [|sub]package.
...
provided by the larger context that embeds the
C<Updater>, which is either a C<Package> or C<System>.  Every C<Updater>
must have a specified I<heading>, but having a specified I<body> is
...
the C<Recipe>, which is either a C<Package> or C<System>.  Every C<Recipe>
must have a specified I<heading>, but having a specified I<body> is
...
=item C<sys_sca_val_stmts> - C<SysScaValStmtNodeSet>
...
=head2 sys.std.Core.Type.Cat.SysScaValStmtNodeSet

An C<SysScaValStmtNodeSet> is a C<DHRelation> that specifies a set of
...
An C<SysScaValStmtNodeSet> has these 4 attributes:
...
An C<SysScaValStmtNodeSet> has a unary primary key on the C<name>
attribute.  Its default value is empty.

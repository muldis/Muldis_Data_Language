...
An C<IvlSelExprNodeSet> is a C<DHRelation> that specifies a set of value
expression nodes where each node represents an interval-union value
selection.

An C<IvlSelExprNodeSet> has these 3 attributes:
...
then you must use a C<RelSelExprNodeSet> rather than an
C<IvlSelExprNodeSet>.

=back

An C<IvlSelExprNodeSet> has a unary primary key on the C<name> attribute.
Its default value is empty.

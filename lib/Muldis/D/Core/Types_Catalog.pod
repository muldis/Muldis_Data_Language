...
1..N C<Name> (represented by a C<Cat.NESeqOfName>), the 1..N elements
...
details); while being a character string like C<Name>, the two are
disjoint.

=head2 sys.Core.Cat.NESeqOfName

A C<Cat.NESeqOfName> is a C<Seq> whose C<value> attribute has a declared
type of C<Name> and that must have at least 1 element; this type is
...
C<name> has the default value of C<Name>, it has zero parameters, zero
...
C<name> (a C<Name>) and C<type> (a C<Cat.NameChain>); the C<name> is
...
and C<expr>, each of those being a C<Name>; the C<name> is the name of

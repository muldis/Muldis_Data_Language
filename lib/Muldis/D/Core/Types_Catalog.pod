...
This document is Muldis::D::Core::Types_Catalog version 0.147.0.
    ...
    sys.std.Core.Type.Universal
        ...
        sys.std.Core.Type.Tuple
            sys.std.Core.Type.DHTuple
                ...
                sys.std.Core.Type.Cat.IfThenExprStmtMap
...
statement.  An C<IfElseStmtNodeSet> is essentially the procedural
version of the functional C<IfElseExprNodeSet>.
...
=item C<if_then> - C<array_of.IfThenExprStmtMap>

This is a sequence of 0..N conditional expressions, each of which
has an associated statement.  Each conditional expression will be tested in
order while the previous conditional resulted in C<Bool:False>; once a
conditional results in C<Bool:True>, its associated statement will be
...
=head2 sys.std.Core.Type.Cat.IfThenExprStmtMap

An C<IfThenExprStmtMap> is a C<DHTuple>.  It defines an element of an
if-else sequential statement.  An C<IfThenExprStmtMap> has 2 attributes,
C<if> (a C<Name>) and C<then> (a C<Name>); C<if> has the name of a
local expression node (or parameter), and C<then> has the name of a
statement node.  The C<if> node is the condition to evaluate; if that
condition evaluates to C<Bool:True>, then the C<then> statement node is

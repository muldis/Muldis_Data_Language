...
This document is Muldis::D::Core::Types_Catalog version 0.126.0.
    ...
    sys.std.Core.Type.Universal
        ...
        sys.std.Core.Type.Relation
            sys.std.Core.Type.DHRelation
                ...
                sys.std.Core.Type.Cat.NilFuncInvoStmtNodeSet
                ...
                sys.std.Core.Type.Cat.KeyConstrSet
                sys.std.Core.Type.Cat.DistribKeyConstrSet
...
C<RoundMeth>, C<RatRoundRule>, C<Singleton>.  The C<OVLScalar> type is
...
namespace.  The C<Name> type explicitly composes the C<Stringy> mixin
...
subdepot hierarchy.  The C<NameChain> type explicitly composes the
...
=head2 sys.std.Core.Type.Cat.APImperativeNC

C<APImperativeNC> is a non-proper subtype of C<APNonTypeNC>.  Its default
...
=head2 sys.std.Core.Type.Cat.RPImperativeNC

C<RPImperativeNC> is a non-proper subtype of C<RelPathMaterialNC> and is
analogous to the C<APImperativeNC> type.
...
expression nodes.  The C<Comment> type explicitly composes the C<Stringy>
...
These are all the system-defined namespaces where not-lexical DBMS entities
may live, or that otherwise always exist due to being system-defined.
Specifically, it declares these 18 standard language namespaces:
...
A C<Depot> has these 14 attributes:
...
stimulus-response rules that a [|sub]depot might directly contain.
...
This is the fully-qualified name, in the C<nlx.[cat|lib|data]> namespace of
any hypothetical immediate child namespace of the depot, of the
stimulus-response rule's parent [|sub]depot.
...
=item C<implements> - C<set_of.RPImperativeNC>
...
invocation of an updater or recipe.  C<MultiUpdStmt> itself has no
restriction on the number of tuples it consists of, but:  At least 1 tuple
is required iff the multi-update statement is composed into a specified
updater or recipe body; otherwise there must be only be zero tuples for an
unspecified updater or recipe body.

A C<MultiUpdStmt> has these 5 attributes:
...
=item C<imperative> - C<MaterialNC>

This is the name of the updater or recipe being invoked.

=item C<upd_args> - C<NameExprMap>

These are the 1..N subject-to-update arguments to the updater
invocation, or the 0..N subject-to-update arguments to the recipe
...
These are the 0..N read-only arguments to the updater or recipe invocation.
...
C<MultiUpdStmt> has a distributed primary key over the C<name>
...
A C<Recipe> has these 10 attributes, of which the 7 C<upd_params>,
C<ro_params>, C<opt_params>, C<upd_global_params>, C<ro_global_params>
C<dispatch_params>, C<implements>, define the I<heading> and the 2
C<exprs>, C<stmt> define the I<body>:
...
=item C<upd_global_params> - C<RcpGlobalVarAliasMap>
...
=item C<ro_global_params> - C<RcpGlobalVarAliasMap>
...
This defines the entire body of the recipe, which is composed of
a single multi-update statement.  This multi-update statement must
reference all of the parameters and global-aliases that the recipe has.
...
allowed actions (it may not invoke any recipes / globals) and its execution
...
name of the root node must be the empty string.  The root node must also be
a compound statement node, meaning a tuple of the procedure's
C<stmt.compound_stmts> attribute; while making this requirement isn't
...
A C<StmtNodeSet> has these 11 attributes:
...
=item C<nil_func_invo_stmts> - C<NilFuncInvoStmtNodeSet>

These are statement nodes that represent niladic function invocations and
the assignment to local variables of the results of those invocations.

=item C<imp_invo_stmts> - C<ImpInvoStmtNodeSet>

These are statement nodes that represent imperative routine (updater,
recipe, or procedure) invocations.
...
=head2 sys.std.Core.Type.Cat.NilFuncInvoStmtNodeSet

A C<NilFuncInvoStmtNodeSet> is a C<DHRelation> that specifies a set of
atomic statement nodes where each node represents the assignment to a
variable (or parameter) of the result of an invocation of a niladic
function (with no arguments).  The invoked function can be either
user-defined or system-defined.  Using one of these statement nodes should
be an effective alternative to declaring (or extending) an updater that
simply combines a single niladic-function-invoking expression node with an
C<assign> updater call, such as when you want to set a variable to an
arbitrarily complex but statically defined value.

A C<NilFuncInvoStmtNodeSet> has these 4 attributes:
...
A C<NilFuncInvoStmtNodeSet> has a unary primary key on the C<name>
attribute.  Its default value is empty.

=head2 sys.std.Core.Type.Cat.ImpInvoStmtNodeSet

An C<ImpInvoStmtNodeSet> is a C<DHRelation> that specifies a set of
statement nodes where each node is an invocation of an imperative routine
(updater, recipe, or procedure) with specific arguments.  The invoked
routine can be either user-defined or system-defined.  Iff the invoked
routine is an updater or a recipe, then that statement node is an atomic
statement node.
...
These are the 0..N subject-to-update arguments to the imperative routine
invocation, as-per C<ro_args>; but iff the routine being invoked is an
updater, then there must instead be 1..N subject-to-update arguments,
because an updater must take at least 1 subject-to-update argument.

=item C<ro_args> - C<IISArgVarMap>

These are the 0..N read-only arguments to the imperative routine
invocation.  Each element defines one argument value, with the element
C<arg> matching the invoked routine's parameter name, and the element
C<var> naming a local or global variable or statement-containing-procedure
parameter which defines the value.
...
C<var> (a C<Name>); the C<arg> is the name of the routine argument,
...
of C<ImpInvoStmtNodeSet> named I<try> and I<catch>, where I<try> is
...
C<if> (a C<Name>) and C<then> (a C<Name>); C<if> has the name of a
...
=item C<distrib_key_constraints> - C<DistribKeyConstrSet>

These are the 0..N explicit simple distributed (unique) key constraints that are
...
C<distrib_key_constraints> may be privileged as the I<primary key>.  Note that
C<distrib_key_constraints> is logically an abstraction syntax (the canonical simplest
...
(foreign key constraints) that are applicable to tuple/database values of the type
...
key constraints) that are applicable to tuple/database values of the type being
...
C<distrib_key_constraints> and C<[|distrib_]subset_constraints>.
...
=item C<key_constraints> - C<KeyConstrSet>

These are the 0..N explicit (unique) key constraints that are applicable to relation
...
key constraints, then there is an implicit (unique) key over all attributes of the
...
may be in it at once.  If any explicit key constraints are defined, then every one
...
consist of no more than one tuple.  At most one key in C<key_constraints> may be
privileged as the I<primary key>.  Note that C<key_constraints> is logically an
...
the C<name> attributes of C<key_constraints>, and the similar elements or C<name>
...
=head2 sys.std.Core.Type.Cat.KeyConstrSet

A C<KeyConstrSet> is a C<DHRelation> that specifies a set of
unique key constraints for a relation type.

A C<KeyConstrSet> has these 4 attributes:
...
=item C<material> - C<KeyConstr>

This defines the entire constraint sans its name.

=back

A C<KeyConstrSet> has a unary primary key on the C<name> attribute.  Its
default value is empty.

=head2 sys.std.Core.Type.Cat.DistribKeyConstrSet

A C<DistribKeyConstrSet> is a C<DHRelation> that specifies a set of
distributed (unique) key constraints over relation-valued attributes of a
tuple/database.

A C<DistribKeyConstrSet> has these 4 attributes:
...
=item C<material> - C<DistribKeyConstr>

This defines the entire constraint sans its name.

=back

A C<DistribKeyConstrSet> has a unary primary key on the C<name> attribute.
Its default value is empty.

=head2 sys.std.Core.Type.Cat.SubsetConstrSet

A C<SubsetConstrSet> is a C<DHRelation> that specifies a set of
(non-distributed) subset constraints over relation-valued
attributes of a tuple/database.

A C<SubsetConstrSet> has these 4 attributes:
...
=item C<material> - C<SubsetConstr>
...
distributed subset constraints over relation-valued
attributes of a tuple/database.

A C<DistribSubsetConstrSet> has these 4 attributes:
...
=item C<material> - C<DistribSubsetConstr>
...
are a union of those from the zero or more other not-C<Empty> types that
...
a relation over an extended term (if multiple key constraints conceptually have all
...
which invokes a recipe or procedure automatically in response to some
...
This is the name of the recipe or procedure being invoked.

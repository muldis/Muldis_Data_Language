...
specifically all the relcons and relvars with the names C<cat|sys|imp|mnt>
and C<(fed|dep|sdp|pkg).cat>.  The system catalog constant named C<cat> is
of the C<Cat.System> type.
...
these subdepots are organized into a tree whose root is the depot.

A C<Cat.SubdepotSet> has these 3 attributes:

=over

=item C<parent> - C<Cat.NameChain>

This is the fully-qualified name, in the C<dep> namespace, of the
subdepot's parent subdepot, which is often just the depot itself.

...
C<sys.std.Core.Type.Cat.MaterialRef> type.
...
context of its creation.  Its default value is as per C<Material>, which is
a reference to the C<sys.std.Core.Type.Universal> data type.
...
function name.  It is used to implement a higher-order function or a
...
C<FuncRef> where every member value's conceptual C<NameChain> is a
C<order_determination> function name.  Its default value is a reference to
the C<sys.std.Core.Scalar.order> function.  This type is conceptually

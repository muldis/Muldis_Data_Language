...
This document is Muldis::D::Core::Types_Catalog version 0.54.0.
...
default to the system-defined q/scalar ordering function when no argument is
given to it.
...
Specifically, it declares these 11 standard types: in the
C<sys.std.Core.Type> namespace, C<Universal>, C<Empty>, C<QScalar>,
C<QTuple>, C<QRelation>, C<Reference>, C<Remnant>; in the
C<sys.std.Core.Type.Cat> namespace, C<(Func|Proc|Type|OrdDetFunc)Ref>.

=item C<sca_npr_types> - C<SysScaNPRTypeSet>

These are all the system-defined scalar root (not union etc) types
...
A C<SysScaNPRTypeSet> is a C<Relation> that specifies a set of
system-defined scalar root (not union etc) types that have no possreps.

A C<SysScaNPRTypeSet> has these 3 attributes:
...
for a q/tuple and q/relation type having that heading, or it defines a subtype
of some other (|quasi-)nonscalar type.  That is, it defines 2 new data
types, a q/tuple type and a q/relation type with matching headings.  Note that
...
Note that you can not use an C<InnerNonscalarType> to declare or
...
q/tuple or q/relation type, the only way to reference it as the other kind of
type is by using the C<type.(|quasi_)(tuple|relation)_from.\w+> referencing syntax.
...
applicable to q/tuple/database values of the type being defined, that range
over and relate q/tuples of specified q/relation-typed attributes of it; they
are a kind of referential constraint.  Each q/tuple of a child attribute must
have a corresponding q/tuple in a specific single parent attribute, where
...
relational difference, where a projection of the parent q/relation is
subtracted from a corresponding projection of the child q/relation (with
...
difference q/tuples are from child q/tuples that violate the foreign key.
...
over and relate q/tuples of specified q/relation-typed attributes of it; they
are a kind of referential constraint.  Each q/tuple of a child attribute must
have a corresponding q/tuple in one member of a specific set of
...
being defined; it is as per C<foreign_keys> except that the parent q/relation
is the result of unioning appropriately renamed projections of the member
q/relations of the distributed key.
...
C<R2>, then a q/tuple insertion into C<V> could be rewritten at least 3 ways,
...
defined as per-q/tuple operations, separately for insert/substitute/delete.>
...
derives a q/tuple of dependent attribute values from a q/tuple of determinant
attribute values.  The implicit function heading that this goes with has a
single C<topic> parameter whose declared type is a q/tuple whose attributes
match those of C<determinant_attrs>; the function's implicit result type is
a q/tuple whose attributes match those of C<dependent_attrs>.  Note that the
...
This defines the 0..N host q/relation attributes comprising the key.  If this
set is empty, then we have a nullary key which restricts the host q/relation
to have a maximum of 1 q/tuple.

=item C<is_primary> - C<Bool>

This is C<Bool:true> if this key has been designated the I<primary key> of
the q/relation (a q/relation may have at most one, or none, of those); it is
...
likely to be treated as immutable in practice for the q/relation's q/tuples,
and hence are the best candidates for identifying q/tuples within a q/relation
...
of a q/tuple having an identity that is distinct from the sum total of all
its attribute values, such that one can say that a q/tuple is being "updated"
(rather than its host q/relation being the only thing that is updated to hold
a different set of q/tuples) is dependent in Muldis D on the host q/relation
having a primary key; if a q/tuple in a q/relation is replaced by a distinct
q/tuple whose values in the primary key attributes are identical, it is only
in this situation that we can consider that we still have the same q/tuple,
which was just updated, and that we have not lost the old q/tuple and gained
a new one.  The q/relation attributes comprising a primary key are the best
mapping values for automatic new foreign keys and join conditions if the
host q/relation type has to be auto-split into several associated ones, for
example because the physical representation of this q/relation doesn't
support RVAs; in fact, some implementations may require that any q/relation
having an RVA must also have an explicit primary key, so it is easier for
them to choose the key to automatically relate a split q/relation on.
...
q/relations participating in the distributed key to have a maximum of 1 q/tuple
between them.  Note that the distributed key attribute names don't have to
match the names of any participating q/relation attributes.

=item C<relations> - C<DKMemRelAttrMap>

This names the 0..N q/relation-valued attributes of the host q/tuple/database
...
q/relation-valued attributes of a host q/tuple/database type that are
...
This is the name of the q/relation-valued attribute that is participating in
...
has more, then the host type attribute is a q/tuple/database and any further
...
a q/relation-valued attribute of a host q/tuple/database type participating in
...
(non-distributed) keys over q/relation-valued attributes of a q/tuple/database;
each tuple specifies one foreign key, which is a kind of referential
constraint, that relates q/tuples of such attributes.  Each q/tuple of a child
...
This is the name of the q/relation-valued attribute that is the parent in the
...
This is the name of the q/relation-valued attribute that is the child in the
...
foreign distributed keys over q/relation-valued attributes of a
q/tuple/database; each tuple specifies one foreign key, which is a kind of
referential constraint, that relates q/tuples of such attributes.  Each q/tuple
of a child attribute must have a corresponding q/tuple in one member of a
...
element; if it has more, then the host type attribute is a q/tuple/database
...
attributes of a child q/relation-valued attribute of a host q/tuple/database
...
An C<InnerRestrType> is a C<Tuple>.  It provides a relatively terse
...
the foundation for most componentized type definitions, including all q/tuple
and q/relation types (for which it is named I<heading>), and it is used also
for the components list of a q/scalar possrep.  It is also used to define
...
that function on each q/tuple of a q/relation it is filtering, and the function
has the criteria for whether the q/tuple passes the filter.

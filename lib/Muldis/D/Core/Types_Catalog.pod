...
This document is Muldis::D::Core::Types_Catalog version 0.146.0.
    ...
    sys.std.Core.Type.Universal
        ...
        sys.std.Core.Type.Relation
            sys.std.Core.Type.DHRelation
                ...
                sys.std.Core.Type.Cat.[Function|Recipe|Procedure]Set
...
A C<Package> has these 18 attributes:
...
=item C<functions|recipes|procedures> -
C<[Function|Recipe|Procedure]Set>

These are all the definitions that this [|sub]package contains of
functions, recipes, procedures.
...
=head2 sys.std.Core.Type.Cat.[Function|Recipe|Procedure]Set

A C<[Function|Recipe|Procedure]Set> is a C<DHRelation> that
specifies a set of functions|recipes|procedures
...
A C<[Function|Recipe|Procedure]Set> has these 5 attributes:
...
This is the fully-qualified name, in the C<nlx.[cat|lib|data]> namespace,
of any hypothetical immediate child namespace of the package, of the
function|recipe|procedure's parent [|sub]package.

=item C<name> - C<Name>

This is the declared name of the function|recipe|procedure within
...
function|recipe|procedure as a whole.
...
=item C<material> - C<Function|Recipe|Procedure>

This defines the entire function|recipe|procedure sans its name.
...
A C<[Function|Recipe|Procedure]Set> has a binary primary key on the
...
function|recipe.  Each tuple of an C<ExprNodeSet> attribute is a
...
Only functions and recipes may contain expression
...
named functions or recipes.  Note that, while the
...
happens or recipes where the expression is used as an argument
for a subject-to-update parameter of a recipe call; in
that case the leaf nodes / only node of the expression must map to a
subject-to-update parameter of the expression-containing recipe.
...
when generating concrete Muldis D code from a C<Function|Recipe>,
...
accessed via the host recipe's subject-to-update parameters
...
indirectly by way of recipe invocations, as each
recipe body is itself composed entirely of 1 multi-update statement (plus
supporting value expressions).  Only recipes may contain
...
encapsulated by named recipes.  Each tuple of a multi-update
statement is a single update statement, which represents a single
invocation of an recipe.  C<MultiUpdStmt> itself has no
...
recipe body; otherwise there must be only be zero tuples for an
unspecified recipe body.
...
recipe's multi-update statement.
...
These are the 0..N read-only arguments to the recipe invocation.
...
This indicates the subset of the recipe's subject-to-update or read-only
parameters that are optional, that is, do not need to be supplied explicit
arguments when the function is invoked; any recipe parameters not named
...
discarded after the recipe finishes executing.  Each element of
...
This is as per the same-named attribute of C<Recipe>.
...
These are statement nodes that represent imperative routine
(updater or procedure) invocations.
...
(recipe or procedure) with specific arguments.  The invoked
routine can be either user-defined or system-defined.  Iff the invoked
routine is a recipe, then that statement node is an atomic
statement node.
...
argument lists for routine invocations by functions or recipes.

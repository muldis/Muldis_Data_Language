...
A C<KeyConstrSet> is a C<DHRelation> that specifies a set of candidate keys or
unique key constraints for a relation type; each tuple specifies one
candidate key.

A C<KeyConstrSet> has these 5 attributes:
...
A C<KeyConstrSet> has a unary primary key on the C<name> attribute, and a unary
(unique) key on its C<attrs> attribute.  Its default value is empty.

=head2 sys.std.Core.Type.Cat.DistribKeyConstrSet

A C<DistribKeyConstrSet> is a C<DHRelation> that specifies a set of candidate
...
A C<DistribKeyConstrSet> has these 6 attributes:
...
This has the same meaning as C<is_primary> of C<KeyConstrSet> but for being
distributed as if the relations distributed over were one relation.

=back

A C<DistribKeyConstrSet> has a unary primary key on the C<name> attribute, and a
...
(non-distributed) subset constraints (foreign key constraints) over relation-valued
...
distributed subset constraints (foreign key constraints) over relation-valued
attributes of a tuple/database; each tuple specifies one subset constraint,
which is a kind of referential constraint, that relates tuples of such
attributes.  Each tuple of a child attribute must have a corresponding

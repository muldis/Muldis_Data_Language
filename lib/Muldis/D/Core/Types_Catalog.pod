...
These are expression nodes that define [|quasi-]scalar literals specified
...
expression nodes where each node represents a [|quasi-]scalar literal that
is specified simply in terms of possrep attributes.

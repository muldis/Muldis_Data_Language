...
This document is Muldis::D::Core::Types_Catalog version 0.103.0.
    ...
    sys.std.Core.Type.Universal
        ...
        sys.std.Core.Type.Relation
            sys.std.Core.Type.DHRelation
                ...
                sys.std.Core.Type.Cat.Sys[Scalar|Nonscalar]TypeSet
                sys.std.Core.Type.Cat.Sys[Union|Subset]TypeSet
                ...
                sys.std.Core.Type.Cat.[Scalar|Nonscalar]TypeSet
                sys.std.Core.Type.Cat.[Union|Subset]TypeSet
                ...
                sys.std.Core.Type.Cat.IvlSelExprNodeSet
...
C<nlx[.par]**0..*.[cat|lib|data]>, C<lex>, C<type>.  One can be used to
reference a material (routine or type or etc) for invocation, either
system-defined or user-defined, or one can be used to reference a variable
(or pseudo-variable or parameter or named expression or statement), either
a system-catalog or normal data variable.  Its default value is a reference
...
C<nlx[.par]**0..*.lib>, C<type>.  One can be used to reference a material
(routine or type or etc) for invocation, either system-defined or
...
or C<fed> (or a C<type>-prefix followed by those) but not C<nlx>.  One is
used when conceptually a routine or type is being passed as an argument to
a routine, such as because it is a higher-order function or closure, and it
is in fact the name of the invocant being passed; only an absolute path can
be used in this situation for the general case because the target is being
invoked from a different context than where the reference to the target is
being selected; a relative path doesn't work because C<nlx> means something
different on each side of the NC-argument-taking-routine.  Conceptually
speaking, an C<AbsPathMaterialNC> that points to a routine I<is> a closure,
or a higher-order function if it points to a function.

=head2 sys.std.Core.Type.Cat.APNonTypeNC

This is an enumeration data type.  C<APNonTypeNC> is a proper subtype of
C<AbsPathMaterialNC> that excludes the C<type>-prefix values.  Its default
...
This is an enumeration data type.  C<APTypeNC> is a non-proper subtype of
C<AbsPathMaterialNC>.  Its default value is a reference to the
...
of C<MaterialNC> where every member value's chain starts with either C<sys>
or C<nlx> (or a C<type>-prefix followed by those) but not C<fed>.  One is
used in a context where a user-defined routine or type may only be invoked
directly when both the invoker and invoked are in the same depot.
...
it declares these 18 standard language namespaces:
C<[sys|mnt|fed|nlx|lex]> (which have the nameless global
...
=item C<[scalar|nonscalar|union|subset]_types> -
C<Sys[Scalar|Nonscalar|Union|Subset]TypeSet>
...
nonscalar types, and union types, and subset types.
...
=head2 sys.std.Core.Type.Cat.Sys[Scalar|Nonscalar|Union|Subset]TypeSet

A C<Sys[Scalar|Nonscalar|Union|Subset]TypeSet> is a C<DHRelation> that
specifies a set of scalar|nonscalar|union|subset system-defined types.

A C<Sys[Scalar|Nonscalar|Union|Subset]TypeSet> has these 4 attributes:
...
This is the fully-qualified name, in the nameless global root namespace, of
the scalar|nonscalar|union|subset type's parent system namespace.

=item C<name> - C<Name>

This is the declared name of the scalar|nonscalar|union|subset type
...
scalar|nonscalar|union|subset type as a whole.

=item C<material> - C<[Scalar|Nonscalar|Union|Subset]Type>

This defines the entire scalar|nonscalar|union|subset type sans its name.

=back

A C<Sys[Scalar|Nonscalar|Union|Subset]TypeSet> has a binary primary key on
...
A C<Depot> has these 10 attributes:
...
=item C<[scalar|nonscalar|union|subset]_types> -
C<[Scalar|Nonscalar|Union|Subset]TypeSet>

These are all the definitions that this [|sub]depot contains of scalar
types with possreps, complete nonscalar types, union types, and subset
types.
...
An C<[Function|Updater|Procedure]Set> is a C<DHRelation> that specifies a
...
An C<[Function|Updater|Procedure]Set> has these 4 attributes:
...
An C<[Function|Updater|Procedure]Set> has a binary primary key on the
C<parent> plus C<name> attributes.  Its default value is empty.

=head2 sys.std.Core.Type.Cat.[Scalar|Nonscalar|Union|Subset]TypeSet

An C<[Scalar|Nonscalar|Union|Subset]TypeSet> is a C<DHRelation> that
specifies a set of scalar|nonscalar|union|subset types that a
[|sub]depot might directly contain.  I<TODO: each type may be either public
for the DBMS as a whole or private to the subdepot.>

An C<[Scalar|Nonscalar|Union|Subset]TypeSet> has these 4 attributes:
...
This is the fully-qualified name, in the C<nlx.[cat|lib|data]> namespace
of any hypothetical immediate child namespace of the depot,
of the scalar|nonscalar|union|subset type's parent [|sub]depot.
...
An C<[Scalar|Nonscalar|Union|Subset]TypeSet> has a binary primary key on
the C<parent> plus C<name> attributes.  Its default value is empty.
...
An C<ExprNodeSet> has these 13 attributes:
...
=item C<ivl_sel_exprs> - C<IvlSelExprNodeSet>

These are expression nodes that represent interval value selections.
...
=head2 sys.std.Core.Type.Cat.IvlSelExprNodeSet

A C<IvlSelExprNodeSet> is a C<DHRelation> that specifies a set of value
expression nodes where each node represents an interval value selection.

A C<IvlSelExprNodeSet> has these 3 attributes:
...
=item C<interval> - C<interval_of.Name>

These represent the attributes of the interval value being selected.

Note that, because of how C<IvlSelExprNodeSet> is defined, the
C<excludes_[min|max]> attribute values of the interval are compile time
constants, since a boolean is stored in the system catalog for each rather
than the name of an expression node like with the C<min> and C<max>
attributes; if you actually want the interval value being selected at
runtime to have runtime-determined C<excludes_[min|max]> attribute values,
then you must use a C<TupSelExprNodeSet> rather than a
C<IvlSelExprNodeSet>.

=back

A C<IvlSelExprNodeSet> has a unary primary key on the C<name> attribute.
Its default value is empty.
...
An C<ProcInvoStmtNodeSet> is a C<DHRelation> that specifies a set of
...
An C<ProcInvoStmtNodeSet> has these 5 attributes:
...
An C<ProcInvoStmtNodeSet> has a unary primary key on the C<name>
attribute.  Its default value is empty.  There is a distributed primary
key over the C<name> attributes of C<upd_args> and C<ro_args>.
...
An C<TryCatchStmtNodeSet> has these 4 attributes:
...
C<Single> whose sole element matches the declared name of a
...
matches the declared name of a C<named-value> function that
...
matches the declared name of an C<order-determination> function.  The
...
matches the declared name of a C<type-constraint> function that
...
This matches the declared name of a C<possrep-map> function that
...
This matches the declared name of an inverse C<possrep-map> function
...
=item C<tuple_constraints> - C<set_of.RPFunctionNC>

This matches the declared names of 0..N C<type-constraint> functions
...
=item C<relation_constraints> - C<set_of.RPFunctionNC>
...
=item C<default_tuple> - C<maybe_of.RPFunctionNC>
...
=item C<default_relation> - C<maybe_of.RPFunctionNC>
...
This matches the declared name of a C<virtual-attr-map> function
...
This matches the declared name of a C<named-value> function that
...
An C<SubsetType> is a C<DHTuple>.  It provides a relatively terse
...
defined attribute list using a C<SubsetType> (or a C<UnionType>).

An C<SubsetType> has these 4 attributes:
...
parameters and its result type is the same as the subset type whose

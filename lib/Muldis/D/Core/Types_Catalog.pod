...
A C<NonscalarType> is a C<DHTuple>.  It defines either a new
...
Note that you can not use a C<NonscalarType> to declare or subtype an
incomplete type, as it (or its supertype) must specify a complete set of
attributes.

A C<NonscalarType> has these 13 attributes:

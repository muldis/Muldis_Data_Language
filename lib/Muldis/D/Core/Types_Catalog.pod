...
This document is Muldis::D::Core::Types_Catalog version 0.105.0.
...
An C<[Scalar|Tuple|Relation|Domain|Subset]TypeSet> is a C<DHRelation> that
...
An C<[Scalar|Tuple|Relation|Domain|Subset]TypeSet> has these 4 attributes:
...
An C<[Scalar|Tuple|Relation|Domain|Subset]TypeSet> has a binary primary key
...
These are expression nodes that represent interval-union value selections.
...
expression nodes where each node represents an interval-union value
selection.
...
=item C<elems> - C<interval_of.Name>

These represent the elements, each of which is a (non-union) interval of
the interval-union value being selected.  When this value expression is
evaluated, if any child expression nodes are such that any duplicate tuples
might be input to this C<IvlSelExprNodeSet> selector, the duplicates are
silently eliminated and do not constitute a failure condition.

Note that, because of how C<IvlSelExprNodeSet> is defined, the
C<excludes_[min|max]> attribute values of the interval-union are compile
time constants, since a boolean is stored in the system catalog for each
rather than the name of an expression node like with the C<min> and C<max>
...
then you must use a C<RelSelExprNodeSet> rather than a
C<IvlSelExprNodeSet>.
...
A C<DomainType> has these 7 attributes:

...
This document is Muldis::D::Core::Types_Catalog version 0.77.0.
    ...
    sys.std.Core.Type.Universal
        ...
        sys.std.Core.Type.Relation
            sys.std.Core.Type.DHRelation
                ...
                sys.std.Core.Type.Cat.DepMaterialSet
                sys.std.Core.Type.Cat.PkgMaterialSet
...
has a different default value, which is C<sys.std.Core.Scalar.order>
...
default to the system-defined scalar ordering function when no argument
is given to it.
...
user-defined types.  Specifically, it declares these 12 standard types: in
the C<sys.std.Core.Type> namespace, C<Universal>, C<Empty>, C<Scalar>,
C<Tuple>, C<Relation>, C<Reference>, C<External>; in the
C<sys.std.Core.Type.Cat> namespace, C<String> and
...
that are each implicit maximal types of 2 of 4 non-remnant type categories
(the other 2
...
A C<Depot> has these 5 attributes:
...
=item C<materials> - C<PkgMaterialSet>

These are all the definitions that this [|sub]depot contains of functions,
updaters, procedures, scalar types with possreps, complete nonscalar types,
union types, and restriction types.
...
A C<PackageSet> is a C<DHRelation> that specifies the set of packages
...
=head2 sys.std.Core.Type.Cat.DepMaterialSet

A C<DepMaterialSet> is a C<DHRelation> that specifies a set of routines and
types that a [|sub]depot might directly contain.

A C<DepMaterialSet> has these 4 attributes:
...
This is the fully-qualified name, in the C<dep.[cat|lib|data]> namespace,
of the routine's or type's parent [|sub]depot.
...
A C<DepMaterialSet> has a binary primary key on the C<parent> plus C<name>
attributes.  Its default value is empty.
...
A C<Package> has these 3 attributes:
...
These are all the definitions that this package contains of functions,
updaters, procedures, scalar types with possreps, complete nonscalar types,
union types, and restriction types.
...
=head2 sys.std.Core.Type.Cat.PkgMaterialSet

A C<PkgMaterialSet> is a C<DHRelation> that specifies a set of routines and
types that a package might contain, where each routine or type may be
either public for the DBMS as a whole or private to the package.

A C<PkgMaterialSet> has these 4 attributes:

=over

=item C<name> - C<Name>

This is the declared name of the routine or type.
...
This is C<Bool:true> if the routine or type is part of the package's own
public interface and may be invoked from outside the package; it is
...
A C<PkgMaterialSet> has a unary primary key on the C<name> attribute.  Its
default value is empty.
...
composed into a C<Depot> or C<Package>.
...
C<[Func|Proc|Type|OrdDetFunc]RefSelExprNodeSet>
...
A C<[Func|Proc|Type|OrdDetFunc]RefSelExprNodeSet> is a C<DHRelation>
that specifies a set of expression nodes where each node represents a value
of the special C<sys.std.Core.Type.Cat.[Func|Proc|Type|OrdDetFunc]Ref>
type.

A C<[Func|Proc|Type|OrdDetFunc]RefSelExprNodeSet> has these 3
attributes:
...
A C<[Func|Proc|Type|OrdDetFunc]RefSelExprNodeSet> has a unary (unique)
...
=item C<default> - C<maybe_of.InnerFuncBody>

Iff it is a C<Single>, then C<default>
defines the entire (main) body of a C<type_default> function that
...
C<base_type> is C<nothing>, then semantics are as if it were a defined body
...
are the same as the scalar root type whose declaration the
...
=item C<constraint> - C<maybe_of.InnerFuncBody>

Iff it is a C<Single>, then C<constraint>
defines the entire (main) body of a C<type_constraint> function that
...
as if it were a defined body that unconditionally resulted in C<Bool:true>.
...
=item C<default_tuple> - C<maybe_of.InnerFuncBody>
...
a defined body that resulted in a value of the type being defined where all
...
=item C<default_relation> - C<maybe_of.InnerFuncBody>
...
C<default_relation> were a defined body that resulted in a value of the
...
derives a tuple of dependent attribute values from a tuple of
...
result type is a tuple whose attributes match those of
...
A C<ConstraintSet> is a C<DHRelation> that specifies a set of named generic

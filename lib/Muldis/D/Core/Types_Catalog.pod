...
This document is Muldis::D::Core::Types_Catalog version 0.80.0.
    ...
    sys.std.Core.Type.Universal
        ...
        sys.std.Core.Type.Tuple
            sys.std.Core.Type.DHTuple
                ...
                sys.std.Core.Type.Cat.Material
                ...
                sys.std.Core.Type.Database
                    ...
                    sys.std.Core.Type.Cat.DepotOrPackage
        ...
        sys.std.Core.Type.Relation
            sys.std.Core.Type.DHRelation
                ...
                sys.std.Core.Type.Cat.MaterialSet
                ...
                sys.std.Core.Type.Cat.AliasExprNodeSet
...
it declares these 29 standard language namespaces:
C<[sys|mnt|tpc|fed|dep|sdp|pkg|inn|lex]> (which have the nameless global
root namespace as their parent, spelled as the empty C<DeclNameChain>
value, that isn't also declared here), C<sys.[cat|std|imp]>,
C<[mnt|tpc].cat>, C<[fed|dep|sdp|pkg].[cat|lib|data]>, C<sys.std.Core>,
...
functions|updaters|procedures.  (Note that C<procedures> also specifies the
system-defined system services and bootloader exclusives.)
...
also specifies the system-defined system services, bootloader exclusives.)
...
A C<SysTypeSet> is a C<DHRelation> that specifies a set of system-defined
types.
...
=head1 TYPES FOR DEFINING DEPOTS AND SUBDEPOTS AND PACKAGES

These are all enumeration data types.

=head2 sys.std.Core.Type.Cat.DepotOrPackage

A C<DepotOrPackage> is a C<Database>.  It specifies the entire system
...
the root.  It also triples to specify the entire system catalog of a single
package, which provides a privatizing namespace for a subset of a depot's
entities; entities outside a package can only directly invoke that
...
subdepots or other packages.  The system catalog variables named C<dep.cat>
and C<sdp.cat> and  C<pkg.cat> are all of the C<DepotOrPackage> type.

A C<DepotOrPackage> has these 5 attributes:

=over

=item C<comment> - C<single_of.Comment>

This is an optional programmer comment about the [|sub]depot or package as
a whole.

=item C<subdepots> - C<SubdepotSet>

These are all the subdepots that this system catalog contains (which might
be none), iff this system catalog is a [|sub]depot; iff this system catalog
is a package, then C<subdepots> is not applicable and is empty.

=item C<packages> - C<PackageSet>

These are all the packages that this system catalog contains (which might
be none), iff this system catalog is a [|sub]depot; iff this system catalog
is a package, then C<packages> is not applicable and is empty.

=item C<materials> - C<MaterialSet>

These are all the definitions that this [|sub]depot or package contains of
functions, updaters, procedures, scalar types with possreps, complete
nonscalar types, union types, and restriction types.

=item C<data> - C<maybe_of.NameChain>

This is the declared data type of the self-local dbvar that this
[|sub]depot or package contains, iff C<data> is a C<Single>; if C<data> is
C<nothing> (the default), then this [|sub]depot or package does not have a
self-local dbvar.

=back

The default value of C<DepotOrPackage> defines an empty [|sub]depot or
package that does not have any self-local dbvar.
...
entities it contains; see the C<DepotOrPackage> which contains it for that.
...
A C<PackageSet> is a C<DHRelation> that specifies the set of packages that
a depot might optionally have for organizing its possibly private entities.
A C<PackageSet> only specifies that a package exists, not which package
entities it contains; see the C<DepotOrPackage> which contains it for that.

A C<PackageSet> has these 3 attributes:
...
=head2 sys.std.Core.Type.Cat.MaterialSet

A C<MaterialSet> is a C<DHRelation> that specifies a set of routines and
types that a [|sub]depot or package might directly contain; in the case of
a package, each routine or type may be either public for the DBMS as a
whole or private to the package.

A C<MaterialSet> has these 6 attributes:
...
This is the fully-qualified name, in the C<dep.[cat|lib|data]> namespace,
of the routine's or type's parent [|sub]depot, or of the routine's or
type's parent package's parent [|sub]depot.

=item C<package> - C<maybe_of.Name>

This is the declared name of the routine's or type's parent package within
the namespace defined by C<subdepot>, iff the routine or type belongs to a
package; iff the routine or type directly belongs to a [|sub]depot, then
C<package> is not applicable and is C<nothing>.

=item C<name> - C<Name>

This is the declared name of the routine or type within the namespace
defined by C<subdepot>, plus C<package> if it is a C<Single>; other Muldis
D code would reference it with the combination of C<subdepot> and
C<package> if applicable and C<name>.

=item C<comment> - C<Comment>

This is an optional programmer comment about the routine or type as a
whole.

=item C<is_public> - C<maybe_of.Bool>

Iff the routine or type being defined lives directly in a [|sub]depot, then
C<is_public> is not applicable and is C<nothing>.  Iff the routine or type
being defined is part of a package, then C<is_public> is a C<Single>:  This
is C<Bool:true> if the routine or type is part of the package's own public
interface and may be invoked from outside the package; it is C<Bool:false>
if it is just part of the package's internals.

=item C<material> - C<Material>

This defines the entire routine or type sans its name.

=back

A C<MaterialSet> has a ternary primary key on the C<subdepot> plus
C<package> plus C<name> attributes.  Its default value is empty.

=head1 TYPES FOR DEFINING ROUTINES AND DATA TYPES

These are all enumeration data types.

=head2 sys.std.Core.Type.Cat.Material

A C<Material> is a C<DHTuple> that specifies a named routine or type, both
its public interface and its implementation details, except for its name
and public/private status; the latter are provided by any depot or package
embedding the material when applicable.  Its purpose is as per
C<Inner[[[Func|Upd|Proc][Head|Body]]|[Scalar|Nonscalar|Union|Restr]Type]>,
one of which it is a proxy-wrapper for along with other inner routines and
types that are composed into it.  A C<Material> is used to define any DBMS
entity that would be invoked via the second-level namespace C<lib> (hence
its name is derived from the concept of a library's materials), such as
C<[fed|dep|sdp|pkg].lib>.  A C<Material> is composed of 1 or more named
inner routines or types, exactly 1 of which must have the empty string as
its name; that inner routine or type defines the actual main routine or
type that the C<Material> is a proxy-wrapper for, and the other inner
routines and types, if any, support it.  A C<Material> is typically
composed into a C<DepotOrPackage>.

A C<Material> has these 8 attributes:

=over

=item C<comment> - C<Comment>

This is an optional programmer comment about the material as a whole.

=item C<inner_funcs> - C<InnerFuncSet>

These are all the inner function definitions that this material contains.

=item C<inner_upds> - C<InnerUpdSet>

These are all the inner updater definitions that this material contains.
This attribute must be empty unless the material is a procedure or updater.

=item C<inner_procs> - C<InnerProcSet>

These are all the inner procedure definitions that this material contains.
This attribute must be empty unless the material is a procedure.
...
that this material contains.

=back

There is a distributed primary key over the C<name> attributes of all 7 of
a C<Material>'s C<DHRelation>-typed attributes.

The default value of C<Material> is as per C<InnerRestrType>.

=head2 sys.std.Core.Type.Cat.Inner[Func|Upd|Proc]Set

An C<Inner[Func|Upd|Proc]Set> is a C<DHRelation> that specifies a set of
...
An C<Inner[Sca|Nonsca|Uni|Restr]TypeSet> is a C<DHRelation> that specifies
...
=head1 TYPES FOR DEFINING INNER ROUTINE HEADINGS
...
An C<InnerFuncBody> is a C<DHTuple>.  It specifies the entire body or
...
=item C<alias_exprs> - C<AliasExprNodeSet>

These are expression nodes that represent aliases of other expression nodes
or attributes of said.

=item C<func_invo_exprs> - C<FuncInvoExprNodeSet>

These are expression nodes that represent [|inner ]function invocations.
...
=head2 sys.std.Core.Type.Cat.AliasExprNodeSet

An C<AliasExprNodeSet> is a C<DHRelation> that specifies a set of value
expression nodes where each node is an alias for either another expression
node or for an attribute of one; what each node is an alias for is defined
in terms of a C<NameChain> in the C<lex> top-level namespace.

A C<AliasExprNodeSet> has these 3 attributes:
...
=item C<original> - C<NameChain>

This is the name of the other expression node (or attribute) being aliased.

=back

A C<AliasExprNodeSet> has a unary primary key on the C<name>
attribute.  Its default value is empty.
...
An C<InnerUpdBody> is a C<DHTuple>.  It specifies the entire body or
...
An C<InnerProcBody> is a C<DHTuple>.  It specifies the entire body or
...
An C<InnerScalarType> is a C<DHTuple>.  It defines either a new
...
by using a C<InnerScalarType>, and you can not define a scalar type
with an incompletely defined attribute list at all.

An C<InnerScalarType> has these 7 attributes:
...
=item C<subtype_constraint> - C<maybe_of.Name>
...
C<Single> whose sole element matches the declared name of an inner
C<type_constraint> function that determines what base type values are part
of the subtype.  The inner function that this names must have a
single C<topic> parameter whose declared type is that named by
C<base_type>, and whose argument is the value to test; the inner function's
...
matches the declared name of an inner C<named_value> function that
...
matches the declared name of an inner C<order_determination> function.  The
inner function that this names must have exactly 4 parameters; the
...
=item C<constraint> - C<maybe_of.Name>

Iff it is a C<Single>, then C<constraint>
matches the declared name of an inner C<type_constraint> function that
...
the attributes individually.  The inner function that this names
...
value to test; the inner function's result type must be C<Bool>.  If the
...
=item C<p2_from_p1> - C<Name>

This matches the declared name of an inner C<possrep_map> function that
derives the representation of the possrep named by C<p2> from that of the
possrep named by C<p1>.  The inner function that this names must
have a single C<topic> parameter whose declared type is a tuple whose
attributes match those of C<p1>; the inner function's result type must be a
...
=item C<p1_from_p2> - C<Name>

This matches the declared name of an inner inverse C<possrep_map> function
...
An C<InnerNonscalarType> is a C<DHTuple>.  It defines either a new
...
=item C<tuple_constraints> - C<set_of.Name>

This matches the declared names of 0..N inner C<type_constraint> functions
...
The inner function that each of these names must have a single
...
=item C<relation_constraints> - C<set_of.Name>
...
applicable to tuple/database values of the type being defined, that range
over and relate tuples of specified relation-typed attributes of it;
they are a kind of referential constraint.  Each tuple of a child
...
relation is subtracted from a corresponding projection of the child
relation (with attribute renaming if necessary), is an empty relation; if
the difference is an empty relation, then the foreign key is satisfied;
otherwise, any difference tuples are from child tuples that violate the
foreign key.
...
over and relate tuples of specified relation-typed attributes of it;
they are a kind of referential constraint.  Each tuple of a child
attribute must have a corresponding tuple in one member of a specific set
...
relation is the result of unioning appropriately renamed projections of
the member relations of the distributed key.

=item C<default_tuple> - C<maybe_of.Name>
...
=item C<default_relation> - C<maybe_of.Name>
...
The default value of C<InnerNonscalarType> defines a relation type that has
...
=item C<virtual_attr_map> - C<Name>

This matches the declared name of an inner C<virtual_attr_map> function
that derives a tuple of dependent attribute values from a tuple of
determinant attribute values.  The inner function that this names must
have a single C<topic> parameter whose declared type is a tuple whose
attributes match those of C<determinant_attrs>; the inner function's
...
A C<ForeignKeySet> is a C<DHRelation> that specifies a set of foreign
(non-distributed) keys over relation-valued attributes of a
tuple/database; each tuple specifies one foreign key, which is a kind of
referential constraint, that relates tuples of such attributes.  Each
tuple of a child attribute must have a corresponding tuple in a
...
This is the name of the relation-valued attribute that is the parent in
...
This is the name of the relation-valued attribute that is the child in
...
A C<ForeignDistribKeySet> is a C<DHRelation> that specifies a set of
...
A C<FKChildAttrParentAttrMap> is a C<DHRelation>.  It maps 0..N attributes
...
An C<InnerUnionType> is a C<DHTuple>.  It defines a new data type whose
...
have "Rectangle", "Rhombus"), you have to first define a C<InnerUnionType>,
and then use that as the base type of a C<InnerScalarType>.
...
=item C<constraints> - C<set_of.Name>

This matches the declared names of 0..N inner C<type_constraint> functions
that (when I<and>-ed together) determine what filter-type-passing
source-type values are part of the union type.  The inner function
...
This matches the declared name of an inner C<named_value> function that
...
An C<InnerRestrType> is a C<DHTuple>.  It provides a relatively terse
...
defined attribute list using a C<InnerRestrType> (or a C<InnerUnionType>).
...
the restriction type.  The inner function that each of these names
...
function C<inn.mytest> and C<dep.lib.mymain> invoked the public function
...
C<inn>, the context is the outer entity containing that inner; for a
reference to C<sys> or C<fed>, the context is global; for C<dep> or C<sdp>
or C<pkg> the context is that same depot/subdepot/package.  I<These spec
...
information such as pre-bound function parameters.  Its default value is as
per C<Material>, which is a reference to the C<sys.std.Core.Type.Universal>
data type.

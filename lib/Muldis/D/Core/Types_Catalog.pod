...
it declares these 25 standard language namespaces:
C<[sys|mnt|tpc|fed|dep|sdp|inn|lex]> (which have the nameless global
...
A C<Depot> has these 3 attributes:
...
types that a [|sub]depot might directly contain; each routine or type may
be either public for the DBMS as a whole or private to the subdepot.

A C<MaterialSet> has these 5 attributes:
...
defined by C<subdepot>; other Muldis D code would reference it with the
combination of C<subdepot> and C<name>.
...
This is C<Bool:true> if the routine or type is part of the subdepot's own
public interface and may be invoked from outside the subdepot; it is
C<Bool:false> if it is just part of the subdepot's internals.
...
A C<MaterialSet> has a binary primary key on the C<subdepot> plus
C<name> attributes.  Its default value is empty.
...
and public/private status; the latter are provided by any depot
...
C<[fed|dep|sdp].lib>.  A C<Material> is composed of 1 or more named

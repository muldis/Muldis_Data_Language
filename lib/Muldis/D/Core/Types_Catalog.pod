...
intended for use as the declared type of a routine parameter that would
take the name of an C<order_determination> function, but that parameter is
optional and should default to the system-defined scalar ordering function
when no argument is given to it.
...
A C<Cat.ExprNodeSet> has these 7 attributes:
...
A C<Cat.VirtualAttrMapSet> has these 4 attributes:
...
=item C<determinant_attrs> - C<set_of.Cat.Name>

These are the declared names of the determinant attributes.

=item C<dependent_attrs> - C<set_of.Cat.Name>

These are the declared names of the dependent attributes; none of these may
be the same as the names of the determinant attributes.
...
=item C<child> - C<Cat.Name>

This is the name of the relation-valued attribute that is the child in the
foreign (non-distributed) key relationship.  Note that C<child> and
C<parent> are allowed to be one and the same.
...
=item C<parent_distrib_key> - C<Cat.Name>
...
proxy the attributes that are being matched on in the foreign key.

=item C<child> - C<Cat.Name>

This is the name of the relation-valued attribute that is the child in the
foreign distributed key relationship.

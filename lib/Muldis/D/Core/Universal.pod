...
This document is Muldis::D::Core::Universal version 0.100.0.
...
argument to C<sys.std.Core.Integer.diff>, which it otherwise wouldn't.  Note
...
This function is an alias for C<sys.std.Core.Universal.is_before> except that it
...
This function is exactly the same as C<sys.std.Core.Universal.is_before> except
...
C<< function sys.std.Core.Universal.is_after_or_same (Bool <-- $topic : Universal,
$other : Universal, $func? : OrdDetFuncRef) >>

This function is an alias for C<sys.std.Core.Universal.is_before_or_same> except
that it transposes the C<topic> and C<other> arguments.  This function is
exactly the same as C<sys.std.Core.Universal.is_after> except that it results in
...
This function is exactly the same as C<sys.std.Core.Universal.min> except that it
...
C<sys.std.Core.Universal.min> and C<sys.std.Core.Universal.max> would result in when
...
This function is to C<sys.std.Core.Universal.max> as C<sys.std.Core.Universal.maybe_min>
is to C<sys.std.Core.Universal.min>.

=head2 sys.std.Core.Universal.maybe_minmax

C<< function sys.std.Core.Universal.maybe_minmax (Relation <-- $topic : Set,
$func? : OrdDetFuncRef) >>
...
whose respective attribute values are what C<sys.std.Core.Universal.min> and
C<sys.std.Core.Universal.max> would result in when given the same arguments; if

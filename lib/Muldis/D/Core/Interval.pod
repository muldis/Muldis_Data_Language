...
This document is Muldis::D::Core::Interval version 0.111.0.
...
that this operation is also known as C<I∋> or C<Ihmem>.
...
arguments.  Note that this operation is also known as C<I∌> or C<I!hmem> or
C<Inot-hmem>.
...
otherwise.  That is, if conceptually the interval represents a set of
values, this function tests if C<value> is a member of C<interval>.  Note
that this operation is also known as C<I∈> or C<Iimem> or C<between>, and
depending on the combination of C<excludes_[min|max]> attributes of its
C<interval> argument, it is alternately also known as one of [C<≤≤>, C<< ≤<
>>, C<< <≤ >>, C<< << >>].
...
that this operation is also known as C<I∉> or C<I!imem> or C<Inot-imem>
or C<!between> or C<not-between>, and depending

...
C<< function sys.std.Core.Bool.implies (Bool <-- $topic : Bool, $other : Bool)
>>
...
This function is exactly the same as C<sys.std.Core.Bool.imp> except that it
...
This function is an alias for C<sys.std.Core.Bool.imp> except that it transposes
...
This function is exactly the same as C<sys.std.Core.Bool.exactly> except that
it results in the opposite boolean value when given the same argument.

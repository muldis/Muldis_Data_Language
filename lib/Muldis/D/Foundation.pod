...
call/invocation takes exactly 1 input I<topic> value and gives exactly 1
output I<result> value.  The result value of a function call is determined
entirely by its topic value, and a function is guaranteed to give exactly
the same single result value for every call with the same topic value.
Multiple distinct topics may yield the same result, but multiple calls with
the same topic never give different results.

A Muldis D function is more constrained than a mathematical function such
that its input I<topic> may only be a I<tuple> (a value of the
...
Each (distinctly named) member I<attribute> of the I<topic> is an input
...
expression, and similarly, the function call's topic must be
...
all (its I<topic> is the sole nullary/zero-attribute I<tuple>) and it
...
C<0> attribute as its topic/arguments.  Its result type is I<FDN__Any>.

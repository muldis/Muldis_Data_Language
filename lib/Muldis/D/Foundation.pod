...
But in the interest of avoiding unnecessary complication for the design and
users of the language, Muldis D requires that every I<actual argument> aka
I<topic> of a function is I<FDN__Tuple> typed, and moreover that type must
exclude the sole I<FDN__Tuple> value having zero attributes.  Therefore,
what would otherwise be the case for most functions in practice is required
for all functions, and so extra logic for dealing with edge cases can be
avoided. To maintain consistency across functions, even functions that take
exactly 1 conceptual argument should expect it wrapped in a I<FDN__Tuple>
such that the actual argument is a I<FDN__Tuple> with exactly 1 attribute
that is the conceptual argument.

Note that for brevity the Muldis D language specifications will only use
the unqualified term (function) I<argument> to refer to a conceptual
argument of the function that is actually an attribute of its I<FDN__Tuple>
typed actual argument.  Complementary to this, the term I<topic> will
always refer to a function's entire actual argument.  As an exception, in
some contexts the unqualified I<argument> could mean either an actual or a
conceptual argument.

Note that for brevity the Muldis D language specifications will refer to
conceptual arguments with the same C<foo> syntax regardless of whether they
...
contract for type I<T1> where the sole argument is value I<V1> (that is,
the actual argument is a I<FDN__Tuple> with exactly 1 attribute whose name
is C<0> and whose asset is I<V1>) is semantically identical to (and is
idiomatic for) testing if I<V1> is a member of the type I<T1>; the result
is I<true> if so and I<false> otherwise.
...
The function C<FDN__is_a_Boolean> requires its topic to be a I<FDN__Tuple>
...
The function C<FDN__is_a_Integer> requires its topic to be a I<FDN__Tuple>
value with just 1 attribute/argument named C<0>, where that argument may be
any value.  This function results in the I<FDN__Boolean> value
...
The function C<FDN__is_a_Array> requires its topic to be a I<FDN__Tuple>
value with just 1 attribute/argument named C<0>, where that argument may be
any value.  This function results in the I<FDN__Boolean> value
...
The function C<FDN__is_a_Bag> requires its topic to be a I<FDN__Tuple>
value with just 1 attribute/argument named C<0>, where that argument may be
any value.  This function results in the I<FDN__Boolean> value
...
The function C<FDN__is_a_Tuple> requires its topic to be a I<FDN__Tuple>
value with just 1 attribute/argument named C<0>, where that argument may be
any value.  This function results in the I<FDN__Boolean> value
...
The function C<FDN__is_a_Capsule> requires its topic to be a I<FDN__Tuple>
value with just 1 attribute/argument named C<0>, where that argument may be
any value.  This function results in the I<FDN__Boolean> value
...
The function C<FDN__is_a_External> requires its topic to be a I<FDN__Tuple>
value with just 1 attribute/argument named C<0>, where that argument may be
any value.  This function results in the I<FDN__Boolean> value
...
The function C<FDN__External_call_function> requires its topic to be a
...
C<0> attribute as its actual argument.  Its result type is I<FDN__Any>.

...
is driving it; the application holds a C<QDRDBMS::Interface::Routine>
object to reference it by (the result of a QDRDBMS object's "prepare" or
"compile" method), and it exists only so long as that object doesn't go out
of scope.  A C<gateway> is mostly like a C<procedure> in every other way.
Main differences are that the only C<gateway> routines are ones that users

...
QDRDBMS itself works, and will spend little time in providing rationale;
...
QDRDBMS type system supports multiple inheritance, so types can form a
...
non-encapsulated collection types in QDRDBMS D, and are like other
...
command processor (CPU), standard user input and output channel, persistent
...
is a large part of what makes the QDRDBMS DBMS a reliable modeler of

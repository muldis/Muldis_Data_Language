...
terms will be used for such instead, to avoid confusion.
...
A I<tuple> is an unordered collection of 0..N elements that are keyed by
the element's name; each element is a name-value pair, and all names in the
tuple are distinct.  While I<tuple> legitimately refers to the same thing
as the QDRDBMS term I<sequence> in other contexts, it does not in this
documentation.  Terms like I<record> or I<row> are not used in this
documentation, the latter in particular because it implies ordinality.

=item relation, relvar, relcon

A I<relation> is like an unordered set of I<tuple> where all member tuples
have identical cardinality and name-sets, but that a relation data type
knows what its allowed names are even if it has no tuples.  Like with
...
and its return value.  A I<function> is not the same as a I<relation>,
...
the C<Sequence> data type, which is a certain binary relation.
...
All values in the QDRDBMS type system are broadly categorized into 3
complementary sets called I<scalar values>, I<tuple values>, and I<relation
values> (the latter 2 are collectively known as I<nonscalar> values).  The
type system has the system-defined data types named C<Scalar>, C<Tuple>,
and C<Relation>, which serve as maximal data types for each category,
respectively.  The 3 types are all mutually disjoint, and C<Universal> is a
union type over all of them.

To keep things simpler, every data type (save C<Universal> and C<Empty>)
must be a proper subtype of exactly 1 of the 3 categories, and can not
include values from several of them.  Therefore, every data type is said to
be either a I<scalar type>, a I<tuple type>, or a I<relation type>,
depending which category all of its values come from.  In similar fashion,
a I<nonscalar type> is any type that is not a scalar type, meaning it
is either a tuple type or a relation type.
...
this distribution by instantiating a new QDRDBMS::Interface::DBMS object),
...
argument values come by way of a single nonscalar-type parameter.
...
invocation both returns and represents a value of a specific data type;
this invocation can only exist as part of a value-expression of another
routine.  A C<function> is pure and deterministic in the
functional-language sense, such that all of its 0..N parameters are
read-only / not subject to update, and that it can only see or influence
its own lexical variables, if it has any (no globals), and that it can only
invoke C<function> and C<update_operator> routines.  The vast majority of
invokable system-defined routines are C<function>; they include all value
selectors, and the typical numeric, string, and relational operators,
...
but that its return value is via a parameter.  Most non C<function>
...
routine.  The C<procedure> and C<gateway> are the only explictly invokable
routines that can directly reference global containers, whether catalog or
data.  The vast majority of C<procedure> that exist will be user-defined.
But some system-defined routines that would otherwise be C<function> or
C<update_operator> are C<procedure> instead soley because they are
non-deterministic; an example is an operator that derives a tuple sequence
from a relation without fully sorting the tuples, because the result is
fundamentally random and non-repeatable.
...
a value of that type is being selected; it asserts whether said
value, which by this time is known to be acceptable to the current data
type's more generic supertype, is within the data type's own more
restricted domain.  This routine can only see its own lexical variables (no
globals).  This routine has 1 read-only parameter, which is the value to
examine, and it returns a Bool.  The DBMS would then throw a
type-constraint-violation exception if the constraint returns False, and
no-op if it returns True.  Conceptually speaking, a C<type_constraint> will
execute before any other kinds of constraints.
...
updates none.  This routine has no parameters and returns a Bool.  The DBMS
responds as per a type constraint; if the constraint fails, then the
...
returns a Bool.  The DBMS responds as per a state constraint.  Conceptually
speaking, a C<transition_constraint> will execute after all other kinds of
constraints.

=item C<gateway>

A C<gateway> is an anonymous procedure that can not be invoked by any other
routine and exists in the limbo between the DBMS and the application that
is driving it; the application holds a C<QDRDBMS::Routine> object to
reference it by (the result of a QDRDBMS object's "prepare" or "compile"
method), and it exists only so long as that object doesn't go out of scope.
A C<gateway> is mostly like a C<procedure> in every other way.  Main
differences are that the only C<gateway> routines are ones that users
define, and that they are the only routines allowed to have explicit
isolated transaction initiation and termination statements (for optional
use in a parent-most transaction); all other updating routine types have
self-contained atomic blocks instead.
...
C<gateway> routines) are bound to specific lexical scopes, such that they
...
a C<gateway> routine, that an application invokes directly, and not in any
named routines, nor within atomic statement blocks in C<gateway> routines.
...
These nonscalar data types, C<Tuple|Relation|Database|Set|Sequence|Bag>,
...
system-defined selector for nonscalar values will return values that are
...
attributes, where all attribute names are mutually distinct, and each
attribute may be of distinct selectable types; the mapping of a tuple's
attribute names and their declared data types is called the tuple's
I<heading>.  Its default value is a zero-attribute tuple.  The cardinality
of this type is equal to the product of the number of permutations drawable
from the values of each of its attributes' declared data types; for a
C<Tuple> subtype to be finite, all of its attribute types must be.

=item C<sys.type.Relation>

A C<Relation> is analagous to a set of 0..N tuples where all tuples have
the same heading (all attribute names and corresponding declared data types
match), but that a C<Relation> data type still has its own corresponding
heading (attribute names and declared data types) even when it consists of
zero tuples.  Its default value is a zero-attribute and zero-tuple
relation.  Matters of its cardinality are generally the same as for
C<Tuple>.
...
A C<Set> is a proper subtype of C<Relation> that has 1 attribute named
C<value>; it can be of any declared type.  A C<Set> subtype is normally
used by any system-defined N-ary operators where the order of their
argument elements or result is not significant, and that duplicate values
are not significant.  Its default value has zero tuples.

=item C<sys.type.Sequence>

An C<Sequence> is a proper subtype of C<Relation> that has 2 attributes
named C<index> and C<value> where C<index> is a unary key and its declared
type is an C<UInt> subtype (C<value> can be non-unique and of any declared
type).  A Sequence is considered dense, and all C<index> values in one are
numbered consecutively from 0 to 1 less than the count of tuples, like
array indices in typical programming languages.  A C<Sequence> subtype is
...
A C<Bag> is a proper subtype of C<Relation> that has 2 attributes named
C<value> and C<count> where C<value> is a unary key (that can have any
...
occurrances for C<values>.  Its default value has zero tuples.
...
=item C<sys.type.Cat.DeclEntityName>

A C<Cat.DeclEntityName> is a declared name of a DBMS entity, or the
declared name of an attribute thereof, that either is defined with.  This
declared name is conceptually multi-part, with the parts forming a sequence
of 1..N C<NEText>, ordered from greatest to least significance; names of
attributes typically have just 1 part and so are conceptually just a
C<NEText>, though often database attribute names, which are its component
...
C<sys> is an example of part 1. C<Cat.DeclEntityName> has multiple
possreps; one is a C<Sequence(1..N) of NEText>; one is a specially encoded
C<NEText> where parts are ordered from greatest to least signficance, and
adjacent parts are separated with a single "period" character, and any
literal period or backslash characters in parts are backslash-escaped (no
other characters are escaped).  Its default value consists of a single part
where that part is a single space character.

=item C<sys.type.Cat.InvokEntityName>

A C<Cat.InvokEntityName> is a name by which some routine invokes or
references a DBMS entity.  It is similar to a C<Cat.DeclEntityName>.
...
This selector function returns the False boolean value.

=item C<sys.rtn.Bool.True of Bool ()>

This selector function returns the True boolean value.

=item C<sys.rtn.Blob_from_bin_digit_Text of Blob (Text $text)>

This selector function returns the Blob value that its C<Text> argument
...
This selector function returns the Int value that its C<Text> argument maps
to when the whole character string is evaluated as a base-10 integer.

=item C<sys.rtn.Int_from_hex_digit_Text of Int (Text $text)>

This selector function returns the Int value that its C<Text> argument maps
to when the whole character string is evaluated as a base-16 integer.

=back

=head2 Simple Nonscalar Data Type Selectors

I<This documentation is pending.>

String-form syntax for declaring collection types, such as when used as
arguments to N-ary constructors etc:
...
sees, ala QDRDBMS::AST), and string-form QDRDBMS D code that users

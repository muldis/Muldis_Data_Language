...
I<QDRDBMS framework> aka the I<core> is composed entirely of all the files
included with this QDRDBMS distribution in its C<lib/> directory.  This
also includes its main test suite, since that is implemented using modules
in C<lib/> such as C<QDRDBMS::Validator>.

By contrast, this QDRDBMS distribution may also include some extra files,
particularly in its C<examples/> directory, which are explicitly not part
of the core, and which may be the copyright of other parties.
...
contribution into the QDRDBMS core distribution, or may possibly rewrite the
contribution instead, in which case the contribution would effectively be
'advice'.
